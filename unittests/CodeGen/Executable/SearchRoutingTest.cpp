...
TEST(SearchRoutingTest,
     ExplicitSearchStopsAtStructureSpecificStorageWithoutActualCandidate) {
  ...
  diagnostics.flush();
  EXPECT_NE(
      diagnosticsText.find("physical-search incomplete "
                           "required_coordinate=structure-specific-storage"),
      std::string::npos)
      << diagnosticsText;
