//===- RankResourceCostValidationTest.cpp ---------------------------===//
...
class RankResourceCostValidationTest : public ::testing::Test {
protected:
  RankResourceCostValidationTest() {
...
TEST_F(RankResourceCostValidationTest,
...
module {
  ...
  auto accepted =
      wafer::compiler::testing::validateRankResourceCost(modules, *config);
...
TEST_F(RankResourceCostValidationTest,
       RejectsSPMTransferAcrossTileResidencyDomains) {
...
module {
  ...
  EXPECT_TRUE(
      mlir::failed(wafer::compiler::testing::validateRankResourceCost(
          modules, *config)));
}

TEST_F(RankResourceCostValidationTest,
       RejectsUnknownRankCandidateTrafficInsteadOfGuessing) {
  auto module = parse(R"mlir(
module {
  func.func @main(
...
module {
  ...
  auto accepted = wafer::compiler::testing::validateRankResourceCost(
      modules, *cardConfig);
  ASSERT_TRUE(mlir::succeeded(accepted));
