//===- WholeCardExecutableLoweringTest.cpp ----------------------------===//

#include "../../lib/Wafer/Compiler/WholeCardExecutableLowering.h"
...
TEST(WholeCardExecutableLoweringFailureTest,
     KeepsClassificationOutOfDiagnosticText) {
  using wafer::compiler::detail::WholeCardExecutableLoweringFailure;
  using wafer::compiler::detail::WholeCardExecutableLoweringFailureKind;

  WholeCardExecutableLoweringFailure stageFailure{
      WholeCardExecutableLoweringFailureKind::PhysicalTileDomain,
      "arbitrary detail"};
  EXPECT_FALSE(stageFailure.isProvenExactRejection());
  EXPECT_EQ(stageFailure.getDiagnosticLabel(), "physical-tile-domain");

  WholeCardExecutableLoweringFailure indeterminate{
      WholeCardExecutableLoweringFailureKind::TargetABILowering,
...
class WholeCardExecutableLoweringTest : public ::testing::Test {
protected:
  WholeCardExecutableLoweringTest() {
...
TEST_F(WholeCardExecutableLoweringTest, ConsumesCompletePhysicalTileDomain) {
  ...
  wafer::compiler::detail::WholeCardExecutableLoweringFailure failure;
  auto executable =
      wafer::compiler::detail::lowerPhysicalTileModulesToExecutable(
  ...
  EXPECT_EQ(statistics.physicalTileModuleLoweringAttempts, 1u);
  EXPECT_EQ(statistics.physicalTileModuleLoweringSuccesses, 1u);
  ...
  EXPECT_EQ(statistics.wholeCardExecutablesProduced, 1u);
}

TEST_F(WholeCardExecutableLoweringTest,
       ReportsTargetABIFailureAfterInstrLowering) {
  auto module = parseTileModule(R"mlir(
  func.func @main(
  ...
  EXPECT_EQ(failure.kind,
            wafer::compiler::detail::WholeCardExecutableLoweringFailureKind::
  ...
  EXPECT_EQ(statistics.targetLoweringVerificationInvocations, 1u);
  EXPECT_EQ(statistics.targetTileLoweringVerificationInvocations, 16u);
  EXPECT_EQ(statistics.wholeCardExecutablesProduced, 0u);
}
...
TEST_F(WholeCardExecutableLoweringTest,
  ...
  EXPECT_FALSE(failure.isProvenExactRejection());
  EXPECT_EQ(statistics.physicalTileModuleLoweringAttempts, 1u);
  EXPECT_EQ(statistics.physicalTileModuleLoweringSuccesses, 0u);
