//===- WholeCardResourceAcceptanceTest.cpp ------------------------------===//
...
class WholeCardResourceAcceptanceTest : public ::testing::Test {
protected:
  WholeCardResourceAcceptanceTest() {
  ...
  completePhysicalTileDomain(mlir::OwningOpRef<mlir::ModuleOp> activeModule) {
...
module {
  func.func @main() {
    ...
    for (int64_t tile = 1; tile < config->getPhysicalTileCount(); ++tile) {
      auto idleModule = parse(idleSource);
      if (!idleModule) {
        ADD_FAILURE() << "failed to parse idle physical Tile module";
        return {};
      }
  ...
  mlir::FailureOr<wafer::analysis::WholeCardInstructionProgramCost>
  accept(llvm::ArrayRef<mlir::ModuleOp> modules,
         const wafer::compiler::ExecutionConfig &executionConfig) {
    llvm::SmallVector<wafer::PhysicalTileId, 16> physicalTileIds;
    physicalTileIds.reserve(modules.size());
    for (size_t tile = 0; tile < modules.size(); ++tile)
      physicalTileIds.emplace_back(static_cast<int64_t>(tile));
    return wafer::compiler::testing::acceptWholeCardResources(
        modules, physicalTileIds, executionConfig);
  }
...
TEST_F(WholeCardResourceAcceptanceTest,
       AcceptsExactResourceSummaryForCompletePhysicalTileDomain) {
...
module {
  ...
  ASSERT_TRUE(config);
  llvm::SmallVector<mlir::ModuleOp, 16> modules =
      completePhysicalTileDomain(std::move(module));
  ASSERT_EQ(modules.size(), 16u);

  auto accepted = accept(modules, *config);
  ASSERT_TRUE(mlir::succeeded(accepted));
  EXPECT_EQ(accepted->tileCosts.size(), 16u);
  EXPECT_EQ(accepted->aggregateDDRReadBytes.value, 8u);
  EXPECT_EQ(accepted->minimumHopLinkByteDemand.value, 0u);
  EXPECT_EQ(accepted->maximumTileSPMHighWaterBytes.value, 8u);
}
...
module {
  ...
  EXPECT_TRUE(mlir::failed(accept(modules, *config)));
}

TEST_F(WholeCardResourceAcceptanceTest,
       RejectsMissingAcceptedSPMHighWaterInsteadOfBypassingCapacity) {
  auto module = parse(R"mlir(
module {
  func.func @main() {
...
module {
  ...
  auto accepted = accept(modules, *config);
  ASSERT_TRUE(mlir::succeeded(accepted));
  EXPECT_FALSE(accepted->aggregateDDRReadBytes.isKnown());
}
...
module {
  ...
  EXPECT_EQ(accepted->maximumTileSPMHighWaterBytes.value, 264u);
}

TEST_F(WholeCardResourceAcceptanceTest,
       AggregatesTheCompleteSixteenPhysicalTileDomain) {
...
module {
  ...
  auto accepted = accept(modules, *cardConfig);
  ASSERT_TRUE(mlir::succeeded(accepted));
  EXPECT_EQ(accepted->tileCosts.size(), 16u);
  EXPECT_EQ(accepted->aggregateInstructionCount.value, 16u);
  EXPECT_EQ(accepted->maximumTileSPMHighWaterBytes.value, 8u);
  EXPECT_EQ(accepted->summedTileSPMHighWaterBytes.value, 128u);
}

TEST_F(WholeCardResourceAcceptanceTest,
       ModelsAcceptedRouteWorkFromPhysicalTopology) {
...
module {
  ...
  auto accepted = accept(modules, *cardConfig);
  ASSERT_TRUE(mlir::succeeded(accepted));
  ASSERT_TRUE(accepted->aggregateNoC.aggregateTransmitBytes.isKnown());
  ASSERT_TRUE(accepted->aggregateNoC.aggregateReceiveBytes.isKnown());
  EXPECT_EQ(accepted->aggregateNoC.aggregateTransmitBytes.value, 4u);
  EXPECT_EQ(accepted->aggregateNoC.aggregateReceiveBytes.value, 4u);
  EXPECT_EQ(accepted->minimumHopLinkByteDemand.value, 4u);
  EXPECT_EQ(accepted->minimumHopMessageDemand.value, 1u);
  EXPECT_EQ(accepted->directedNoCLinkCount.value, 48u);
  EXPECT_EQ(accepted->idealizedMinimumPeakLinkByteDemand.value, 1u);
  EXPECT_EQ(accepted->modeledNoCRoute.peakDirectedLinkByteDemand.value, 4u);
  EXPECT_EQ(accepted->maximumNoCHopCount.value, 1u);
}

TEST_F(WholeCardResourceAcceptanceTest,
       RejectsDuplicateExplicitPhysicalTileIdentity) {
  constexpr llvm::StringLiteral source = R"mlir(
module {
  func.func @main() {
    return
  ...
  llvm::SmallVector<mlir::ModuleOp, 16> modules;
  llvm::SmallVector<wafer::PhysicalTileId, 16> physicalTileIds;
  for (int64_t tile = 0; tile < config->getPhysicalTileCount(); ++tile) {
    owners.push_back(parse(source));
    ASSERT_TRUE(owners.back());
    modules.push_back(*owners.back());
    physicalTileIds.emplace_back(tile == 15 ? 14 : tile);
  }
  mlir::ScopedDiagnosticHandler suppress(
      context.get(), [](mlir::Diagnostic &) { return mlir::success(); });

  EXPECT_TRUE(mlir::failed(wafer::compiler::testing::acceptWholeCardResources(
      modules, physicalTileIds, *config)));
}
