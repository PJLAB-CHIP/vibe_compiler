//===- WholeCardCandidateEvaluationTest.cpp
//----------------------------===//
...
class WholeCardCandidateEvaluationTest : public ::testing::Test {
protected:
  WholeCardCandidateEvaluationTest() {
...
TEST(WholeCardCandidateEvaluationSchedulerTest,
     StartsWithSeedAndRotatesAfterRejectedAttempts) {
  using Coordinator =
      wafer::compiler::detail::WholeCardCandidateEvaluationScheduler;
  using Lane = wafer::compiler::detail::WholeCardCandidateEvaluationLane;
  ...
  for (Lane expected :
       {Lane::Seed, Lane::Expansion, Lane::Seed, Lane::Expansion}) {
  ...
  ASSERT_TRUE(mlir::succeeded(
      coordinator.recordAttempt(*forcedExpansion, /*exactAccepted=*/false)));
...
TEST(WholeCardCandidateEvaluationSchedulerTest,
  ...
  for (unsigned index = 1;
       index < wafer::compiler::detail::kMaximumCoordinatedExactScheduleActions;
       ++index) {
    auto lane = coordinator.chooseNextLane(/*seedAvailable=*/true,
                                           /*expansionAvailable=*/true);
    ASSERT_TRUE(lane);
...
TEST(WholeCardCandidateEvaluationSchedulerTest,
  ...
  wafer::compiler::detail::WholeCardInstrLoweringFailure failure;
  failure.kind = wafer::compiler::detail::WholeCardInstrLoweringFailureKind::
      RankInstrLowering;
  EXPECT_TRUE(
      wafer::compiler::detail::isRecoverableWholeCardInstrLoweringFailure(
          failure));
  failure.kind =
      wafer::compiler::detail::WholeCardInstrLoweringFailureKind::SPMAllocation;
  EXPECT_TRUE(
      wafer::compiler::detail::isRecoverableWholeCardInstrLoweringFailure(
          failure));
  failure.kind =
      wafer::compiler::detail::WholeCardInstrLoweringFailureKind::RankDomain;
  EXPECT_FALSE(
      wafer::compiler::detail::isRecoverableWholeCardInstrLoweringFailure(
          failure));
  failure.kind =
      wafer::compiler::detail::WholeCardInstrLoweringFailureKind::WorkLedger;
  EXPECT_FALSE(
      wafer::compiler::detail::isRecoverableWholeCardInstrLoweringFailure(
          failure));
}

TEST_F(WholeCardCandidateEvaluationTest,
       ReturnsReadyOrderOnlyAsACompleteRankAction) {
  ...
  // same all-rank action; it never disappears into a partial rank candidates.
...
TEST_F(WholeCardCandidateEvaluationTest,
  ...
  for (const auto &action : *actions) {
    ...
    EXPECT_EQ(action.communicationActionKey->stableOrdinal, 7u);
...
TEST_F(WholeCardCandidateEvaluationTest,
       SerializedDirectDTEComputeIsATypedPreSelectionActionSibling) {
  std::vector<mlir::OwningOpRef<mlir::ModuleOp>> parents;
...
TEST_F(WholeCardCandidateEvaluationTest,
  ...
  auto makeEstimate =
      [](uint32_t ordinal,
         wafer::compiler::detail::CoordinatedReadyOrderKind readyOrderKind,
        ...
        estimate.readyOrderKind = readyOrderKind;
...
TEST_F(WholeCardCandidateEvaluationTest,
  ...
  variant.evaluationReservation = ledger->getMandatoryBaselineReservation();
  ...
  wafer::support::ScopedCompileWorkStatisticsActivation workActivation(
      workSession);
  auto gated = wafer::compiler::detail::evaluateWholeCardTileCandidate(
  ...
  EXPECT_TRUE(llvm::any_of(
      *gated, [](const auto &survivor) { return survivor.reservedBaseline; }));
  ...
  // production candidates: equivalent-cost actions may be deduplicated after
  ...
  EXPECT_EQ(work.evaluationInstructionLowerings, 1u);
  EXPECT_GT(work.evaluationCandidateClones, 1u);
}
...
TEST_F(WholeCardCandidateEvaluationTest,
  ...
  wafer::compiler::detail::WholeCardInstrLoweringFailure failure;
  wafer::compiler::detail::WholeVariantSelectionStatistics statistics;
  auto cursor = wafer::compiler::detail::beginWholeCardCandidateEvaluation(
  ...
  auto seed = wafer::compiler::detail::advanceWholeCardCandidateEvaluation(
      **cursor, variant.evaluationReservation);
  ASSERT_TRUE(mlir::succeeded(seed)) << diagnosticsText;
  ASSERT_EQ(
      seed->kind,
      wafer::compiler::detail::WholeCardCandidateEvaluationStepKind::Accepted)
      << diagnosticsText;
  ...
  auto expansion = wafer::compiler::detail::advanceWholeCardCandidateEvaluation(
      **cursor, *reservation);
  ASSERT_TRUE(mlir::succeeded(expansion)) << diagnosticsText;
  EXPECT_NE(
      expansion->kind,
      wafer::compiler::detail::WholeCardCandidateEvaluationStepKind::Exhausted);
...
TEST_F(WholeCardCandidateEvaluationTest,
  ...
  ASSERT_TRUE(mlir::succeeded(seed)) << diagnosticsText;
  EXPECT_EQ(seed->kind,
            wafer::compiler::detail::WholeCardCandidateEvaluationStepKind::
                RecoverableRejected);
  EXPECT_TRUE((*cursor)->seedAttempted());
  EXPECT_FALSE((*cursor)->exactSeedAccepted());
...
TEST_F(WholeCardCandidateEvaluationTest,
  ...
  auto reservation = ledger->tryReserveCandidateEvaluation(*evaluation);
  ASSERT_TRUE(reservation);

  wafer::compiler::detail::CoordinatedTileVariant variant;
  variant.stableSemanticOrdinal = 1;
  variant.evaluationReservation = *reservation;
  variant.ranks.emplace_back(0, std::move(tile), nullptr);
  auto executionConfig = wafer::compiler::ExecutionConfig::createForSingleCard(
      rankCount, wafer::RuntimeLaunchKind::Kernel);
  ASSERT_TRUE(static_cast<bool>(executionConfig))
  ...
  EXPECT_EQ(failure.kind,
            wafer::compiler::detail::WholeCardInstrLoweringFailureKind::
                WholeVariantExactGate);
  EXPECT_EQ(failure.gate, "no-valid-executable");
...
TEST_F(WholeCardCandidateEvaluationTest,
  ...
  auto ledger =
      wafer::compiler::detail::CoordinatedWorkLedger::create(rankCount);
  ASSERT_TRUE(mlir::succeeded(ledger));
  wafer::compiler::detail::WholeCardCandidateSearchConfig searchConfig;
  ...
  for (const auto &tileParent : *candidates) {
    ...
    ASSERT_TRUE(mlir::succeeded(gated))
        ...
        << diagnosticsText;
    EXPECT_EQ(failure.kind,
              wafer::compiler::detail::WholeCardInstrLoweringFailureKind::None);
    ASSERT_FALSE(gated->empty());
    for (const auto &survivor : *gated) {
...
TEST_F(WholeCardCandidateEvaluationTest,
       ExactAdmittedCandidateSetDigestIsIndependentOfRankParallelism) {
  constexpr int64_t rankCount = 16;
  ...
  auto run = [&](mlir::ModuleOp source,
    ...
    std::vector<wafer::compiler::detail::EvaluatedWholeCardCandidate>
        evaluatedCandidates;
    wafer::compiler::detail::WholeVariantSelectionStatistics statistics;
    ...
    for (const auto &tileParent : *candidates) {
      ...
      if (baseline != evaluatedCandidates.end())
      ...
      for (auto &executable : *gated)
        evaluatedCandidates.push_back(std::move(executable));
    }
    if (llvm::count_if(evaluatedCandidates, [](const auto &candidate) {
          return candidate.reservedBaseline;
        }) != 1)
      return std::nullopt;
    const auto beforeSelection = ledger->getSnapshot();
    if (beforeSelection.evaluationWorkReserved != 0)
      return std::nullopt;
    Result result;
    result.candidateDigest =
        wafer::compiler::detail::computeEvaluatedWholeCardCandidateSetDigest(
            evaluatedCandidates);
    result.candidateCount = evaluatedCandidates.size();
    result.maximumWorkers = statistics.maximumRankPipelineWorkers;
    auto winner = wafer::compiler::detail::selectEvaluatedWholeCardCandidate(
    ...
    if (beforeSelection.capacity != afterSelection.capacity ||
        ...
        beforeSelection.evaluationWorkReserved !=
            afterSelection.evaluationWorkReserved ||
      ...
      return std::nullopt;
    llvm::ArrayRef<wafer::compiler::detail::EvaluatedWholeCardCandidate>
        winnerView(&*winner, 1);
    result.winnerDigest =
        wafer::compiler::detail::computeEvaluatedWholeCardCandidateSetDigest(
            winnerView);
  ...
  EXPECT_EQ(serial->candidateCount, parallel->candidateCount);
  EXPECT_EQ(serial->candidateDigest, parallel->candidateDigest);
...
TEST_F(WholeCardCandidateEvaluationTest,
  ...
  EXPECT_TRUE(foundAdmittedPartial)
      ...
      << ", evaluatedCandidates partial executables: "
      << admittedPartialExecutables
      << ", evaluatedCandidates partial ranks with DTE wait: "
      << admittedPartialRanksWithWait
      << ", last partial failure gate: " << lastPartialFailureGate << '\n'
      << diagnosticsText;
  EXPECT_EQ(ledger->getSnapshot().evaluationWorkReserved, 0u);
}

TEST_F(WholeCardCandidateEvaluationTest,
       ExactOversizedSPMCapacityCheckPreservesTileParentAndClosesAttempt) {
  constexpr int64_t rankCount = 16;
  auto ledger =
      wafer::compiler::detail::CoordinatedWorkLedger::create(rankCount);
  ASSERT_TRUE(mlir::succeeded(ledger));
  ...
  wafer::support::ScopedCompileWorkStatisticsActivation workActivation(
      workSession);
  EXPECT_TRUE(
      mlir::failed(wafer::compiler::detail::evaluateWholeCardTileCandidate(
          variant, program, *executionConfig,
  ...
  EXPECT_EQ(
      ...
      rankCount);
  const wafer::support::CompileWorkStatistics work = workSession->snapshot();
  EXPECT_EQ(work.evaluationInstructionLowerings, 0u);
