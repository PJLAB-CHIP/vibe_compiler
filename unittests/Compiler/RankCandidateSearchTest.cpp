//===- RankCandidateSearchTest.cpp ---------------------------------===//

#include "../../lib/Wafer/Compiler/RankCandidateSearch.h"
...
class RankCandidateSearchTest : public ::testing::Test {
protected:
  RankCandidateSearchTest() {
...
module {
  ...
  mlir::OwningOpRef<mlir::ModuleOp> makeConstantConditionalTileModule() {
    return mlir::parseSourceString<mlir::ModuleOp>(R"mlir(
module {
...
TEST_F(RankCandidateSearchTest,
       ReservesGenerationEvaluationAndRepairBeforeWork) {
  auto evaluation = wafer::compiler::detail::CoordinatedWorkLedger::
      getCandidateEvaluationUpperBound(/*rankCount=*/4);
  auto action = wafer::compiler::detail::CoordinatedWorkLedger::
      getExecutableScheduleAttemptUpperBound(/*rankCount=*/4);
  ASSERT_TRUE(mlir::succeeded(evaluation));
  ASSERT_TRUE(mlir::succeeded(action));
  ASSERT_TRUE(evaluation->getTotal());
  constexpr uint64_t rankCount = 4;
  static_assert(
      wafer::compiler::detail::kMaximumCoordinatedExactScheduleActions == 8,
      "the exact-success candidates must remain independently bounded");
  EXPECT_EQ(
      evaluation->get(
          wafer::compiler::detail::CoordinatedWorkKind::TileToInstrLowering),
      rankCount);
  EXPECT_EQ(evaluation->get(wafer::compiler::detail::CoordinatedWorkKind::
  ...
  uint64_t capacity =
      mandatoryGeneration + *evaluation->getTotal() + repair + 2;
  ...
  EXPECT_EQ(snapshot.evaluationWorkReserved, *evaluation->getTotal());
...
TEST_F(RankCandidateSearchTest,
  ...
  auto completeSeed =
      [&](wafer::compiler::detail::CandidateEvaluationReservation reservation) {
        ...
        return ledger->completeCandidateEvaluation(reservation, actual);
      };
  ...
  auto seedUpperBound = wafer::compiler::detail::CoordinatedWorkLedger::
      getCandidateEvaluationUpperBound(1);
  ASSERT_TRUE(mlir::succeeded(seedUpperBound));
  for (unsigned index = 0; index < 7; ++index) {
    auto reservation = ledger->tryReserveCandidateEvaluation(*seedUpperBound);
  ...
  EXPECT_EQ(snapshot.evaluationWorkReserved, 0u);
  EXPECT_FALSE(ledger->tryReserveExecutableScheduleAttempt());
  EXPECT_FALSE(ledger->tryReserveCandidateEvaluation(*seedUpperBound));
}
...
TEST_F(RankCandidateSearchTest,
  ...
  auto ledger = wafer::compiler::detail::CoordinatedWorkLedger::create(
      /*rankCount=*/4);
  ASSERT_TRUE(mlir::succeeded(ledger));
  wafer::compiler::detail::RankCandidateSearchConfig config;
  config.rankCount = 4;
  auto candidates = wafer::compiler::detail::buildRankCandidateSet(
      *source, config, *ledger);
  ASSERT_TRUE(mlir::succeeded(candidates));
  ASSERT_GT(candidates->size(), 1u);
  const auto &baseline = candidates->front();
  EXPECT_TRUE(baseline.reservedBaseline);
  EXPECT_EQ(baseline.evaluationReservation.id,
  ...
  for (const auto &variant : *candidates) {
    ...
    EXPECT_TRUE(reservations.insert(variant.evaluationReservation.id).second);
    ...
    EXPECT_EQ(variant.candidateSetDigest,
              wafer::compiler::detail::computeRankCandidateSetDigest(
                  *candidates));
  }
  EXPECT_EQ(baselineCount, 1u);
  auto snapshot = ledger->getSnapshot();
  EXPECT_EQ(snapshot.mandatoryGenerationReserved, 0u);
  EXPECT_GT(snapshot.evaluationWorkReserved, 0u);
}
...
TEST_F(RankCandidateSearchTest,
  ...
  wafer::compiler::detail::StructuredCandidateSearchStatistics stats;
  auto session =
      wafer::compiler::detail::RankCandidateSearchSession::create(
  ...
  EXPECT_FALSE((*session)->getActualCandidateSetDigest().empty());
  ...
  EXPECT_EQ(stats.retainedActualCandidates, 1u);
...
TEST_F(RankCandidateSearchTest,
  ...
  auto closeExactWork = [&](const auto &candidate) {
    ...
    actual.set(
        wafer::compiler::detail::CoordinatedWorkKind::ExecutableScheduleAction,
        1);
    return ledger->completeCandidateEvaluation(candidate.evaluationReservation,
                                               actual);
  };
...
TEST_F(RankCandidateSearchTest,
  ...
  auto run = [&](bool reverseProviderOrder,
    ...
    if (mlir::failed(session))
      return std::nullopt;
    std::string structuralDigest =
        (*session)->getStructuredCandidateSetDigest().str();
    while (true) {
      auto next = (*session)->admitNextActualCandidate();
      if (mlir::failed(next))
        return std::nullopt;
...
TEST_F(RankCandidateSearchTest,
       ProviderBudgetCoversResidencyBeforeParallelPartitionDetails) {
  auto source = makeProgram();
  ASSERT_TRUE(source);
  auto ledger = wafer::compiler::detail::CoordinatedWorkLedger::create(1);
  ASSERT_TRUE(mlir::succeeded(ledger));
...
TEST_F(RankCandidateSearchTest,
       ProviderBudgetLeavesHeadroomForCompleteActualCandidate) {
  auto source = makeProgram();
  ASSERT_TRUE(source);
  auto ledger = wafer::compiler::detail::CoordinatedWorkLedger::create(1);
  ASSERT_TRUE(mlir::succeeded(ledger));
...
TEST_F(RankCandidateSearchTest,
  ...
  EXPECT_EQ(stats.implementationAlternativeProposals,
            wafer::compiler::detail::kMaximumStructuredCandidateStates * 3u);
...
TEST_F(RankCandidateSearchTest,
  ...
  const unsigned baselineTransfers =
      countTransfers(*candidates->front().ranks.front().module);
  EXPECT_TRUE(
      llvm::any_of(llvm::drop_begin(*candidates), [&](const auto &variant) {
...
TEST_F(RankCandidateSearchTest,
  ...
  auto materialize = [&](wafer::CompleteRankTraversalComposition composition,
    ...
    auto candidate = wafer::materializeCompleteRankCandidateTileModule(
  ...
  auto candidates = wafer::compiler::detail::buildRankCandidateSet(
      *source, config, *ledger);
  ASSERT_TRUE(mlir::succeeded(candidates));
  EXPECT_TRUE(llvm::any_of(*candidates, [&](const auto &variant) {
...
TEST_F(RankCandidateSearchTest,
  ...
  auto materializeChain = [&](Action action) {
    failureReason.clear();
    auto candidate = wafer::materializeCompleteRankConnectionTileModule(
  ...
  auto mixed = wafer::materializeCompleteRankConnectionTileModule(
...
TEST_F(RankCandidateSearchTest,
  ...
  auto candidate = wafer::materializeCompleteRankConnectionChoicesTileModule(
  ...
  auto crossRegion = wafer::materializeCompleteRankConnectionChoicesTileModule(
      *source, /*logicalRank=*/0,
      llvm::ArrayRef<wafer::CandidateTraversalConnectionChoice>{choice},
      wafer::CandidateBoundaryMovementAction::Staged,
      wafer::CandidateLoopMovementAction::AsConstructed, &failureReason);
...
TEST_F(RankCandidateSearchTest,
  ...
  auto rankZero = wafer::materializeCompleteRankCandidateTileModule(
      *source, /*logicalRank=*/0, /*candidateTileSizes=*/{8},
      /*candidateReductionTileSizes=*/{},
  ...
  ASSERT_TRUE(mlir::succeeded(ledger->completeCandidateEvaluation(
      parent.evaluationReservation, failedAttempt)));
  ...
  EXPECT_NE(repaired->evaluationReservation.id,
            parent.evaluationReservation.id);
...
TEST_F(RankCandidateSearchTest,
       ExactDirectMappedBoundaryMovementIsAnActualCandidateSetSibling) {
...
TEST_F(RankCandidateSearchTest,
  ...
  ASSERT_TRUE(source);
  std::string materializationFailure;
  auto direct = wafer::materializeCompleteRankCandidateTileModule(
  ...
  auto full = wafer::materializeCompleteRankCandidateTileModule(
  ...
  EXPECT_TRUE(candidates->front().reservedBaseline);
  ASSERT_EQ(candidates->front().ranks.size(), 2u);
  for (const auto &rank : candidates->front().ranks) {
    unsigned branches = 0;
    unsigned loads = 0;
...
TEST_F(RankCandidateSearchTest,
       CandidateSetDigestIsIndependentOfGenerationParallelism) {
  ...
  wafer::compiler::detail::RankCandidateSearchConfig serial;
  serial.rankCount = 4;
  serial.candidateParallelism = 1;
  auto parallel = serial;
  parallel.candidateParallelism = 8;
  wafer::compiler::detail::StructuredCandidateSearchStatistics serialStatistics;
  wafer::compiler::detail::StructuredCandidateSearchStatistics
      parallelStatistics;
  auto first = wafer::compiler::detail::buildRankCandidateSet(
      *firstSource, serial, *firstLedger, &serialStatistics);
  auto second = wafer::compiler::detail::buildRankCandidateSet(
  ...
  // complete rank domain; candidateParallelism cannot change candidate order
  // or introduce competing reservations.
  EXPECT_EQ(parallelStatistics.connectionWorkerCount, 1u);
  EXPECT_EQ(
      wafer::compiler::detail::computeRankCandidateSetDigest(*first),
      wafer::compiler::detail::computeRankCandidateSetDigest(*second));
}
...
TEST_F(RankCandidateSearchTest,
  ...
  auto run = [&](mlir::OwningOpRef<mlir::ModuleOp> source,
    ...
    auto candidates = wafer::compiler::detail::buildRankCandidateSet(
        *source, config, *ledger, &stats);
    EXPECT_TRUE(mlir::succeeded(candidates));
    if (mlir::failed(candidates))
      return stats;
    ...
    auto evaluation = wafer::compiler::detail::CoordinatedWorkLedger::
        getCandidateEvaluationUpperBound(1);
    EXPECT_TRUE(mlir::succeeded(evaluation));
    EXPECT_TRUE(evaluation->getTotal());
    EXPECT_EQ(ledger->getSnapshot().evaluationWorkReserved,
              candidates->size() * *evaluation->getTotal());
    return stats;
  };
...
TEST_F(RankCandidateSearchTest,
  ...
  ASSERT_TRUE(mlir::succeeded(evaluation));
  ASSERT_TRUE(evaluation->getTotal());
  const uint64_t capacity = 2 + *evaluation->getTotal();
  ...
  auto parallel = serial;
  parallel.candidateParallelism = 8;
  wafer::compiler::detail::StructuredCandidateSearchStatistics firstStats;
  wafer::compiler::detail::StructuredCandidateSearchStatistics secondStats;
  auto first = wafer::compiler::detail::buildRankCandidateSet(
  ...
  EXPECT_EQ(
      wafer::compiler::detail::computeRankCandidateSetDigest(*first),
      wafer::compiler::detail::computeRankCandidateSetDigest(*second));
  EXPECT_EQ(firstStats.evaluationReservationDenied, 0u);
  EXPECT_EQ(secondStats.evaluationReservationDenied, 0u);
  EXPECT_EQ(firstStats.generationReservationDenied, 0u);
  EXPECT_EQ(secondStats.generationReservationDenied, 0u);
...
TEST_F(RankCandidateSearchTest,
  ...
  using Facts = wafer::compiler::detail::StructuredCandidateCost;
  using Dimension = wafer::compiler::detail::StructuredCandidateCostDimension;
  using Knowledge = wafer::compiler::detail::StructuredCandidateMetricKnowledge;
  using Order = wafer::compiler::detail::StructuredCandidateDominance;
  ...
  EXPECT_EQ(
      wafer::compiler::detail::compareStructuredCandidateCost(left, right),
      Order::LeftDominates);
  ...
  EXPECT_EQ(wafer::compiler::detail::compareStructuredCandidateCost(
...
TEST_F(RankCandidateSearchTest,
  ...
  using Facts = wafer::compiler::detail::StructuredCandidateCost;
  using Dimension = wafer::compiler::detail::StructuredCandidateCostDimension;
  using View = wafer::compiler::detail::StructuredCandidateCostView;
  ...
  auto run = [&](std::array<size_t, 5> order) {
    ...
    for (size_t candidateIndex : order) {
      ...
      auto plan = wafer::compiler::detail::planStructuredParetoInsertion(
          {static_cast<int64_t>(candidateIndex), false, &facts[candidateIndex]},
          existing);
...
TEST_F(RankCandidateSearchTest,
  ...
  auto makeVariant = [](mlir::OwningOpRef<mlir::ModuleOp> module) {
    ...
    variant.evaluationReservation.id = 1;
    variant.ranks.emplace_back(/*logicalRank=*/0, std::move(module), nullptr);
    return variant;
  };
  ...
  auto stagedFacts =
      wafer::compiler::detail::deriveStructuredCandidateCost(stagedVariant);
  auto renamedFacts =
      wafer::compiler::detail::deriveStructuredCandidateCost(renamedVariant);
  auto directFacts =
      wafer::compiler::detail::deriveStructuredCandidateCost(directVariant);
  ...
  EXPECT_EQ(wafer::compiler::detail::compareStructuredCandidateCost(
            ...
            wafer::compiler::detail::StructuredCandidateDominance::Equivalent);
  EXPECT_EQ(
      wafer::compiler::detail::compareStructuredCandidateCost(*stagedFacts,
                                                              *directFacts),
      wafer::compiler::detail::StructuredCandidateDominance::Incomparable);
}
...
module {
  ...
  const auto &compute = facts->selection[static_cast<size_t>(
      wafer::compiler::detail::StructuredCandidateCostDimension::
          ComputeAggregateWork)];
  EXPECT_EQ(compute.knowledge,
            wafer::compiler::detail::StructuredCandidateMetricKnowledge::Known);
...
module {
  ...
  EXPECT_EQ(
      movement.knowledge,
      wafer::compiler::detail::StructuredCandidateMetricKnowledge::Overflow);
  EXPECT_FALSE(movement.disposition.empty());
}

TEST_F(RankCandidateSearchTest,
       ProductionReductionKeepsExactEvaluationReservations) {
  auto run = [&](mlir::OwningOpRef<mlir::ModuleOp> source) {
    auto ledger = wafer::compiler::detail::CoordinatedWorkLedger::create(1);
    EXPECT_TRUE(mlir::succeeded(ledger));
    wafer::compiler::detail::RankCandidateSearchConfig config;
    ...
    if (mlir::failed(candidates))
      return uint64_t{0};
    EXPECT_EQ(stats.retainedStates, candidates->size());
...
TEST_F(RankCandidateSearchTest,
  ...
  // executable evaluation, while target ring/tree selection remains a
...
TEST_F(RankCandidateSearchTest,
       PeerMessagesRequireAllAndOnlyTypedEndpointsBeforeEvaluation) {
...
module {
  ...
  conditionVariant.evaluationReservation.id = 2;
...
module {
  ...
  ASSERT_TRUE(mlir::succeeded(
      wafer::compiler::detail::verifyCoordinatedStructuredCommunication(matched,
                                                                        2)));
  auto facts = wafer::compiler::detail::deriveStructuredCandidateCost(matched);
  ASSERT_TRUE(mlir::succeeded(facts));
  const auto &payload = facts->selection[static_cast<size_t>(
      wafer::compiler::detail::StructuredCandidateCostDimension::
          NoCAggregatePayloadBytes)];
  EXPECT_EQ(
      payload.knowledge,
      wafer::compiler::detail::StructuredCandidateMetricKnowledge::Estimated);
