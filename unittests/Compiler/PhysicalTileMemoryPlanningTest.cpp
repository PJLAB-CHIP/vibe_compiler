//===- PhysicalTileMemoryPlanningTest.cpp ---------------------------------===//

#include "../../lib/Wafer/Compiler/PhysicalTileMemoryPlanning.h"
...
class PhysicalTileMemoryPlanningTest : public ::testing::Test {
protected:
  PhysicalTileMemoryPlanningTest() {
...
TEST_F(PhysicalTileMemoryPlanningTest,
  ...
  wafer::compiler::detail::PhysicalTileMemoryPlanningFailure failure;
  auto memoryPlanned = wafer::compiler::detail::planPhysicalTileMemory(
      std::move(module), &failure);
  EXPECT_TRUE(mlir::failed(memoryPlanned));
  EXPECT_EQ(
      failure.kind,
      wafer::compiler::detail::PhysicalTileMemoryPlanningFailureKind::Contract);
  EXPECT_NE(diagnostics.find(
                "physical_tile_memory_planning_requires_canonical_instr_ir"),
            std::string::npos)
      << diagnostics;
}

TEST_F(PhysicalTileMemoryPlanningTest, ReportsSPMFailureForOwnedTileModule) {
  mlir::OwningOpRef<mlir::ModuleOp> module =
  ...
  EXPECT_EQ(failure.kind,
            wafer::compiler::detail::PhysicalTileMemoryPlanningFailureKind::
                SPMAllocation);
...
TEST_F(PhysicalTileMemoryPlanningTest,
       RejectsWholeCardPlacementBeforeTileMemoryPlanning) {
...
TEST_F(PhysicalTileMemoryPlanningTest,
  ...
  wafer::support::ScopedCompileWorkStatisticsActivation workActivation(
      workSession);
  auto memoryPlanned =
      wafer::compiler::detail::planPhysicalTileMemory(std::move(module));
  ...
  EXPECT_EQ(work.physicalTileMemoryPlanningInvocations, 1u);
