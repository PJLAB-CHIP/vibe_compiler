//===- RankCandidateEvaluationTest.cpp
//----------------------------===//

#include "../../lib/Wafer/Compiler/RankCandidateEvaluation.h"
#include "../../lib/Wafer/Compiler/CompilationInternal.h"
#include "../../lib/Wafer/Compiler/RankCandidateSelection.h"
...
class RankCandidateEvaluationTest : public ::testing::Test {
protected:
  RankCandidateEvaluationTest() {
  ...
  mlir::OwningOpRef<mlir::ModuleOp> makeOversizedTileModule(int64_t elements) {
    std::string source;
    llvm::raw_string_ostream os(source);
    os << R"mlir(module {
...
module {
  ...
  mlir::OwningOpRef<mlir::ModuleOp>
  makeRepairableSPMPressureTileModule(int64_t elements) {
    std::string source;
    llvm::raw_string_ostream os(source);
    os << R"mlir(module {
  wafer.target.topology @default
  ...
  mlir::OwningOpRef<mlir::ModuleOp>
  makeDDRBoundaryRepairableSPMPressureTileModule(int64_t elements) {
    std::string source;
    llvm::raw_string_ostream os(source);
    os << R"mlir(module {
  wafer.target.topology @default
...
module {
  ...
  mlir::OwningOpRef<mlir::ModuleOp> makeReadyOrderTileModule() {
    return mlir::parseSourceString<mlir::ModuleOp>(R"mlir(
module {
  wafer.target.topology @default
      {card_grid = array<i64: 1, 1>, card_interconnect = "mesh",
...
TEST_F(RankCandidateEvaluationTest,
  ...
  auto tile = makeReadyOrderTileModule();
  ASSERT_TRUE(tile);
  ...
  wafer::support::ScopedCompileWorkStatisticsActivation workActivation(
      workSession);
  auto gated = wafer::compiler::detail::evaluateRankCandidate(
...
TEST_F(RankCandidateEvaluationTest,
  ...
  wafer::compiler::detail::RankInstrLoweringFailure failure;
  wafer::compiler::detail::RankCandidateSearchStatistics statistics;
  auto cursor = wafer::compiler::detail::beginRankCandidateEvaluation(
  ...
  auto expansion = wafer::compiler::detail::advanceRankCandidateEvaluation(
      **cursor, *reservation);
  ASSERT_TRUE(mlir::succeeded(expansion)) << diagnosticsText;
  EXPECT_NE(
      expansion->kind,
      wafer::compiler::detail::RankCandidateEvaluationStepKind::Exhausted);
...
TEST_F(RankCandidateEvaluationTest,
  ...
  ASSERT_TRUE(mlir::succeeded(seed)) << diagnosticsText;
  EXPECT_EQ(seed->kind,
            wafer::compiler::detail::RankCandidateEvaluationStepKind::
                RecoverableRejected);
  EXPECT_TRUE((*cursor)->seedAttempted());
  EXPECT_FALSE((*cursor)->exactSeedAccepted());
...
TEST_F(RankCandidateEvaluationTest,
  ...
  auto gated = wafer::compiler::detail::evaluateRankCandidate(
      ...
      wafer::OptimizationConfig::production(), *ledger, diagnostics, failure,
      /*statistics=*/nullptr,
      wafer::compiler::detail::RankCandidateSelectionMode::ReservedBaseline);
  ASSERT_TRUE(mlir::succeeded(gated)) << diagnosticsText;
...
TEST_F(RankCandidateEvaluationTest,
  ...
  ASSERT_TRUE(static_cast<bool>(executionConfig))
      << llvm::toString(executionConfig.takeError());
  wafer::analysis::CardInstructionProgramCost zeroBaseline;
  std::string diagnosticsText;
  llvm::raw_string_ostream diagnostics(diagnosticsText);
  wafer::compiler::detail::RankInstrLoweringFailure failure;
  wafer::compiler::detail::RankCandidateSearchStatistics statistics;
  auto gated = wafer::compiler::detail::evaluateRankCandidate(
      ...
      wafer::OptimizationConfig::production(), *ledger, diagnostics, failure,
      &statistics,
      wafer::compiler::detail::RankCandidateSelectionMode::Production,
      &zeroBaseline);
  EXPECT_TRUE(mlir::failed(gated));
  EXPECT_EQ(failure.kind,
            wafer::compiler::detail::RankInstrLoweringFailureKind::
...
TEST_F(RankCandidateEvaluationTest,
  ...
  auto ledger =
      wafer::compiler::detail::CoordinatedWorkLedger::create(rankCount);
  ASSERT_TRUE(mlir::succeeded(ledger));
  wafer::compiler::detail::RankCandidateSearchConfig searchConfig;
  ...
  for (const auto &tileParent : *candidates) {
    ...
    ASSERT_TRUE(mlir::succeeded(gated))
        ...
        << diagnosticsText;
    EXPECT_EQ(failure.kind,
              wafer::compiler::detail::RankInstrLoweringFailureKind::None);
    ASSERT_FALSE(gated->empty());
    for (const auto &survivor : *gated) {
...
TEST_F(RankCandidateEvaluationTest,
  ...
  auto run = [&](mlir::ModuleOp source,
    ...
    std::vector<wafer::compiler::detail::EvaluatedRankCandidate>
        evaluatedCandidates;
    wafer::compiler::detail::RankCandidateSearchStatistics statistics;
    ...
    auto winner = wafer::compiler::detail::selectEvaluatedRankCandidate(
...
TEST_F(RankCandidateEvaluationTest,
  ...
  variant.evaluationReservation = ledger->getMandatoryBaselineReservation();
  for (int64_t rank = 0; rank < rankCount; ++rank) {
    auto module = makeOversizedTileModule(/*elements=*/2'000'000);
    ASSERT_TRUE(module);
