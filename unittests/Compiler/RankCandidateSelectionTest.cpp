//===- RankCandidateSelectionTest.cpp ------------------------------===//
...
using wafer::compiler::detail::RankCandidateCostView;
using wafer::compiler::detail::RankCandidateSelection;
using wafer::compiler::detail::RankQualificationSelection;
using wafer::compiler::detail::RankSelectionBasis;
using wafer::compiler::detail::RankCandidateSelectionMode;

static CardInstructionProgramCost makeCost(uint64_t ddrReadBytes) {
  CardInstructionProgramCost cost;
  cost.aggregateDDRReadBytes.value = ddrReadBytes;
  return cost;
}

static void addSingleRankCost(CardInstructionProgramCost &cost) {
...
static mlir::FailureOr<RankCandidateSelection>
plan(std::vector<RankCandidateCostView> views,
     RankCandidateSelectionMode mode = RankCandidateSelectionMode::Production) {
  return wafer::compiler::detail::selectRankCandidateByCost(views, mode);
}

TEST(RankCandidateSelectionTest,
     SelectsOnlyACompleteTargetModelMarginWinner) {
  CardInstructionProgramCost baseline = makeCost(1'000'000'000);
  CardInstructionProgramCost candidate = makeCost(100'000'000);
  std::vector<RankCandidateCostView> views = {
      {0, true, &baseline},
      {7, false, &candidate},
  };
  ...
  EXPECT_EQ(result->basis, RankSelectionBasis::EstimatedBenefit);
...
TEST(RankCandidateSelectionTest,
  ...
  CardInstructionProgramCost baseline = makeCost(1'000'000'000);
  CardInstructionProgramCost candidate = makeCost(1);
  std::vector<RankCandidateCostView> views = {
      {0, true, &baseline},
      {1, false, &candidate},
  };

  auto result = plan(views, RankCandidateSelectionMode::ReservedBaseline);
  ASSERT_TRUE(mlir::succeeded(result));
  EXPECT_EQ(result->selectedIndex, 0u);
  EXPECT_EQ(result->basis, RankSelectionBasis::ReservedBaseline);
}
...
TEST(RankCandidateSelectionTest,
  ...
  CardInstructionProgramCost baseline = makeCost(1'000'000'000);
  CardInstructionProgramCost candidate = makeCost(995'000'000);
...
TEST(RankCandidateSelectionTest,
  ...
  CardInstructionProgramCost baseline = makeCost(150'000'000);
  CardInstructionProgramCost candidate = makeCost(149'000'000);
...
TEST(RankCandidateSelectionTest,
  ...
  CardInstructionProgramCost baseline = makeCost(0);
  CardInstructionProgramCost candidate = makeCost(0);
  baseline.aggregateCompute.vectorF16Bf16LogicalOps.value = 64'000'000;
...
TEST(RankCandidateSelectionTest,
  ...
  CardInstructionProgramCost baseline = makeCost(595'722'240);
  CardInstructionProgramCost candidate = makeCost(8'519'680);
...
TEST(RankCandidateSelectionTest,
  ...
  CardInstructionProgramCost baseline = makeCost(0);
  CardInstructionProgramCost candidate = baseline;
...
TEST(RankCandidateSelectionTest,
  ...
  CardInstructionProgramCost baseline = makeCost(1'000'000'000);
  CardInstructionProgramCost first = makeCost(100'000'000);
  CardInstructionProgramCost second = first;
  std::vector<RankCandidateCostView> views = {
      {0, true, &baseline},
      {9, false, &first},
      {3, false, &second},
...
TEST(RankCandidateSelectionTest,
  ...
  CardInstructionProgramCost baseline = makeCost(2'000'000'000);
  CardInstructionProgramCost ddrCandidate = makeCost(100'000'000);
  CardInstructionProgramCost computeCandidate = makeCost(0);
...
TEST(RankCandidateSelectionTest, DecisionIsIndependentOfCandidateOrder) {
  CardInstructionProgramCost baseline = makeCost(1'000'000'000);
  CardInstructionProgramCost worse = makeCost(400'000'000);
  CardInstructionProgramCost winner = makeCost(100'000'000);
  std::vector<RankCandidateCostView> first = {
      {0, true, &baseline}, {8, false, &worse}, {4, false, &winner}};
  std::vector<RankCandidateCostView> second = {
...
TEST(RankCandidateSelectionTest, RejectsMissingOrDuplicateBaseline) {
  CardInstructionProgramCost cost = makeCost(1);
...
protected:
  ...
  wafer::compiler::detail::EvaluatedRankCandidate
  makeExecutable(int64_t semanticOrdinal, uint32_t actionOrdinal,
    ...
    for (int64_t logicalRank = 0; logicalRank < 2; ++logicalRank) {
      ...
      EXPECT_TRUE(module);
      ranks.push_back(wafer::compiler::CardExecutableBuilder::makeRank(
    ...
    wafer::compiler::detail::RankExecutableSet accepted(
        std::move(ranks), std::move(launch),
        wafer::analysis::CardInstructionProgramCost{});
    return wafer::compiler::detail::EvaluatedRankCandidate(
        semanticOrdinal, reservedBaseline, std::move(accepted), actionOrdinal,
        wafer::compiler::detail::RankScheduleActionKey{},
...
TEST_F(CoordinatedQualificationSelectionTest,
  ...
  std::vector<wafer::compiler::detail::EvaluatedRankCandidate> variants;
  ...
  mlir::FailureOr<RankQualificationSelection> selected =
      wafer::compiler::detail::selectRankQualificationCandidate(
          variants, RankCandidateSelectionMode::CharacterizeAllGatherDirect);
...
TEST_F(CoordinatedQualificationSelectionTest,
  ...
  mlir::FailureOr<RankQualificationSelection> selected =
      wafer::compiler::detail::selectRankQualificationCandidate(
          variants, RankCandidateSelectionMode::QualifyNoCResidentAllReduceRing);
  ASSERT_TRUE(mlir::succeeded(selected));
  ...
  mlir::FailureOr<wafer::compiler::detail::EvaluatedRankCandidate> winner =
      wafer::compiler::detail::selectEvaluatedRankCandidate(
          std::move(variants),
          RankCandidateSelectionMode::QualifyNoCResidentAllReduceRing,
          diagnosticsStream);
...
TEST_F(CoordinatedQualificationSelectionTest,
  ...
  ASSERT_TRUE(mlir::succeeded(
      wafer::compiler::detail::reduceEvaluatedRankCandidates(
          variants, RankCandidateSelectionMode::CharacterizeAllGatherDirect)));
