//===- StructuredDAGScheduleTest.cpp -----------------------------------------===//

#include "../../lib/Wafer/Compiler/StructuredDAGSchedule.h"
...
SymbolicWaveClass wave(StructuredDAGNodeID node, SymbolicWaveKind kind) {
  return {node, kind};
}

llvm::SmallVector<TileId, 4>
tiles(std::initializer_list<int64_t> values) {
  llvm::SmallVector<TileId, 4> result;
  for (int64_t value : values)
    result.push_back(TileId(value));
  return result;
}

class StructuredDAGScheduleTest : public ::testing::Test {
protected:
  StructuredDAGScheduleTest() {
  ...
  StructuredDAGAnalysis buildDAG(mlir::ModuleOp module) {
    ...
    auto dag = StructuredDAGAnalysis::create(getOnlyFunction(module), &failureReason);
  ...
  StructuredDAGScheduleState buildState(const StructuredDAGAnalysis &dag,
                                   llvm::ArrayRef<TileId> tileIDs) {
    std::string failureReason;
    auto state = StructuredDAGScheduleState::create(dag, tileIDs, &failureReason);
  ...
  StructuredDAGScheduleEvent runWave(StructuredDAGScheduleState &state,
    ...
    return mlir::succeeded(event) ? std::move(*event) : StructuredDAGScheduleEvent{};
  }
...
TEST_F(StructuredDAGScheduleTest, BuildsStableDiamondNodeEdgeAndWaveIDs) {
  ...
  EXPECT_EQ(dag.getObservableDependencyComponents().front().nodes,
            (llvm::SmallVector<StructuredDAGNodeID, 4>{0, 1, 2, 3}));
  ...
  llvm::SmallVector<std::tuple<StructuredDAGNodeID, StructuredDAGNodeID>, 4> edges;
  for (const StructuredDAGEdge &edge : dag.getEdges())
    edges.emplace_back(edge.producer, edge.consumer);
  EXPECT_EQ(edges,
            (llvm::SmallVector<std::tuple<StructuredDAGNodeID, StructuredDAGNodeID>, 4>{
                {0, 1}, {0, 2}, {1, 3}, {2, 3}}));

  auto secondModule = parse(kDiamondProgram);
  ASSERT_TRUE(secondModule);
  StructuredDAGAnalysis second = buildDAG(*secondModule);
  ...
  StructuredDAGScheduleState state = buildState(dag, tiles({8, 2, 5}));
  ASSERT_EQ(state.getTileStates().size(), 3u);
  EXPECT_EQ(state.getTileStates()[0].tile, TileId(2));
  EXPECT_EQ(state.getTileStates()[1].tile, TileId(5));
  EXPECT_EQ(state.getTileStates()[2].tile, TileId(8));
  EXPECT_EQ(state.getReadyWaves(), (llvm::SmallVector<SymbolicWaveClass, 8>{
                                       wave(0, SymbolicWaveKind::Prologue)}));
}

TEST_F(StructuredDAGScheduleTest,
...
module {
  ...
  EXPECT_EQ(dag.getObservableDependencyComponents()[0].nodes,
            (llvm::SmallVector<StructuredDAGNodeID, 4>{0}));
  ...
  EXPECT_EQ(dag.getObservableDependencyComponents()[1].nodes,
            (llvm::SmallVector<StructuredDAGNodeID, 4>{1}));
...
module {
  ...
  EXPECT_FALSE(dag.supportsIndependentComponentPlacement());
  EXPECT_EQ(dag.getObservableDependencyComponents().front().nodes,
            (llvm::SmallVector<StructuredDAGNodeID, 4>{0, 1, 2}));
  EXPECT_EQ(dag.getObservableDependencyComponents().front().observableOutputs,
...
TEST_F(StructuredDAGScheduleTest,
  ...
  StructuredDAGScheduleState first = buildState(dag, tiles({3, 0}));
  StructuredDAGScheduleState second = buildState(dag, tiles({0, 3}));
  ...
  EXPECT_EQ(first.getTileState(TileId(0))->availableTime, 4u);
  EXPECT_EQ(first.getTileState(TileId(3))->availableTime, 4u);
}
...
TEST_F(StructuredDAGScheduleTest,
  ...
  StructuredDAGScheduleState state = buildState(dag, tiles({0, 1}));
  ...
  StructuredDAGScheduleEvent producerPDone =
  ...
  StructuredDAGScheduleEvent consumerTDone =
      runWave(state, consumerT, tiles({1}), 1);
  EXPECT_EQ(consumerTDone.time, 12u);
  EXPECT_TRUE(state.isComplete());
}

TEST_F(StructuredDAGScheduleTest, DiamondFanoutAndFaninUseEventReadiness) {
  auto module = parse(kDiamondProgram);
  ASSERT_TRUE(module);
  StructuredDAGAnalysis dag = buildDAG(*module);
  StructuredDAGScheduleState state = buildState(dag, tiles({0, 1, 2}));
...
TEST_F(StructuredDAGScheduleTest, RejectsOverlappingTileSetsAtOneEvent) {
  auto module = parse(kIndependentProgram);
  ASSERT_TRUE(module);
  StructuredDAGAnalysis dag = buildDAG(*module);
  StructuredDAGScheduleState state = buildState(dag, tiles({0, 1, 2}));
  ...
  EXPECT_EQ(state.getRunningWaves()[1].tiles,
            (llvm::SmallVector<TileId, 4>{TileId(0),
                                                  TileId(2)}));
}
