...
TEST(TargetModelMemoryTest,
  ...
  EXPECT_NE(error.find("address-misaligned"), std::string::npos);
  EXPECT_EQ(llvm::cantFail(registry.readSnapshot(
                0, TargetModelAddressSpace::RankSPM, spm, 2, 1)),
  ...
  ASSERT_FALSE(registry.applyAtomically(
      {TargetModelByteWrite{0,
       ...
       TargetModelByteWrite{0,
                            ...
                            everyOtherByte}}));
  EXPECT_EQ(llvm::cantFail(registry.readSnapshot(
                0, TargetModelAddressSpace::RankSPM, interleaved, 4, 1)),
            (std::vector<uint8_t>{1, 2, 3, 4}));
...
TEST(TargetModelMemoryTest,
  ...
  // individual segments. The first segment is writable rank-0 output, while
  // the last resolves to rank-1 read-only input. Neither this descriptor nor
  // the valid write before it may publish any bytes.
