...
#include "Wafer/Transforms/Transport/NoCCommunicationAction.h"
...
class NoCIntermediateRoutingTest : public ::testing::Test {
protected:
  NoCIntermediateRoutingTest() {
...
module {
  ...
  mlir::OwningOpRef<mlir::ModuleOp> parseOutputWriterRank() {
    return mlir::parseSourceString<mlir::ModuleOp>(
        R"mlir(
module {
  func.func @main(
...
TEST_F(NoCIntermediateRoutingTest,
...
TEST_F(NoCIntermediateRoutingTest,
       RoutesEquivalentProducedValueToEveryRequiredOutputWriter) {
  ...
  for (size_t rank = 0; rank < 2; ++rank) {
    auto module = parseOutputWriterRank();
    ASSERT_TRUE(module);
  ...
  EXPECT_EQ(wafer::compiler::detail::materializeNoCReplicatedOutputDataflow(
...
TEST_F(NoCIntermediateRoutingTest,
  ...
  EXPECT_FALSE(wafer::compiler::detail::hasNoCIntermediateRoutingOpportunity(
      parents, program));
  int64_t communicationId = 0;
  EXPECT_EQ(wafer::compiler::detail::materializeNoCIntermediateRoutes(
...
TEST_F(NoCIntermediateRoutingTest,
  ...
  EXPECT_TRUE(wafer::compiler::detail::hasNoCIntermediateRoutingOpportunity(
      parents, program));
