...
| native wrapper | `TsmDataMove::{Transpose, Nchw2nhwc, Nhwc2nchw, Pad, Img2col, TensorNom}` | public wrapper/header 暴露能力，CRT 只是少量调用样例；`ChannelNorm` 样例更偏向 `GatherScatter` | V0 production 只允许 pad/img2col；transpose-like movement 和 TensorNom 不作为 V0 native lowering surface，继续用 GatherScatter 或结构化失败；concat/maskgather variants 另行设计 |
