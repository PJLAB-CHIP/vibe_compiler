...
| 阶段 | 纳入范围 | 暂不纳入原因 |
| --- | --- | --- |
| V0 | RDMA/WDMA contiguous 和 strided descriptor；CT elementwise/relation/logic/activation/convert 子集；native `TsmReduce` `sum/avg/max/min`；NE GEMM 基础子集；基础 Conv/Depthwise/BackwardConv；Pool/UnPool；structured TDMA DataMove；public-wrapper Peripheral；GatherScatter；Direct DTE unicast。LLM 主线优先 GEMM/attention/reduce/layout materialization，Conv V0 不含 bias/scale/sparse/INT8/fused activation，psum 与 input feature 同 dtype/layout | packet 和 wrapper 路径相对清楚；Conv/Pool/Peripheral 的 target LLVM lowering 和板端 case 仍按后续 gate 验证 |
...
| native wrapper | `TsmDataMove::{Transpose, Nchw2nhwc, Nhwc2nchw, Pad, Img2col, TensorNom}` | public wrapper/header 暴露能力，CRT 只是少量调用样例；`ChannelNorm` 样例更偏向 `GatherScatter` | 已进入 instruction IR；`TensorNom` 不作为 V0 channelnorm 主路径；concat/maskgather variants 另行设计 |
