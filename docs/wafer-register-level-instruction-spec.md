...
| native wrapper | `TsmPool`、`TsmUnPool` | pool/unpool wrapper header 存在 | 已进入 instruction IR；Pool/UnPool 输入输出默认 aligned，semantic layout 为 `NHWC`；target LLVM call 已生成，CRT/golden packet 仍待验证 |
