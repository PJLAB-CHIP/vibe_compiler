...
| IR 需求 | 推荐发射 |
| --- | --- |
| layout transpose/permutation | V0 compiler 主路径全部用 `GatherScatter` 或结构化失败；硬件/现有后端虽然重点支持 `[0,2,1,3]`、`[0,2,3,1]`、`[0,3,1,2]` 等少数 permutation，但不作为默认 production lowering surface |
...
| 阶段 | 纳入范围 | 暂不纳入原因 |
| --- | --- | --- |
| V0 | RDMA/WDMA contiguous 和 strided descriptor；CT elementwise/relation/logic/activation/convert 子集；native `TsmReduce` `sum/avg/max/min`；NE GEMM 基础子集；基础 Conv/Depthwise/BackwardConv；Pool/UnPool；TDMA pad/img2col；public-wrapper Peripheral；GatherScatter；Direct DTE unicast。LLM 主线优先 GEMM/attention/reduce/layout materialization，Conv V0 不含 bias/scale/sparse/INT8/fused activation，psum 与 input feature 同 dtype/layout；layout transpose/permutation/TensorNom 主路径走 GatherScatter | packet 和 wrapper 路径相对清楚；target LLVM call emission 已覆盖 V0 production surface；Wafer CRT symbol closure、Conv/Pool/Peripheral 的 CRT/golden packet 和板端 case 仍按后续 gate 验证 |
| V1 | reduce_mul composite/helper；TDMA concat/maskgather variants；Raw DTE non-unicast collective ABI（broadcast/scatter/shuffle，gather 需要单独确认真实编码） | 语义、参数约束、性能收益以及自定义 Wafer communication ABI 都需要板端验证 |
...
| wrapper 调用样例 | `TsmArith`、`TsmRelation`、`TsmLogic`、`TsmTranscendental`、`TsmActivation` | Tx81 CRT 有大量 VV/VS/unary/bool 调用样例 | target LLVM call emission 生成 Wafer-owned `wafer_tx81_*` elementwise call；`__*` 只可作为 Wafer CRT 内部实现证据，不作为 compiler ABI；不继承 CRT 的同步策略 |
| wrapper 调用样例 | `TsmConvert` | Tx81 CRT 有 INT8/INT16/INT32/BF16/FP16/FP32/TF32 普通转换样例 | target LLVM call emission 生成 Wafer-owned convert call；MXFP 另做 helper，不当作单条 convert 指令 |
...
| native wrapper | `TsmPool`、`TsmUnPool` | pool/unpool wrapper header 存在 | 已进入 instruction IR；Pool/UnPool 输入输出默认 aligned，semantic layout 为 `NHWC`；target LLVM call emission 已生成，CRT/golden packet 和 symbol closure 仍待验证 |
| native wrapper | `TsmDataMove::{Mirror, Transpose, Rotate90/180/270, Nchw2nhwc, Nhwc2nchw, Pad, Img2col, TensorNom}` | public wrapper/header 暴露能力，CRT 只是少量调用样例；`ChannelNorm` 样例更偏向 `GatherScatter` | V0 production target surface 只允许 pad/img2col；transform-like movement 和 TensorNom 不作为 V0 production target surface，继续用 GatherScatter materialize 或结构化失败；concat/maskgather variants 另行设计 |
| native wrapper | `TsmPeripheral::{Count, ArgMax, ArgMin, Bilinear, Lut16, Lut32, RandGen, Factorize, ElemMask}` | public wrapper/header 暴露能力，CRT 只是少量调用样例 | 已进入 instruction IR；bitcount 因没有 public wrapper 不纳入 |
