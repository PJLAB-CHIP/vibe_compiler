...
| [tx81-current-crt-conformance-matrix.md](tx81-current-crt-conformance-matrix.md) | Current Wafer CRT conformance matrix derived from the old TX81 CRT source audit: direct-wrapper-derived families, public-header-only families, intentional exclusions, and static checker gates. |
| [tx81-extended-crt-surface-triage.md](tx81-extended-crt-surface-triage.md) | Extended CRT surface triage for old TX81 source functions that are not in the current production closure: promote-now candidates, composite/layout/DTE requirements, and permanent exclusions. |
