...
| GEMM | `__Gemm` records input/config/output calls and optional feature setters；current V5.6 Kcore source与SDK example均显示RHS raw hardware transpose bit和semantic transpose相反，semantic NN发`SetTransflag(0, 1)` | Wafer CRT保持public call中的orientation为semantic值，只在packet wrapper边界把RHS bit取反，发`(lhs_orientation, !rhs_orientation)`；psum由显式operand启用，input/output/psum各传独立format；缺省psum使用`Fmt_UNUSED`关闭。F32 psum与独立F16/BF16 output的三段K组合已board-observed，输入partial回读未改变；同址复用未资格化，compiler/model拒绝。bias、scale、quant和activation仍显式关闭 |
