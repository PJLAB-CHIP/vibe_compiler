...
- Explicit non-goals:
  ...
  外推到其它firmware/runtime/SKU，不以reset/power cycle作为测试步骤，不用缺失typed gate与本轮会话资格的
...
后续queue边界probe使用独立进程、单case；同一software/runtime身份的会话只在总runner开始时运行一次
known-good Add资格测试，不在结尾或每个case重复。资格测试或case timeout都停止全部板测，由用户决定恢复方式。
...
`documented-depth-manual`校准：一次只选一个engine和一个case、只运行一遍，并复用本次重启会话开场时的一次
known-good Add资格结果；它只验证连续提交恰好`D`条后的completion、instruction count、完整结果和guard，
...
board-positive row仍是独立launch和外层timeout；同一合格会话只在总runner开始时做一次known-good Add资格测试，
queue边界、安全random observation和multi-writeback等board row也不逐case重复资格化。所有
...
| TDMA Memset | element count、byte stride、raw logical iteration、inclusive range和dtype packet encoding | whole/128B×32/64B×64 geometry，I8/F16/BF16 raw与CRT，全range和guard | 10/11/14 | 普通dtype descriptor `calibrated`；本轮targeted FP16 `128B×32`与BF16 `64B×64` strided case为2/2 exact。该结果只闭合已测geometry、physical span、holes、guard和completion，不外推其它shape/dtype |
...
| SPM alignment/bank | 256B preferred alignment、非1024-bit访问资格/代价、bank/color映射 | bounded非preferred geometry round-trip、disjoint offset sweep、多offset engine-pair serial/window control | 09 placement与06 cost | 当前SPM inventory有146行：原19个本地case及新增5个`base+64/+128/+192`、`length=128/384`均已有板端证据。用于性能区分的16个sustained cell组成4个同invocation matched board group，本轮4/4通过完整SPM/canary、count、address、PMU、readback和lifecycle oracle。physical bank/color identity仍没有owner-backed bank/port mapping或bank-specific attribution；offset、port counter或aggregate execution方向都不得代签bank编号 |
| DDR/cache/coherence | host H2D、Kcore cache、DMA completion和host publication是不同域 | Kcore read前invalidate对照、DMA round-trip、matching drain后D2H | 09/12/14/15 | memory-descriptor 155行、独立cache 58行、16-rank tile-offset与40GiB sparse probe已有板端证据；当前FP16 operator focused批次中的DDR tile-offset与DDR sparse均fresh通过。DDR active-rank的RDMA-only、WDMA-only、双向三类 × 4KiB/64KiB/64KiB-stride三种workload组成9个五点matched board group，本轮9/9通过完整active/inactive rank oracle和16-rank lifecycle；该结果不形成bank/controller结论 |
| queue shape与连续提交边界 | register/header中CT/NE/RDMA/WDMA静态depth为6、TDMA为4；该数值描述pending storage，不是完整lifetime总提交上限，active occupancy和full行为不能由形状或总提交数推出 | 普通calibration只跑1/2/4（TDMA 1/2）；隔离manual gate验证`D`、tight `D+1`和较长workload观察点，完整检查count/output/guard | 10/11/16 | 五类engine的`D`与tight `D+1`已有板端证据；本轮30个`short/sustained × D-1/D/D+1`区分case全部通过count/output/guard、terminal和cleanup。当前versioned record没有head/tail/occupancy/resident字段，因此resident数量仍由`queue-active-resident-count-unobservable` host gate typed fail closed，不能由task-done、静态depth或完成总数代签 |
| worker scope | worker0/1/2 routing、default wait和`bywork` scope | CT三worker；matching wait后、safety drain前读CSR/result与completion marker | 10/11/14/15 | 本轮wait-scope 18/18、subset-scope 12/12和worker placement/progress 14/14 matched group全部通过真实device protocol、result/guard、join/idle、deadline和cleanup oracle。fixture曾把NE转置B的物理stride错误写成`N+1`，修正为`K+1`后只由fresh输出闭合。absolute per-worker completion timestamp与physical arbiter policy没有owner-backed观测面，仍由typed host boundary拒绝；跨worker仍必须显式join |
...
| Direct DTE | source read、destination visibility、participant、channel/FSM、terminal status | 16-rank receiver-first；producer→DTE、DTE→consumer、disjoint顺序、event后复用、multidestination mode及同步错误返回 | 13-16 | 修复后的独立64元素production baseline已在16 ranks exact；modes 1--12均已有各自expected/status/guard/cleanup证据。四源fan-in、本轮raw broadcast 8/8、scatter 8/8和single-destination source-1D shuffle 8/8均fresh通过。broadcast/scatter按zero-based final active slot写`dest_num=fanout-1`；shuffle固定单目的`dest_num=0`，以128B element、stride256B和sections 2/4/8/15验证source gather，不再测试无owner依据的multi-destination shuffle。8/15源fan-in、device phase、physical route和跨卡transport仍fail closed；sender时间重叠仍`unknown` |
...
| --- | --- | --- | --- | --- |
| CT | Tensor vector对已测FP16/BF16/FP32 unary、`VV/VS/VuV` main/tail按current exact oracle完成；合同内两个`VuVLoop unit=64` geometry exact。reduce的128B logical result实际写满256B physical block。ArgMin全正普通值返回`0.5@index42`，负数输入稳定返回错误结果；本轮tie/NaN各一个case均通过三组coherent value/index、padding/guard和lifecycle oracle | current catalog 653项现已全部通过result、span、guard、record和lifecycle oracle，`PASS=653, FAIL=0, MISSING=0`。ArgMin fixture修复了跨sample旧`DATA_VALID`代次被误认完成的问题：issue前保存CSR pair，只接受与本轮输入coherent的新pair，并在drain后重读确认 | 已有653个row按其实际板端证据消费；`VuVLoop`只接受unit64和checked base/full关系；ArgMin只开放已验证全正普通值及tie/NaN观察域，负数拒绝 | 不由已有CT special-value或F16域外推BF16/F32或其它ArgMin域；这些未列域由`argmin-unqualified-domain` typed host gate拒绝 |
...
| Pool/Unpool | Pool的16个exact覆盖BF16/F32 symmetric及FP16 asymmetric unpadded；8个padding/tie只形成bounded behavior。indexed Pool的value与index是双writeback，F32 index占u32宽度、完整span 1024B。Unpool的index参数是same-shape i16 SPM buffer地址而非scalar；Avg与已验证symmetric mask组合exact，ordinary indexed及F32/large组合仅bounded，其中large FP16 mask只命中最后64-channel window；重复overlap尚无板端规则 | Pool 24个当前case及既有Unpool exact/observation有板端结果。`unpool-index-f16-repeated-overlap-observed`与`unpool-mask-f16-repeated-overlap-observed`先由indexed-max产生四个都指向同一global位置、local index分别为5/3/2/0的真实aux，再以same-worker NCC依赖链分别快照producer aux、把四组按sample轮换的不同FP16 sentinel搬入pooled value并发射Unpool；host分别逐bit检查consumer前快照和consumer后aux，64个collision channel各自由候选四组sentinel的非空subset解释，并记录uniform或lane-varying的value mask与histogram。mask首轮执行的一个aux lane从5变0且同一lane错误scatter到position 0，证明consumer实际读到错误index，但旧post-only aux与Kcore staging边界不能区分producer writeback和consumer读取；修正case只完成host/no-card，仍待新板端执行 | IR显式携带index SSA buffer及其dtype；Pool按value/index分别分配。Unpool只开放明确exact组合；ordinary indexed、large及新重复overlap在得到板端分类前不进入通用numeric lowering | 不把F32 Pool的u32 index直接接到i16 mask-Unpool，不由完成性或case准备推导collision winner/accumulate规则，不由一个window外推任意padding/tie语义 |
...
| ArgMin是否支持浮点负数 | **不支持当前负数域。** 全正普通值exact，负数对照稳定给出错误结果；F16 tie/NaN两个区分case本轮均通过coherent value/index、guard和lifecycle oracle | 只准已验证全正普通值、tie与NaN观察域；负数域由capability/verifier拒绝；其它dtype/domain由typed host gate拒绝 |
| Unpool是否通用exact | **不是。** 已列Avg和symmetric mask组合exact；ordinary indexed及其它已有组合只取得bounded observation；本轮index/mask两条重复overlap case均通过其bounded observation oracle | 只对明确exact组合开放numeric资格；重复overlap通过不定义通用winner、覆盖或累加语义 |
...
| Direct DTE与barrier | 有序producer→DTE、DTE→consumer、两种disjoint顺序、event后复用和两destination broadcast均完成payload矩阵；四种同步错误观察得到预期transport error后clean success；raw async serial/window各3×16 exact且返回码全0；full-card barrier两个反向错峰epoch均16/16正确；四源fan-in以及raw broadcast/scatter/source-shuffle本轮全部fresh通过 | DTE以显式event/token和wait表达；raw async允许在send/wait之间发射独立CT，但正确性证据不等于时间重叠收益；当前只准16-rank full-card barrier，subgroup在发包前拒绝 | broadcast/scatter各8项验证fanout 2/4/8/15的adjacent/interleaved destination order；shuffle的8项只验证单目的source-1D strided gather，sections 2/4/8/15和target1/8。8/15源fan-in、可信device phase、physical route和跨卡transport继续typed fail closed |
...
2. **暂不允许**：未匹配expanded白名单的自动overlap、用queue depth选择pipeline window、SPM bank coloring、
   ...
   out-of-contract observation，不再列为production待执行边界。
...
| --- | --- | --- | --- |
| CT / peripheral | current CT catalog 653/653通过current exact oracle；旧63项mismatch在修正value-truth及BOOL VuV 40-bit physical period/tail zero-fill oracle后的新板端执行中全部得到`execute_result=1, mismatches=0`。合同内`VuVLoop unit=64`的两个区分geometry已闭合；ArgMin只在全正普通值域得到`0.5@index42` exact，负数域返回错误结果；普通indexed Unpool只得到有界执行观察，FP16 indexed-max→mask-unpool组合得到exact | 按实际已有板端证据准入；ArgMin tie/NaN两个case及Unpool index/mask repeated-overlap两个case已接入board CTest和runner但尚未上板，不能混入本列“已经得到的结论” | BOOL VuV 40-bit周期不外推其它unit/layout/form；ArgMin tie/NaN和Unpool repeated-overlap待板端分类；其它ArgMin dtype/domain由typed gate fail closed |
...
| DTE / barrier | 修复后的64元素production baseline为16-rank exact；modes 1--12均通过各自oracle。16-rank full-card barrier、四源fan-in、raw broadcast 8/8、scatter 8/8和single-destination source-1D shuffle 8/8均已闭合；DTE channel0 transfer raw只取得profile内相对量 | 保留显式event/wait，只支持当前16-rank full-card participant合同；raw async window只开放调用顺序正确性，不据此写时间重叠收益；broadcast/scatter写`dest_num=fanout-1`，shuffle固定`dest_num=0` | 8/15源fan-in、device phase、physical route和跨卡transport保持fail closed；subgroup继续typed-negative；不再把shuffle解释成multi-destination或AllToAll |
...
- Unpool按实际语义分层：Avg的FP16/BF16/F32与symmetric mask composite的FP16/BF16保留exact；
  ...
  `unpool-mask-f16-repeated-overlap-observed`先用真实indexed-max生成`5/3/2/0`四组aux，再以same-worker
  NCC RAW/WAW链把producer aux快照到独立区域并用compact TDMA搬入按sample轮换的四组不同FP16 sentinel，
  不在两条NCC指令之间插Kcore mapped-SPM写。host分别逐bit检查consumer前快照和consumer后aux，要求64个重叠
  ...
  因而不会再把“任意bounded writeback”冒充collision观察。mask首轮中aux第17个lane从5变0，并在同一
  channel把对应sentinel错误scatter到position 0；该相关性排除host slot offset和纯readback误判，但旧case
  只有consumer后aux，不能区分producer writeback与consumer读取。修正case已通过host/no-card，仍需新板端
  执行后才能形成collision winner/覆盖/累加规则。
...
3. `production-vertical`从真实source/IR编译到package，证明typed consumer真的使用该能力。
...
| CT vector × NCx | 至少两个N slice，slice使用不同pattern和独立padding poison；逐slice执行Cx逻辑 | 不得把NCx静默flatten成单个Cx |
| reduce/pool/unpool × Cx/NCx | 原生wrapper声明的shape/layout逐kind正向；compact Tensor版本若需转换则执行movement | 不由GEMM或CT vector结果外推 |
...
Tensor materialize→NE identity GEMM、FP16 `128B×32`与BF16 `64B×64` strided TDMA，以及FP16/BF16 raw-vs-CRT
...
catalog/probe从weight shape和Cx dtype宽度推导8192B，8192B之后的suffix guard继续严格。修正schema的
FP16/BF16各3个新板端样本均完成request/result echo、8192B physical
...
| runtime publication | device terminal→runtime completion→D2H→cleanup；失败路径使用外层timeout | terminal schema、完整output和cleanup；管理面idle不替代本轮execution资格 |
...
6. CT会话资格的ordinary Add heartbeat（CTest 125）以3.10s通过。CTest 106 `ct-capability`以32.77s
   ...
   heartbeat观察到的卡状态污染。审计确认此前63项均是host oracle语义错误，不是硬件数值失败：
   op78--87的value logic对每个数值取truth后执行
   ...
   读取为0。修正后在`board-ct-vector-corrected63-v2`通过新板端执行验证id
...
`board-observed`。memory descriptor 155项已按current 2MiB schema完成板端执行和当场校验，
...
pending inventory把每个可安全观测的未闭合项绑定到真实catalog对象、device request/record、host
...
每个新会话只在开始时执行一次known-good Add资格测试。
runner只允许选择尚无结论或代码修复后明确要求重新执行的case。本批全部待板端case的单一机器可审计入口为：
...
cell，并校验每个CTest恰有runner step。归一化后的pending target参数无重复；同invocation内作为matched
control共同新执行的既有baseline不计入1231个pending target，例如SPM equivalence的
...
有意重叠，不应在同一命令中机械叠加。选择master batch后只自动加一次开场资格测试；每个group CTest在单一
...
不属于合法pending inventory，且不存在任何board case或runner step。历史板端输出不得作为runner输入，
已有结论的全量suite不进入runner。

`f16-common-factor`的每个baseline/winner输出都通过显式
`--expected-f16-relaxed`交给`wafer-run`逐元素比较：`+0/-0`相等，finite同时满足固定abs/rel与1 ULP
边界，NaN/Inf拒绝；资源dtype/长度、全输出覆盖、lifecycle及其它结构oracle仍严格。host侧oracle预检不能
替代这一步实际板端output比较。
...
本轮绑定TX8110、16 tile、runtime version 1300及同一runtime library identity执行。以下数字只记录
当时进程结果，不是当前operator qualification汇总；dtype已替换的case必须fresh执行：
...
此前79个通过项中，3个I8 AllGather与11个I8 AllToAll/Permute只执行了不代表真实训练dtype的
workload，现已撤销其通信算子覆盖结论；替换后的FP16 case必须重新上板，旧输出不得重判或回放。
其余65项只保留各自result、guard、count、completion及已声明proxy合同的板端证据；未满足promotion
条件的cost、bank/color、resident和scope结论仍保持Unknown。

当前operator-level正向workload已统一以FP16/BF16为默认：rank-one/all-rank Add、Direct-DTE
collective、collective algorithm/traffic、通用single/pair engine、DDR offset、strided TDMA及
reciprocal/resident/recompute/ready-order均使用FP16或BF16并保持原byte边界。只有明确验证
model BootParam v1 F32 ABI、I8量化、convert、index/mask或raw字节协议的专项保留对应类型；这些专项
不得外推为FP16/BF16算子资格。

重启后的本轮fresh串行执行得到当前operator qualification结论：

- FP16 exact Add资格入口1/1通过。
- production optimizer的common-factor、reciprocal、resident fanout、consumer-local recompute、
  ready-order、两个GEMM和tree all-reduce共8/8通过；复用的Direct-DTE production vertical也通过。
- FP16 collective algorithm 9/9、AllToAll/Permute traffic 11/11通过。
- generic engine single activation group 1/1、pair activation group 60/60通过。case key中的
  `i2/i4/i8`表示iteration 2/4/8，payload为FP16，不是I8 dtype。
- Direct-DTE、DDR tile-offset、DDR sparse focused组3/3通过；targeted strided TDMA的FP16与BF16
  两个case为2/2 exact。

首次AllGather、Direct-DTE、DDR tile及tree all-reduce被host oracle仍匹配旧cluster launch字符串误报；
修正后只由本轮新构建、新启动和新输出fresh执行通过，旧输出没有回放或重判。上述通过只证明已测输入、
accepted结构、完整result/guard、completion和各case已有oracle成立；不能外推未测shape、dtype、route、
并发行为或production cost。

9个ordinary failure按根因边界分类如下：

1. production common-factor与resident-fanout-share各1项：前者使用了错误的旧数值比较合同，后者仅在
   `Neg(+0)`的signed-zero位上不一致；两项旧输出都不作为当前FP16 paired qualification。本轮已按统一
   typed tolerance由fresh板端输出重新执行并通过。
2. ReduceScatter Direct 256B使用了无效的I8算子workload；该结果已从当前通信算子结论中删除，
   不能用于判断FP16 ReduceScatter或DTE transport；当前FP16 algorithm结论来自本轮fresh 9/9。
3. single-engine seed在RDMA起始处返回`SEED_FAILED`；host catalog把`Fmt_INT8`误写成值8，而
   version-matched ABI中INT8为0、值8为UINT8。这是测试枚举事实源错误，不是engine rate证据；当前
   generic engine payload已经改为FP16并由本轮single 1/1、pair group 60/60重新取得结论。
...
本轮按“普通项在前、可能污染会话项在后、raw shuffle最后”的顺序，将此前真正未闭合的104个CTest
全部fresh执行并104/104通过：普通18项；SPM sustained 4、queue saturation 30、wait-scope 18、
subset-scope 12、worker placement/progress 14及raw shuffle 8。ArgMin、NE tail和NE wait/subset先修复
各自fixture，再只由新构建、新启动和新输出取得结论；没有读取、回放或重判旧板端输出。
...
4个matched group执行；DDR active-rank的45个case按9个五点matched group执行；Direct-DTE有25个逐case项：
四源fan-in 1项、raw broadcast/scatter各8项，以及single-destination source-1D shuffle的sections
2/4/8/15 × target1/8共8项；
...
- 每类engine使用`short/sustained × D-1/D/D+1`六格factorial control。`D-1`是确认阈值信号不会在
  ...
  `D+2`或更深overflow。
- 每格至少保留三次独立样本。强oracle包括packet/request echo、目标worker/engine
...
- 语义key为`worker-wait-scope-exclusion`。分别让worker0/1/2成为target，并以NE和RDMA两种
  ...
  推迟逐packet观察。NE held-out使用固定slot内的`M=64,K=256,N=128`有界packet，不增加round数；
  read0/read1/result和guard仍全部位于各自`0x20000` slot。最后一条target issue返回后只经过`rdcycle`，
  target control必须成为第一条MMIO读取，其余worker control随后在requested wait前补齐。该target采样必须
  证明仍pending，否则样本无区分力，不计入scope结论。
...
- physical-tile轴使用独立16-rank shared package执行一个显式选择的paired coordinate。每个coordinate固定
...
第一批固定16 rank、FP16和相同placement/ABI。`B`表示每rank参与的logical collective payload：
AllGather输入`B/16`、输出`B`；ReduceScatter输入`B`、输出`B/16`；AllReduce输入/输出`B`。
sentinel同时编码source rank、目标slice位置和logical lane。AllGather使用有限、可精确表示的FP16
payload；ReduceScatter/AllReduce使用小整数FP16贡献，使16-rank求和仍精确且与归约顺序无关。4KiB AllReduce
复用现有source/payload，不另建重复case。

catalog mutation gate逐AllGather source chunk、逐reduction source contribution（ReduceScatter按
destination segment）及逐ReduceScatter output slice检查rotation，要求16个source contribution两两
不同、16个ReduceScatter destination output两两不同；对每个reduction source逐一删除并以其余每个
source替换，每个destination slice都必须改变。AllGather另要求16个concat chunk两两不同，且逐source
相邻rank swap改变expected。

| case key | collective | algorithm A / B | B | accepted Instr结构oracle | runtime oracle | 状态 / 允许结论 |
| --- | --- | --- | ---: | --- | --- | --- |
| `all-gather-direct-vs-ring-256b` | AllGather | Direct / Ring | 256B | Direct逐rank验证15-peer cyclic round和source slice；Ring逐rank验证固定前后继、`P-1`轮、round→forwarded-slice递推及单一16-rank cycle；完整message tuple跨rank一一匹配 | 每rank按rank-group顺序完整concat；共同lifecycle/status合同见表后 | `board-passed`；correctness资格，不产生cost |
| `all-gather-direct-vs-ring-4096b` | AllGather | Direct / Ring | 4096B | 同上 | 同上 | `board-passed`；correctness资格，不产生cost |
| `all-gather-direct-vs-ring-65536b` | AllGather | Direct / Ring | 65536B | 同上 | 同上 | `board-passed`；correctness资格，不产生cost |
| `reduce-scatter-direct-vs-ring-256b` | ReduceScatter | Direct / Ring | 256B | Direct逐rank验证source-round/destination-slice的15-peer owner exchange；Ring验证`P-1`个chunk round、round→slice递推和单一16-rank cycle；完整message tuple跨rank一一匹配 | 每rank只得到自己的destination slice，全部source贡献exact；共同lifecycle/status合同见表后 | `board-passed`；correctness资格，不产生cost |
| `reduce-scatter-direct-vs-ring-4096b` | ReduceScatter | Direct / Ring | 4096B | 同上 | 同上 | `board-passed`；correctness资格，不产生cost |
| `reduce-scatter-direct-vs-ring-65536b` | ReduceScatter | Direct / Ring | 65536B | 同上 | 同上 | `board-passed`；correctness资格，不产生cost |
| `all-reduce-ring-vs-tree-256b` | AllReduce | Ring / ordered Tree | 256B | Ring验证`2(P-1)`轮两阶段round→slice递推、chunk和单一16-rank cycle；Tree验证15条reduce边、broadcast exact reverse、`slice=child`、`round=child depth`、单root/无环/二叉且rank-group inorder；完整message tuple跨rank匹配 | 16个replicated FP16 exact output；共同lifecycle/status合同见表后 | `board-passed`；correctness资格，不产生cost |
| `all-reduce-ring-vs-tree-4096b` | AllReduce | Ring / ordered Tree | 4096B | 同上；复用原`tree-all-reduce` payload，但Ring必须由actual phase确认，不能由reserved-baseline名字推断 | 同上 | `board-passed`；correctness资格，不产生cost |
| `all-reduce-ring-vs-tree-65536b` | AllReduce | Ring / ordered Tree | 65536B | 同上 | 同上 | `board-passed`；correctness资格，不产生cost |
...
不得写成“16条raw Success已独立观察”；若后续promotion要求逐rank raw status，必须先扩展runtime output surface。

此前I8版本的package/no-card与板端输出均不计入当前FP16 case证据。重启后的FP16 exact Add资格入口
1/1通过；9组algorithm与11组AllToAll/Permute均由新构建、新启动和新输出fresh串行执行，通过完整
result、status/completion与既有oracle。首次AllGather只因host oracle仍匹配旧cluster launch字符串被误报；
修正后重新fresh执行通过，旧输出没有回放或重判。旧RS 256B实现曾
...
| --- | --- | --- | --- | --- |
| AllToAll / Permute traffic semantics | 全交换fanin/fanout、cycle与send-only/recv-only/self/unmapped-zero-fill角色 | 3个AllToAll payload、3个Permute forward-cycle payload，加reverse、opposite-pairs、disjoint-pairs、sparse-roles和two-epoch chain，共11个FP16 structured source→package→ELF→wafer-run case | 全rank exact、status-v2、accepted source graph、terminal/cleanup；单一Direct schedule只测traffic behavior，不伪造算法A/B或device cost | `board-passed`（11/11）；旧I8执行结论已撤销；只闭合已测traffic correctness，ragged AllToAll与same-physical-buffer alias缺少typed surface，host fail closed |
| DTE raw remote / fan-in | broadcast、scatter的multi-destination payload/mapping、shuffle的single-destination source gather，以及current receiver protocol可承载的fanin | broadcast/scatter各8项；source-1D shuffle为sections 2/4/8/15 × target1/8共8项；另有四源fan-in 1项 | 全payload/guard/nonparticipant/status/cleanup；scatter设置`sg_flag=1`；broadcast/scatter写`dest_num=fanout-1`，shuffle写`dest_num=0`、element128B、stride256B | 四源fan-in及24个raw case全部`board-passed`；8/15源fan-in、device phase、physical route与跨卡transport继续typed fail closed |
| single-engine throughput | CT/NE/RDMA/WDMA/TDMA启动成本、tail和steady slope | raw dispatcher有20个small/steady/movement cell；NE另有`f16 m65/k129/n129` tail真实source→package case，共21 cell | exact result/padding/guards/count、manifest绑定的唯一terminal、ordered lifecycle与device PMU，至少3次；engine组按sample-major正/反/半程轮转。NE tail只在session id和完整board qualification fingerprint一致时与`single/ne/small`、`single/ne/steady-16k`合并，三点device-PMU方向完整才激活rate | raw activation group `board-passed`（1/1）；证明组内已测payload/result/guard/completion与既有oracle，不自动形成rate/cost，NE tail独立状态不由该组代签 |
| engine-pair stage balance | 10个pair的A-bound/balanced/B-bound、A→B/B→A方向、iteration与same/cross-worker | 360个board-executable cell组成60个完整activation group；每组冻结serial/window、ratio、方向和held-out controls | 每个group单进程按sample-major正/反/半程轮转，full result/guard/count、manifest绑定的唯一terminal、三次重复、per-engine/global-union PMU齐全才可分类；host elapsed不作cost | `board-passed`（60/60 group）；`i2/i4/i8`为iteration而非dtype，payload为FP16。只闭合已测调度、result/guard/completion与既有oracle；未形成稳定device方向的cell仍不得外推cost |
...
| worker placement/arbitration | w0/w1/w2 fixed-total placement、bounded backlog progress与low/high outstanding区分 | fixed-total placement 14、NE/RDMA progress 18、CT/RDMA × worker × low-2/high-6 outstanding 12，共44 case、14 matched group | sentinel/concurrent固定同一physical slot；完整result/guards/routing count/tail canary、join/idle、deadline与cleanup | `board-passed`（14/14 group）；absolute per-worker timestamp和physical arbiter policy仍有typed host blocker |
| DDR active-rank contention | RDMA-only、WDMA-only、双向在1/2/4/8/16 active ranks的contention slope | 3方向 × 4KiB/64KiB/64KiB-stride × 5 active-count，共45 case、9 matched group；所有16 rank参与lifecycle | active exact/guards/count/device cycle、inactive output/PMU canary及逐rankmatching terminal | `board-passed`（9/9 group）；不由active-rank趋势命名bank/controller |
| SPM conflict pilot | matched candidate/control是否存在可复现非零差异 | `4/16KiB × A→B/B→A`四组，共16 cell | owned-SPM/gap canary、address/count、owner-backed port0/6 T2/T3、matched WDMA readback、completion/lifecycle | `board-passed`（4/4 group）；physical bank/port mapping和bank-specific attribution仍由typed host gate拒绝 |
| DDR bank/coloring | compiler能否控制稳定physical class | rank-one/cross-tile conflict-equivalence board CTest只测actual-address scoped proxy；bank命名必须另有owner-backed mapping与bank-specific counter | 任何equivalence结果都不恢复controller/channel/hop或bank编号 | equivalence `board-passed`（2/2）；physical bank mapping、bank counter及缺owner的WDMA cross-allocation axis仍`typed-fail-closed` |
...
- `depth + 1`不得混入普通calibration；它只允许进入typed tight gate：单engine、单case CTest、有界重复，
  ...
  drain和正常lifecycle。同一software/runtime身份的会话只由总runner在开始时资格化一次，不在结尾或每个case重复。
