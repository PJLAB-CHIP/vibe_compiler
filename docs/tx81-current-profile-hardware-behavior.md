...
旧 oracle 因把两端都当成同一种展开方式，在三个 standalone case 中分别出现 26、32、44 个 mismatch。
修正 oracle 后，3 个 standalone 加 36 个 dependency case，共 39/39 fresh 通过。
...
- physical write span 用于分配、guard、alias/hazard 和 expected-output oracle；
...
## 8. Case F：地址结果只支持到“当前映射”，不支持微架构归因
...
- 当前 profile 的 offset 映射可进入 package/no-card/board oracle。
...
- 负值、边界 dtype、guard 区域是否进入 oracle？
