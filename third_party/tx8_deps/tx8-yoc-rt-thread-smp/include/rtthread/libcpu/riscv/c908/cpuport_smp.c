...
volatile static uint32_t __smp_cores_ready_flag[RT_CPUS_NR] = {0};
...
static inline int spinlock_trylock(spinlock_t *lock)
{
    ...
    //int res = rt_atomic_exchange((volatile rt_atomic_t *)&lock->lock, -1);
    int res = rt_atomic_compare_exchange(&lock->lock, 0, 1);
#else
...
{
    //int id = rt_hw_cpu_id();
    //tx8_kernel_printf("%s begin cpu id:%d\n", __FUNCTION__, id);
...
void rt_hw_secondary_cpu_up(void)
{
    ...
    for (int i = 1; i < RT_CPUS_NR; i++) {
        while (__smp_cores_ready_flag[i] != 0x1);
    }

    __DSB();
    __ISB();

    //tx8_kernel_printf("%s set 0 core\n", __FUNCTION__);
    __smp_cores_ready_flag[PRIMARY_STARTUP_CORE_ID] = 1;
}
...
static void secondary_cpu_init(void)
{
    ...
    __smp_cores_ready_flag[cpu_id] = 0x1;

    __DSB();
    __ISB();

    while (__smp_cores_ready_flag[PRIMARY_STARTUP_CORE_ID] != 0x1);
...
{
    secondary_cpu_init();
    //tx8_kernel_printf("%s begin lock _cpus_lock\n", __FUNCTION__);

    rt_hw_spin_lock(&_cpus_lock);
    //tx8_kernel_printf("%s lock done _cpus_lock\n", __FUNCTION__);
    ...
    //tx8_kernel_printf("%s begin rt_system_scheduler_start %d\n", __FUNCTION__, rt_hw_cpu_id());
    rt_system_scheduler_start();
    //tx8_kernel_printf("%s done rt_system_scheduler_start\n", __FUNCTION__);
}
