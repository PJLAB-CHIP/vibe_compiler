...
struct rt_thread* gdb_contthread = NULL;
struct rt_thread* gdb_usethread = NULL;
...
int gdb_long2hex(unsigned long *long_val, char *buf)
{
    int count = sizeof(long_val[0]);
    char *tmp = (char *)(long_val + 1);
    char ch;
...
{
    return probe_kernel_write((void *)addr, (void *)bundle, BREAK_INSTR_SIZE);
}
...
int gdb_isswbreak(unsigned long addr)
{
    int i;
    for (i = 0; i < GDB_MAX_BREAKPOINTS; i++) {
        if ((gdb_break[i].state == BP_ACTIVE) &&
...
#define put_unaligned_t __put_unaligned_t

static void put_unaligned_be32(uint32_t val, void *p)
{
	put_unaligned_t(u32, cpu_to_be32(val), p);
}


static inline char *hex_byte_pack(char *buf, uint8_t byte)
{
    *buf++ = tohex((byte & 0xf0) >> 4);
	*buf++ = tohex(byte & 0x0f);
	return buf;
}
...
extern unsigned long __stext;
extern unsigned long __sdata;
extern unsigned long __sbss;

extern int get_list_thread(struct rt_thread* threads[], int max_num);
...
static struct gdb_info  debug_infos[MAX_THREAD_NUM] = {
    [0 ... MAX_THREAD_NUM-1] = { .thread = NULL }
};

static int debug_cnt = 0;
static int debug_no = 0;

static void get_threads()
{
    int i,j;
    struct rt_thread* threads[MAX_THREAD_NUM];
    int out_num = get_list_thread(threads, MAX_THREAD_NUM);
    if(0 == debug_cnt){   
        for(i = 0; i < out_num; ++i)
        {
            debug_infos[i].thrdno = ++debug_no;
            debug_infos[i].thread = threads[i];
        }
        debug_cnt = out_num;
    }else{
        // save old debug threads info
        struct gdb_info gdb_infos[MAX_THREAD_NUM];
        memcpy((void*)gdb_infos, (void*)debug_infos, sizeof(struct gdb_info) * debug_cnt);
        ...
        for(i = 0; i < debug_cnt; ++i)
        {
            ...
            for(j = 0; j < out_num; ++j)
            {
                if(gdb_infos[i].thread == threads[j]){
                    index = j;
                    break;
                } 
            }
            // old exist threads insert
            if(index != -1){
                debug_infos[cnt].thread = gdb_infos[i].thread;
                debug_infos[cnt++].thrdno = gdb_infos[i].thrdno;
                threads[index] = NULL;
            }
        }
        // new threads insert
        for(i = 0; i < out_num; ++i){
            if(threads[i]){
                debug_infos[cnt].thread = threads[i];
                debug_infos[cnt++].thrdno = ++debug_no;
            }
        }
...
static rt_thread_t getthread(int tid)
{
    int i;
    for(i = 0; i < debug_cnt; ++i){
        if(debug_infos[i].thrdno == tid){
            return debug_infos[i].thread;
        }
    }

    return (rt_thread_t)RT_NULL;
}

static int getthreadno(rt_thread_t thread)
{
    int i;
    for(i = 0; i < debug_cnt; ++i){
        if(debug_infos[i].thread == thread){
            return debug_infos[i].thrdno;
        }
    }
...
extern char g_sharename[2560];//128*20
extern rt_uint64_t g_sharemem[20];
...
static void gdb_cmd_query(struct gdb_state *gs)
{
    ...
    case 's':
    case 'f':
        if(strncmp(remcom_in_buffer+2, "ThreadInfo", 10))
            break;
       ...
       for(j = 0; j < debug_cnt; ++j){
            if(i >= gs->thr_query && !finished){
                int_to_threadref(thref, debug_infos[i].thrdno);
                ptr = pack_threadid(ptr, thref);
                *(ptr++) = ',';
                gs->thr_query++;

                if(gs->thr_query % KGDB_MAX_THREAD_QUERY == 0)
                    finished = 1;
            }
    ...
    case 'C':
        ...
        gs->threadid = getthreadno(rt_thread_self());
        int_to_threadref(thref, gs->threadid);
        ptr = pack_threadid(remcom_out_buffer + 2, thref);
        *(ptr++) = '\0';
        break;
    case 'T':
        if(strncmp(remcom_in_buffer+1, "ThreadExtraInfo,", 16))
            break;
        gs->threadid = 0;
        ptr = remcom_in_buffer + 17;
        gdb_hex2long(&ptr, &gs->threadid);
        rt_thread_t thread = getthread(gs->threadid);       
        if(thread){     
            gdb_mem2hex(thread->parent.name, remcom_out_buffer, strlen(thread->parent.name));
        }else{
            static char tmpstr[23 + BUF_THREAD_ID_SIZE];
            sprintf(tmpstr, "shadowCPU%d", (int)(-gs->threadid - 2));
            gdb_mem2hex(tmpstr, remcom_out_buffer, strlen(tmpstr));
        
        }
        break;
    case 'O':
        if (!strncmp(remcom_in_buffer, "qOffsets", 8)) {
            strcpy(remcom_out_buffer, "Text=");
            unsigned long addr = (size_t)&__stext;
            gdb_long2hex(&addr, remcom_out_buffer + strlen(remcom_out_buffer));

            strcat(remcom_out_buffer, ";Data=");
            addr = (size_t)&__sdata;
            gdb_long2hex(&addr, remcom_out_buffer + strlen(remcom_out_buffer));

            strcat(remcom_out_buffer, ";Bss=");
            addr = (size_t)&__sbss;
            gdb_long2hex(&addr, remcom_out_buffer + strlen(remcom_out_buffer));
        }
        break;
    case 'X':
        if (!strncmp(remcom_in_buffer, "qXfer:libraries:read", 20)) {
            strcat(remcom_out_buffer, "l<library-list>");
            for(int i = 0; i < g_sharecnt; ++i)
            {
                char name[128] = {0};
                rt_strncpy(name, (char*)&g_sharename[i * 128], 128);
                char libname[256] = {0};
                char address[256] = {0};
                

                sprintf(libname, "<library name=\"%s.so\">", name);
                snprintf(address, sizeof(address), "<segment address=\"0x%lx\"/>", g_sharemem[i]);

                strcat(remcom_out_buffer, libname);
                strcat(remcom_out_buffer, address);

                strcat(remcom_out_buffer, "</library>");
            }

            strcat(remcom_out_buffer, "</library-list>");
        }
        break;
    default:
        break;
...
static void gdb_cmd_getregs(struct gdb_state *gs)
{
    ...
    if(gdb_usethread == rt_thread_self()){
        gdb_get_register(gdb_regs);
    }else{
        gdb_get_regbythrd(gdb_regs, gdb_usethread->sp);
    }
...
static void gdb_cmd_thread(struct gdb_state *gs)
{
	char *ptr = &remcom_in_buffer[1];
	struct rt_thread *thread;

	gdb_hex2long(&ptr, &gs->threadid);
	thread = getthread(gs->threadid);
	if (thread)
		strcpy(remcom_out_buffer, "OK");
	else
		error_packet(remcom_out_buffer, -1);
...
static void gdb_cmd_task(struct gdb_state *gs)
{
	rt_thread_t thread;
	char *ptr;
	...
	case 'g':
        ...
        if(gs->threadid == 0)
        {
            gdb_usethread = rt_thread_self();
        }else{
            gdb_usethread = getthread(gs->threadid);
        }
	...
	case 'c':
        ...
        if((int)gs->threadid <= 0){
            thread = rt_thread_self();
        }else{
            thread = getthread(gs->threadid);
        }

        gdb_contthread = thread;
        if (thread)
		    strcpy(remcom_out_buffer, "OK");
	    else
		    error_packet(remcom_out_buffer, -22);
...
int gdb_handle_exception(int signo, void *regs)
{
    ...
    if (gdb_connected) {
        ...
        *ptr++ = tohex(gs.signo & 0xf);
        ...
        int_to_threadref(thref, getthreadno(rt_thread_self()));
        ptr = pack_threadid(ptr, thref);
        *ptr++ = ';';
        put_packet(remcom_out_buffer);
    }
