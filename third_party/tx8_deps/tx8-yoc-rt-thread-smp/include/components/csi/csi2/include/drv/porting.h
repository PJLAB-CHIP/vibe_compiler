...
uint64_t soc_get_cpu_freq(uint32_t idx);
...
uint64_t soc_get_timer_freq(uint32_t idx);
...
uint64_t soc_get_coretim_freq(void);
uint64_t soc_get_cur_cpu_freq(void);
uint64_t soc_get_sys_freq(void);
...
/**
  ...
  \return      none
*/
void soc_set_sys_freq(uint64_t freq);
