...
__STATIC_INLINE void csi_vic_enable_irq_core(int32_t IRQn, int hartid)
{
...
__STATIC_INLINE void csi_vic_enable_irq(int32_t IRQn)
{
    ...
    csi_vic_enable_irq_core(IRQn, hartid);
}
...
__STATIC_INLINE void csi_vic_disable_irq_core(int32_t IRQn, int hartid)
{
    PLIC_Type *plic = (PLIC_Type *)CONFIG_PLIC_BASE;
...
{
    int hartid = csi_get_cpu_id();
    csi_vic_disable_irq_core(IRQn, hartid);
}
...
__STATIC_INLINE uint64_t _csi_clint_config2(unsigned long coret_base, uint16_t hartid, uint32_t ticks, int32_t IRQn)
{
    ...
    if ((value != 0) && (value != 0xffffffffffffffff)) {
        ...
        uint64_t t = (((uint64_t)(CLINT_TIMECMPn_VAL(&clint->MTIMEH, 0))) << 32) + \
                     (uint64_t)(CLINT_TIMECMPn_VAL(&clint->MTIMEL, 0));
        value = t + ticks;
    }
...
__STATIC_INLINE uint64_t csi_clint_config(unsigned long coret_base, uint32_t ticks, int32_t IRQn)
{
    return _csi_clint_config2(coret_base, 0, ticks, IRQn);
}

__STATIC_INLINE void csi_coret_reset_value2_by_id(int hartid)
{
    uint32_t value = 0x0;
    CLINT_Type *clint = (CLINT_Type *)CORET_BASE;
...
{
    int hartid = csi_get_cpu_id();
    csi_coret_reset_value2_by_id(hartid);
}
 ...
 */
__STATIC_INLINE uint64_t csi_coret_config(uint32_t ticks, int32_t IRQn)
{
...
__STATIC_INLINE void csi_dcache_invalid_range(unsigned long *addr, size_t dsize)
{
    ...
    unsigned long unaligned_size = (unsigned long)addr & (linesize - 1);
    long op_size = dsize + unaligned_size;
    unsigned long op_addr = (unsigned long)addr - unaligned_size;
