...
function(get_current_dir_name base  output_var)
...
# 定义一个函数来移除路径中的最后一个目录
function(remove_last_directory input_path result_var)
  # 找到最后一个斜杠的位置
  string(FIND "${input_path}" "/" LAST_SLASH REVERSE)

  # 移除最后一个目录
  if(LAST_SLASH GREATER -1)
    string(SUBSTRING "${input_path}" 0 ${LAST_SLASH} temp_path)
  else()
    set(temp_path "")
  endif()

  # 如果原始路径和新路径相同，设置为根目录
  if(input_path STREQUAL temp_path)
    set(temp_path "/")
  endif()

  # 将结果存储在输出变量中
  set(${result_var} "${temp_path}" PARENT_SCOPE)
endfunction()
function(zephyr_include_build_directories)
  target_include_directories(zephyr_interface INTERFACE "$<BUILD_INTERFACE:${ARGV}>")
  install(DIRECTORY  ${ARGV}/
          DESTINATION  ${CMAKE_INSTALL_PREFIX}/include/build
          USE_SOURCE_PERMISSIONS
  )
  set_property(GLOBAL APPEND PROPERTY TX8FW_INCLUDE_INSTALL_DIR  include/build)
endfunction()
# https://cmake.org/cmake/help/latest/command/target_include_directories.html
function(zephyr_include_directories)
  target_include_directories(zephyr_interface INTERFACE ${ARGV})
  if(NO_INSTALL_ZEPHYR_INCLUDE_DIR)
    return()
  endif ()
  foreach(item ${ARGV})
    cmake_path(IS_ABSOLUTE item is_abs)
    if(is_abs)
      file(RELATIVE_PATH name ${TX8FW_BASE} ${item})
      if( ${name} STREQUAL ${item} )
        MESSAGE(WARNING "${item} is not base to ${TX8FW_BASE}")
        set(INSTALL_DIR include/${item})
        remove_last_directory("${item}" ITEM_REMOVE_LAST_PART_PATH)
      else ()
        set(INSTALL_DIR ${name})
        remove_last_directory("${name}" ITEM_REMOVE_LAST_PART_PATH)
      endif ()
      set(dest_dir ${ITEM_REMOVE_LAST_PART_PATH})
    else ()
      get_current_dir_name(${TX8FW_BASE} result)
      set(INSTALL_DIR include/${result}/${item})

      remove_last_directory("${item}" ITEM_REMOVE_LAST_PART_PATH)
      if(ITEM_REMOVE_LAST_PART_PATH)
        set(dest_dir ${result}/${ITEM_REMOVE_LAST_PART_PATH})
      else ()
        set(dest_dir ${result})
      endif ()
    endif()
    ...
    install(DIRECTORY  ${item}
            DESTINATION  ${CMAKE_INSTALL_PREFIX}/include/${dest_dir}
            USE_SOURCE_PERMISSIONS
    )
    set_property(GLOBAL APPEND PROPERTY TX8FW_INCLUDE_INSTALL_DIR  ${INSTALL_DIR})
  endforeach ()
endfunction()
...
macro(zephyr_library)
  zephyr_library_get_current_dir_lib_name(${TX8FW_BASE} lib_name)
...
# TX8FW_BASE/drivers/entropy/CMakeLists.txt
...
function(generate_inc_file
  ...
  add_custom_command(
    ...
    COMMAND
    ${PYTHON_EXECUTABLE}
    ${TX8FW_BASE}/scripts/build/file2hex.py
...
function(zephyr_linker_sources location)
  ...
  foreach(file IN ITEMS ${L_UNPARSED_ARGUMENTS})
    ...
    file(RELATIVE_PATH relpath ${TX8FW_BASE}/include ${path})
...
# Usage:
#   assert(TX8FW_TOOLCHAIN_VARIANT "TX8FW_TOOLCHAIN_VARIANT not set.")
#
...
Relative paths are only allowed with `-D${ARGV1}=<path>`")
  ...
  if(ZFILE_CONF_FILES)
    ...
    if(ZFILE_NAMES)
      ...
      set(shortened_filename_list ${ZFILE_NAMES})
    else()
...
function(zephyr_check_cache variable)
  ...
  if(${CACHE_VAR_REQUIRED} AND NOT DEFINED ${variable})
    message(NOTICE "${variable} is not being defined on the CMake command-line,"
...
function(add_llext_target target_name)
  ...
  if (CONFIG_LLEXT_EXPORT_BUILTINS_BY_SLID)
    set(slid_inject_cmd
      ${PYTHON_EXECUTABLE}
      ${TX8FW_BASE}/scripts/build/llext_inject_slids.py
