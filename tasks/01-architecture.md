...
source model / exported program / pre-exported StableHLO
  ...
  -> execution-mesh / launch-block / boundary-shards contract
...
- HAL-like 原则：device binding、allocation/import/query、fence/completion、package manifest 是
  ...
  accepted offset 和 boundary shard 这些不可从 local IR 重算的事实。
...
- 从 committed instruction IR、accepted offsets、topology/execution mesh、boundary shard 和
...
- package metadata 中的 tile endpoint、block id、local slice metadata 和 available/excluded tile
  信息来自 `wafer.target.topology`、`wafer.execution.mesh`、`wafer.boundary.shards` 和薄
...
| Target topology / execution mesh / boundary shards | topology import、unavailable tile metadata、valid execution mesh、slice metadata verifier |
...
ModelImport/FrontendProgram
  ...
  -> topology/execution-mesh/boundary-shards + launch-block binding
  -> ABI / LLVM lowering
...
- 没有独立 placed memref / access descriptor realization 主线阶段。committed instruction IR 已经通过
  ...
  memory facts；topology/execution-mesh/boundary-shards contract 只保存 regular topology、SPMD rank
  domain policy、optional explicit endpoint 和 boundary shard slice 这类不能从 local IR 重算的事实。
  ...
  topology/execution-mesh/boundary-shards 派生，不能再引入 placed memref / access descriptor 旁路协议。
...
- ABI/LLVM lowering 只消费 committed instruction IR、accepted offset facts、
  topology/execution-mesh/boundary-shards contract、薄 launch/block binding 和使用点重算的 resource view，
...
| Target topology / execution mesh / boundary shards | `tasks/04-topology-execution-mesh-boundary-shards.md` | 草案 | regular card/tile topology、unavailable endpoint exceptions、valid SPMD rank-domain policy、derived/optional endpoint view、boundary shard slice | Cx/NCx、packet queue、C ABI、tensor 重新切分 |
...
| C ABI / golden packet | `tasks/14-abi-golden-packet.md` | 草案 | committed instruction IR + topology/execution-mesh/boundary-shards + on-demand resource view 到 ABI / LLVM / packet emission 的参数单位、wait policy、golden packet | 上层 IR formation、layout search 和 SPM memory planning |
