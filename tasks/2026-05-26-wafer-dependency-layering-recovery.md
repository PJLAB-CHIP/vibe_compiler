...
状态：R0.3 完成记录；2026-05-26 修正为 PyTorch/XLA-rooted dependency stack 和统一 Shardy CMake gate
...
| SPMD bridge | `ShardySdy*` CMake shim target、future Shardy/SPMD pass target | Shardy/SDY source dependency、MLIR dialect registration、import/export/propagation pass compile gate | physical tile id、DTE algorithm、runtime package |
...
- `cmake/third_party/WaferDependencyVersions.cmake` 是 PyTorch/XLA、LLVM/MLIR、StableHLO、Shardy、
  OpenXLA/XLA、GTest、lit 和 importer Python wheel pin 的唯一事实源。
- `cmake/third_party/WaferThirdParty.cmake` 负责 MLIR/LLVM/Python discovery、pinned LLVM fallback、
  ...
  importer Python tooling boundary、future runtime/driver SDK roots 和 GTest fallback。
- `third_party/` 是默认 dependency root；source dependencies 作为一级 git submodule 维护：
  ...
  `third_party/shardy`、`third_party/xla`、`third_party/googletest`。Python tooling 和 downloads
  也放在该目录；`.deps/` 仅作为旧 build cache 的兼容输入。
- PyTorch/XLA source pin 是 frontend importer 的版本根；它的 `WORKSPACE` `xla_hash` 必须等于
  `WAFER_OPENXLA_XLA_COMMIT`。当前 PyTorch/XLA `396608c7...` 选择 OpenXLA/XLA
  `32ebd694...`。
- OpenXLA/XLA source pin 是 C++/MLIR dependency stack 的事实源；`third_party/llvm-project` 和
  ...
  commit；当前使用 `1838e285...`，它是 XLA base `1142aa48...` 的后代并把 standalone LLVM pin 对齐到
...
- `requirements-importer.txt` 固定 frontend importer Python wheels：`torch==2.5.0`、
  ...
  PyTorch/XLA source checkout 只能穿透到 frontend/importer tooling，不能穿透到 core compiler
  public dependency。
...
- `cmake/third_party/WaferShardyCMake.cmake` 在 `WAFER_ENABLE_SPMD_PARTITIONER_DEPS=ON` 时从
  ...
  dependency gate。
- `wafer-shardy-cmake-gate` 是 dependency compile gate；开启 SPMD deps 时 `check-wafer` 依赖它。
  该 gate 只证明公共 Shardy/SPMD 依赖能在同一套 pinned compiler stack 下编译和注册，不表示 R2.2
  artifact bridge 已完成。
...
- PyTorch/XLA、LLVM/MLIR、StableHLO、Shardy、OpenXLA/XLA 和 googletest checkout HEAD 是否匹配 pin
...
- Shardy CMake shim、`wafer-shardy-cmake-gate` 和 `check-wafer` 的依赖关系存在；Shardy target 使用
  `llvm_update_compile_flags`，避免和 pinned no-RTTI LLVM/MLIR 产生 ABI/link mismatch。
...
- R2.1/R2.2：真实 frontend importer artifact、sidecar、Shardy bridge 和 GSPMD-compatible
  partitioner integration 语义；当前只完成 public dependency pin、拉取、隔离检查和统一 Shardy
  CMake compile gate。
- torch-mlir source-tree adapter 的精确 commit 仍属于 R2.1 frontend importer 恢复任务；R0.3
  已先用 PyTorch/XLA 2.5 source baseline 收敛 XLA/LLVM/StableHLO/Shardy 版本根。
...
明确不再作为当前 Wafer dependency gate 的项：
...
- 不运行 `third_party/xla` 自己的 Bazel workspace 作为 Wafer core compile gate；XLA checkout 在 R0.3
  只作为 PyTorch/XLA-rooted dependency stack 的 pin 来源和 future GSPMD integration source。
