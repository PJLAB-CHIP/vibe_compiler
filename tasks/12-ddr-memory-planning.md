...
下游 target LLVM call emission、package metadata 和 runtime adapter 通过同一 resource view analysis 从已接受的
...
- Downstream consumer:
  ...
  target LLVM call emission、package metadata 和 runtime adapter 在使用点从 committed IR、
...
That check belongs to target LLVM call emission and runtime adapter binding.
