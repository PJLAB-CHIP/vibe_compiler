...
DeviceExecutable构造只能从accepted DDR facts和当前IR demand重算launch-facing requirements，并形成typed
...
不作为长期 attr 字段保存。
DDR planner接收complete candidate TileModule set；本stage不生成、排序或修补candidate。
...
- 从complete candidate TileModule set中all-and-only Tile modules的完整instruction-level programs重算DDR
...
- 只在整个top-level TileModule set通过时原子提交offsets；任一Tile/task/transport/event/target gate失败都擦除未提交subtree，
  不把已通过的DDR ranges部分写回。
...
- 不把单task/plan output、representative tile或full-shape initial coordinate特判当作完整DDR
...
Pipeline position:
- Upstream IR / input:
  complete candidate TileModule set after Instr conversion/SPM placement。
...
- User-level driver / named pipeline:
  complete candidate TileModule set→DeviceExecutable pipeline；wafer-opt只测试相同leaf stage。
- Explicit non-goals:
...
分配offset，也不写external access summary attr。DDR memory planning只在selected current IR中验证
...
launch-facing binding，selected `DeviceExecutable` formation必须把上述验证结果与frontend parameter boundary、accepted transport binding及
...
`DeviceExecutable` binding fields。
...
本节只记录未来extension约束，当前planner、DeviceExecutable和current source-to-package路径均不实现streaming/state resource；
它不能作为当前output、typed field或completion事实。

streaming是TileModule set-wide actual resource/lifetime形态，不是package读取优化，也不是DDR planner内部的window search。
...
本stage只计划其可见slices。无法证明时结构化拒绝selected streaming plan。若未来实现，不能新增`StreamWindowId`、
...
Demand recovery从selected current IR的memref SSA use-def、view relation和instruction effects推导role。external IO与
...
调用边界固定为：对selected finalized complete TileModule set原子执行actual evaluation；下列步骤按current
explicit arenas/placement domains形成fixed-capacity problems。problem/query数量只作budget diagnostic，逐Tile结果
不形成Tile-local survivor或partial apply，只有all-and-only Tile modules全部通过才原子接受整个candidate TileModule set。
...
2. Build structured lifetime dataflow and a pass-local provisional placement map before descriptor validation, so
   RDMA/WDMA root checks can consume the selected range without reading or materializing `wafer.ddr.offset`。
...
11. Materialize accepted offset facts only after every function/scope, descriptor and resource-limit check succeeds；
    ...
    DeviceExecutable atomic formation。

The solver order, lifetime bounds, intent merge and rejected placements are analysis. The accepted offset is the
...
retention/release，并擦除未提交top-level TileModule subtrees。外层controller可以继续其它explicit choice，但不得clone/replay失败candidate。
...
当前没有executable dialect或独立resource-view analysis对象。candidate TileModule set在DeviceExecutable formation前的verification gate
...
通过上述gate的candidate完整TileModule set在transaction-local storage中构造全部typed Tile records；不能从IR重算的boundary
...
通过后才能在同一transaction中构造并验证`DeviceExecutable`；只有DeviceExecutable也通过才把winning Tile modules与执行对象一次
...
package emission只从accepted `DeviceExecutable`及其verified target writing view序列化runtime-observable
