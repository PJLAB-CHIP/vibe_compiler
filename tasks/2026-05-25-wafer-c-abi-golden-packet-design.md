...
`wafer.abi.reduce` 作为 wrapper-first C ABI issue 点。它们保留 SSA/type verifier，用 explicit
byte count、M/K/N、elementwise/reduce kind、reduce dimensions、init value 和 `issue_only` wait
policy 表达参数单位、address direction 和 wait 边界；它们不是 raw packet dialect，也不保存
runtime physical address 或 BO handle。后续 C/LLVM lowering 可以把这些 issue op 转成实际
`wafer_*` C shim 调用，golden packet 测试再验证 shim 到 wrapper/packet field 的映射。
