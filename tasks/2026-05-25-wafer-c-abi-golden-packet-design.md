...
same-shape / limited-broadcast 子集的 `wafer.abi.elementwise` 作为 wrapper-first C ABI issue 点。
它们保留 SSA/type verifier，用 explicit byte count、M/K/N、elementwise kind 和 `issue_only` wait policy
