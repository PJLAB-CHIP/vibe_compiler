...
| resource view analysis | committed instruction IR with accepted SPM/DDR offset facts + topology/execution-mesh/boundary-shards contract + optional launch/block binding | launch signature、external binding、workspace、resident constant resource view，或结构化失败 | 从 committed IR 按需派生 launch-visible resource demand，不落第二份 metadata，不重新决定 layout/SPM/DDR plan，不 allocate/import/query runtime object |
| ABI / LLVM lowering | committed instruction IR + accepted offsets + topology/execution-mesh/boundary-shards + launch/block binding + resource view | LLVM dialect call / C ABI call / packet builder input | 从当前 IR 派生 ABI/packet 参数，生成具体 ABI call 或 packet emission，不回头修改 schedule/layout |
