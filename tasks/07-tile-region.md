...
- Current stage responsibility:
  ...
  return boundary 转成 `#wafer.memory<ddr, tensor>` memref signature。closed-loop selector提交后、One-Shot
  Bufferize之前先canonicalize已无use的外层tensor常量，避免把已在region内以fill表达的
  splat再生成无consumer的`memref.global`。
- Output artifact / IR:
