...
| `linalg.generic` simple elementwise / relation / select | 单 result、单 `linalg.yield`，typed scalar mapper 识别 add/sub/mul/div/min/max/neg/exp/sqrt/rsqrt/tanh、cmp eq/ne/lt/le/gt/ge，以及 `arith.select`；生成 `wafer.tile.elementwise` 并带原 `indexing_maps` | supported for simple CT family and basic select | 不靠 op name string 恢复语义；select 要求 predicate 为 `i1`、true/false/result value type 一致；复杂 region、多 result、dynamic mask policy 和 convert 仍需要明确 kind / op contract。 |
...
| `wafer.linalg_ext.collective.all_to_all` / nested collective | 当前无对应 buffer-level op 或 nested control-flow materialization | explicitly deferred | 需要先补可验证的 buffer slice / peer / token / schedule 表达；不能把 unsupported collective 静默降成名字约定或 pass-local side table。 |
...
5. Wafer instruction legalization / selection：instruction lowering 把 target-abstract executable op
   合法化并选择成 instruction-level `wafer.instr.*`。p2p Direct DTE schedule lowering 在这一层把
   accepted `wafer.tile.*` collective algorithm materialize 成 `wafer.instr.dte_send` /
