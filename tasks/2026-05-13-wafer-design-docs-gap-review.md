...
归属：`2026-05-11` 架构文档把整理后的 Wafer 主文档和 reverse-engineering 合同放在一级依据。
硬件事实和 runtime 事实来自 `docs/`；task 文档只把它们放到正确 IR stage 的 verifier / lowering /
runtime boundary 中。
...
归属：架构、compute、comm、layout、SPM 文档已经承接主要 verifier 事实的 IR stage
