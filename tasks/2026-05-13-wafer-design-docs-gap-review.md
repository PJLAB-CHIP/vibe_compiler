...
归属：硬件事实和 runtime 事实来自 `docs/`；task 文档只负责把这些事实落到对应 IR stage 的
verifier / lowering / runtime boundary。
...
承接：target-abstract compute/communication 留在 tile 层；lower-level C ABI 和 instruction-form
由 `tasks/2026-05-25-wafer-c-abi-golden-packet-design.md` 承接。
...
归属：layout/SPM、compute、comm、runtime/package、C ABI 和 verification plan 分别承接对应
verifier 事实，避免在单个文档里重复建模。
...
承接：layout 合同由 `tasks/2026-05-21-wafer-layout-materialization-design.md` 维护；本节只保留历史
gap 和实现检查点。
...
承接：SPM range、reserved range、layout storage size、liveness、allocation 和 failure feedback
由 `tasks/2026-05-21-wafer-spm-bufferization-design.md` 维护；`wafer.group` 不保存 offset。
...
承接：allocation、layout padding、overlap policy 和 issue/drain 分别由 SPM、compute/comm 和
scheduler/PMU 处理，不反向写成 `wafer.group` attr。
...
承接：DTE V0 由 `tasks/2026-05-25-wafer-communication-dialect-design.md` 维护；本节只保留实现和
测试时的事实检查。
...
归属：`wafer.group` 不保存 phase plan 或 resource schedule attr；issue/drain、communication wait
和 group barrier 在 `wafer.tile.region` 及更低层表达。
...
归属：group planner 只保留 abstract resource demand；SPM、DDR、compute、comm 和 runtime resource
事实分别由对应子设计维护。
