...
| R0.3 | done | 补依赖层级清单 | 记录见 `tasks/2026-05-26-wafer-dependency-layering-recovery.md`；public C++ source deps 已收敛为 OpenXLA/XLA 版本栈（LLVM/MLIR、StableHLO、Shardy、XLA、GTest）并按 `third_party/<name>` submodule 组织；PyTorch/XLA、torch-mlir 源码树不再作为 Wafer public source deps；target 可见范围、XLA stack pin 和 XLA/Shardy StableHLO patch 一致性由 `tools/check_deps.py` 检查；统一栈已通过 Wafer lit/CTest、LLVM install、Shardy build 和 XLA CPU build |
