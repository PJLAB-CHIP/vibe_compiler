...
| --- | --- | --- |
| 01、04、06、07、10、14、16、18、20 | device-scope-terminology-closure | builtin module直接拥有top-level `TileModuleOp(card_id, tile_id)`；per-Tile fan-out不依赖额外device wrapper；current analysis/cost/executable/DDR/workspace各只有一个受支持形式；`DeviceExecutable`是final whole-device owner；真实`CardId`/`card_id`/`card_count`保持原协议；退役名称无current引用；完整build与全部本地gate通过 |
...
| Owner | 状态 | 当前作用 | 证据入口 |
| --- | --- | --- | --- |
| 16、18 | `done` | checked-in `default` preset与唯一主工程`build/`；全部本地registered CTest、unit/model/lit/Tools、安装组件和managed dependency closure | 16、18；`tasks/archive/completed-task-index.md` |
