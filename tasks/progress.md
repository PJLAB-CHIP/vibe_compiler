...
| Q49.P | `deterministic-baseline-functional-closure` | `done` | Q54；Q59；Q50.A；Q50.0、Q45 | `none`直接构造一个live canonical coordinate，不枚举placement候选或调用recursive CSP/search；temporal fallback是确定性贪心，reduction temporal tiling已存在，reduction spatial partition/partial merge仍归Q50.B，完整temporal搜索域归Q50.E。每个closed coordinate只构造一个actual CardModule并运行一次Q50.0，accepted owner直接下传；root/Tile/function capacity probe、shadow schedule及旧ledger/API/test已删除。root/edge immutable闭包与support facts每coordinate按semantic key只分析一次，多root final region只复制必要SSA closure；16 Tile actual entry bounded并发并按Tile ID稳定归并。fresh定向unit 106/106、FP16 CHAIN/CROSS/GEMM source-to-package/no-card lit 1/1、三棵构建树及public link smoke通过。Q51完整new-search链闭合前不运行旧search或重型LLaMA；首次重型model profile归Q52。 | 06、14、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
