...
| Q50.A | `exact-placement-demand-boundary` | `done` | Q50.0、Q54 | 四项review阻塞项（2026-08-17）已闭合：`LogicalExecutionShard`/`ExactDestinationDemand`使query对每个consumer logical shard求exact demand（adapter按result indexing map逐Tile preimage构造execution domain，query按Tile id排序输出per-destination facts）；`LogicalTileBinding.resultIndex`按producer result绑定ownership，multi-result经production路径；真实invalidation由query的structural IR snapshot（function operation identity + body operation count）每次入口校验，`IREpoch`仅为borrow identity；Satisfied payload与确定性闭合（consumer domain/dependency/result identity/role填充，overlap witness单遍累加，intersections按Tile id排序）。canonical carrier（demand planner/evaluator/enumerator）只消费query fact，旧`deriveExactEdgeRelation`/`getBalancedShard`重算机制已删除；evaluator与enumerator复用同一trial+verdict组装。性能：`fromAffineMap`/`compose`/`preimage`/矩形image算术fast path与(edge, rectangle content) typed-content memo，纯优化不改结果。验证：fast suites 688/688、query suite 24/24（含4个新per-destination合同case）、enumeration 11/11（排除3个已登记病理长跑case）、默认lit 216/216、三棵构建树fresh build；`CardExecutableSynthesisTest.NoneJointlyRefines...`与baseline lit CROSS hang均为已登记Q49.P既有失败（非本批引入）。 | 05-07、09-13、16、18；`tasks/plans/physical-dataflow-synthesis.md` | | 05-07、09-13、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
