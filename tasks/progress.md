...
- R3.2h/R3.3 已接入 `wafer-select-group-tile` 和 `wafer-lower-groups-to-selected-instr`。候选由实际
  ...
  组合也覆盖 full-tile footprint 超过 72KB SPM planning window 后选择 smaller traversal tile +
  `K` split 并 commit 回主 IR。需要 tiled materialization 的 `scf.if` root 当前仍结构化
