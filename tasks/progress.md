...
| --- | --- | --- |
| Q52 | search-scalability | 本项17个线性checkpoint全部通过；05号基于pinned `egg` staticlib与typed C ABI的bounded access-relation e-graph只在attention recognition之后、policy分叉前对ordinary pure Tensor/Linalg graph运行一次：Importer只导入原始节点，request-local `IndexRelation`服务签发dynamic rule relation结果，至少一个fresh真实component由两条以上egg rules连续闭合；attention不创建e-node且保持opaque，candidate及后续stage不调用e-graph，不存在预构造candidate、自研C++或外部进程第二实现；预算耗尽保持原IR，on/off以exact proof保持语义和downstream representability而不要求raw choice identity；compact tile-and-fuse只从actual current SSA决定ordinary producer位置，exact query不缩减自由raw domain且前后保持attention opaque；独立selected-attention lowering每candidate恰运行一次并在layout前形成actual Linalg/Tensor/SCF，future attention action/value/materialization owner为零；baseline/search共同使用通过flat oracle的current-IR PBQP layout optimizer，可精确投影时分别使用NE、Vector/CT、movement和instruction-control term，必要事实未知时整组禁用；唯一full-transfer cleanup已接production；actual leaf内bufferization、completion repair和allocation sinking调用为零，普通bufferization/allocation、rotating allocation与Instr completion分别只有layout、execution-structure和completion owner，SPM/DDR high-water只从accepted offsets重算；observable result直接写唯一destination，冗余DDR copy为0、必要copy有witness且typed，Instr不创建copy-only TileRegion；search原子建立structural producer→compact tile/fuse→selected attention lowering→layout→movement→execution-structure→Instr/order/completion并从Accepted current Instr比较winner；旧shadow/future delivery/手写temporal/fusion/copy fallback零production残留；同一current FP16 LLaMA source的两次fresh package/no-card验收通过 |
