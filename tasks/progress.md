...
| 2 | `board-testing` | `doing` | 16；关联05、06、08、10、11、13--15、17 | current compiler/runtime、全workload/多预算审计及actual IR、实卡数值/profile证据 | 五项通用优化已实施，最终验收未闭合。重启后fresh Add与本轮GEMM实卡PyTorch通过，基础执行恢复；风险prefill的真实completion超时根因未明，按用户要求最后处理。完整tensor循环state的重复buffer type推导、嵌套子集提升遗漏及必执行循环的虚假零次路径已修复，GEMM大预算搜索与no-card闭合；异构search已在子集提升版本通过no-card。LLaMA有限预算可行性、DDR访问粒度排序、conv主机长测和最终模型/BF16数值及匹配性能仍待闭合；本项保持doing，分版本证据与失败范围见性能计划和记录，不以主机机制加速代签设备性能 | `tasks/plans/board-performance-optimization.md` |
