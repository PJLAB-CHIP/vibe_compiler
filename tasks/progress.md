...
interface skeleton 已落地；下一步进入最小 op/verifier。
...
继续从 P1.4 开始实现最小 `wafer.group` op/verifier，然后推进 `wafer.tile_region` 和 tile buffer
type。不要越过 P1 verifier 直接实现 transformer block 逻辑。
