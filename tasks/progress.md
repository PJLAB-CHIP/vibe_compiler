...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `board-testing` | `doing` | 16；关联13--15、17 | current compiler/runtime、正确性case的PyTorch source/input/eager reference、性能case的手写IR/input、通过本轮no-card的原样ExecutablePackage与可用设备会话 | Add、基础Direct-DTE和三组GEMM已有实卡通过证据；区域/选路、跨Tile DDR发布/获取及参数行cache可见性已修复，AllGather/AllToAll/ReduceScatter产品与通信专项的整除/尾部完整PyTorch实卡验收全部通过，最终主机构建、回归和no-card闭合。AllReduce的产品none/search及通信专项整除/尾部验收也已闭合；当前第6项已修复native归约降rank错读及Conv权重HWOI/Cx合同，独立Conv/Reduce的整除与尾部none/search均通过本轮完整PyTorch实卡。前端bias/logistic舍入、F32除法/比较及四路F32卷积累加已修复，独立整除/尾部和原组合none/search均通过本轮完整PyTorch实卡，第6项完成。Attention prefill已完成通用state/destination/fill有效期与coupled切分修复，1024/1025/1031×none/search的6项FP16实卡完整PyTorch比较均通过，原FP16/BF16及FP16尾部8项no-card通过；最终51项source/no-card与完整check-wafer通过，package身份与本轮launch一致，第7项正确性及公共elementwise写回优化完成：通用alias/effect证明消除相邻冗余copy，AllReduce每Tile原128B local写回消失；36个结构cell、最终51项source/no-card与check-wafer通过，AllReduce九项和prefill六项fresh PyTorch实卡复验全部通过，无新增join。第8项KV cache decode当前主机编译阻塞：none的区域级DDR/DTE检查产生假环，search合并集合共享本地依赖造成失效IR；尚未board-ready。按用户要求当前先推进第10项cost实卡校准：profiler报告port协议及通信候选profile传递已修复；已完成手写IR的2/8 KiB×DDR/DTE四组性能采集，无外部数值对比，actual指令、PMU、动态DTE阶段和正常清理通过。绝对时延及同步补测已闭合：有界时钟差分、两种payload的DTE首条/后续与空闲/搬运后NCC join均已实卡复验，guard/status与cleanup通过；首条/后续DTE及NCC固定调用/逐participant参数已回写同一cost cohort，多join合并/拆开已实卡对照，整段拟合参数已撤回。采样收口；参数/cohort、估时和比较已统一归属Analysis/Instr/CostModel，controller只消费评分，主机回归闭合；统一评分未完成，DDR/DTE tradeoff仍可能不可比。随后继续decode两步实际KV链，Llama与性能资格尚未验收。完整矩阵的PyTorch比较、guard/status、正常清理和计划内性能记录齐全后才可完成 | `tasks/plans/board-correctness-qualification.md` |
...
| 3 | `production-host-readiness` | `queued` | Q53 / 16 | `mesh-communication-materialization`产出的 current-IR 与 target-independent/target transport 分界、frontend、interface、package/runtime | 历史主纵向与smoke通过不能代签current模型资格；本轮精度no-card及完整check-wafer通过，但模型扩展此前发现prefill search lifetime、decode completion/崩溃和Llama manifest限制；prefill已在board-testing完成公共修复和6项FP16实卡，decode与Llama仍待解决。等待board-testing统一闭合模型问题后重签完整source/IR/package/oracle/runner/no-card/SystemC矩阵；本轮证据见板测计划，本项不运行真实设备 | `tasks/plans/physical-dataflow-synthesis.md` |
