...
| 2 | `board-testing` | `doing` | 16；关联05、06、08--11、13--15、17 | current compiler/runtime、全workload/多预算审计及actual IR、实卡数值/profile证据 | 已有循环state/lifetime修复与本轮Add/GEMM资格保留原范围，模型搜索和匹配性能未闭合。遗留步骤已规划，下一步为主机重复清理/分析；随后精确容量反馈→搜索工作量/存活IR与覆盖→DDR粒度估时→completion环→conv/BF16→非风险产品/预算/实卡A/B→风险4K prefill。步骤合同与矩阵见计划文首；未实施/未执行不算通过，主机机制改善不代签设备性能；全部规定资格闭合才done | `tasks/plans/board-performance-optimization.md` |
