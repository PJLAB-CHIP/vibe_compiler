...
| 2 | `board-testing` | `doing` | 16；关联05、06、08--11、13--15、17 | current compiler/runtime、原默认search全配置证据、扩展模型source及actual IR、实卡数值/profile | 按[扩展矩阵与三轮方案](plans/board-workload-matrix.md)推进：先完成原42配置及新增模型的正确性压测，再冻结基线/profile，依次调优复用与DDR、流水与等待、搜索选择及剩余热点。原42配置已有历史实卡计时，BF16 LLaMA数值问题仍待修复。当前GQA 1024/1025及长KV两步4094→4095→4096已通过本轮完整package/no-card，达到board-ready；长KV的tensor assembly输入需求归因已通用修复，组件门禁通过。固定FP16 LLaMA本轮no-card通过，完整运行包与上轮无卡包相同，已保存final IR；上轮view修改相对初始正确快包的设备数值与性能回归仍待验。ViT的GELU公开扩展/LayerNorm opmath已通用补齐，1024/1025 source通过，1024已到含attention的structured IR，随后达到1,800秒主机期限，One-Shot循环状态分析单次活跃至少1,628秒、内部根因未定；独立GELU默认search包/no-card及frontend门禁通过，LLaMA重新export的14个源文件与初始快版本相同。ResNet、大GEMM、batch共享RHS的整网编译/lowering、容量或数值缺口尚未闭合；混合dtype端口和整数exact比较已完成主机验证；完整单层LM已注册四个配置，S16 FP16/BF16完整CPU logits通过，但HF导出在pinned PyTorch装饰器/ModuleList追踪处失败，尚无source/package。YOLO尚未接入。QKV诊断曾发生真实设备超时，会话未恢复，实卡批次保持停止，风险项目最后处理。按影响与编译成本分层验证，LLaMA为可能相关修改稳定后及每轮末的强制检查项，保留初始正确快版本防累计退化；三轮均要求通用根因修复及关键case无可确认退化，最终冻结版本后全矩阵实际板测一次。用户已移除DLRM；正确性收口、三轮调优与最终实卡验收均未完成，主机局部通过不代签总任务 | `tasks/plans/board-performance-optimization.md` |
