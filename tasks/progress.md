...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `search-scalability` | `doing` | Q52 | Q51 search controller、structural choice与attention展开donor，current baseline regression，Q50.0 actual memory/target leaf | 按current plan的20个线性checkpoint施工：第6--10项已经闭合actual leaf、Instr completion、execution structure、movement和structured logical normalization。当前下一直接项为第11项，并在同一work item内严格按“切断产品caller→隔离保留资产→从末端删除shadow graph→删除孤儿/test-only接口→全仓零残留”五批执行；PBQP、SCF pipeline/rotation和attention Linalg算术必须先解除`RepresentationPlan`、`EventGraph/BufferPlan`及future attention inventory依赖，不能原样搬文件。两种policy在第17/18项重启前明确unavailable、互不fallback，direct stages与actual leaf保持可构建。第12项从保留的Spatial/ExactDemand/Region partition资产实际创建Card/TileRegion；第13项compact tile/fuse；第14项selected attention lowering；第15项layout/bufferization；第16项显式重接movement→execution→Instr/completion→actual leaf；第17、18项分别重启独立none和search；第19项闭合inventory；第20项同源LLaMA验收。SPM legality仍只由actual MiniMalloc决定 | 读AGENTS/progress→读05/06及本项20个矩阵→按每个semantic work item分别完成算法调研、pinned API确认、代码/测试、fresh验证和MLIR规范复审→更新状态并提交 |
