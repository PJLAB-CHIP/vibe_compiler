...
| device-code compile/link gate | done | LLVM IR artifact + repo-vendored TX8 deps + repo-local Wafer CRT lib dir | LLVM `clang++` `.ll -> .o`、default `wafer_cabi_shim.c -> shim.o` 和 repo-vendored `tx8_deps` GCC `.o + shim.o -> kcore .so` 的命令形态固定；本地 smoke 已证明 LLVM IR 和 shim source 可生成 RISC-V relocatable object，vendored toolchain 提供 `rv64imafdc/lp64d` multilib，最终 shared object 仍需要在 `third_party/wafer_crt/lib` 提供 Wafer CRT `libvr`；manifest validator 要求 `device_code` 记录 `kcore_shared_object` |
