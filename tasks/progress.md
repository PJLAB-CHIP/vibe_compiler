...
- R2.4-pre 已完成库级 Wafer named pipeline / driver contract：`WaferPipelines` 注册
  ...
  两层。`wafer-import-model --prepare-stablehlo-spmd-bundle` 从 bundle verifier 进入同一条
  Shardy propagation pipeline；`wafer-import-model --compile-stablehlo-bundle-to-cabi` 从 verified
  local/partitioned StableHLO bundle 进入 StableHLO->C ABI pipeline，并拒绝带 pre-SPMD sharding
  seed 但没有 post-SPMD marker 的 bundle，防止绕过 Shardy/XLA SPMD。XLA SPMD partitioner 当前仍由
...
- R2.4-pre 已固定主链路 gate 的 named pipeline / driver 入口：pre-SPMD bundle 可通过
  `wafer-import-model --prepare-stablehlo-spmd-bundle` 运行 default input seed + Shardy
  propagation；post-XLA-SPMD partitioned bundle 或明确 replicated-local StableHLO bundle 可通过
  `wafer-import-model --compile-stablehlo-bundle-to-cabi` 进入编译。`wafer-opt` 层的
