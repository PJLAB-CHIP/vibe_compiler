...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `board-testing` | `doing` | 16；关联06、13--15、17 | current compiler/runtime、静态计算的通用 temporal 合同、三条模型的 fresh FP16 PyTorch source/input/reference、profile package与合格设备会话 | 既有正确性与单次计时资格保留在原归档；按用户新授权，采集prefill/decode/LLaMA block真实profile，定位瓶颈根因并实施通用优化，完成host、fresh no-card、PyTorch与匹配性能复验 | `tasks/plans/board-performance-optimization.md` |
