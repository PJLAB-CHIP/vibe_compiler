...
| Q42 | `test-system-scope-refactor` | `next` | 无 | 一次性重构全部现有case和公共测试入口：每个test只判直接合同，局部、integration和qualification分层，重复资格移出case；迁移后默认及后续任务不再全仓扫描或运行无关suite。 | 16；`tasks/plans/test-gate-scope-reduction.md` |
