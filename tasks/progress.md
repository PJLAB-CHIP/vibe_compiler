...
| Q19 | `reference-executor-core` | `doing` | Q16 | 已落owner-backed immutable `ReferenceProgram`和执行前capability preflight：flat/tile-region、static view、movement、GEMM、f32 elementwise/fill、非zero-point convert、single-block SCF/acyclic CFG及direct non-recursive call closure均投影为value-id function/block graph，执行期不读mutable MLIR；Q16/Q17已统一为唯一typed entry加private closure，RND_MODE 0..3、0/1/2 trip、branch/CFG/call forwarding已通过；fixed-seed residual MLP以所有hidden channel和两层非零bias都影响完整输出的独立CPU-loop oracle闭合；test-only slow coordinate oracle已跨compact/Cx/NCx、rank/dtype、tail对齐和channel block边界逐坐标证明physical footprint/offset/唯一性，并使所有layout统一拒绝越界坐标；convert type pair/parameter policy已收敛为IR typed helper，TableGen全枚举gate执行全部非zero-point kind并在input/arena前拒绝全部zero-point kind；executor已按internal immutable graph、projection、numeric/storage、interpreter和薄orchestration拆成单向职责文件。仍需zero-point/stochastic证据。 | 10、11、16 |
