...
| 2 | `board-testing` | `doing` | 16；关联05、06、08、10、11、13--15、17 | current compiler/runtime、全workload/多预算审计及actual IR、实卡数值/profile证据 | 已实现可恢复分层搜索及布局/通信/共享/流水组合；4K预算14→42实卡514.896→276.943 ms且PyTorch通过。保留K搜索并贯通独立format、FP32 psum及最后GEMM原生输出；两种dtype三段K定向实卡逐bit通过、partial只读/guard通过；生产none/search的fresh GEMM package均已消除独立add/convert、no-card通过；私有DDR链同步最终format，F32观察者保留。前轮FP16 GEMM尾部实卡通过。完整K的BF16 prefix仍有2项超原容差，分块F32中间输出复验满足原BF16容差；完整block的42次主机搜索在第37次layout物化/bufferization耗时增长处主动停止，尚无该模型新板端资格。继续闭合数值、编译耗时及LLaMA/decode预算曲线，总任务未完成 | `tasks/plans/board-performance-optimization.md` |
