...
当前没有 `doing`。当前唯一 `next`：Q0 Target LLVM semantic correctness。2026-07-10 审计确认
structured control flow 会被 flatten 成无条件调用，因此原 call-emission 局部闭环不再满足完成门槛。
Q0 完成后依次重排 Q0.1-Q0.4；这些 P0/P1 correctness/architecture closure 都先于 extended CRT surface。
...
| Q0.2 | `blocked` | Rank-aware executable identity | 先在编号设计中收敛 rank-parametric ABI 或显式 per-rank variant；删除 partition/replica id 常量 0、pass-only rank 和默认 rank 0 语义通道。 | `tasks/03-shardy-spmd.md`, `tasks/04-topology-execution-mesh.md`, `tasks/07-tile-region.md`, `tasks/15-launch-runtime-package.md`, `tasks/16-verification-plan.md` |
| Q0.3 | `blocked` | Committed executable composition | 定义唯一 committed executable artifact；direct full-shape 只能作为 candidate policy；同时决定 scheduled group 是否是正式 IR，并闭合跨 group/region 的 persistent/transient resource lifetime。 | `tasks/01-architecture.md`, `tasks/06-group.md`, `tasks/09-spm-memory-planning.md`, `tasks/12-ddr-memory-planning.md`, `tasks/15-launch-runtime-package.md`, `tasks/16-verification-plan.md` |
| Q0.4 | `blocked` | Async completion and reuse contract | 收敛 compute/DMA/DTE/host completion ownership；证明 hardware busytable/worker mode 假设或用显式 event/fence 保守建模，闭合 SPM reuse、timeout 和 error propagation。 | `tasks/09-spm-memory-planning.md`, `tasks/10-compute-movement.md`, `tasks/11-instruction-ir.md`, `tasks/13-communication.md`, `tasks/15-launch-runtime-package.md`, `tasks/16-verification-plan.md` |
...
| Q4 | `blocked` | Typed package ABI and auto-export mainline | 阻塞于 Q0.2-Q0.4；先修复 exact typed entrypoint/workspace binding，并携带已验证 endpoint projection 或可确定性重建它的 target/topology identity；随后只消费 committed executable 的派生产物。 | `tasks/15-launch-runtime-package.md`, `tasks/16-verification-plan.md` |
...
| Q4 | package metadata 由 committed executable 的结构化派生产物自动导出；entrypoint typed arguments 与 binding order 完整、唯一且一一对应；`modules` 绑定真实 kcore `.so` digest/target/exports；endpoint projection 被携带或由绑定的 target/topology identity 确定性重建；同一 validator contract 被 runtime 消费。 | schema roundtrip 只覆盖手写 JSON/YAML；从参数数量、文件名或 fixture 猜接口；漏/重 workspace binding；既不保存 endpoint projection 也不保存其可重建身份；复制 instruction schedule。 |
