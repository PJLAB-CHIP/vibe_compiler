...
| Q49.P | `deterministic-baseline-functional-closure` | `done` | Q59；Q50.A；Q50.0、Q54、Q45 | `none`已从正常TensorProgram直接构造一个live canonical coordinate，不枚举placement候选、不调用recursive CSP/search；exact conflict只驱动预定义、单调、无分支且不回溯的spatial/temporal functional legalization。session级immutable source preparation一次，trial从第一个coordinate起只物化一个root/参与Tile shard，全部fit后完整CardModule与Q50.0 compile各一次；IndependentDDRStages按actual endpoint直接形成一root一region与显式DDR carrier，post-hoc multi-root splitter退出baseline。current relation/scratch evidence、function-scope escalation、raw SPM certificate、rank-0 typed unpartitioned、bounded Presburger preflight、deterministic spatial advance、source-only/peer carrier effect与sealed-result cache均闭合。baseline public result/ledger与旧search statistics/header分离，accepted result不依赖cost、shadow schedule或默认IR trace；一般workload仍使用16个active Tiles并由bounded executor并发处理，rank-0退化为一个参与Tile但保留16-entry ABI。fresh bounded非search host gate 117/117、CHAIN/CROSS/GEMM/no-card及组织/negative lit 3/3、main/board-runtime/SystemC-model三树构建通过；未运行search或重型LLaMA。reduction temporal tiling和greedy functional fallback已存在；reduction spatial partition/partial merge仍归Q50.B，完整temporal domain归Q50.E，new search归Q51。 | 06、14、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
