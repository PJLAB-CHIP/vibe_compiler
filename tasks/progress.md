...
| 2 | `board-testing` | `doing` | 16；关联05、06、08--11、13--15、17 | current compiler/runtime、原默认search全配置证据、扩展模型source及actual IR、实卡数值/profile | 原默认search全配置已有实卡计时，除BF16 LLaMA数值超差外完整PyTorch通过，FP16 LLaMA已恢复历史性能目标。当前按[扩展矩阵与三轮方案](plans/board-workload-matrix.md)推进：先修BF16 LLaMA并完成新旧case正确性压测，再冻结基线/profile，依次调优复用与DDR、流水与等待、搜索选择及剩余热点；每轮要求通用根因修复、完整数值和固定关键case无可确认性能退化。已接入ResNet、ViT、大GEMM、batch共享RHS及GQA；嵌套view地址修复通过主机门禁及batch package/no-card；ResNet的BN合法化和opmath已通过主机数值及Linalg，整网编译及窗口归约lowering仍待修复。E-graph复用已完成LLaMA no-card且包与原正确快版本相同；后续Region提案的精确计分/合法性查询优化已通过主机门禁，该修改的LLaMA no-card通过且包与原快版本相同，ResNet整网仍触发主机编译期限。GQA source/主机reference已通过；多M/N轴contraction已通用修复并通过精确坐标及多Tile主机验证，正式search已越过lowering，现仍全部被实际SPM容量拒绝。多轴修复已通过完整Transforms/Driver及LLaMA no-card，LLaMA包与初始正确快版本逐文件相同。后续容量修正已复用current-map状态协调，主机门禁通过；GQA仍被actual容量拒绝，实际IR另确认metadata reshape阻断局部加载、导致完整输入进入SPM，下一步修复该通用搬运缺口。协调修改的LLaMA no-card已通过，完整包与初始正确快版本相同。数值、容量和ViT前端缺口尚未修复，YOLO、完整LM和长cache尚未接入。QKV独立诊断发生真实设备超时，已停止实卡批次，继续主机准备/分析。按影响范围与编译成本分层验证，不每轮全量重编；LLaMA为可能相关修改稳定后及每轮末的强制检查项，保留初始正确快版本防累计退化；最终冻结版本后全矩阵实际板测一次。用户已移除DLRM；正确性收口、三轮调优及原预算/主机开销/completion/lowering边界均未完成，风险设备执行最后处理；局部通过不代签总任务 | `tasks/plans/board-performance-optimization.md` |
