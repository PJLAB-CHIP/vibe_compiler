...
| Q58 | `program-data-ownership` | `doing` | Q50.0、Q54、Q55 | 2026-08-16代码review重新打开：现有实现已建立`ProgramDataSource`/`ProgramDataRange`/`ProgramDataHandoff`与Card/target handoff，但尚未满足transaction ownership合同。阻断项是大payload的默认`MemoryBuffer::getFile`会mmap用户文件，不能保证establish后bytes不受同inode原地写入影响；helper输出、tensor-program readback和CardExecutable边界仍存在无`ProgramPayloadResolver`的payload reopen/read，`program-data-io`账本因此不完整；range未证明source shape与global/local shape的对应关系；共享dtype width表把target尚未支持的`i1`重新放入`ProgramTensor`；materialization与shard digest I/O错误未稳定填充typed failure。此前小payload mutation、region digest和shared-view单测没有覆盖这些production分支，0.5/2/8 MB账本与RSS只保留为部分性能背景，不能签发完成。完成要求见现有计划中的2026-08-16 review记录；Q56继续等待Q58闭合。 | 02、14、16、18；`tasks/plans/program-data-and-whole-program-scale.md` |
