...
| 2 | `board-testing` | `doing` | 16；关联05、06、08--11、13--15、17 | current compiler/runtime、全workload/多预算审计及actual IR、实卡数值/profile证据 | 通用复用排序、容量冲突证据汇总和搜索调度已实施；同配置LLaMA FP16默认预算完成fresh package/no-card及完整PyTorch实卡，已达到用户要求的历史性能目标，早期无可行解回归已修复。动态Subview物化缺口修复后，全部默认search产品通过本轮复验，最终LLaMA包及fresh输入/reference与实测产物精确一致。继续原前7步：主机开销→容量反馈→搜索覆盖/预算/存活IR→DDR估时→completion环→conv/BF16→非风险产品与匹配实卡验收；风险4K设备执行最后处理。合同、覆盖矩阵和当前边界见实施计划；全部默认search配置已完成本轮实卡单次计时；除BF16 LLaMA正常执行后的数值超差外，其余配置含两步decode及默认预算4K均完整PyTorch通过，无设备超时。后续先处理BF16 LLaMA数值缺陷；用户指定的新网络、单层完整LM及4096³ GEMM的测试矩阵已形成，具体输入/输出、DDR/DTE配对和选定补充case见[实施矩阵](plans/board-workload-matrix.md)，新增case尚未实现/验证。完整预算曲线、completion/lowering遗留、匹配profile与历史高预算风险候选仍未闭合，单次计时清单完成不代签整个板测项 | `tasks/plans/board-performance-optimization.md` |
