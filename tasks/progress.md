...
Q50.0 + Q54 -> Q50.A exact placement-demand boundary          [doing]
Q50.A -> Q49.P canonical-plan foundations + Q50.F closed-plan core
                                                               [queued]
Q49.P/F core -> Q49.P deterministic baseline plan-only closure
                                                               [doing]
Q49.P -> Q50.S semantic-root query/algorithm                  [queued]
Q50.S + Q50.A + Q64 -> Q50.B graph-level spatial placement   [queued]
Q50.B -> Q50.C root-work query / selected leaf emitter        [queued]
...
Q49.P + Q50.F full closure + Q50.S–K + Q50.I post-K closure
  -> Q51.Core planning controller
                                                               [queued]
...
`Q49.P canonical-plan foundations + Q50.F closed-plan core`是一个协调纵向，不是循环依赖：先把baseline当前已真实消费的component
迁入final typed plan，再实现这些component的F resource description/core，最后Q49 controller消费proof。该纵向不实现或宣称完整
Q50.S–K search domains。K之后的I重闭是独立必经checkpoint，不能折叠进J或F。
...
| Q49.P | `deterministic-baseline-functional-closure` | `doing` | Q54、Q59、Q50.A、Q50.0、Q45及Q50.F closed-plan core | 已迁入的consumer-operand exact demand、structured-producer截断、single-root final construction和local/peer/DDR coverage继续保留。当前首要纵向先把baseline真实使用的canonical spatial/region/temporal/representation/movement/single-slot/Serialized/order事实迁入最终policy-free plan component，并交Q50.F closed-plan core证明resource；不等待完整search domains。随后删除per-coordinate CardModule/Q50.0：ExactRejection沿预定义first-fit推进，FullFeasibilityProof关闭plan，planning CardModule/Q50.0为零、selected各一次。旧181.70秒LLaMA只证明功能链；新纵向完成后重跑定向oracle、轻量package/no-card和一轮fresh FP16 LLaMA才能恢复`done`。 | 06、14、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.A | `exact-placement-demand-boundary` | `doing` | Q50.0、Q54 | 本轮设计重审已确认旧完成结论无效：`LogicalShardTrial`重复execution/ownership事实，partial contribution被误作下游result owner，node-wide merge Tile不能表达M/N/K mixed partition，per-edge与operand support walk重复，manual epoch/fingerprint和generic Presburger fallback也不满足新合同。终态以`SpatialAssignment`为唯一输入，operand-level SSA worklist派生`ExactDemandProof`、final owners和per-output-piece reduction requirements；construction normal form在操作前受work bound约束，standard analysis invalidation替代manual snapshot。实现须协调迁移全部baseline、Q50.B及G–J consumers，保留relation/query/carrier测试资产并替换旧错误witness后才能重新标done。 | 05-07、09-13、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
| Q51.Core | `physical-dataflow-search-kernel` | `queued` | Q50.0、Q50.A、Q49.P重新完成、Q50.S–K、Q50.I post-K及Q50.F/J closure | 建立driver-owned、IR-free planning session：closed state variants与typed continuations按S→B→D→E→G→H→I-initial→K→I-post-K→J依赖惰性展开；proposal不移动canonical cursor。exact work、admissible bound和estimate分层，同cohort best-first只凭bound prune；direct causal `ForbiddenAssignment`不猜scope。planning allowance限制successor/query/solver work而非candidate编译；coverage区分objective-optimal、feasible-with-bound、feasible-unranked和budgeted。`none`/`search` controllers独立，只在final plan schema/materializer/downstream pipeline汇合；selected plan只commit一次。 | 06、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.S | `structured-semantic-alternatives` | `queued` | Q50.A、Q49.P plan-only closure（施工顺序门禁） | 终态设计不再枚举或clone whole-program alternatives：search TensorProgram先把已证明的softmax-weighted-sum一次性归一为显式semantic op；每个root只选择MaterializedSoftmax/OnlineRecurrence，block归Q50.E、partition/merge归Q50.B/A。planning读取typed logical facts但零IR materialization；winner进入Q50.C唯一CardModule transaction后由selected emitter直接构造actual work，不写临时algorithm property。先完成Q49是为了清掉已有compile-to-query和建立真实common plan/F core，不表示baseline调用Q50.S。实现须迁移current/旧attention、decode、Flash tile/partial算法与tests，删除Module clone、Original/Online/Split(block,partition)旧domain和decode-only gate。 | 05-06、08、10、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.B | `spatial-partition-and-placement` | `queued` | Q50.S、Q50.A、Q64 | 终态设计以compact `SpatialPlan`保存per-iterator balanced/uniform-tail scheme、injective logical-mesh embedding和per-output reduction-group merge Tile；exact domains只在关闭plan时形成Q50.A ephemeral assignment。domain lazy覆盖全部current regular partitions、available subsets/embeddings/merge placements，仅verified topology automorphism可dedup。compact、independent-component及chain/tree/general-DAG DP只作proposal/bound；旧live-boundary estimated Pareto不在下游轴未关闭时删state。实现须迁移current/旧placement tests并移出edge carrier、observable axis、movement/schedule/resource signature。 | 05-08、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.C | `maximal-single-root-tile-region` | `queued` | Q50.B | 终态设计把Q50.C收窄为derived `RootRegionWork`和single-root leaf emitter：Q50.A operand reconstruction是唯一support closure，按`(root,Tile)`合并selected cells及per-output merge work并在其它structured results/program inputs处截断。production不先建singleton CardModule再由Q50.D改写；完整winner在同一source Module下建立可RAII擦除的CardModule subtree并直接调用singleton/coupled emitter一次。实现须删除trial/epoch、recursive second walker、implicit DDR/pending fallback及Module/Func/DAG clone，补强multi-producer exact-boundary和atomicity witnesses。 | 05-08、10、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.D | `region-formation-and-producer-execution` | `queued` | Q50.C | 终态把region membership、producer execution instance和consumer-use binding拆开：`RegionPlan`保存connected mandatory-root partitions；每个required piece或显式replica选择top-level/consumer-nested placement，每个exact use fragment绑定stored region value、direct nested value或cross-region boundary。recompute归D的显式execution count而非H movement action；nested relation从A exact relation与standard interfaces派生，temporal要求交E关闭。exact successor与proposal、winner emitter及donor迁移见计划；实现须删除fused bool、三态/五态混合action、greedy repair、movement反控fusion、tile-size/DDR混入和whole-module apply。 | 05-10、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.E | `complete-temporal-tiling` | `queued` | Q50.D | 终态temporal坐标锚定per-Tile `ExecutionInstancePlan`派生的scope：top-level exact work pieces及parent wave产生的nested invocation classes各自选完整iterator vector/order，nested producer的内部reduction/window维不因consumer-driven而消失。可tile维的每个`1..extent`整数都在exact-tail合法域；per-Tile variables独立，等价scope只共享domain query。Q51以integer intervals惰性branch，order枚举structural precedence DAG全部linear extensions；proposal算法与baseline greedy见计划。实现须删除node-wide/global-max vector、query/apply reduction错位、preferred-size legality、allocation feedback/post-hoc retile，并迁移multi-piece/multi-reduction/tail/nested actual mechanics。 | 05-10、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.F | `scoped-feasibility-analysis` | `queued` | closed-plan core依赖Q50.A/Q50.0与Q49 canonical component extraction；partial foundation依赖Q50.E；full closure依赖K→I-post-K→J | 三次接入同一实现：先交付resource problem schema、validated placement/status和plan/actual parity，让Q49 canonical closed plan零IR取得`FullFeasibilityProof`；再以closed sum为A–E partial prefix返回Consistent/Deferred/ExactRejection/Unsupported/Indeterminate及minimum-storage/interference proof；最后对S–K→I→J完整plan闭合SPM/DDR、DTE/FSM、descriptor、worker/event和target-field问题及独立oracle。后续choice只扩同一component resource description，不建baseline专用schema。Q50.0从winner actual IR重建同一normalized problems并分配offset；不一致是compiler bug。 | 06-11、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.G | `layout-and-physical-representation` | `queued` | Q50.F foundation | 终态domain以`RegionValueVersionId`为logical identity，显式保存primary `PhysicalVersionId`、execution/boundary/alias/conversion producer DAG及per-use binding；不再按`(Tile,node,operand/result)`覆盖source identity。finite constraint graph以exact support propagation、R0/R1/R2和residual branch-and-bound产生proposal/lower bound，fixed Top-k/local winner不缩域。完整winner在C transaction中由`PhysicalVersionBuilder`按ID一次绑定/物化，H只提供boundary/transport versions；禁止`lookupAny`、operand map save/restore、hidden DDR load/alloc clone。independent oracle、PhysicalLayoutRelation point proof、atomic actual witnesses和donor迁移见计划。 | 06-13、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.H | `explicit-data-movement` | `queued` | Q50.G | 终态只处理D/A explicit boundaries，不再拥有retention、recompute或layout选择。current Direct DTE建模为任意available endpoints间的target-routed fixed unicast；physical mesh只给minimum-hop/cut facts，`SimpleRoute`语义迁为显式software relay graph，opaque routing不伪造link load。统一`TileCommunicationProblem`以chunk的initial/required payload states和transfer/combine proof DAG定义完整有限域；Ring/Tree/double-tree、recursive exchange、row/column、Bruck/pairwise及tree-packing都只是checked topology-aware proposals，不是算法枚举值或local winner。winner apply、alias/lifetime/slice proof、message/token及donor迁移见计划H-3；raw hardware multicast未qualification前不进入production。 | 06、08-13、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.I | `storage-binding-and-rotating-slots` | `queued` | initial domain依赖Q50.H；structure-specific closure依赖Q50.K | initial I在Serialized occurrences上定义`PhysicalVersionId` storage、fresh/exact-alias/reuse、`1..U` slots、rotation和J requirements，只供J-foundation/K eligibility。K改变prologue/steady/epilogue occurrence与live distance后必须进入post-K I closure：Serialized归一single-slot，Pipelined重新枚举并证明multiplicity/rotation/ReuseAfterCompletion，再以新generation交J closure。winner由construction-time builders扩展E/K loops；旧actual-loop scan、clone/reorder、wait/NCC混装和fixed cap退出。 | 08-13、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.J | `event-resource-scheduling` | `queued` | foundation依赖I-initial/Q63；closure依赖固定K和I-post-K | foundation从S–I-initial plan构造query-local EventGraph：hard/disjunctive dependencies、async completion、ranges和typed resources，供K eligibility；不选择最终schedule。K后由I重闭storage/reuse edges，J closure才枚举worker/resource/control sequence和completion boundaries并应用critical/resource/recurrence/transport bounds。winner由ScheduleEmitter按EventId创建order/worker/DTE waits/min NCC joins/releases，Card verifier重证deadlock/pending/resource；calendar/timestamps不进state。 | 08、11、13-19；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.K | `conditional-execution-structure` | `queued` | Q50.J event/resource foundation | 每个recurrence scope有唯一Serialized identity；Pipelined是typed ordered stage partition、launch distance和inter-iteration dependences，只有E occurrence、H data-ready/completion、I slots/lifetime、J foundation及target cyclic capability全部闭合时才广告，multi-slot本身不触发。固定K后重算I/J。winner对distance-0/1使用pinned `scf::pipelineForLoop`机械构造，对qualified static period使用独立periodic builder；二者只作用于新Card subtree，不clone source/loser。旧StagePipeline/FixedSlot/selected-buffer混装及DTE/NCC/alias/periodic/tail donor迁移门禁见计划。 | 06、08-13、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
| Q51 | `physical-dataflow-unified-search` | `queued` | Q50.S–K、Q50.I post-K、Q50.J schedule closure、Q50.F full closure、Q51.Core | independent reference composer按S→B→D→E→G→H→I→K→I→J parent/full plan证明全轴domain、F与cost；test-only fresh actualization验证plan→IR/Q50.0，不进入production。production planning对任意proposal/no-good/bound/parallel开关保持exact winner，CardModule/Instr/Q50.0均为零；selected `PhysicalDataflowPlan`经一个Card subtree transaction和一次Q50.0形成winner。旧Planning/Search每项能力迁入对应Q50/Q51 owner并有direct witness后整目录退役；public search不调用/fallback baseline。 | 06-13、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
...
| Q32.I | `mlir-native-implementation-relation-foundation` | `done` | 历史MLIR-native IndexRelation与typed lowering foundation闭合；旧implementation-choice资产是否由Q50.S–K/Q51完整承接正在重审，不能再由current source删除直接推断迁移完成。 | `tasks/archive/mlir-native-implementation-relation-foundation.md` |
