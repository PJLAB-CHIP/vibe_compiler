...
| Q49.P | `deterministic-baseline-functional-closure` | `doing` | Q54；Q59；Q50.A；Q50.0、Q45 | `none`直接构造一个live canonical coordinate，不枚举placement候选或调用recursive CSP/search；temporal fallback是确定性贪心，reduction temporal tiling已存在，reduction spatial partition/partial merge仍归Q50.B，完整temporal搜索域归Q50.E。fresh FP16 LLaMA baseline暴露current materializer仍从result/edge endpoint无条件回溯SSA闭包，并在carrier apply后递归重建support graph：一个destination上的exact-empty producer被重新拉入，导致合法multi-producer `insert_slice` fan-in报未组装structured producer；因此此前done结论和“重型LLaMA不属于门禁”均撤回。修复必须从全部Tile root execution domain一次性反向传播exact demand，在structured producer处形成typed boundary obligation，先证明local/peer/DDR all-and-only coverage，再一次性构造最终Tile IR；删除过宽closure及post-hoc support rebuild/replay，不在edge strategy增加empty布尔旁路。baseline不运行search；fresh FP16 LLaMA `optimization-none` source-to-package/no-card通过且work ledger证明relation共享、没有16次整图分析后才能完成。 | 06、14、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
