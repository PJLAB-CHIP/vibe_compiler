...
| 2 | `board-testing` | `doing` | 16；关联05、06、08、10、11、13--15、17 | 已交付通用优化的current compiler/runtime、actual candidate IR、三条模型的fresh FP16 PyTorch source/input/reference、既有profile证据 | 本轮Halo/中间buffer、已定位搬运、主机机制正确性与编译开销修复已完成；后续按完整LLaMA→KV decode→4K prefill完成fresh no-card、全量PyTorch及匹配实卡profile/性能复验，decode包含hidden/K/V与历史prefix；新增优化由新profile确认的热点启动，搜索预算比较继续延后 | `tasks/plans/board-performance-optimization.md` |
