...
| Q42 | `test-system-scope-refactor` | `next` | 无 | 一次性简化全部现有case和公共测试入口：每个test只判直接合同，共享检查按需单独运行；不建新分层框架，迁移后不再全仓扫描或运行无关suite。 | 16；`tasks/plans/test-gate-scope-reduction.md` |
