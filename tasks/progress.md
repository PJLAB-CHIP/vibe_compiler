...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `search-scalability` | `doing` | Q52 | Q51 search controller、structural choice与attention展开donor，current baseline regression，Q50.0 actual memory/target leaf | 按current plan的16个线性checkpoint以consumer-first施工：第6项抽出completion-closed Instr→actual memory/target leaf，第7--10项依次闭合Instr completion、execution structure、movement和layout/bufferization；第11项让observable output在bufferization前绑定actual destination，消除compiler-created DDR→DDR `memref.copy`及Instr copy-only TileRegion fallback；第12项以pinned MLIR SCF tile-and-fuse替换手写`first/steady/tail`递归展开，保持stored/reconstructed/direct-nested delivery并只为必要remainder做late peeling；第13项运行完整baseline gate；第14项原子切换search current-IR production、resource-aware winner和旧shadow/complete路径删除；第15项闭合IR/layout/copy/movement/Instr inventory；第16项以同一current FP16 LLaMA source完成`none`/`search`各自15分钟package与strict readback/no-card。最终baseline不进入search state，search每个choice先物化actual IR；同region优先local SSA/SPM reuse，只有actual boundary需要时才与DDR交互，SPM legality仍只由actual MiniMalloc决定 | 读AGENTS/progress→读06及本项16个矩阵→读相关硬件/ABI事实并把未证同步、engine overlap和performance rate保持其证据边界→调研论文和成熟编译器算法/实现并比较取舍→查官方及pinned LLVM/MLIR API→按checkpoint改代码/测试→fresh验证→按设计与LLVM/MLIR规范复审完整diff和下游witness→更新状态并提交 |
...
| --- | --- | --- |
| Q52 | search-scalability | 本项16个线性checkpoint按current consumer-first施工顺序全部通过；baseline/search共同使用修复全局tie、通过flat oracle且按NE、Vector/CT、movement和instruction-control分别投影的current-IR PBQP layout optimizer，baseline每attempt一次、search首proposal且PBQP on/off不改raw域或exhaustive accepted set；budgeted差异保持typed coverage并报告；第9项唯一full-transfer cleanup已接production；第6项actual leaf内bufferization、completion repair和allocation sinking调用为零，普通bufferization/allocation、rotating allocation与Instr completion分别只有第10、8、7项owner，SPM/DDR high-water只从accepted offsets重算；第11项闭合output destination与standard copy，movement closure后compiler-created DDR→DDR copy为0且Instr conversion不创建copy-only TileRegion；第12项以唯一SCF tile-and-fuse mechanics保留selected local delivery，stored producer只物化一次、direct-nested all-and-only、remainder不复制first或按wave展开compute closure；第14项原子建立search structural producer和current-IR layout→output/copy→movement→execution-structure→Instr/order/completion choices，以Accepted current Instr的resource-aware objective比较winner，`FirstAccepted`未比较其它accepted candidate时不称best；baseline/search保持controller和actual IR owner隔离；旧physical-value、movement、storage、execution-structure、schedule shadow state、domain rebuild、parity verifier、complete materializer、通用手写temporal/fusion路径和Instr copy fallback零production残留；同一current FP16 LLaMA source的两次fresh package/no-card验收通过 |
