...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `board-testing` | `doing` | 16；关联13--15、17 | current compiler/runtime、逐case PyTorch source/input/eager reference、通过本轮no-card的原样ExecutablePackage与可用设备会话 | Add与基础Direct-DTE已有实卡通过证据；FP16 Ring AllGather的1024/1025/1031完整PyTorch比较及实卡均通过，GEMM与AllToAll三组整除/尾部实卡及完整PyTorch比较也通过，区域/选路修改已通过主机门禁，但新none的AllGather实卡数值失败，正在补跨Tile DDR完成合同；修复并重新实卡验收后，再完成ReduceScatter及剩余通信、组合计算与模型，正确性通过后在同一任务内测性能。完整矩阵的PyTorch比较、guard/status、正常清理和计划内性能记录齐全后才可完成 | `tasks/plans/board-correctness-qualification.md` |
