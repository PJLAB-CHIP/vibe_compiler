...
instruction op contract、DialectConversion 和 named pipeline 接入已落地；当前 active task 是
**R3.2d.2 Cx/NCx reshape physical mapping correction**，然后再进入 R3.2e SPM placement /
offset assignment。R3.2d 的 IR 设计主文档
...
- R3.2d 只做 Wafer instruction legalization / selection：把 target-abstract executable op 合法化并
  ...
  reshape lowered to standard memref view、nested `scf.if` / `scf.for` 递归 legalization，以及
  communication / padding layout materialization structured failure。`Cx/NCx` reshape 需要按
  block-major logical-to-physical mapping 修正，不能按 physical byte count 或 dense
...
| R3.2d.2 | active | 输入：R3.2d.1 instruction ops + R3.2c tile-region IR；输出：instruction-level `wafer.instr.*` IR 或结构化失败 | 已实现 `--wafer-convert-tile-region-to-instr` DialectConversion 的主体；为 load/store、静态可证明的 layout materialize、tile.gemm/reduce/elementwise、copy 选择 instruction-level IR，并把 compact metadata reshape 降成标准 memref view 或 identity；剩余 gate 是把 `Cx/NCx` reshape 改为按 block-major logical-to-physical mapping 判定 identity/materialization/structured failure，不能用 physical byte count 或 dense aligned row 近似 |
...
下一步先收口 R3.2d.2：修正 `Cx/NCx` reshape 的 logical-to-physical mapping，让 lowering 按
`[CxBlock][outer][lane]` / `[N][CxBlock][HW][lane]` 和 retained `C0` tail 判定 metadata-only、
materialization 或 structured failure。之后再进入 R3.2e，在 instruction-level IR with unplaced
Wafer-tagged memref 上做真实 SPM placement / offset assignment。
