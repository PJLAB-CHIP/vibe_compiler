...
| Q43 | `compiler-collaboration-review-materials` | `done` | Q9、Q37-Q39完成证据 | 2026-08-03冻结的168页PPTX/PDF历史汇报快照已完成；2026-08-05起optimizer架构以Q49为准，deck中旧per-task/rank frontier、region/join与cost叙述不是当前合同；116–132页同步/内存章节保持不变，旧1–115页、133–150页及附录的可见正文已逐页重写为问题来源、IR/算法机制、case结果、失败条件与下游影响。技术图、IR/code/data和既有板端结论未改；168页PPTX/PDF/PNG/Notes已重新生成，spec、页数、备注、对象边界、contact sheet和关键页原尺寸复核通过。 | 01、06、16–18；`tasks/plans/vibe-compiler-collaboration-review.md` |
...
| Q49 | `whole-rank-tile-dataflow-synthesis` | `doing` | Q32 actual-clone/relation/mechanism/exact-gate基础；Q38–Q40 execution/NoC/overlap机制；Q41计时、并发与配置基础；Q46/Q47 compiler-side current合同。Q40/Q41/Q46/Q47尚缺的板端外部门禁不阻塞本项施工 | C1要求每个current static rank entry恰好一个outer `tile.region`作为真实execution/SPM epoch，region边界SPM data为零；不同traversal/tile、耦合或解耦schedule、selective spill/recompute及internal DDR materialization全部在同一region内表达，不搜索region partition，也不用sibling region表达分离。C2 candidate generation与C3 terminal evaluation从generation前起共用唯一all-rank coordinator/global ledger；terminal candidate必须先原子预留all-rank Tile→Instr、worker/order、每个拟评估terminal rank-entry Instr variant一次whole-entry 3 MiB MiniMalloc，以及每个complete all-rank variant一次DDR/transport/ABI exact evaluation，后者内部每rank default DDR arena恰好一次solve并原子汇总。无per-rank/component winner、`N^R` tuple或分立的C2/C3预算。Dominance覆盖DDR、GS/local movement、SPM movement、NoC/link/endpoint、compute/recompute、tile utilization、completion/wait/critical path、Instr/descriptor/resource pressure和all-rank coupling等全部selection-sensitive dimensions；只有各已知维度均不差且至少一维严格更好才支配，known与`Unknown`或不同`Unknown`理由/界不可比。最终只在全部late gate通过的fresh IR上由当前校准hardware cost model选winner；SPM high-water不是代理成本。无artifact笛卡尔积、packing quality probe、bank relocation或ILP/CP-SAT oracle。C3删除旧join后fresh重建，C6删除旧per-task lower/placement/commit与task-return join代码；通用graph、Llama、cached-attention和prefill完成完整package/fresh no-card后为`board-ready`，真实板端matched correctness/performance后才能`done`。 | 01、06-13、16、18；`tasks/plans/whole-rank-tile-dataflow-synthesis.md` |
