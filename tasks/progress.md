...
当前执行独立的板端正确性验收，逐case消费已有且通过本轮no-card的产品产物。编译器通信收敛余项排队保留；
其完成门禁只包含编译器实现与主机验证。板测进度不再写入该项，也不通过拆分归属将其标为完成。

| 顺序 | Work item | 状态 | Owner | 直接输入 | 完成门禁 | 实施计划 |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `board-correctness-qualification` | `doing` | 16；关联13--15、17 | current compiler/runtime、逐case PyTorch source/input/eager reference、通过本轮no-card的原样ExecutablePackage与可用设备会话 | Add与基础Direct-DTE已有实卡通过证据；AllGather仅L=1024实卡通过，当前先补1025/1031，再依次验收GEMM、其余通信和组合计算。各项覆盖矩阵全部完成真实执行、完整PyTorch比较、guard/status与正常清理后才可完成；性能另行验收 | `tasks/plans/board-correctness-qualification.md` |
