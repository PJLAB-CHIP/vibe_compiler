...
| 2 | `board-testing` | `doing` | 16；关联02、05、06、08--11、13--15、17 | current compiler/runtime、原始Torch XLA模型及actual IR、独立PyTorch reference | 用户当前优先级是统一直接Torch XLA并先跑通ResNet-18。共享板测helper和reference/GEMM/MLP工具已调用同一直接XLA入口，产品抓图禁止Dynamo。通用`wafer.tile.pool`以kind区分max/min/sum/avg，支持map证明的二维窗口及缺失N/C轴；原始MaxPool/AvgPool的FP16/BF16 × 1024/1025共8项完整输出、verified package与16 Tile no-card已通过。AvgPool保持F32 opmath、padding计数及原归一化；规则双值矩形常量经既有fill/insert_slice物化。ResNet-18的224输入、完整网络与全部1000个logits已通过baseline构包和16 Tile no-card；局部卷积保留C/KH/KW归约、partial仅有输出维度，全局关系检查移至temporal批次边界，静态Pad guard已修复。默认8/42 search已完成：42个实际候选、13个accepted、29次actual SPM容量拒绝，unsupported/indeterminate为零，生成verified package并通过本轮16 Tile no-card。已修复Region合并跨远端producer、Pad在PBQP后生成未建模计算导致的布局失配；通信proposal在actual Instr上闭合联合依赖，容量反馈优先推进Repair，同一accepted owner只评分一次。编译2147.294秒，含no-card共2153.622秒，峰值RSS 14,907,472 KiB；输入FP16 [1,3,224,224]、输出FP16 [1,1000]完整保留。当前XLA CPU结果相对原PyTorch仍有180/1000超默认容差，不能据此归因为Wafer目标程序。本轮ResNet默认search目标已闭合，整网设备数值与性能仍未验收。前序embedding24项及固定FP16 LLaMA block no-card通过属于旧抓图路径，不能代签统一入口后的资格；完整LM、ViT、大GEMM、共享RHS、BF16 LLaMA数值和YOLO仍按原矩阵待办。QKV设备超时后板卡未恢复，本轮没有实卡动作，也没有性能无退化或三轮调优完成结论。详细已测边界及下一步见当前板测实施计划 | `tasks/plans/board-workload-matrix.md` |
