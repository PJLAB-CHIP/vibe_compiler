...
| 2 | `board-testing` | `doing` | 16；关联05、06、08--11、13--15、17 | current compiler/runtime、全workload/多预算审计及actual IR、实卡数值/profile证据 | 用户已授权按Spatial、融合与多轴Temporal系统方案推进完成；有界搜索、跨Region容量反馈及共享DDR参数去冗余已实施，当前注册的默认search产品全部通过fresh package/no-card，LLaMA FP16本轮PyTorch实卡通过；低预算首次可行成本与设备性能仍未恢复。继续原前7步：主机开销→容量反馈→搜索覆盖/预算/存活IR→DDR估时→completion环→conv/BF16→非风险产品与匹配实卡验收；当前优先收敛修正深度及通信物化次序，风险4K设备执行最后处理。合同、覆盖矩阵和当前边界见实施计划；完整预算曲线、completion/lowering遗留与匹配模型性能未闭合，局部产品通过不代签完成 | `tasks/plans/board-performance-optimization.md` |
