...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `board-testing` | `doing` | 16；关联13--15、17 | current compiler/runtime、逐case PyTorch source/input/eager reference、通过本轮no-card的原样ExecutablePackage与可用设备会话 | Add与基础Direct-DTE已有实卡通过证据；AllGather仅L=1024实卡通过，当前先补1025/1031，再依次验收GEMM、其余通信、组合计算与模型，正确性通过后在同一任务内测性能。完整矩阵的PyTorch比较、guard/status、正常清理和计划内性能记录齐全后才可完成 | `tasks/plans/board-correctness-qualification.md` |
