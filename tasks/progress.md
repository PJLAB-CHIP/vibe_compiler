...
| Q58 | `program-data-ownership` | `done` | Q50.0、Q54、Q55 | 2026-08-16代码review重开后同批修复并闭合：establishment把用户文件流式复制到transaction独占owned file（1MiB window边复制边SHA-256，header bounded parse，用户path此后不再打开，同inode原地改写回归测试覆盖）；helper输出shard经TensorPayloadResolver建立为handoff candidate并成为唯一一次读取，region digest相等复用原source、真实partition经adoptCandidate成为新source，constant由owned content回填tensor program；tensor verification、readback与CardExecutable边界全部经resolver消费owned content，无账本外payload I/O；range携带显式来源合同（OriginalSource source shape==global / MaterializedShard source shape==local），同字节数不同shape被typed拒绝；source dtype width与program boundary admitted dtype分离，i1在establishment typed拒绝；全部可恢复失败填充typed kind+locator+detail（新增MaterializationIO）。`ProgramDataTest` 13/13（含verifyShardAgainstSource dedup/adopt集成、16 Tile一次materialize共享视图、原地改写、i1拒绝、ledger精确断言）；覆盖Q58全部触及suite的filtered单测149/149；三档byte-volume（0.5/2/8MB f16）本轮新构建fresh source→package exit 0且no-card 16 Tile通过，账本三档一致（source_opens=5 header_reads=23 digest_passes=13 helper_output_readbacks=2 materialized_file_writes=4），peak RSS不随payload增长；card-baseline lit 1/1。证据细节见计划文档checkpoint 2-6与4.1历史review记录。 | 02、14、16、18；`tasks/plans/program-data-and-whole-program-scale.md` |
