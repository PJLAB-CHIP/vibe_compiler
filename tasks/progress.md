...
| Q62 | `target-numeric-contract-reconstruction` | `next` | Q55、Q56达到`board-ready`、current TargetCall/SystemC/model mechanics | 保留target operation、physical codec、formal arithmetic和bulk qualification的真实能力，但拆掉Q22.N遗留的全局`NumericSemantics` umbrella：target command字段回到TargetOperation/TargetCall，physical tensor成为无model identity的descriptor，TargetTensor materialization显式携带转换并让compiler/package退出formal model，formal/model按family直接执行或typed拒绝，bulk evidence只绑定concrete GEMM problem/payload/backend/environment。所有profile/pattern/resolver、compiler/evidence占位字段、semantic/resolution digest、兼容alias与专属registry测试删除；01、11、14、16-18合同、CMake/tools/fixtures同批切换。完成证明只跑fresh direct target/codec/package/formal/bulk/model/no-card和组织检查，不运行旧numeric registry基准、Q49.P/Q51长搜索或历史板端路径。 | 01、11、14、16-18；`tasks/plans/target-numeric-contract-reconstruction.md` |
| Q49.P | `deterministic-baseline-functional-closure` | `doing` | Q59；Q50.A；Q50.0、Q54、Q45 | current early-exit、真实TileRegion capacity query和一次完整CardExecutable compile只是起点。完成时`none`从未选placement/region/temporal/representation/buffer的正常TensorProgram自行完成确定性功能合法化：canonical controller按完整semantic tie-break遍历不被beam/cap/budget截断的有限baseline fallback，初始完整tile放不进SPM时重新推导operand/halo/result/temporary/movement/alignment/bank/lifetime并沿合法breakpoint缩小，直到第一个required scoped probe全部fit的canonical completion，再经一次完整Q50.0 gate成为accepted executable；selected assignment是该过程的输出而非前置输入。它仅复用policy-free placement-domain机制和Q50.A logical demand/coverage query，不依赖search state/candidate、search-oriented domain/ranking evaluator、proposal order/group materializer或candidate统计；每个baseline TileRegion恰有一个structured root和必要non-root support closure，同Tile多root形成多个顺序region，跨root shaped dependency显式DDR。Q50.A完成后已证明支持的reduction、broadcast、window/stride与multi-piece exact set由baseline canonical correctness carrier有限分解并走通，dense/descriptor carrier失败不得改判logical placement。region-local query需要call/function lifetime时提升到最近合法isolated ancestor；capacity witness把all-and-only conflict owner关联到当前single root的temporal assignment，unsupported witness命名typed lifetime/call relation和scope，均不得按type/shape、其它root或diagnostic字符串猜测。scoped query不造card-shaped/no-work-Tile wrapper；完整CardModule materialization与CardExecutable compilation各一次。resolved-assignment single-root apply与deterministic feasibility probe成为Q50.B/C/F复用的共同机制，不建立baseline-private路径；fresh overfull-to-fit、prefill/decode/Llama证明IR/package digest、oracle/no-card、结构、调用闭包和work count；声明支持且baseline域有合法completion时必须产出accepted executable，accepted baseline由Q51直接复用为incumbent。两个既有失败已fresh修复：probe/final一致性gate（`NoneJointlyRefinesExplicitProducerStageAndConsumerDemand`）与CROSS 16-Tile挂起（storage-root memo）均闭合。本轮按用户指示删除fastmath/保序旧假设：`requiresLexicographicFloatReductionOrder`/`isOrderPreservingReductionRefinement`全仓删除，`verifyReductionSplitNumericLegality`对addf无条件合法（整数no-wrap/overflow是唯一barrier），search多start rationale改按单调descent表述，死函数`refineResourceScheduleTemporalVariantOnce`与test fixture中`fastmath<reassoc>`清除；「最小tile超SPM」浮点负例因自由重结合不可构造、且其纯reduction标量输出在placement domain之外，已删除并在plan/06/07文档如实记录。验证：CardExecutableSynthesis 23/23、本轮4套件70/70、默认lit gate 216/216、三棵树fresh构建通过、baseline lit fresh通过；Tools/Runtime lit 33/38（5 unsupported、0 failed）——原4个失败全部为2026-08-11 `76f68e29`把SPMD层从16-rank改成card-level后未跟上的stale测试，本轮已修：reference capture按dtype政策改f16（target明确拒绝f32 GEMM）、capture改用card-level reference模式、mesh pass补显式shape、manifest消息/role_index断言同步current合同；source-to-package以`none`编译f16 reference/AddModel走通完整gate（f16下默认search同样通过，~60s，非本任务gate）。未闭合：Llama block fresh capture成功但`none`编译wall 2h21m未完成（100% CPU、RSS 46MB，baseline block级wall-time未闭合，Q52域）；related suites中`TileMemoryPlanningTest.ReportsSPMFailureForOwnedTileModule`在当前二进制上失败（spmLargestDemands为空，位于Q49.P改动的TileMemoryPlanning.cpp域，根因未定性）。 | 06、14、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
...
| Q51.Core | `physical-dataflow-search-kernel` | `queued` | Q49.P；复用Q50.0 | 从零建立new-search control kernel：直接接收Q49.P已准入的move-only baseline executable与同cohort actual cost作为session-level incumbent，只实现current typed assignment/ephemeral transition apply、deterministic frontier、stable dedup、global work/budget ledger、typed evaluation routing和coverage/bound evidence；未来Q50轴不预声明placeholder，mechanism/priority/incumbent/derived lifetime/calendar/SPM/cost不进入candidate identity。Core不适配或调用current `deriveShortlist`、`TileExecutionCandidate`、coordinate sweep、candidate-local schedule/evaluator、allocator/buffer feedback、beam、accepted cohort、`StaticSchedulePlan` selector或winner rematerialization；本checkpoint同批让public `search`只进入新Core并删除上述旧控制链、专属统计和长耗时integration。production mechanism set为空时直接返回Q49.P accepted incumbent。独立finite state-graph model证明control mechanics；每轴reference enumerator随Q50.S/B–K交付并同批清理对应旧实现，actual-region probe归Q50.F；full production flat exhaustive runner与new source-to-package链归Q51 closure。 | 06、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
