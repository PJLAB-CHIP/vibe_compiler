...
| Q49.P | `deterministic-baseline-functional-closure` | `doing` | Q59；Q50.A；Q50.0、Q54、Q45 | `none`必须从未选placement/region/temporal/representation/buffer的正常TensorProgram自行完成确定性功能合法化，并只复用typed iterator/topology facts、单coordinate legality/materialization机制和Q50.A demand query。follow-up review确认current baseline仍展开全部iterator-axis×connected-rectangle placement options后递归取一个结果、在capacity refinement前后重复完整CardModule materialization、accepted后构造并丢弃`StaticSchedulePlan`/duration estimate、普通compile无条件打印Tile IR、并与旧search共享百字段statistics/result header；这些均属于Q49.P而非Q52。完成时canonical controller只按typed rejection推进必要coordinate，所有trial走single-root/single-Tile最窄probe，全部fit后完整CardModule与Q50.0 compile各一次；accepted result不依赖未消费cost/trace，baseline实现/result/窄ledger不引用旧search domain/candidate/statistics。每个TileRegion一structured root、跨root显式DDR、direct causal witness与ancestor-scope probe继续闭合；fresh overfull-to-fit、prefill/decode/Llama以显式`none`证明package/oracle/no-card和work count，不运行旧search或历史winner对照。当前Llama `none` 2h21m未完成在上述浪费清除前仍是Q49.P work-closure缺口；`TileMemoryPlanningTest.ReportsSPMFailureForOwnedTileModule`的空`spmLargestDemands`仍待定性。 | 06、14、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.A | `exact-placement-demand-boundary` | `done` | Q50.0、Q54 | policy-free trial、typed four-state verdict、nested IR snapshot invalidation、bounded exact relation、all-and-only execution/ownership coverage、stable per-Tile payload、partial-reduction merge obligation和multi-result production carrier均已闭合。baseline support carrier直接消费同一per-destination exact demand/ownership intersections，empty destination不再被伪造为矩形或carrier失败。reduction、broadcast、affine window、strided view和multi-piece overwrite五类正常TensorProgram均经canonical baseline carrier完成一次完整Q50.0 CardModule compile并产出accepted CardExecutable；fresh relation/query/carrier回归62/62、五类production gate 5/5、Q50.0 baseline/稳定性/reduction 3/3、source-to-package 1/1和主构建通过。约163秒的旧placement枚举不是本logical boundary门禁：baseline调用由Q49.P删除，旧search测试随Q51.Core清理，Q50.B只以新惰性domain/tiny oracle重新建立需要的轴。 | 05-07、09-13、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
