...
Q50.0 + Q54 + Q55 -> Q58 program data ownership                [doing]
Q58 -> Q56 executable-package data closure                     [queued]
Q56 board-ready -> Q59 compiler entry transaction closure      [queued]
Q59 + Q50.0 + Q54 + Q45
...
| Q58 | `program-data-ownership` | `done` | Q50.0、Q54、Q55 | 2026-08-15闭合：一次compiler transaction经`ProgramDataSource::establish`打开、验证header/exact extent/SHA-256 digest并以pinned content持有all-and-only payload；source snapshot只隔离IR/metadata/目录结构；frontend verification通过`ProgramPayloadResolver` seam消费owned content；SPMD helper只消费transaction materialize并digest readback的helper-input view；helper shard逐文件readback，byte-identical region按digest 复用原source、真实partition成为新`ProgramDataSource`；16个Tile binding只引用stable `ProgramTensorId`和checked `ProgramDataRange`，`ProgramDataHandoff`由`CardExecutable`持有；`prepareProgramInvocations`不再接收packageRoot，parameter/constant每range一次materialize并以shared view共享，target model的card-shared resource只encode一次。失败分类（truncated/trailing/header/dtype/shape/overflow/digest）与mutation负例由`ProgramDataTest` 9/9覆盖；3档递增byte规模（0.5MB/2MB/8MB elementwise f16，2 parameter+1 constant all-and-only inventory）fresh source→package exit 0且`wafer-run --no-card` 16 Tile通过，账本`program-data-io`三档一致（opens=3 headers=5 digests=12 shard_readbacks=2 materialized_files=3），peak RSS 60/64/70 MiB不随payload总量增长；`wafer-compile-card-baseline.test` lit 1/1通过；Q58相关定向单测全部通过（覆盖Q58全部触及suite的filtered-137项中136通过，1个失败与main树A/B确认预先存在；ProgramDataTest 9/9；SYSTEMC构建TargetModelMemoryTest 8/8）；666项全量套件运行至约2h仍停在慢速synthesis单测（>35min无输出）而终止，剩余部分为Q58未触及suite，回归覆盖由filtered-137承担；model codec-hoist经两套构建编译验证（SystemC集成测试需bootstrap numeric deps，环境缺口）。不改CLI、package schema、device memory或resident runtime。 | 02、14、16、18；`tasks/plans/program-data-and-whole-program-scale.md` |
| Q56 | `executable-package-data-closure` | `queued` | Q58 | 只改final `CardExecutable`/target output→package→one-shot runtime边界：将当前误导性的`KernelABISlot`原位收敛为每Tile有序`TileEntryArgument`；对16个Tile arguments做all-and-only join，分离logical ProgramTensor、selected TargetTensor和runtime device address。Compiler为package-owned TargetTensor预排`program-data.bin`的deterministic offset/span/alignment，每种selected representation只转换一次；external input/output无package bytes。Manifest直接记录target/kernel launch、program data、program/target tensors、ports、modules和entries，不序列化provider allocation表。Runtime对non-empty program data以一块`BoardDeviceMemory`整体H2D，empty program data不调用provider；input/output、每Tile workspace/status/profile和pointer rows预排在一块invocation `BoardDeviceMemory`中。所有entry addresses均由对应base+checked offset构造，不逐tensor或逐Tile分配。current产品provider只保留`txLaunchKernel` family；同批从Wafer-owned compiler/package/runtime/CLI接口原位删除`txLoadGraph`/`txLaunchModel`分支、枚举值和选择开关，反向工程资料留在`docs/`。同步替换writer/loader、no-card、board one-shot、target model、CLI和fixtures，无旧reader。fresh source→package/readback/no-card、layout/digest负例、fake-provider与完整kernel板端case达到`board-ready`后，Q59/Q49.P/Q53才消费。 | 14-17、20；`tasks/plans/executable-package-and-resident-runtime.md` |
| Q59 | `compiler-entry-transaction-closure` | `queued` | Q56达到`board-ready` | 让public compiler library的primary result、package readback/commit、CLI exit status和installed tool形成同一transaction合同；success当且仅当`ExecutablePackage`已原子提交，返回值直接持有已验证package而非中间`CardExecutable`。target-model/IR dump退出production compile的post-commit gate；filesystem/external-tool/package failure使用typed `Error/Expected`；installed compiler不依赖source/build绝对路径。不改frontend格式、search语义或runtime execution。 | 01、15、18-20；`tasks/plans/compiler-entry-productization.md` |
| Q49.P | `baseline-control-flow-isolation` | `queued` | Q59；Q50.0、Q54、Q45 | 不改变Q49已签发的正确性合同，只清理`none`控制流：不得构造search state/candidate family，不得为每个temporal尝试反复编译whole graph；scoped probe直接消费Q54形成的真实TileRegion或最近合法isolated-anchor seam，不得把每个region包装成synthetic Module/Func后重跑完整Tile memory planning和verification；最后只对accepted baseline编译一次完整CardExecutable。fresh prefill/decode/Llama证明CardModule、CardExecutable与package digest稳定，oracle/no-card通过，阶段计时证明重复materialization热点已移除。 | 06、14、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
