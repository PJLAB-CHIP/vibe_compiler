...
| 2 | `board-testing` | `doing` | 16；关联05、06、08、10、11、13--15、17 | current compiler/runtime、全workload/多预算审计及actual IR、实卡数值/profile证据 | 五项方案的主要代码改动已实施，分项主机检查通过：e-graph保留限额内有效改进并提高默认额度；物理copy复用；descriptor/issue/completion估时；整数近邻和分层探索。访问吸收后的LLaMA/decode实卡PyTorch已通过，prefill物理复用未测得加速。最终验收未闭合：新prefill大预算候选发生真实设备completion超时；2026-09-13重启前Add也超时，重启后fresh完整16 Tile Add已通过PyTorch，基本执行恢复但prefill根因未明；完整tensor循环state造成的buffer type重复推导已用标准子集提升修复；本轮GEMM 126次搜索完成、48 accepted，no-card及525825输出PyTorch实卡通过（0.973 ms）。LLaMA 14次仍0 accepted，模型性能和剩余catalog长测未闭合。Region domain漏partial→merge边已修复并通过独立反例，异构none通过而search长测未完成。按用户最新顺序，先处理主机长测、搜索质量和LLaMA/decode性能退化，补最终PyTorch与匹配性能；有卡死风险的prefill候选最后处理；不得以中间版本通过或静态估时标done | `tasks/plans/board-performance-optimization.md` |
