...
| Q49.P | `deterministic-baseline-functional-closure` | `doing` | Q59；Q50.A；Q50.0、Q54、Q45 | `none`从正常TensorProgram直接构造一个canonical coordinate，不枚举placement候选、不调用recursive CSP/search；capacity conflict只允许由direct typed witness驱动确定、单调且不回溯的temporal/spatial legalization。每个root/参与Tile必须从一开始走真正all-and-only shard-local materializer/probe，全部fit后完整CardModule与Q50.0 compile各一次；每个TileRegion恰一structured root、跨root显式DDR、relation和probe evidence在current IR/lifetime内完整，accepted result不依赖旧search statistics、cost、shadow schedule或默认IR trace。2026-08-18对合并后current HEAD的综合review撤销此前“P1/P3/P5/P6/P7已闭合”声明：可保留function-scope escalation、raw demand evidence copy、storage-root memo、accepted-result move、shadow schedule删除和显式trace；但`IndexRelation` total/restriction/complete-reduction shortcut不exact，`externalBoundaryCarrier`无consumer导致所谓narrow probe仍物化sibling，controller仍full-card→per-root重复prepare→full-card，witness traversal会扩散到无关descendant且scratch `mlir::Value`逃逸，baseline仍调用post-hoc splitter，typed unpartitioned未被consumer/hash/order使用，maximum-participation exact reject后也没有deterministic spatial legalization，product入口仍携旧statistics bag。reduction iterator的temporal tiling和greedy capacity legalization已经存在；缺的是reduction spatial partition/partial-result merge（Q50.B）及完整temporal domain（Q50.E），constant-map relaxation还须收紧。Q51完整new-search链闭合前不跑重型LLaMA `none/search`；Q49.P只用有界direct unit、CHAIN/CROSS/GEMM/no-card、relation反例和work-count gate验收，详细阻塞项及修复顺序见计划文档。 | 06、14、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
