...
| Q19 | `reference-executor-core` | `doing` | Q16 | 已落owner-backed immutable `ReferenceProgram`和执行前capability preflight：当前flat/tile-region、static view、movement、GEMM、f32 elementwise/fill及非zero-point convert逐op投影成value-id command，执行期不读mutable MLIR；convert用APInt/APFloat闭合RND_MODE 0..3、整条成功后commit，并对stochastic/未证zero-point formula fail closed。仍需补SCF/CFG/direct-call、zero-point/stochastic证据、完整capability矩阵、非平凡differential及独立layout property oracle。 | 10、11、16 |
