...
Q55 + Q56 board-ready + current TargetCall/model
  -> Q62 target numeric contract reconstruction                [next]
...
Q49.P + Q50.0 -> Q51.Core search control kernel               [queued]
...
| Q54 | `mlir-infrastructure-conformance` | `done` | Q50.0 | 已按19号合同完成pass/scope/transaction整改：semantic Location pointer payload、raw semantic attr、ordinal/print relation和synthetic local wrapper退出active路径；TileRegion/Func/Module按真实scope组织，16个atomic pass组成7条常驻named semantic pipeline与1条Shardy条件pipeline，production只由统一runner建PM；RegionBranch/effect/call、ODS、AnalysisManager、transactional pattern/conversion和typed host error闭合。fresh core/model build、定向单测、非搜索单测、受影响card关系、lit、feature-on依赖/模型/link及IR/source organization检查均通过。2026-08-17 follow-up确认原contract map中的`NCCSynchronizationContract` pure target/MLIR adapter分层并未实际完成，不能继续计入Q54结论；该缺口由Q63排队并成为Q50.J前置。baseline/Q52长搜索未冒充本任务验证。 | 18-19；`tasks/plans/mlir-engineering-remediation.md` |
...
| Q49.P | `deterministic-baseline-functional-closure` | `doing` | Q59；Q50.A；Q50.0、Q54、Q45 | `none`必须从未选placement/region/temporal/representation/buffer的正常TensorProgram自行完成确定性功能合法化，并只复用typed iterator/topology facts、single-coordinate legality/materialization机制和Q50.A demand query。follow-up review确认current baseline仍展开全部iterator-axis×connected-rectangle placement options并递归propagate/backtrack取一个结果、在capacity refinement前后重复完整CardModule materialization、accepted后构造并丢弃`StaticSchedulePlan`/duration estimate、普通compile无条件打印Tile IR、并与旧search共享百字段statistics/result header；这些均属于Q49.P而非Q52。`2f2e8de0`增加的function-scope probe、per-Tile scoped入口、storage-root memo、DDR split和reduction temporal materialization只作为新baseline机制资产融合，不建立独立DS修复线；current relation remap/retain会丢witness，root split不retarget consumer reload relation且接受zero-root/固定256轮，direct-witness fallback还有唯一root猜测与bool更新错误。完成时baseline只有一个live coordinate，直接构造canonical spatial placement；该coordinate可以覆盖多个甚至全部Tiles，无parallel result轴的root才退化为all-factor=1、单参与Tile，不能把后者写成baseline全局Tile数。不得生成option domain、调用recursive CSP/search state/candidate/evaluator、保留alternative或比较winner。只有direct typed rejection可触发确定、单调且不回溯的functional legalization transition；每个root在其spatial placement的每个参与Tile上从一开始走shard-local最窄materializer/probe，全部fit后完整CardModule与Q50.0 compile各一次；accepted result不依赖未消费cost/trace，baseline实现/result/窄ledger不引用旧search对象。每个TileRegion恰一structured root、跨root显式DDR、relation在current consumer buffer上语义完整、direct causal witness与ancestor-scope probe继续闭合；fresh overfull-to-fit、非空relation类别、actual region IR及轻量source-to-package/no-card以显式`none`证明package/oracle和work count。Q51完整new-search链闭合前，Q49.P、Q50各checkpoint和Q51.Core均不得执行重型LLaMA block的`none`或`search`；既有2h21m未完成运行只作为旧baseline误入全option/CSP及无界exact-relation热路径的诊断，不是Q49.P门禁或重跑项，首轮重型LLaMA执行归Q52显式scalability profile，正式package/no-card归Q53。`TileMemoryPlanningTest.ReportsSPMFailureForOwnedTileModule`的空evidence已定位为无`materializationRelations`分支只复制scalar字段而漏传三个demand集合，需在Q49.P内修复。2026-08-17 session：P1 relation completeness、P3 narrow materializer、P5 direct-witness attribution、P6 policy-free canonical coordinate + narrow ledger、P7 shadow schedule/显式trace均已闭合并有fresh测试；P4的one-root-per-region以per-component构造（独立root）+ split kernel（多波spill）落地，probe的boundary supplies阻止sibling compute进入窄scope，typed postcondition与trace request线程已加。**P4剩余**：canonical carrier的RegionCut spill在consumer temporal tiling下被wave-clone（store写clone alloc、reload读原alloc），跨Tile CROSS gate因此仍走split kernel；carrier需拥有wave-hoisted exact store/reload发射后才能退出baseline调用链。P6-C剩余：标量输出大shape reduction的movement-path IndexRelation::compose仍走generic Presburger。 | 06、14、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
...
| Q51.Core | `physical-dataflow-search-kernel` | `queued` | Q49.P；复用Q50.0 | 从零建立new-search control kernel：接收Q49.P move-only accepted baseline和同cohort typed cost，只拥有typed assignment/transition、deterministic frontier/dedup、global ledger/budget、typed evaluation routing与coverage evidence；未来轴、derived analysis/cost和policy不进identity。public `search`同批切到new Core，空production mechanism时直接返回baseline。删除旧`deriveShortlist`/`TileExecutionCandidate`、coordinate sweep、string failure-gate状态机、feedback/beam/accepted cohort、`StaticSchedulePlan` selector、winner rematerialization、百字段statistics及Rank/coordinated source/test island；删除读取旧source symbol的CTest、`none/search` paired optimization catalog/driver和hardware inventory binding、旧stderr/winner/golden、14处旧长链CTest及search-only fault-injection限制。新Core不适配旧行为、不保留回归基准、不在candidate compile打印IR；finite state-graph model只证明control mechanics，真实轴/oracle随后由Q50/Q51新链交付。 | 06、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
...
| Q52 | `physical-dataflow-search-scalability` | `queued` | Q51；复用现有计时/RSS/work ledger | 先拆`TargetScheduleCostPolicy`/`WaferTargetPolicy`跨owner aggregate：exact Instr resource metrics、hard memory facts、package/profile reference rates和search estimate priors分别归唯一owner；不保留`TileSearchEffort::{Quick,Default,Deep}`或固定candidate/beam cap。随后在通用DAG、HF prefill/decode和Llama representative load上记录state/work/clone/packing/cost/wall/RSS、estimate误差和best-found/recall/regret，再按真实热点引入memo、DP、no-good、dominance、branch-and-bound或局部solver。启发式/top-k/LNS等永久丢状态策略只在profile后启用并准确标`budgeted-feasible`；长时间运行只属于本任务的显式profile批次，不进入每次功能改动回归，也不得用旧search耗时或winner作基准。 | 06、14-18；`tasks/plans/physical-dataflow-synthesis.md` |
