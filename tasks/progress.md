...
| Q56 | `executable-package-data-closure` | `board-ready` | Q58 | `TileEntryArgument` closed union、16-Tile all-and-only join、`program-data.bin` deterministic预排、canonical manifest（program/target/launch/card_count/tile_count/program_data/program_tensors/target_tensors/inputs/outputs/modules/entries，无版本分支）与两块`BoardDeviceMemory` one-shot runtime已落地；`txLoadGraph`/`txLaunchModel`与`Tx81ModelABI`从compiler/package/runtime/CLI/测试原位删除，反向工程事实留在`docs/`。package只含manifest.json+modules+data/program-data.bin，source tree/NPY/IR/exporter payload不进入。验证：q55-current-fresh、wafer-board-check（BOARD_RUNTIME=ON）、q54-fresh-model（SystemC）三棵树fresh build通过；Q56定向单测169/170（BoardRuntimeTest 40/40、PackageManifest+ProfileInstrumentation 41/41、TargetABI/CodeGen/CallFrontend、CardExecutable系列全过；唯一失败`CardExecutableSynthesisTest.NoneJointlyRefinesExplicitProducerStageAndConsumerDemand`与Q58证据同源的SPM feedback路径问题，不在本任务范围）；WaferRunBoardIOUnitTests 42/42、RuntimePublicLinkSmoke exit 0、SystemC integration 3/3、SystemC model 41/41+8/8；默认lit gate 216/216；Tools/Runtime lit 27/34（4 unsupported），3个失败均已在HEAD复现且成因定位（见`memory/bugs.md`），归属外部helper/既有search gate排期：stablehlo-sharding-propagation（wafer-opt mesh-explicit要求，76f68e29引入，测试不在本批改动内）、structured-tensor-program（XLA partitioner helper INVALID_ARGUMENT）、spmd-partition（selected-buffer-materialization gate失败，d5cfeee4后的既有问题，归Q50.I范围）；board纯unittest 6/6、全部board python py_compile通过；no-card fresh验证：参数add（program_tensors/target_tensors/digest/file_offset与`launch_argument base=program_data`）和pytorch x+x（canonical empty program data）均通过。真实板测未执行：single-op add case/oracle/runner完整（FP16），Q56=board-ready，上板通过前不标done。 | 14-17、20；`tasks/plans/executable-package-and-resident-runtime.md` |
