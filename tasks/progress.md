...
Q58 -> Q56 executable-package data closure                     [doing]
...
| Q56 | `executable-package-data-closure` | `doing` | Q58 | 首轮实现已落下current package、program-data和one-shot kernel runtime，但review证明完成门禁尚未闭合：TargetTensor排序后entry ID可能失配；writer只支持identity representation且整tensor materialize；TileRow重复申请16块row memory；strict readback未拒绝overlap/gap/trailing/nonzero padding和package root额外member；neutral package support尚未与BoardRuntime执行库拆分。必须按计划3.4修复producer、consumer、verifier、CMake边界和直接负例，重新通过受影响unit、source-to-package、strict readback/no-card、fake provider、public link/source organization及完整package检查后才能恢复`board-ready`；真实板测通过前仍不得标`done`。 | 14-18、20；`tasks/plans/executable-package-and-resident-runtime.md` |
