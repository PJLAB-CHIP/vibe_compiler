...
| 2 | `board-testing` | `doing` | 16；关联02、05、06、08--11、13--15、17 | current compiler/runtime、原始Torch XLA模型及actual IR、独立PyTorch reference | 用户顺序：闭合通用循环DDR/DTE与大GEMM后，完成带embedding及LM head的单层LLaMA2；YOLOv5s已移除。大GEMM的4096³ FP16/BF16、4097³ FP16均已通过默认8/42、verified package及本轮fresh no-card，各有2个共享候选实际通过并选出DTE；FP32 psum局部化，末块直接窄输出，4096³ DDR读取由1024 MiB降至256 MiB。循环复制/Ring/回边与8项实际SystemC数值验证通过；此前ResNet search/no-card及通信构造优化保留。完整单层LM的S16 FP16/BF16均已通过直接Torch XLA source、search、verified package及本轮fresh no-card；原始token IDs进入embedding，保留decoder、final norm和全部词表logits。概率舍入与SiLU opmath已修复；原逐点超差已定位到归约及K分块差别；用户已授权完整LM采用显式余弦与相对L2联合验收，原逐点误差保留为诊断。本轮FP16/BF16完整source→search→package→TargetModel全输出比较→fresh no-card均已通过，单层主机数值资格闭合；通用比较器和板端runner使用同一显式策略，其它case保持原标准，尚无设备通过结论。访问复用（AccessReuse）方案已收紧为统一分析、候选前轻量净收益筛选和少量实际验证，低收益机会不物化、不占trial；统一分析、候选筛选及受限物化已实现；驻留/滑动/两级和三输入联合已通过actual下游，SystemC完整数值、大GEMM与LLaMA2完整source/search/package/fresh no-card、driver与canonical构建回归均已通过；用户要求补齐大GEMM闭环并检查attention的同类累加状态布局；已修复读取子视图、源关系/多scope容量反馈、树转发及跨循环完成约束。GEMM的64 MiB读取候选已在默认42次中通过实际SPM并评分，性能最优与读取最少分开记录；通用SCF状态布局已与初始化边界解耦，GEMM及online attention状态main/tail验证通过，真实attention完整输出及大GEMM fresh no-card通过，通用布局与主机回归已通过；4097尾块的读取最少候选仍高于一次读取目标，性能效果与设备资格继续独立记录；范围为原跨Tile共享、基础驻留、固定步长滑动窗口和最多两级驻留，暂缓替换。边界见06号3.5，详细方案见tasks/plans/access-reuse.md。主机/no-card与设备资格分开；板卡仍按未恢复处理，无新增实卡结果。详见同一计划的当前检查点。 | `tasks/plans/board-workload-matrix.md` |
