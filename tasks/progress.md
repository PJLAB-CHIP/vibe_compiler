...
| 2 | `board-testing` | `doing` | 16；关联02、05、06、08--11、13--15、17 | current compiler/runtime、原始Torch XLA模型及actual IR、独立PyTorch reference | 用户当前优先级是统一直接Torch XLA并先跑通ResNet-18。共享板测helper和reference/GEMM/MLP工具已调用同一直接XLA入口，产品抓图禁止Dynamo。通用`wafer.tile.pool`以kind区分max/min/sum/avg，支持map证明的二维窗口及缺失N/C轴；原始MaxPool/AvgPool的FP16/BF16 × 1024/1025共8项完整输出、verified package与16 Tile no-card已通过。AvgPool保持F32 opmath、padding计数及原归一化；规则双值矩形常量经既有fill/insert_slice物化。ResNet-18的224输入、完整网络与全部1000 logits端口已通过baseline构包和16 Tile no-card；局部卷积保留C/KH/KW归约、partial仅有输出维度，全局关系检查移至temporal批次边界，静态Pad guard已修复。旧版默认search已停止；已修复非连续Region合并跨过远端producer造成的DDR/DTE等待环，交错依赖用例通过实际completion/SPM，同一ResNet失败候选已通过completion推进到SPM；3次候选预算均为typed capacity，默认search仍未闭合。当前XLA CPU结果相对原PyTorch仍有180/1000超默认容差，不能据此归因为Wafer目标程序。正在定位整网直接阻塞，不扩展其它模型批次。前序embedding24项及固定FP16 LLaMA block no-card通过属于旧抓图路径，不能代签统一入口后的资格；完整LM、ViT、大GEMM、共享RHS、BF16 LLaMA数值和YOLO仍按原矩阵待办。QKV设备超时后板卡未恢复，本轮没有实卡动作，也没有性能无退化或三轮调优完成结论。详细已测边界及下一步见当前板测实施计划 | `tasks/plans/board-workload-matrix.md` |
