...
| Q50.D | `coupled-traversal-region-fusion` | `doing` | Q50.C | 从SSA、iterator/index relation和effect构造合法region boundary与consumer-driven coupled traversal；actual fused candidate必须处于共同TileRegion并避免中间值无意义DDR round-trip。maximal closure与合法cut都进入共同candidate set，不能用单边`fused`标记或group字段代替实际融合。 | 05-10、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
