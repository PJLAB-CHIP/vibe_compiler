...
Q9 + Q45 + Q47 current interfaces -> Q55 version consolidation [done]
...
| Q55 | `interface-version-consolidation` | `done` | Q9 profile合同、Q45命名规则、Q47 current target/package/runtime接口 | package manifest、target runtime ABI、device profiler record、Direct-DTE status和独立raw evidence保留为真实外围版本边界；frontend nested metadata、profile重复字段、repo-owned dependency record、target内部metadata、workload及算法/model/hash-domain名称均收敛为一种current表示，旧reader和内部版本后缀已删除。runtime profile collection与optimization comparison测试名称同步职责。fresh 419步构建、runtime IO 39/39、其余非搜索unit 629/629、受影响CTest 19/19及Wafer lit 216/216、dependency helper 21/21+7/7+7/7和profile report通过；未运行Q49/Q52长搜索或真实板测。 | 02、11、14-17、20；`tasks/plans/interface-version-consolidation.md` |
