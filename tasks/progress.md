...
Q50.0 + Q54 -> Q50.B spatial-assignment representation foundation
                                                               [doing]
Q50.B foundation -> Q50.A exact placement-demand boundary     [doing]
...
Q50.S + Q50.A + Q64 -> Q50.B full graph-level spatial domain [queued]
...
Q50.A只消费这份closed assignment。Q50.S/A完成后，Q50.B再交付full domain/proposals，所以不存在A输入凭空产生或A/B循环。
`Q49.P remaining canonical-plan foundations + Q50.F closed-plan core`同样是一个协调纵向：把baseline其余已真实消费的component
...
| Q50.B | `spatial-partition-and-placement` | `doing` | representation foundation依赖Q50.0/Q54；full domain依赖Q50.S、Q50.A、Q64 | 当前只推进foundation：定义compact `SpatialPlan`、closed `SpatialAssignment`、structural validation和baseline canonical producer，给A真实输入但不枚举search domain；A/Q49纵向完成后任务保持打开，full stage再惰性覆盖全部regular partitions、available subsets/embeddings和per-output reduction-group merge placements，并逐point调用A。仅verified topology automorphism可dedup，DP只作proposal/bound；旧`LogicalShardTrial`、edge carrier、movement/schedule/resource signature退出。 | 05-08、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
