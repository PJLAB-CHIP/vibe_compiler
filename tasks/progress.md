...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `search-scalability` | `doing` | Q52 | Q51 search controller、structural choice与attention展开donor，current baseline regression，Q50.0 actual memory/target leaf | 按current plan的14个线性checkpoint以consumer-first施工：第6项从现有Card→Executable入口抽出completion-closed Instr→Q50.0 actual leaf合同，不重复实现allocator/target且memory leaf不再重建completion；第7--10项继续沿baseline唯一production路径逐层向上替换Instr/order/completion、current Tile execution structure/rotating storage、movement/boundary和layout/bufferization，其中第9项接入唯一full-transfer cleanup，第10项一次闭合function-boundary/region-local bufferization、current-IR layout域、修复后的exact PBQP和layout apply，并使PBQP按NE、Vector/CT、movement与instruction-control分别投影cost；baseline每attempt调用PBQP一次但不进入layout frontier。第11项只运行完整baseline gate；最后在第12项让search复用同一PBQP首proposal并保留raw layout域，原子建立其它current-IR choice枚举、重接controller、以Accepted current Instr的resource-aware makespan选择winner、切换production并删除flat instruction objective、future physical value/movement/storage/event/schedule shadow chain和旧complete materializer。最终baseline从current TensorProgram和固定规则直接构造actual IR，不进入search choice/domain/state；search在spatial/region/temporal choice后物化自己的actual IR；完成IR膨胀/融合/最终Instr与per-engine cost汇总；同一current FP16 LLaMA block的`none`/`search`各自在15分钟Release门限内生成package并strict readback/no-card | 读AGENTS/progress→读06及本项14个矩阵→读相关硬件/ABI事实并把未证同步、engine overlap和performance rate保持其证据边界→调研论文和成熟编译器算法/实现并比较取舍→查官方及pinned LLVM/MLIR API→按checkpoint改代码/测试→fresh验证→按设计与LLVM/MLIR规范复审完整diff和下游witness→更新状态并提交 |
...
| --- | --- | --- |
| Q52 | search-scalability | 本项14个线性checkpoint按current consumer-first施工顺序全部通过；baseline/search共同使用修复全局tie、通过flat oracle且按NE、Vector/CT、movement和instruction-control分别投影的current-IR PBQP layout optimizer，baseline每attempt一次、search首proposal且PBQP on/off不改raw域或exhaustive accepted set；budgeted差异保持typed coverage并报告；第9项唯一full-transfer cleanup已接production；第6项actual leaf内bufferization、completion repair和allocation sinking调用为零，普通bufferization/allocation、rotating allocation与Instr completion分别只有第10、8、7项owner，SPM/DDR high-water只从accepted offsets重算；第12项已原子建立search structural producer和current-IR layout→movement→execution-structure→Instr/order/completion choices，接通completion-closed actual memory/target leaf，并以Accepted current Instr的resource-aware makespan选择winner；统一instruction cost已删除，缺失相关work/rate及依赖未证NE/CT overlap的排序保持typed incomparable；baseline/search保持controller和actual IR owner隔离；baseline直接构造actual attempts且search state/domain调用为零；search的spatial/region/temporal choice闭合后每个candidate均以actual TileRegion/Instr IR作唯一事实源；verifier按LLVM/MLIR职责闭合；旧physical-value、movement、storage、execution-structure、schedule shadow state、domain rebuild、parity verifier和complete materializer零production残留；已确认的IR膨胀、双路径、baseline root grouping和layout/movement问题已修复；同一current FP16 LLaMA source的两次fresh package/no-card验收通过 |
