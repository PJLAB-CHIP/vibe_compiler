...
| Q50.A | `exact-placement-demand-boundary` | `doing` | Q50.0、Q54 | 已有四态`ExactDemandStatus/Result`、data/init/support-chain exact `IndexRelation`组合、typed coverage witness及production legality接线；2026-08-17 review确认边界尚未闭合。当前production trial以整个node loop domain求一次image，未对每个consumer logical shard建模，逐destination Tile demand仍由旧direct-data carrier重算；trial ownership没有producer-result identity且adapter显式拒绝multi-result；进程级`IREpoch`无production mutation owner推进，old query + old trial可跨mutation继续使用relation cache；`consumerIterationDomain`结果字段未填充，ownership/witness顺序仍受caller binding顺序影响。完成必须以typed assignment显式表达并查询每个consumer shard、按producer result表达ownership并通过multi-result production路径、建立真实per-IR mutation invalidation、补齐result payload和确定性；现有query tests未覆盖这些完成门禁，详细review与证据由对应设计文档owner。 | 05-07、09-13、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
