...
| Q59 | `compiler-entry-transaction-closure` | `done` | Q56达到`board-ready` | 2026-08-16闭合：`VerifiedPackage`原位改名move-only `ExecutablePackage`，`compileProgram`返回`llvm::Expected<CompilationResult>`（package+optional compiler-owned `ProfileInstrumentationProduct`），`compileProgramWithTargetLLVMModules`返回`llvm::Expected<CompiledProgram>`；失败由`CompilationStage`+`CompilationFailure`分类。transaction在no-replace commit后按installed root readback manifest并digest绑定profile activation；production `wafer-compile`成功当且仅当commit+readback完成，`--target-model*`/`--dump-compiler-ir`/旧`--output-program-dir`报unknown argument，internal/test入口收敛到`wafer-compile-test`；`--output-package-dir`原位替换并同步全部consumer。install closure含compiler+helper+device linker script+CRT/ABI资源，单一resolver `resolveDriverToolFacts`按executable-relative/PATH/`TX8_DEPS_ROOT`发现并验证外部facts，二进制不烘焙source/build绝对路径；install规则被importer+SPMD deps+configured helper门控。验证：unit 28/28、request/internal-options/atomicity/install-relocate（fresh source→package readback/no-card于relocated tree）通过；source/deps/IR organization检查通过；feature-off不安装production compiler由`wafer-compile-install-feature-off.test`钉住（本feature-on build中unsupported）。pre-existing外部缺口：`wafer-compile-spmd-partition.test`的32x32 dot输入被pinned XLA helper拒绝（helper调用byte-identical，与Q59无关）、同输入search路径45分钟+未收敛（候选持续枚举，search cost属Q52）。 | 01、15、18-20；`tasks/plans/compiler-entry-productization.md` |
