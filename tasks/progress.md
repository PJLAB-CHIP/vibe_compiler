...
| Q19 | `reference-executor-core` | `doing` | Q16 | 已落owner-backed immutable `ReferenceProgram`和执行前capability preflight：flat/tile-region、static view、movement、GEMM、f32 elementwise/fill、非zero-point convert以及single-block SCF/acyclic CFG均投影为value-id/block graph，执行期不读mutable MLIR；RND_MODE 0..3和0/1/2 trip、branch/CFG forwarding已通过。仍需把Q16/Q17单函数限制收敛为唯一entry+private closure后补direct-call，并补zero-point/stochastic证据、完整capability矩阵、非平凡differential、独立layout property oracle及executor文件职责拆分。 | 10、11、16 |
