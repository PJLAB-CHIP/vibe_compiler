...
| 2 | `board-testing` | `doing` | 16；关联05、06、08、10、11、13--15、17 | current compiler/runtime、全workload/多预算审计及actual IR、实卡数值/profile证据 | 全workload、多预算与既有DTE实现审计已完成；总任务尚有归约tail与BF16 LLaMA数值失败，及已确认的搜索覆盖/搬运热点。按同一计划先闭合数值和completion witness，再修正通用搜索/复用机制；不得以no-card或单个模型收益代签 | `tasks/plans/board-performance-optimization.md` |
