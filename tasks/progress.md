...
| 2 | `board-testing` | `doing` | 16；关联05、06、08--11、13--15、17 | current compiler/runtime、原默认search全配置证据、扩展模型source及actual IR、实卡数值/profile | 通用复用/容量反馈/搜索调度修改及全部默认search产品主机门禁已通过；原全部默认search配置已有本轮实卡计时，除BF16 LLaMA数值超差外完整PyTorch通过，FP16 LLaMA已恢复历史性能目标。当前按[扩展矩阵与三轮方案](plans/board-workload-matrix.md)推进：先修BF16 LLaMA并完成新旧case正确性压测，再冻结基线/profile，依次调优复用与DDR、流水与等待、搜索选择及剩余热点；每轮要求通用根因修复、完整数值和固定关键case无可确认性能退化；按影响范围/编译成本分层验证，试修先跑机制，稳定后测相关整网，最终补齐全矩阵资格，不每轮全量重编。用户已移除DLRM；新增八类case尚未实现，正确性收口及三轮调优均未完成。原预算覆盖、主机开销、completion/lowering及历史高预算4K风险继续对账，风险设备执行最后处理；完成局部矩阵不代签总任务 | `tasks/plans/board-performance-optimization.md` |
