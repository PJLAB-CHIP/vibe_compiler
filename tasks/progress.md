...
- P2.F1 主链路已能从 source-built PyTorch/XLA capture adapter 产出 PyTorch/XLA StableHLO bundle，
  并通过 bundle metadata / data verifier；P2.S1 / R3 / R4 / R5 / R6
...
| P2.S1 | ready | 集成完整 Shardy propagation / SPMD partitioner pipeline | 从 P2.F1 4096 matmul verified artifact 出发，通过 framework frontend mark 覆盖 data/batch、column parallel、row/contracting、2D output、2D contracting+output 和 partial replication sharding，不以手写 `sdy.sharding` fixture 或 sidecar 作为主 gate；运行 Shardy propagation/partitioning 并产出 per-rank artifact；multi replica group、rank selection、shard slicing、collective legality 和 placement input 由 IR/attr/verifier 明确表示；完成证明必须从真实图 artifact 跑到 per-rank artifact 正确导出和自校验。若后续 collective normalization、placement 或 ring/resource lowering 尚未覆盖某个硬件可表达语义，应记录为对应下游恢复任务或补 IR 表示，不能作为 P2.S1 拒绝理由 |
