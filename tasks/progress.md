...
| 2 | `board-testing` | `doing` | 16；关联05、06、08--11、13--15、17 | current compiler/runtime、原默认search全配置证据、扩展模型source及actual IR、实卡数值/profile | 原默认search全配置已有实卡计时，除BF16 LLaMA数值超差外完整PyTorch通过，FP16 LLaMA已恢复历史性能目标。当前按[扩展矩阵与三轮方案](plans/board-workload-matrix.md)推进：先修BF16 LLaMA并完成新旧case正确性压测，再冻结基线/profile，依次调优复用与DDR、流水与等待、搜索选择及剩余热点；每轮要求通用根因修复、完整数值和固定关键case无可确认性能退化。已接入ResNet、ViT、大GEMM与batch共享RHS；嵌套view地址修复通过主机门禁及batch package/no-card；ResNet的BN合法化和opmath已通过主机数值及Linalg，整网编译及窗口归约lowering仍待修复。E-graph复用已完成LLaMA no-card且包与原正确快版本相同；后续Region提案的精确计分/合法性查询优化已通过主机门禁，该修改的ResNet及LLaMA整网资格继续验证。数值、容量和ViT前端缺口尚未修复，其余新增模型尚未接入。QKV独立诊断发生真实设备超时，已停止实卡批次，继续主机准备/分析。按影响范围与编译成本分层验证，不每轮全量重编；LLaMA为可能相关修改稳定后及每轮末的强制检查项，保留初始正确快版本防累计退化；最终冻结版本后全矩阵实际板测一次。用户已移除DLRM；正确性收口、三轮调优及原预算/主机开销/completion/lowering边界均未完成，风险设备执行最后处理；局部通过不代签总任务 | `tasks/plans/board-performance-optimization.md` |
