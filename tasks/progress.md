...
Q50.A -> Q50.S semantic-root query/algorithm                  [queued]
...
Q59 + Q50.A + Q45 + Q50.F closure
  -> Q49.P deterministic baseline plan-only resource closure [doing]
Q49.P + Q50.F closure + Q50.S–K -> Q51.Core planning contract and actual planner
                                                               [queued]
...
| Q49.P | `deterministic-baseline-functional-closure` | `doing` | Q54、Q59、Q50.A、Q50.0、Q45及Q50.F full closure | 已迁入的consumer-operand exact demand、structured-producer截断、single-root final construction、local/peer/DDR coverage、职责拆分和旧probe/rebuild/default-stat删除继续保留；旧FP16 LLaMA package/no-card证明功能链可走通。但current baseline仍为每个temporal legalization coordinate构造CardModule并运行Q50.0，约22,968次TileRegion→Instr正是compile-to-query反模式，不能转交Q52或靠actual IR cache处理。终态每个coordinate只调用Q50.A/F等pure typed query，ExactRejection沿预定义单调first-fit推进，FullFeasibilityProof关闭plan；planning CardModule/Q50.0均为零，selected plan各一次，actual/resource parity失败终止且不继续fallback。迁移后重跑定向oracle、轻量package/no-card和一轮fresh FP16 LLaMA，才能恢复`done`。 | 06、14、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
...
| Q51.Core | `physical-dataflow-search-kernel` | `queued` | Q50.0、Q50.A、Q49.P、Q50.S–K、Q50.F/J closure | 建立driver-owned、IR-free planning session：closed state variants与typed continuations按S→B→D→E→G→H→I→K→I→J依赖惰性展开；proposal不移动canonical cursor。exact work、admissible bound和estimate分层，同cohort best-first只凭bound prune；direct causal `ForbiddenAssignment`不猜scope。planning allowance限制successor/query/solver work而非candidate编译；coverage区分objective-optimal、feasible-with-bound、feasible-unranked和budgeted。`none`/`search` controllers独立，只在final plan schema/materializer/downstream pipeline汇合；selected plan只commit一次。 | 06、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.S | `structured-semantic-alternatives` | `queued` | Q50.A | 终态设计不再枚举或clone whole-program alternatives：search TensorProgram先把已证明的softmax-weighted-sum一次性归一为显式semantic op；每个root只选择MaterializedSoftmax/OnlineRecurrence，block归Q50.E、partition/merge归Q50.B/A。planning读取typed logical facts但零IR materialization；winner进入Q50.C唯一CardModule transaction后由selected emitter直接构造actual work，不写临时algorithm property。实现须迁移current/旧attention、decode、Flash tile/partial算法与tests，删除Module clone、Original/Online/Split(block,partition)旧domain和decode-only gate。 | 05-06、08、10、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
...
| Q50.F | `scoped-feasibility-analysis` | `queued` | foundation依赖Q50.E；full closure依赖Q50.J/K | foundation以closed sum返回Consistent/Deferred/ExactRejection/Unsupported/Indeterminate，不把“未发现矛盾”叫feasible；required G–K objects由typed partial state和实际check自动推导。它从A–E facts建立minimum storage与mandatory interference，只凭必要certificate reject，partial state不调用packer。full closure只对S–K已关闭的contender构造typed SPM/DDR packing、DTE/FSM/descriptor/worker/event及target-field问题；fast placement只提供validated witness，heuristic no-fit/solver耗尽保持Indeterminate，只有完整proof才形成无offset `FullFeasibilityProof`。Q50.0从winner actual IR重建同一normalized problems并分配真实offset；不一致是compiler bug。旧SPM clone/probe、caller missing-list、packer feedback均不恢复。 | 06-11、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
...
| Q50.I | `storage-binding-and-rotating-slots` | `queued` | Q50.H | 终态`BufferPlan`按`PhysicalVersionId`选择fresh/exact-alias/reusable storage object，并为E/H同一exact occurrence relation建立`1..U` slot families、rotation expression和必须由J满足的def/use/completion/release ordering。schedule-dependent reuse只产生hard requirement；K改变execution occurrence后使I/J失效重算。完整winner由construction-time object/rotation builders直接扩展E/K loops，per-scope lifetime verifier重证alias/range/backedge/release；旧edge subset+bufferCount、actual-loop scan、1900-line clone/reorder materializer、wait/NCC混装和fixed slot cap退出。 | 08-13、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
| Q50.J | `event-resource-scheduling` | `queued` | foundation依赖Q50.I、Q63；closure依赖Q50.K | foundation从S–I plan构造query-local EventGraph：hard dependencies、resource-order alternatives、async issue/completion、exact ranges和typed resources；opaque NoC不伪造link。kernel惰性覆盖worker/resource binding、resource/control sequences和completion boundaries，calendar/timestamps不进state。固定K并重闭I后，full resource-constrained search用critical/resource/recurrence/transport sound bounds；winner由ScheduleEmitter在construction时按EventId创建order/worker/DTE waits/min NCC joins/releases，Card verifier重证deadlock/pending/resource。旧Instr pointer/snapshot domain、move-op apply、post-hoc join rebuild、false link/resource、registry/clone退出。 | 08、11、13-19；`tasks/plans/physical-dataflow-synthesis.md` |
