...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `search-scalability` | `doing` | Q52 | Q51 search controller、structural choice与attention展开donor，current baseline regression，Q50.0 actual memory/target leaf | 按current plan的13个线性checkpoint以consumer-first施工：第6项从现有Card→Executable入口抽出Q52 current Instr与已完成Q50.0 actual leaf之间的typed合同，不重复实现allocator/target；第7--9项继续沿baseline唯一production路径逐层向上替换Instr/completion、movement/boundary和layout/region-local bufferization，每项必须由fresh TensorProgram实际进入已闭合下游；第10项只运行完整baseline gate；最后在第11项原子建立search structural producer、切换production并删除future physical value/movement/storage/event/schedule shadow chain和旧complete materializer。最终baseline从current TensorProgram和固定规则直接构造actual IR，不进入search choice/domain/state；search在spatial/region/temporal choice后物化自己的actual IR；完成IR膨胀/融合/最终Instr汇总；同一current FP16 LLaMA block的`none`/`search`各自在15分钟Release门限内生成package并strict readback/no-card | 读AGENTS/progress→读06及本项13个矩阵→读相关硬件/ABI事实并把未证同步语义保持unknown→调研论文和成熟编译器算法/实现并比较取舍→查官方及pinned LLVM/MLIR API→按checkpoint改代码/测试→fresh验证→按设计与LLVM/MLIR规范复审完整diff和下游witness→更新状态并提交 |
...
| --- | --- | --- |
| Q52 | search-scalability | 本项13个线性checkpoint按current consumer-first施工顺序全部通过；第11项已原子建立search structural producer、接通current-IR layout→movement→Instr/completion→actual memory/target consumers并删除旧路径；baseline/search保持controller和actual IR owner隔离；baseline直接构造actual attempts且search state/domain调用为零；search的spatial/region/temporal choice闭合后每个candidate均以actual TileRegion/Instr IR作唯一事实源；verifier按LLVM/MLIR职责闭合；旧physical-value、movement、storage、execution-structure、schedule shadow state、domain rebuild、parity verifier和complete materializer零production残留；已确认的IR膨胀、双路径、baseline root grouping和layout/movement问题已修复；同一current FP16 LLaMA source的两次fresh package/no-card验收通过 |
