...
| Q43 | `compiler-collaboration-review-materials` | `done` | Q9、Q37-Q39完成证据 | 2026-08-03冻结的168页PPTX/PDF历史汇报快照保持不变。独立4页开场材料已按探索出发点、AGENTS.md开发流程、工程规则和六次架构重构整页返工；旧四张错误图片已删除。四张新Image2主图先固定真实技术对象再逐页生成和校正，第一页不展开LLVM/ABI/backend artifact，第二页使用真实SPM planning owner与token/wait lifetime case，第三页以side-table失效、验证范围和共享checkout三类失败机制组织规范，第四页按下游消费驱动的边界重构展开且明确当前whole-rank工作仍未完成。PPTX/PDF、4页PNG、contact sheet、source map和嵌入notes已fresh生成；原尺寸复核图文一致、无裁切遮挡。 | 01、06、16–19；`tasks/plans/vibe-compiler-collaboration-review.md` |
...
| Q49 | `whole-rank-tile-dataflow-synthesis` | `doing` | Q32 actual-clone/relation/mechanism/exact-gate基础；Q38–Q40 execution/NoC/overlap机制；Q41计时、并发与配置基础；Q46/Q47 compiler-side current合同。Q40/Q41/Q46/Q47尚缺的板端外部门禁不阻塞本项施工 | `tile.region`只表达SPM residency domain；每个static rank entry可有一个或多个non-nested regions，data边variadic且只允许DDR，SPM root/alias不跨界。region partition与traversal/tile、layout/version、resident/spill/recompute、materialization和communication共同进入actual-clone搜索；task、loop、collective或单个spill都不自动切region，region数量不作为成本代理。C2 candidate generation与C3 terminal exact feedback从generation前起共用唯一all-rank coordinator/global ledger并共同闭合搜索；允许不可发布、不可独立选winner的rank/component factor summary，但无rank-local commit、`N^R` tuple或分立预算。terminal current IR删除旧join后fresh重建completion，从SPM roots/lifetime/coexistence/conflict与explicit DDR arenas/placement domains分别派生fixed allocation problems并all-and-only覆盖，problem/query数量只作预算诊断。Dominance分别覆盖DDR、GS/local movement、SPM movement、NoC/link/endpoint、compute/recompute、tile utilization、completion/wait/critical path、Instr/descriptor/resource pressure和all-rank coupling；known与不同`Unknown` disposition不可直接支配。最终只在全部late gate通过的fresh IR上由当前校准hardware cost model选winner；bank phase不改变hard feasibility，无artifact笛卡尔积、packing quality probe或ILP/CP-SAT production oracle。C6删除旧per-task lower/placement/commit与task-return join代码；通用graph、Llama、cached-attention和prefill完成完整package/fresh no-card后为`board-ready`，真实板端matched correctness/performance后才能`done`。 | 01、06-13、16、18；`tasks/plans/whole-rank-tile-dataflow-synthesis.md` |
