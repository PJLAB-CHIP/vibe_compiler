...
| 2 | `board-testing` | `doing` | 16；关联06、13--15、17 | current compiler/runtime、IndexRelation与actual candidate IR、三条模型的fresh FP16 PyTorch source/input/reference、既有profile证据 | 持续完成halo/buffer、主机正确性、编译开销、decode退化、完整FA block、prefill采集及剩余搬运七项；先闭合可独立推进的host/fresh no-card，设备恢复后完成模型PyTorch与匹配性能复验；搜索预算比较继续延后 | `tasks/plans/board-performance-optimization.md` |
