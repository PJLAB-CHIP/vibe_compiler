...
- R2.4-pre 已完成库级 Wafer named pipeline / driver contract：`WaferPipelines` 注册
  ...
  `wafer-lower-stablehlo-to-cabi` 明确组合 StableHLO->Linalg 与 Linalg->C ABI 两层。
  `wafer-import-model --compile-stablehlo-bundle-to-cabi` 作为用户级 artifact driver，先校验
  PyTorch/XLA / SPMD 产出的 bundle，再调用同一条 StableHLO->C ABI pipeline；`wafer-opt` 只负责
  注册和调用，单 pass flags 保留给 Transforms/StageConnections 这类 unit/debug 覆盖。用户级
...
- R2.4-pre 已固定主链路 gate 的 named pipeline / driver 入口：StableHLO bundle 可通过
  `wafer-import-model --compile-stablehlo-bundle-to-cabi` 直接进入编译；`wafer-opt` 层的
  StableHLO->Linalg、Linalg->C ABI、StableHLO->C ABI 和 tile-level communication->C ABI gate 都通过
  库中注册的 Wafer pipeline 重放。pipeline option `target=wafer` 会 materialize/校验
...
| R2.4-pre | done | 收敛 Wafer named pipeline / driver contract | 新增 `WaferPipelines` 库并在 `wafer-opt` 注册 `wafer-lower-stablehlo-to-linalg`、`wafer-lower-linalg-to-cabi`、`wafer-lower-stablehlo-to-cabi` 和 `wafer-lower-tile-communication-to-cabi`；`wafer-lower-stablehlo-to-cabi` 组合 StableHLO->Linalg 与 Linalg->C ABI body；`wafer-import-model --compile-stablehlo-bundle-to-cabi` 从 StableHLO bundle verifier 进入同一条 compile pipeline；lit 覆盖手写最小 bundle 和真实 source-built PyTorch/XLA 小尺寸 export bundle 两条入口；pipeline option `target=wafer` materialize/校验 `#wafer.target<wafer>`，非 `wafer` target 诊断；单 pass flag 保留给 unit/debug，StageConnections 仍用于逐层连接覆盖 |
