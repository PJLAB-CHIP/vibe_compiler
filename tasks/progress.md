...
| Q51.Core | `physical-dataflow-search-kernel` | `queued` | Q49.P；复用Q50.0 | 直接接收Q49.P已准入的move-only baseline executable与同cohort actual cost作为session-level incumbent，不用search carrier重建baseline，也不把baseline choice当未来轴default。Core只建立current typed assignment/ephemeral transition apply、deterministic frontier、stable dedup、global work/budget ledger、typed evaluation routing和coverage/bound evidence；尚未施工的Q50轴不预声明placeholder，mechanism/priority/incumbent/derived lifetime、calendar、SPM与cost均不进入candidate identity。本checkpoint原子接管public `search`的frontier/actual comparison/winner control；既有bounded generation原位改为只返回current typed complete proposal的显式lossy source，其beam/cap/shortlist必须报告coverage loss，不能更新winner或repair accepted IR。Core以独立finite state-graph model证明frontier mechanics；每轴真实reference enumerator随Q50.S/B–K交付，通用actual-region probe归Q50.F，full production flat exhaustive runner、exact domain、有效fusion与有损过渡proposal清理归Q51 closure。现有`TileExecutionCandidate`的metrics、stable ordinal、feedback history和beam控制不得升级为Core合同；Core不固定best-first、默认budget、no-good/dominance或LNS。 | 06、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
...
| Q51 | `physical-dataflow-unified-search` | `queued` | Q50.S、Q50.B–Q50.K | 在Q51.Core唯一control owner上闭合完整typed domain：同一可回溯assignment state联合表达semantic root、spatial mapping、TileRegion/融合、temporal tile与wave-loop order、layout/version、movement、buffer和resource-event choices，派生analysis不进入identity，并删除Core阶段显式lossy的bounded过渡proposal。tiny DAG以逐轴独立reference enumerator证明域完整、production flat exhaustive runner证明candidate set/剪枝/winner；真实负载以baseline incumbent和显式预算返回accepted actual winner并按coverage标结果等级。必须以共享TileRegion、coupled traversal或明确retained SSA、tile-sized intermediate和无中间DDR round-trip证明有效融合；`search`与`none`共用Q50.0 exact gate。 | 06-13、16、18；`tasks/plans/physical-dataflow-synthesis.md` |
