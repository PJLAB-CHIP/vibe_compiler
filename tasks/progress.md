...
| 2 | `board-testing` | `doing` | 16；关联05、06、08、10、11、13--15、17 | current compiler/runtime、全workload/多预算审计及actual IR、实卡数值/profile证据 | 五项方案的主要代码改动已实施，分项主机检查通过：e-graph保留限额内有效改进并提高默认额度；物理copy复用；descriptor/issue/completion估时；整数近邻和分层探索。访问吸收后的LLaMA/decode实卡PyTorch已通过，prefill物理复用未测得加速。最终验收未闭合：新prefill大预算候选发生真实设备completion超时；2026-09-13用户指定单次Add复查也同样超时（fresh no-card通过、boot/runtime未变），当前停止全部板端执行；search catalog与部分大预算主机编译暴露长链buffer type重复推导，已保留verified current-IR复现。Region domain漏partial→merge边已修复并通过独立反例，异构none通过而search长测未完成。先处理主机长测及设备超时根因，再补最终PyTorch与匹配性能；不得以中间版本通过或静态估时标done | `tasks/plans/board-performance-optimization.md` |
