...
Q51.Core -> Q64 source registration and library ownership    [doing]
...
| Q64 | `source-registration-truth-closure` | `doing` | Q51.Core；Q62/Q63继续拥有各自语义清理 | 用户要求优先关闭全仓library ownership：逐文件扫描Wafer-owned source/header/CMake/tool/test/runtime，删除`Compiler`中间目录和无独立语义的顶层`WaferPipelines`，按Analysis/Planning/Conversion/Transforms/CodeGen/Driver/Program/Package/Target/Runtime/Model唯一owner移动实现与unit mirror；named subpipeline回到实际IR owner，单pass wrapper删除。同步建立repo-wide active registration或current task-owned dormant disposition，不建立compat include/target alias。Q62/Q63后续语义重构在新owner内继续。 | 18；`tasks/plans/source-registration-truth-closure.md` |
