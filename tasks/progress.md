...
PyTorch/XLA StableHLO Wafer program directory
  ...
  -> boundary-shards materialization
       function boundary attr `wafer.boundary_shards = @symbol` 指向
       `wafer.boundary.shards` payload；payload 引用 execution mesh；不重新切分 tensor
  ...
  -> ABI / LLVM lowering
       committed instruction IR + accepted offsets + topology/execution-mesh/boundary-shards
...
| boundary-shards migration | done | partitioned StableHLO program + parameter shard metadata + `wafer.execution.mesh` | function boundary attr 指向 `wafer.boundary.shards` payload；payload 引用 execution mesh；rank coverage 对 mesh 校验；IR 不再保存独立 rank count |
...
| ABI / LLVM lowering | pending | committed instruction IR + accepted SPM/DDR offset facts + topology/execution-mesh/boundary-shards + launch-block binding + communication/sync lowering | LLVM dialect call sequence 或 `wafer_*` C ABI / packet builder input；按需重算 launch/resource view，固定参数单位、address domain、wait/completion policy 和 ABI version |
| object + package manifest | pending | ABI/LLVM artifact + committed IR + topology/execution-mesh/boundary-shards | object/program id、entrypoint、ABI version 和 IR-derived package manifest；endpoint/resource/constant metadata 由同一 resource view analysis 从 IR 重算 |
...
1. 让 communication、ABI/LLVM 和 package manifest 从 topology/execution mesh、`wafer.boundary.shards`、
