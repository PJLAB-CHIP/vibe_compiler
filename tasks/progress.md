...
numeric gate接受支持的f16/bf16/f32；scalar algebraic candidate显式消费target profile的typed
`SourceExact`/`Relaxed`策略，前者保持source DAG，后者允许候选且replacement只继承原op fast-math flags交集。
...
| Q37 | `tx81-compiler-hardware-calibration-and-multi-engine-software-pipelining` | `doing` | Q32、Q6.B + configured board | 已有raw台账闭合对应legality/correctness风险并保留Unknown的保守fallback，但不再称硬件行为完备；已上板case不重复。当前fresh会话的FP16 exact Add资格为1/1；8个production optimizer paired case为8/8，复用的Direct-DTE vertical也通过；9个FP16 collective algorithm与11个FP16 AllToAll/Permute traffic case为9/9与11/11；generic engine single activation group为1/1、pair activation group为60/60；Direct-DTE、DDR tile-offset、DDR sparse focused组为3/3；FP16/BF16 targeted strided TDMA为2/2 exact。engine case名中的`i2/i4/i8`只表示iteration 2/4/8，payload仍为FP16，不是I8 dtype。此前I8通信结果均已撤销，不进入当前资格。首次AllGather、Direct-DTE、DDR tile和tree all-reduce曾因host oracle仍匹配旧cluster launch字符串被误报；修正后只用新构建、新启动和新输出fresh执行通过，不回放或重判旧输出。这些结论只闭合已测输入、结构、result/guard、completion与既有oracle，不外推其它shape/dtype/route或production cost。旧清单误纳的native Concat `dims=HW`已经按永久非法删除全部board入口，永远不测试且不计入合法分母。明确的model F32 ABI、convert、index/mask、量化和raw字节协议专项不得冒充主线FP16/BF16算子资格。DTE raw `dest_num`编码和destination mapping只作板端观察；8/15源fan-in、device phase、physical route和跨卡transport持续blocked。worker partial issue/observation failure按全部attempted worker至多做一次bounded cleanup，cleanup deadline失败即poison停批且不retry。SPM pilot现读取owner-backed port0/6 T2/T3 counter并做matched WDMA readback及partial-accept安全drain，但没有physical bank/port mapping或bank-specific attribution。缺production three-stage producer、per-worker timestamp或physical arbiter的项同样持续blocked。software-pipeline仍需在完整Instr IR上以typed MemoryEffects和SSA alias/root/path建立edge、物化multi-buffer和latest-legal completion，并经过完整late gate。 | 06、08-17；`docs/tx81-compiler-hardware-calibration.md`；`tasks/plans/multi-engine-software-pipelining.md`；`tasks/plans/production-optimization-board-campaign.md`；`tasks/plans/collective-hardware-characterization.md` |
| Q32.N | `numeric-algebraic-extension` | `done` | Q32 | scalar algebraic candidate消费target profile的typed `SourceExact`/`Relaxed`策略；exact保持float source DAG，relaxed允许actual mutation/frontier且replacement只传播共同fast-math flags。paired qualification统一signed-zero、finite tolerance和nonfinite合同，每个variant的f16板端数值由`wafer-run`执行同一typed tolerance，结构/长度/guard/index/completion exact。generic reduction、named GEMM K切分和Ring collective继续按各自typed numeric gate覆盖支持float；integer overflow/no-wrap及真实结构、资源和target负例保持。未新增frontend mode或IR carrier。 | 05-07、10-11、13、16；`tasks/plans/numeric-algebraic-extension.md` |
