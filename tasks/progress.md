...
| 2 | `board-testing` | `doing` | 16；关联02、05、06、08--11、13--15、17 | current compiler/runtime、原始Torch XLA模型及actual IR、独立PyTorch reference | 用户确认LLaMA2单层、ResNet18和大GEMM已完成各自已记录的主机链路，本轮收尾将已验证实现及配套记录一并提交推送。LLaMA2单层FP16/BF16已通过source、search、verified package、TargetModel完整输出比较及fresh no-card，采用用户确认的余弦相似度与相对L2联合合同，保留embedding、decoder、final norm和LM head；ResNet18此前整网search、verified package和fresh no-card通过，不能外推为完整数值通过；大GEMM整除/尾块及两种半精度的已登记矩阵通过search、verified package和fresh no-card，最新单项重跑确认默认预算下仍有可行驻留候选。通用循环状态布局已与初始化解耦，GEMM及online attention机制和attention源程序数值回归通过；读取最少与评分最优分别记录，尾块流量、输入转换复用及未校准模型效果不作为已完成的性能承诺，本轮不继续扩展优化。AccessReuse范围仍为跨Tile共享、基础驻留、固定步长滑动窗口和最多两级驻留，暂缓替换；YOLOv5s不在范围内。主机资格的版本、矩阵、最新重跑及限制见同一计划；真实板端未通过，总任务继续doing。 | `tasks/plans/board-workload-matrix.md` |
