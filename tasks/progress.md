...
| Q49.P | `deterministic-baseline-functional-closure` | `doing` | Q59；Q50.A；Q50.0、Q54、Q45 | `none`必须从未选placement/region/temporal/representation/buffer的正常TensorProgram自行完成确定性功能合法化，并只复用typed iterator/topology facts、single-coordinate legality/materialization机制和Q50.A demand query。follow-up review确认current baseline仍展开全部iterator-axis×connected-rectangle placement options并递归propagate/backtrack取一个结果、在capacity refinement前后重复完整CardModule materialization、accepted后构造并丢弃`StaticSchedulePlan`/duration estimate、普通compile无条件打印Tile IR、并与旧search共享百字段statistics/result header；这些均属于Q49.P而非Q52。`2f2e8de0`增加的function-scope probe、single-Tile入口、storage-root memo、DDR split和reduction temporal materialization只作为新baseline机制资产融合，不建立独立DS修复线；current relation remap/retain会丢witness，root split不retarget consumer reload relation且接受zero-root/固定256轮，direct-witness fallback还有唯一root猜测与bool更新错误。完成时baseline只有一个live coordinate，直接构造canonical spatial placement；不得生成option domain、调用recursive CSP/search state/candidate/evaluator、保留alternative或比较winner。只有direct typed rejection可触发确定、单调且不回溯的functional legalization transition；所有trial从一开始走single-root/single-Tile最窄materializer/probe，全部fit后完整CardModule与Q50.0 compile各一次；accepted result不依赖未消费cost/trace，baseline实现/result/窄ledger不引用旧search对象。每个TileRegion恰一structured root、跨root显式DDR、relation在current consumer buffer上语义完整、direct causal witness与ancestor-scope probe继续闭合；fresh overfull-to-fit、非空relation类别、actual split IR及轻量source-to-package/no-card以显式`none`证明package/oracle和work count。Q51完整new-search链闭合前，Q49.P、Q50各checkpoint和Q51.Core均不得执行重型LLaMA block的`none`或`search`；既有2h21m未完成运行只作为旧baseline误入全option/CSP及无界exact-relation热路径的诊断，不是Q49.P门禁或重跑项，首轮重型LLaMA执行归Q52显式scalability profile，正式package/no-card归Q53。`TileMemoryPlanningTest.ReportsSPMFailureForOwnedTileModule`的空evidence已定位为无`materializationRelations`分支只复制scalar字段而漏传三个demand集合，需在Q49.P内修复。 | 06、14、16、18-19；`tasks/plans/physical-dataflow-synthesis.md` |
