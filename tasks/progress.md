...
Q42 + Q32.C -> Q41 compiler search scalability               [next]
Q42 + Q39 -> Q40 composed search and DTE overlap             [later: after Q41]
```
...
| Q41 | `compiler-search-scalability` | `next` | Q32.C bounded executor、Q42；M-sharded K=1024复现 | 无卡阶段必须推进到`board-ready`：搜索上界、编译时间、winner不变、K=1024 case完整package和no-card闭合，但不得标`done`。真实板端exact-output及winner profile有效后完成；不跑target model或无关suite。 | 06、14-16、18；`tasks/plans/compiler-search-scalability.md` |
