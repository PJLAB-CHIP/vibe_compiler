...
| transformer staged gaps | pending | static transformer local shard IR / staged IR gaps | HF Llama config snapshot + PyTorch/XLA `mark_sharding` + 16-rank Megatron-style tensor parallel 已能到 `stablehlo-spmd-to-group`；剩余是 full-block group->tile/instr schedule 或拒绝原因、mask/select、dynamic-bound policy、constant/weight slice 到后续 memory/runtime |
