...
| Q16.T | `direct-dte-transport-activation` | `doing` | Q18 | logical identity与post-memory physical acceptance checkpoints已闭合：`channel_id`贯穿collective/tile，typed `DTEMessageAttr`唯一标识现有ring/direct/tree/permute/all-to-all消息；完整rank domain按source/destination/message匹配并核对peer/bytes/planned SPM range/same-block wait，normal profile验证单live sender并分配4个无重叠receiver FSM，成功后才原子写`DirectDTEBindingAttr`并形成`TransportContract::DirectDTE`。all-to-all已改为逐semantic distance issue/wait；canonicalizer删除read-only wait的effect bug已修复。checkpoint gate执行40个C++ unit和233个lit（232 pass、1个feature-inverse unsupported）。下一步闭合target CRT/event/status lowering、Q17 atomic publication及Q18 runtime-observable transport requirements；不把raw packet或shadow schedule塞入bundle/manifest。 | 13、14、15、16 |
