...
Q56 board-ready -> Q58 program data backing lifecycle          [queued]
Q56 board-ready + Q58 -> Q59 compiler entry transaction closure [queued]
Q59 + Q50.0 + Q54 + Q45
...
| Q56 | `executable-package-data-closure` | `doing` | Q50.0、Q54、Q55 | 只改`CardExecutable`之后的compiler→package→one-shot runtime边界：把调用端口、package-owned immutable data和compiler-planned internal storage从单一resource role/host-visible协议中拆开；将package allocation descriptor、package view、logical program tensor与selected physical tensor/layout/span分离；parameter/external captured-constant按final target ABI的每个selected physical version物化为digest-bound package data，runtime不再要求caller逐次绑定；每个initialized physical tensor独占allocation/full-span view/segment，external input/output不生成compile-time segment。compiler writer与runtime loader统一返回一个move-only `ExecutablePackage` owner，package root只含all-and-only runtime成员并完整readback。同步替换manifest、writer/loader、no-card、board one-shot、target model、CLI和fixtures，不引入旧reader。fresh source→package/no-card、package data正负例、fake-provider one-shot与完整板端case达到`board-ready`后，Q58/Q59及后续Q49.P/Q53才消费新current package。 | 14-17、20；`tasks/plans/executable-package-and-resident-runtime.md` |
| Q58 | `program-data-backing-lifecycle` | `queued` | Q56达到`board-ready` | 只改verified source→外部SPMD helper→`CardExecutable`同事务data handoff的大数据边界：每个parameter/external captured-constant binding以immutable source backing、checked range和logical view表达，transaction持有可验证owner而不复制整树；helper消费content-stable ranges并以bounded I/O返回owned shard，post-SPMD只保留必需backing/view。current single-card完整inventory记录source、selected physical和package bytes、read/write、peak RSS/disk；多partition只作helper/frontend isolated gate。path、name、shape或相同digest不推断跨binding sharing。不改CLI、framework adapter、package schema或device residency。 | 02、14-16、18；`tasks/plans/program-data-and-whole-program-scale.md` |
| Q59 | `compiler-entry-transaction-closure` | `queued` | Q56达到`board-ready`、Q58 | 让public compiler library的primary result、package readback/commit、CLI exit status和installed tool形成同一transaction合同；success当且仅当`ExecutablePackage`已原子提交，返回值直接持有已验证package而非中间`CardExecutable`。target-model/IR dump退出production compile的post-commit gate；filesystem/external-tool/package failure使用typed `Error/Expected`；installed compiler不依赖source/build绝对路径。不改frontend格式、search语义或runtime execution。 | 01、15、18-20；`tasks/plans/compiler-entry-productization.md` |
...
| Q57 | `resident-static-execution` | `later` | Q56达到`board-ready`且Q53按新package合同达到`board-ready` | 在不改变selected `CardExecutable`语义的前提下建立Wafer-owned device failure domain、完整whole-card loaded executable、device buffer/view和带device generation的显式submission/completion；code与immutable data可驻留并按exact digest/physical descriptor/device qualification共享lease，one-shot执行只保留为同一路径便利封装。TX provider初始只single context、`max_inflight=1`、无cancel，并显式报告kernel module set/model graph各自的loaded上限；板端必须证明load-once/run-many、immutable H2D一次、每种开放的跨launch visibility crossing、workspace read-before-overwrite/initialization、public terminal及poison quarantine。provider library的process lifetime与Wafer child handle ownership分开。Q56两步状态只能经caller output/input往返，Q57只允许资格化的external device-buffer crossing；runtime-owned state、multi-inflight、persistent device loop、跨卡和external execution-engine policy均不在本项。 | 12、14-17；`tasks/plans/executable-package-and-resident-runtime.md` |
...
- Q58 program data backing与Q61 whole-program scale共用实施计划：
