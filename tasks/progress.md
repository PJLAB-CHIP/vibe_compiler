...
| 2 | `board-testing` | `doing` | 16；关联05、06、08--11、13--15、17 | current compiler/runtime、全workload/多预算审计及actual IR、实卡数值/profile证据 | 模型搜索和匹配性能未闭合。用户已授权按Spatial、融合与多轴Temporal系统方案继续实施；搜索组织修改仍在工作区验收，方案与覆盖矩阵见实施计划链接。原前7步范围保留：主机开销→容量反馈→搜索覆盖/预算/存活IR→DDR估时→completion环→conv/BF16→非风险产品与匹配实卡验收；风险4K设备执行最后处理。拟议方案、历史测试和中间accepted不代签fresh package/no-card或板端完成；共享 DDR 参数去冗余已通过20项产品no-card、组件和一次AllToAll尾部PyTorch实卡验证（见14/15号与统一性能记录） | `tasks/plans/board-performance-optimization.md` |
