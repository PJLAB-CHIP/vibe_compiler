...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `mesh-communication-materialization` | `doing` | 06、13、14、16 | verified card-local TensorProgram、抽象 collective participant/payload/combine 语义、layout-resolved current TileRegion；target transport 只消费已物化 peer IR | 通用 Ring/recursive-doubling/ordered AllToAll/reduction 算法与 TX81 transport 完全分层；execution/residency、layout/bufferization、frontend helper 和 search cost 的 current-IR 合同闭合；host/no-card/SystemC fresh 矩阵通过。真实板端明确延后，不把`board-ready`冒充`done` | `tasks/plans/physical-dataflow-synthesis.md` |
| 2 | `production-host-readiness` | `queued` | Q53 / 16 | item 1 的 current-IR 与 target-independent/target transport 分界、frontend、interface、package/runtime | FP16 主纵向 source/IR/package/oracle/runner/no-card/SystemC 矩阵通过；BF16 只执行 dtype/ABI/conversion smoke；本项不运行真实设备 | `tasks/plans/physical-dataflow-synthesis.md` |
