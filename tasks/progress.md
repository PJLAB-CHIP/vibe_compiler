...
| 2 | `board-testing` | `doing` | 16；关联06、13--15、17 | current compiler/runtime、统一ordinary fusion入口与IndexRelation需求分析合同、三条模型的fresh FP16 PyTorch source/input/reference、profile package与合格设备会话 | 既有正确性与单次计时资格保留在原归档；ordinary入口收敛及host/fresh no-card已闭合；模型profile、PyTorch与匹配性能复验仍由本项板端边界拥有 | `tasks/plans/board-performance-optimization.md` |
