...
| 2 | `board-testing` | `doing` | 16；关联05、06、08、10、11、13--15、17 | current compiler/runtime、全workload/多预算审计及actual IR、实卡数值/profile证据 | 已实现可恢复分层搜索及布局/通信/共享/流水组合，最新完整主机门禁通过；4K预算14→42实卡514.896→276.943 ms且PyTorch通过。低精度K分块额外舍入已定位，处理方向待确定；随后重签LLaMA/decode及受影响产品的数值与预算曲线，总任务未完成 | `tasks/plans/board-performance-optimization.md` |
