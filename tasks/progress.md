...
| Q56 | `executable-package-data-closure` | `board-ready` | Q58 | `TileEntryArgument` closed union、16-Tile all-and-only join、`program-data.bin` deterministic预排、canonical manifest与两块`BoardDeviceMemory` one-shot runtime已落地；`txLoadGraph`/`txLaunchModel`/`Tx81ModelABI`从compiler/package/runtime/CLI/测试原位删除，反向工程资料留在`docs/`。首轮review六个闭环项全部修复并配直接负例：canonical TargetTensor id在确定性placement后回填全部entry引用（三种重映射类回归）；`writeProgramData`消费`PhysicalTensorCodec`新增bounded-window packer按窗口materialize selected representation（identity路径流式source range、Cx块padding canonical零、每个TargetTensor只转换一次并计一次range materialization）；TileRow pointer rows H2D到`invocation base + planned offset`（fake provider断言allocation次数/row地址/每Tile失败注入/reverse cleanup与同一plan一致）；strict verifier按writer同一tie-break重建non-overlap placement并证明id顺序、offset、全零padding、精确total/base alignment（5个拒绝路径负例）；strict loader验证package root恰好`manifest.json`+`modules/`+`data/program-data.bin`（额外member/symlink负例）；中立`WaferPackageSupport`（lib/Wafer/Package+include/Wafer/Package）拥有typed model、canonical spelling、parser/serializer、verifier、readback与profile instrumentation model，`WaferCompiler`链接它且不再链接`WaferRuntime`（CMake依赖边界检查+双public link smoke）。验证：三棵树fresh build通过；Q56定向单测161/161、WaferRunBoardIOUnitTests 42/42、Compiler/Runtime link smoke exit 0、默认lit gate 216/216、SystemC integration 3/3+model 41/41、source organization与board python检查通过；Tools/Runtime lit 27/34（4 unsupported），3个失败均为HEAD既有并已在HEAD复现定位（mesh-explicit pipeline、XLA helper、selected-buffer gate，见`memory/bugs.md`）；fresh参数add source→package→no-card通过（`base=program_data offset=0`）。真实板测未执行：single-op add case/oracle/runner完整（FP16），Q56=board-ready，上板通过前不标done。 | 14-18、20；`tasks/plans/executable-package-and-resident-runtime.md` |
