...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `board-testing` | `doing` | 16；关联06、13--15、17 | current compiler/runtime、正确性case的PyTorch source/input/eager reference、纯性能case的手写IR/input、fresh no-card package与可用设备会话 | 已完成计划内Add/DTE、通信、GEMM、卷积组合及prefill正确性；cost时延/同步采样、参数回写与Analysis/Instr/CostModel归属修正已完成。当前先收尾统一评分，解决信息充分时DDR/DTE tradeoff仍不可比的问题；随后依次完成两步KV cache decode、完整LLaMA block、同源none/search产品性能验收。Decode仍有none联合顺序假环与search重叠closure失效IR待修复；LLaMA的历史manifest规模阻塞需fresh重现，两者尚未board-ready。具体修复步骤、覆盖矩阵与停止条件见同一实施计划前部；全部正确性、生命周期和matched性能证据闭合后才可done | `tasks/plans/board-correctness-qualification.md` |
