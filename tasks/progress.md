...
| Q58 | `program-data-ownership` | `doing` | Q50.0、Q54、Q55 | 先闭合verified source→SPMD helper→`CardExecutable`/target consumer的parameter/constant数据owner：一次compiler transaction打开、验证并以`ProgramDataSource`持有all-and-only source files，用stable ProgramTensor identity和checked `ProgramDataRange`交接，最终形成move-only `ProgramDataHandoff`；helper只消费transaction提供的内容，真实partition产生新owned file，连续slice只改变range。16个Tile只引用identity/range，不复制payload；target consumer禁止按Tile重复打开或转换。以bounded I/O、完整大parameter inventory、read/write/hash/RSS/disk账本和mutation负例证明后，Q56才能消费该seam。不改CLI、package schema、device memory或resident runtime。 | 02、14、16、18；`tasks/plans/program-data-and-whole-program-scale.md` |
