...
Q49 -> Q50.A exact edge logical demand correctness             [done]
...
| Q50.A | `exact-edge-logical-demand` | `done` | Q49 | dependent edge logical demand已闭合：current structured semantics统一导出query-local `IndexRelation`，relation image生成layout-independent exact demand plan并证明producer shard ownership覆盖；plan不携带layout、bytes、action、route或fusion。reduction、broadcast/window、stride、ownership正负测试和dense local/peer actual Card/Tile compatibility lowering均通过；placement transition直接消费demand planner，target carrier限制不再反写为logical legality。 | 05-07、09-13、16、18；`tasks/plans/whole-card-tile-dataflow-synthesis.md` |
