...
`wafer.compute.gemm` 的最小 shape/layout/memory-space verifier 已落地。
...
继续从 P1.8 开始实现最小 `wafer.comm` p2p、sync token、local wait 表示。不要越过 P1
verifier 直接实现 transformer block 逻辑。
