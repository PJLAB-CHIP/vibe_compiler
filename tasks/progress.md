...
**R3.2d.2 instruction legalization / selection DialectConversion**。R3.2d 的 IR 设计主文档
...
- R3.2d 只做 Wafer instruction legalization / selection：把 target-abstract executable op 合法化并
  ...
  lit/unit 覆盖。R3.2d.2 下一步实现 tile-region 到 instruction IR 的 conversion。
...
| R3.2d.2 | active | 输入：R3.2d.1 instruction ops + R3.2c tile-region IR；输出：instruction-level `wafer.instr.*` IR 或结构化失败 | 实现 `--wafer-convert-tile-region-to-instr` DialectConversion；为 load/store、layout materialize、tile.gemm/reduce/elementwise、tile movement/view op 选择硬件指令形态；递归处理 `scf.if` / `scf.for` body；不新增 storage/buffer IR |
...
下一步进入 R3.2d.2 DialectConversion：在 R3.2c 的 DDR/SPM Wafer-tagged memref tile-region IR
边界上，将 load/store、layout materialize、tile.gemm/reduce/elementwise 和 tile movement/view op
选择成 R3.2d.1 已落地的 `wafer.instr.*`，递归处理 `scf.if` / `scf.for` body，并在 unsupported
instruction form 上结构化失败。随后 R3.2d.3 接入 pipeline 和 planner scratch path。
