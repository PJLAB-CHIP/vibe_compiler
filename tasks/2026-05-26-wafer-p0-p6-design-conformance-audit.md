...
工程组织也没有按 `tasks/2026-05-11-wafer-ai-compiler-architecture.md` 第 7 节收敛。当前源码
把 frontend lowering、group/tile materialization、layout/SPM/DDR、communication 和 ABI skeleton
基本都堆在 `lib/Wafer/Transforms`，ODS 也集中在 `WaferOps.td`，verifier 集中在
`WaferDialect.cpp`。这仍是 prototype skeleton 形态，不是设计文档要求的按 IR stage / op prefix
分层组织。
...
| 范围 | 当前实现 | 与设计文档的偏差 |
| --- | --- | --- |
| P0 工程/依赖/组织 | CMake、`wafer-opt`、lit/gtest、dependency pin/checker 已有；源码目录仍以少数 IR/Transforms 聚合目录承载多数阶段 | 最小工程入口可用，但没有按第 7 节的 Frontend / IR / Transforms / Conversion 和 IR stage 组织收敛；不代表后端链路完成 |
...
4. P3-P6 继续叠加功能时，没有先关闭 P3 的真实 artifact/package 闭环，导致后续 M1/M6/M2-M4 都继承
   skeleton 出口。
5. 源码组织没有跟随设计文档的 IR stage / op prefix 边界演进，导致 frontend、planner、
