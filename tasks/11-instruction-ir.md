...
| concat / maskgather variants | 无单独 op | V0 composite or future target extension | source-level任意轴concat在V0展开为typed `gather_scatter` movement；current production wrapper只证明last logical dim映射到native CT concat的C编码，W/H虽有bounded raw completion观察但未exact-qualified，HW曾timeout，均不得因header存在编码而进入native lowering。maskgather variants仍需bool/index operand policy；未定义前不能复用`tdma_data_move` |
