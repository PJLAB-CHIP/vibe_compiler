...
Q32.M/S已把这些能力接入共同candidate基础，Q49负责让完整rank Tile clone只lower一次并在最终worker/effect/range上
...
- committed instruction artifact 不是脱离完整entry/all-rank domain的 `wafer.tile.region` body、task、tile 或 representative sample，
  而是每个 logical rank 一份覆盖完整 static traversal 的 structured instruction program；这些 rank
  programs 只能作为完整 variant set 原子提交。

`wafer.instr` 的作用是把每个完整rank entry的target-abstract outer tile-region epoch body变成可执行硬件动作或硬件通信调用，并让下游能从
...
- `wafer.instr.*` op 只读写 Wafer-tagged memref，不产生 buffer result，不携带 SPM offset或raw
  ...
  `DisjointComponents` sibling并写actual worker attrs；统一completion owner随后删除全部`wafer.instr.ncc_join`并fresh重建fixed-frontier
...
- tile-region lowering 已产出memref-backed `wafer.tile.region`；Q49终态要求current static rank entry恰好一个
  non-nested outer region，多个traversal/loop nest、不同tile shape和逐root lifetime都在其body内。candidate materialization 已为
...
- Upstream artifact / IR:
  whole-variant evaluation clone 中每个 static rank entry 的完整 structured traversal；其中恰好一个non-nested outer
  `wafer.tile.region`表示完整SPM ownership/device-execution epoch，并包含带 Wafer memory attr 的 memref values、
  ...
  structured control-flow和显式tile-level async event/dependency。source-observable ordering由structured control-flow、
  SSA event/token dependency和typed MemoryEffects表达；compiler-derived `wafer.instr.ncc_join`尚未物化。
- Current stage responsibility:
  ...
  graph。对 `scf.if` / `scf.for` 递归转换其 region body并保留control-flow结构；loop backedge、内部traversal/task
  boundary和candidate commit不是NCC completion boundary，本conversion不能仅因结构scope插入blocking join。outer
  tile-region/rank-entry exit是真实epoch exit，但本conversion尚无final worker/range事实；terminal completion由下游owner在
  final sibling上显式重建并证明。same-worker RAW/WAR/WAW先由issue order和显式buffer lifetime暴露给后续completion owner；
  source-observable ordering继续由control-flow、SSA event/token与typed effects保持；当前没有source-authored Instr fence，
  `wafer.instr.ncc_join`也不承载这类来源语义。若未来source fence无法由这些事实表达，必须先增加独立typed op/event及verifier，
  不能借用join或provenance。NCC到Kcore、Direct DTE、其它worker、host publication或terminal的participant join只在最终
  worker/slot/range sibling上fresh构造。对 accepted
  `wafer.tile.*` collective，生成 explicit
  `wafer.instr.dte_send` / `dte_recv` / `dte_wait` p2p schedule。instruction op 通过
  interface显式暴露current instruction family、memref read/write、descriptor attrs、issue effect和completion relation；
...
- Output artifact / IR:
  ...
  control flow + exactly one non-nested outer `wafer.tile.region` epoch + memref values with
...
- Downstream consumer:
  ...
  typed worker/fixed-slot siblings，删除全部compiler-derived`wafer.instr.ncc_join`并从current effects/ranges fresh重建latest-necessary
  completion。每个complete-entry Instr variant随后只运行一次覆盖完整outer region body的whole-entry SPM planning、
  whole-variant DDR planning、event/physical-transport/all-rank transport/
  target-entry verification 和
  closed-loop whole-variant candidate driver；这里的每个Instr variant都是完整rank entry；atomic commit 后才由
...
- User-level driver / named pipeline:
  ...
  stop-stage，也不能把
  `DirectFullShape`、单 task 或单 tile-region 结果独立lower后送入 committed target/package flow；用户级
...
- Completion gate:
  ...
  不能让 SPM memory planning 从 target-abstract op 猜 demand。每个issue必须由explicit async
  token/wait或由下游fresh typed worker ordered-pending/participant join合同闭合；generic async task identity、Direct DTE
  completion和NCC worker pending set分别验证，每条function exit path在typed terminal completion后均为空；variant-set gate
...
canonical/unplaced current Instr派生all-rank atomic typed worker sibling并在clone中重写actual worker attrs；fixed-slot再从保留
该assignment的siblings派生，统一completion owner随后删除全部`wafer.instr.ncc_join`并重建fixed-frontier latest-necessary joins。不能用metadata
...
| TDMA | `wafer.instr.gather_scatter` | `wafer.tile.materialize_layout`、tile movement ops | byte-counted SPM movement；contiguous copy 是 descriptor 特例，但complete-rank actual clone中只有无法安全coalesce storage的copy才保留 |
...
最终TargetCall sequence。Q46补齐existing encoding implementation和concrete verifier后，Cx/NCx CT
relation/select/logic/convert/bitpacked candidate必须从现有字段重建full-physical traversal，并完成
TargetCall/SystemC、tail/padding和canary纵向；不能因存在padding而固定回Tensor。
...
| TDMA `TsmDataMove::GatherScatter` | `wafer.instr.gather_scatter` | V0 production target op | 真实layout materialization、SPM copy和可静态证明的slice/transpose/broadcast movement展开为一条或多条gather/scatter；完整copy在actual clone由08用IndexRelation、physical map、effect/lifetime/completion重证，能安全coalesce时删除，不能证明或无法压成V0 descriptor时分别保留或结构化失败 |
...
| CT arithmetic / relation / logic / activation / selected transcendental | `wafer.instr.elementwise` + `#wafer.instr_elementwise_kind` | V0 production target op | 覆盖当前enum中的target kind；tile-level map必须先materialize为movement/同形状operand并strip，terminal op不携带`indexing_maps`。Q46补齐existing encoding implementation的Cx/NCx `i1` mapping后，可用既有full-traversal fields承载value-form relation/logic的bitpacked result；scalar immediate、VuV/VuVLoop和缺失rounding field的形态仍需显式target variant |
...
| CT reduce `sum/avg/max/min` | `wafer.instr.reduce` + `#wafer.instr_reduce_kind` + target `dim` code | target-native leaf；Q0.L source-reduce correctness baseline不直接生成 | terminal op不携带init operand/attr；Q46只有在完整domain/dimension/combiner/init映射和numeric admission闭合时才生成native candidate。floating leaf order可变化，integer保持exact/modular |
...
| concat / maskgather variants | 无单独 op | V0 composite；native `dims=HW`永久target-illegal | source-level任意轴concat在V0一律展开为typed `gather_scatter` movement；current production wrapper只证明last logical dim映射到native CT concat的C编码，W/H的bounded raw completion不构成production资格。header虽公开HW编码，但native Concat `dims=HW`是错误/非法指令组合：不得构造、序列化或提交板端packet，不保留board case，也没有复测或重新资格化入口。maskgather variants仍需bool/index operand policy；未定义前不能复用`tdma_data_move` |
| VuV / VuVLoop / scalar-immediate BOOL variants | 当前`elementwise`只覆盖value-form opcode family kind，不单独建模这些variant | future extension | Q46的bitpacked result只复用existing value-form instruction与encoding traversal；VuV/VuVLoop、scalar immediate和其它缺字段variant仍须独立typed target合同，不能靠`elementwise`名称吞掉 |
...
worker进入ordered-pending frontier，matching `wafer.instr.ncc_join`只完成其canonical participant集合。
...
drain。不携带resident data的pending NCC completion frontier可按精确event/control relation跨内部traversal、loop nest和
traversal separation；same-worker ordered reuse的SPM root可在同一outer region及已证明安全的loop backedge内延续。
`wafer.tile.region`表示SPM ownership/device-execution epoch，current static rank entry恰好一个non-nested outer region；
每条region/rank-entry exit path必须证明participant completion闭合，并证明没有未消费DTE token、generic async task或未完成recv。
SPM buffer/root/alias不得跨epoch boundary，region operand/result也不得携带SPM data。region body内的selective spill仍必须保留
完整DDR store/completion/load并只结束目标root。SPM planner对每个terminal rank-entry只执行一次whole-entry solve，把该
outer region内全部distinct roots纳入同一physical arena；current static entry拒绝nested或multiple sibling tile-region。
...
`wafer.tile.materialize_layout`/GatherScatter。Q46只联合选择这些已有actual-IR路径，不改变instruction合同。
...
target-native leaf；Q46 production source path只有在完整logical reduction domain、dimension、combiner和init合同没有丢失，
...
#### 7.5.1 Relation-Guided Cx/NCx Physical-Version Absorption
...
Q46若在actual clone中删除前置Tensor↔Cx/NCx `materialize_layout`/GS，GEMM、native reduce及physical-traversal-compatible
...
```text
Pipeline position:
- Upstream artifact / IR: future target ABI revision下的verified instruction module中显式存在的
...
- legal：`memref.alloc`、standard memref view ops、`wafer.instr.*`、
  typed `wafer.instr.ncc_join`、SPM-ownership/device-execution epoch `wafer.tile.region`、
  `scf.if` / `scf.for`
...
| `wafer.tile.reduce` | typed baseline验证init后显式构造fill、slice、map-free elementwise ping-pong和final movement。Q46另生成native candidate，但必须证明完整logical reduction domain/dimension/combiner/init映射并通过numeric admission；floating leaf order可变化，integer保持exact/modular。dynamic-init/combiner/budget或native proof失败只拒绝对应candidate |
...
| `wafer.tile.all_gather` | requires matching `tensor/ntensor` SPM layouts and infers the unique gather axis from compact local/gather buffer shapes. `ring` copies the local chunk into the local gather slot and leaves typed NCC producer effects plus DTE token/exact wait before first DTE observation, then forwards one slot per round around the topology-derived rank order; conversion itself inserts no participant join or round/backedge/final structural join. `direct` uses the deterministic cyclic order of semantic group indices to send the local slot to every other logical rank while receiving each peer chunk into its result slot；it does not invoke topology Ring search. |
| `wafer.tile.all_reduce` | pre-Q46 implementation要求matching compact `tensor` SPM buffer。Q46已允许全rank相同Cx/NCx encoding的Tree使用完整physical footprint，也允许每个chunk具有exact contiguous physical-interval proof的Ring；其它blocked case保留compact或显式materialization。`ring`是chunked reduce-scatter+all-gather，`tree`使用topology-derived ordered binary tree；两者显式物化local reduction、typed NCC effects和DTE token/exact wait，participant join只由post-worker统一completion owner fresh构造，并接受integer及支持的floating type。 |
| `wafer.tile.reduce_scatter` | compact full `tensor` input保留为baseline；Q46允许full Cx/NCx input在每个owned chunk都有exact contiguous physical-interval proof时生成Ring。`direct`按owner累计，`ring`每轮只传输/归约一个typed chunk；无法证明连续、互斥和full cover时保留显式pack/unpack或拒绝该extra candidate。 |
...
- pre-Q46 current `wafer.tile.all_reduce`已能在`rank_group`、group-local `local_rank`、`group_size`、`bytes`、
  compact `tensor` SPM buffer type和sum/max/min reduce kind均可验证时materialize fixed-size unicast schedule；Q46的
  Cx/NCx full-footprint/exact-chunk扩展状态只看`tasks/progress.md`和独立gate。
...
`tile.reduce_scatter lowering requires compact buffers or exact physical-chunk traversal proof` or
...
- NE GEMM and CT reduce require supported aligned layout marker, dtype and rank. current plain GEMM按implicit
  ...
  reduce只有compiler-owned full-domain mapping与numeric-admission proof后才可进入production。
- relation-guided physical-version absorption不增加Instr字段；verifier只从current Instr/operands的memref type与
  existing encoding证明shape/tail对应的packing，并核对geometry、valid/padding lane、range、alias、effect和completion。
  current target helpers只回答instruction/format capability，不参与physical encoding查询。Q32既有GEMM absorption证据保持不变；
  Q46广义movement消失及删除前后等价性只由其独立集成gate证明，不由Instr verifier反推。
...
- 每个issue都有可验证completion relation；内部traversal/loop/separation不触发completion，pending NCC frontier可沿
  显式SSA/control flow传播。`wafer.tile.region` exit是真实SPM ownership/device-execution epoch exit；每条rank function/epoch
  exit在显式terminal participant join/exact wait后pending-event set为空，variant gate覆盖每个rank唯一outer region的完整body。
...
placement，也不绑定runtime symbol、packet bit或worker window。Q49 SPM allocator对每个terminal rank-entry Instr variant只执行一次
whole-entry fixed-capacity MiniMalloc query，覆盖其唯一outer region的完整body；actual high-water只作headroom。
accepted-offset-derived bank phase若参与，只能打破solver
自然遇到的execution-equivalent hard-valid placement，不新增query/search/relocation，也不得改变spill/resident、epoch boundary、
DDR movement、order或join。
...
本例中的outer `tile.region`是完整rank-entry device-execution epoch；WDMA在epoch exit仍属于pending observable effect，
因此post-worker completion owner依据final worker/effect/range在exit前fresh物化participant join。join来自真实
...
completion proof。完整traversal、once-per-terminal-rank-entry whole-entry SPM、whole-variant DDR、terminal event closure、transport/target binding、
...
R3.2d.2 当前 mechanics 与 Q49 gap：
...
9. conversion不在WDMA后、`scf.for` backedge、内部traversal/task boundary或仅因看到`wafer.tile.region`/
   ...
   普通NCC op进入worker-aware ordered-pending frontier；统一completion placement只在value effect/root alias证明的
   NCC→Direct DTE/Kcore/call/return cut插入覆盖实际pending worker的latest typed join；其中current rank唯一outer
   region/rank-entry exit是必须闭合的真实epoch exit，空join和相邻冗余
   join不进入accepted IR。same-worker RAW/WAR/WAW与loop-carried slot reuse由issue order和lifetime证明，
   不逐edge drain；SPM仍独立按exact DTE token/wait跟踪communication lifetime。当前production converter仍有
   standalone task `func.return` terminal join遗留，随后被task commit复制。Q49 C1新production path不得调用该builder；
   C3在完整rank final worker/effect/range上先erase-all再fresh重建；C6只删除已经零consumer的旧代码。完成前本条终态
   合同尚未闭合。
...
11. Q49 pipeline gate必须把legacy多region正例迁为“每个static rank entry恰好一个non-nested outer region”，并覆盖
    同一body内多个traversal/loop nest、不同tile shape、逐root lifetime与internal DDR spill/reload；multiple/nested region及
    boundary携带SPM data为稳定负例。structured `scf.if` / `scf.for`、tile communication structured failure，以及转换后
...
本边界消费final verified typed Instr，并产出只可lower到current TargetCall的完整rank instruction program；TargetCall/CRT与module publication
...
本计划不改变entry ABI、DTE status schema或其它独立版本化Instr字段，也不引入superoptimizer、
solver或新的instruction semantics interface。

## 14. 规划中的 Typed Instruction Synthesis 与 Numeric 收口

`semantic-superoptimization`只覆盖current target-admitted typed Instr surface，不把
