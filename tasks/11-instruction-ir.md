...
floating reassociation/tree也不通过Instr attr恢复：Q32.N先要求production source无损携带standard permission，再把选择物化为
显式SSA DAG/SCF；instruction lowering只消费该actual DAG，不读取隐藏order或“已重结合”标志。
