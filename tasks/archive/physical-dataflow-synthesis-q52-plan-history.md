...
> 归档说明：本文保留Q52第1--11项施工、旧路径删除账本和早期第12--20项展开。Current实施顺序与
> 覆盖矩阵只读`tasks/plans/physical-dataflow-synthesis.md`。
...
| TensorProgram→TileModule/TileRegion construction | 旧整体assembly不保留；只把不依赖group/delivery recipe、直接调用pinned `TilingInterface`并创建actual IR的operand tiling与partial-reduction tiling放入`WaferLinalgTransforms`；`createStandaloneTileModules`继续只消费actual TileModule IR；第12项重新建立唯一structural construction | 整个旧TensorProgram→TileModule set和TensorProgram→TileRegion library、test-only projected-permutation helper、`StructuredNodeShardGroup` complete recipe、`SelectedRegionMaterialization` replay、`rewireDirectSSA`、手写三段wave递归和通用future fusion worklist/cache |
| Representation与PBQP | exact R0/R1/R2/residual solver core改为layout-owner内部的中性query-local PBQP；flat oracle保留；`LayoutOptimization`只保留已验证的current cleanup mechanics，不代签第15项完整layout assignment | `RepresentationPlan/Domain/State`、`PhysicalVersion*`、canonical representation replay及所有future value binding；迁移后API和状态名不再使用`RepresentationPBQP*` |
...
- 第11项当时的构建/测试数字只属于历史施工证据，不继续写作current gate。canonical build、全部repository-managed本地依赖和
  Tools lit现在由16号`canonical-build-gate`统一签发。8个产品no-card与24个calibration no-card registration都必经旧`none`路径；旧
  target-model source纵向已从current suite删除；case、CPU oracle和runner资产保留，第17项必须从current pipeline重新构造并注册
  新纵向，不能恢复旧协议或用未执行/feature-off测试代签。
- source/IR organization、public-link smoke和current retained-asset检查当时通过；active source/header/CMake/test及
