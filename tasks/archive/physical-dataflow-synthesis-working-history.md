# Physical Dataflow Planning 与 Selected Execution 历史施工记录

> 本文件保存截至2026-08-25的Q49–Q51施工、Q52重基线审计和原Q53板端设想。它不是当前计划或设计
> 事实源；当前状态读取`tasks/progress.md`，稳定设计读取`tasks/06-physical-dataflow-synthesis.md`，当前Q52/Q53
> 实施步骤读取`tasks/plans/physical-dataflow-synthesis.md`。
...
4. **Policy隔离**：deterministic-baseline-closure与search work items不互相消费；二者拥有不同complete plan、materializer和verifier，
   只共享policy-free基础query与无policy窄lowering leaf。
...
| 19 | `search-scalability` | `doing` | Q52 | 合并baseline失效与baseline/search各自的IR expansion closure；baseline和search不是一个complete-candidate实现，各自只保留一套materializer；只在operation-scoped lowering、TileRegion→Instr、MiniMalloc/DDR/target等无policy leaf复用实现。接通movement/layout优化并删除旧shared/edge-driven路径，最后完成search scalability及同源LLaMA `none/search` package/no-card硬验收 | 读AGENTS/progress→读编号设计与本项覆盖矩阵→读相关硬件/ABI事实并把未证同步语义保留为unknown→调研论文/成熟编译器中的相关算法与实现并比较取舍→查官方文档及pinned LLVM/MLIR确认API→实现代码/测试→fresh验证→按设计与LLVM/MLIR规范复审实现、diff和下游witness→更新状态并提交 | production-host-readiness |
...
controller交给baseline自有materializer的是窄immutable resolved baseline assignment。它是上述feasibility resolution的输出而非入口
...
`none`与`search`可以共享immutable structured/relation/target facts和单operation lowering、TileRegion→Instr、SPM/DDR、target/package等
无policy leaf；不能共享complete plan、Card/Tile materializer、completion owner或materialization callback，也不共享search state/candidate/evaluator、group
...
3. baseline-owned CardModule/TileRegion materializer直接构造final single-root region；search materializer是另一套实现。两者只调用
   相同的窄operation lowering leaf，不能通过shared facade、policy mode或callback切换complete construction。
...
| policy isolation | 同一source各开独立transaction运行none/search，输入顺序扰动 | 共同normalization和无policy lowering/SPM leaf；baseline/search materializer及verifier不同 | search失败不返回none package | diagnostics/call tree无互调、无shared complete facade；各自commit一次；search coverage typed | matched tests只在外层比较package facts |
...
baseline和search分别完成policy-specific Instr construction后调用同一个actual `PlanSPMMemory`/MiniMalloc及DDR/target leaf；它们不调用同一个
candidate materializer或preparation。任何estimate API、plan-side packing problem、fast allocator、fallback allocator或parity proof都不是
...
| Q50.0 leaf parity | baseline与search各自完成policy-specific construction后的Instr IR，named leaf pipeline与正式driver | 两policy分别调用相同TileRegion→Instr kernel及SPM/DDR/transport/target leaf；不共享complete preparation/callback；每candidate各leaf一次 |

完成证明必须包含真实规模aligned/ragged full pipeline和actual allocator结果；局部packing unit、shape dump或plan object不能代签。

2026-08-25 Q52审计推翻了此前“Card construction移到一个policy-free complete facade即可同时服务baseline/search”的结论。终态由
baseline owner和search owner分别完成Card/Tile及policy-specific Instr construction；`CompleteCandidatePlan`与
`CompleteCandidatePreparation`只属于search，不得成为baseline输入。两条实现分别调用窄TileRegion→Instr kernel，随后在各自completion/
storage/schedule已经落入verifier-legal Instr IR后，才调用共同SPM/DDR/transport/target leaf。current shared facade和callback结构按Q52删除清单
退役。
...
- User-level driver / named pipeline:
  `wafer-compile`的public `search` policy；`none`进入独立baseline controller。两者使用不同actual candidate materializer，只复用named
  无policy lowering leaf；policy controller绝不互调。
- Explicit non-goals:
...
两个controller只共享明确列出的immutable基础analyses与无policy lowering/SPM/DDR/target leaf；不共享complete assignment schema、
candidate materializer、preparation或publication owner。各自形成policy-complete Instr后调用相同actual memory/target leaf。
...
- call-tree/source gate证明partial-state expansion不含materialize/Q50.0/clone，search complete-candidate path只调用search-owned materializer，
  在policy-complete Instr后调用共同memory/target leaf，且不含baseline、repair或同candidate重建；旧post-hoc apply和第二search candidate
  materializer零残留；
...
`search`先跑baseline拿一个executable。两者共享的是基础事实与无policy lowering leaf，不是final plan schema、complete materializer、
controller状态或结果。
...
### search-scalability施工前调研与覆盖矩阵

| Primary source / implementation | 实际方法 | 本项采用 | 明确不采用 |
| --- | --- | --- | --- |
| [Halide learned autoscheduler](https://halide-lang.org/papers/autoscheduler2019.html)及[production implementation](https://github.com/halide/Halide/tree/main/src/autoschedulers/adams2019) | coarse-to-fine backtracking、beam及对partial schedule的cost-model priority | 分层profile、constructive proposal和priority与合法域分离 | beam/learned score不签发exact coverage、bound或legality |
| [Ansor](https://www.usenix.org/conference/osdi20/presentation/zheng) | hierarchical sketch sampling、evolutionary search及多subgraph task scheduling | 分离global task/component工作与local proposal，profile time-to-first及quality曲线 | sketch子集、固定population和测量cost不成为Wafer默认domain |
| [Dechter context-minimal AND/OR caching](https://ics.uci.edu/~dechter/publications/r218a.pdf) | 以pseudo-tree context/separator合并conditioned suffix，复杂度随induced width为`O(N*K^(w+1))` | `FutureBoundaryKey`必须覆盖全部cross-boundary constraints；先完整key再由perturbation证明投影 | prefix hash、shape/bytes/Tile count相同不能合并future-distinct state |
| [MLIR Pass/Analysis infrastructure](https://mlir.llvm.org/docs/PassManagement/#analysis-management) | analysis按operation惰性缓存；pass后默认失效，只有显式preserved才继续使用 | IR-derived只读fact留在对应operation/analysis边界；planning session memo只持pure typed value | 不把`Analysis *`、operation pointer、manual epoch或IR clone放进跨candidate memo |
| [OR-Tools CP-SAT LNS implementation](https://github.com/google/or-tools/blob/stable/ortools/sat/cp_model_lns.cc) | 从active variables、constraints、intervals、precedence和connected components构造多类neighborhood | destroy closure从Wafer typed dependency/resource graph产生，repair复用Q51 transitions | random/adaptive completion order、solver-local feasible/optimal不能改变coverage或绕过actual admission |
| [Pisinger--Røpke LNS](https://doi.org/10.1007/978-3-319-91086-4_4) | destroy/repair大邻域与多operator组合 | 只有已有actual incumbent后启动改进lane；released binding带完整downstream closure | 不做single-axis随机扰动、LNS-only fallback或隐式丢state |

本项写代码前冻结下列覆盖矩阵。所有production-style positive使用rank至少3、主要维度1024/1025/1031；tiny domain只用于可穷举
oracle，并与真实规模case成对存在。profile数字只解释是否实施优化，不进入IR legality或candidate admission。

| 输入等价类 | 代表输入/结构 | on/off或对照路径 | typed failure | 精确断言 | 直接下游witness |
| --- | --- | --- | --- | --- | --- |
| profile隔离 | generic mixed DAG，16 Tile，1024/1025；显式profile与普通search | 同一source/policy，只有profile sink不同 | sink/report I/O失败不改变search result | 普通compile零profile对象/字符串；profile逐axis/query/actual gate计数，`complete==CardModule==Q50.0`，publication=1 | Q53原样消费无instrumentation production policy |
| assignment memo | tiny全轴exact oracle + rank-3 1024/1025 chain/branch | cache off/on、容量eviction、不同hash seed、重复resume | key依赖缺失为contract failure；allocation/resource exhaustion不伪装cache miss | complete keys、actual result、objective、winner、coverage逐项相等；value无IR/offset/solver/diagnostic | Q51 controller只看到相同typed successor/result |
| dependency perturbation | placement、fanout、relay、layout、slot、recurrence、worker、output/effect逐字段扰动 | 完整prefix key对比经证明的projected key | result变化但key不变必须失败 | 每个实际读取字段至少一个改变result/key的witness；pointer/hash digest不进key | memo eviction只增加work，不改变下游actual集合 |
| component/separator DP | chain、branch、diamond、independent components；1024/1025 | flat Q51 reference vs conditioned component DP | opaque fabric/shared DDR/DTE/FSM/alias/join未进separator时拒绝拆分 | 每separator assignment的local/global set、objective、Unknown/incomparable项一致；无Top-k | 组合complete plan仍走normal J/F actual admission |
| 16-Tile query sharing | all-16 symmetric workload与hole/asymmetric/fanout变体 | sharing off/on | equivalence proof缺失则回退独立query | symmetric显著减少pure query count；hole/asym自动拆类；每Tile assignment、IR、SPM offsets仍独立 | 每complete candidate仍16次Tile materialization、一次whole-card gate |
| safe bound/priority | Known/Unknown cost、local component optimum、opaque route | Q51 bound vs stronger admissible bound；priority替换 | timeout/FEASIBLE/unknown只作Indeterminate或proposal | exact mode visited/pruned proof与winner一致；相等bound不误剪semantic tie | retained actual incumbent及coverage不变 |
| constructive + LNS | generic联合陷阱、fanin/fanout、pipeline/resource-window | constructive-only、PreserveAll+LNS、显式budgeted retention | 无incumbent不启动LNS；repair work耗尽保持typed incomplete | dependency closure与flat repair set一致；每complete assignment actualize一次；PreserveAll不降coverage，budgeted最多`BudgetedFeasible` | 唯一incumbent进入同一Q51 comparison/publication owner |
| heavy campaign | HF prefill、functional decode、代表LLaMA block；FP16/BF16及1024/1025/1031 | fresh none一次；profile search checkpoints；最终production policy | deadline无plan=`WorkExhaustedWithoutPlan`，不得fallback baseline | finite policy内search自己的Accepted winner、一次publication、package strict readback/no-card；记录wall/RSS/work/quality曲线 | Q53直接消费current production policy与fresh package case |

### Q52-0：IR expansion closure

2026-08-25 fresh Llama block profile证明scalability问题首先发生在actual candidate construction，而不是SPM estimate、frontier state数或
source模型规模。该case的source StableHLO只有142个operation，post-SPMD为144，Tensor/Linalg为434；首个constructive candidate在
selected Tile dataflow边界已经有554,688个operation。16个Tile各有34,668个operation且结构相同。Tensor层106个structured compute经
16-way node placement应形成1,696个node/Tile execution，但actual relation记录40,960个compute emission，Tile IR另含92,976个
`wafer.tile.extract_slice`、57,600个`memref.global`、44,384个`memref.alloc`、27,504个load、19,520个layout materialization、
16,656个insert slice及13,120个transpose。15分钟Release lowering只处理约13%的extract slice，尚未进入actual SPM planner；该timeout
不能解释为SPM rejection。

本轮fresh inventory冻结为修复前基线，后续不得用不同source、不同candidate或partial lowering结果替换：

| IR边界或关系 | 数量 | 相对倍率 | 结论 |
| --- | ---: | ---: | --- |
| source StableHLO | 142 | 1.000x | 单个Llama decoder block，不是完整32层模型 |
| post-SPMD StableHLO | 144 | 1.014x / source | SPMD不是异常膨胀点 |
| Tensor/Linalg | 434 | 3.014x / post-SPMD | 含106个structured compute；该层仍是单份logical program |
| selected Tile dataflow | 554,688 | 1,278.083x / Tensor，3,906.254x / source | 异常首次完整出现在actual candidate construction |
| selected node/Tile executions | 1,696 | `106 * 16` | 当前plan要求的execution inventory |
| actual compute emissions | 40,960 | 24.151x / selected execution | 同一selected execution被edge demand重复物化 |

Tile dataflow中的主要分类也作为同一基线保存：`wafer.tile.extract_slice=92,976`、`bufferization.to_memref=57,856`、
`memref.global=57,600`、`memref.alloc=44,384`、`memref.subview=42,464`、`wafer.tile.load=27,504`、
`wafer.tile.materialize_layout=19,520`、`wafer.tile.insert_slice=16,656`、`wafer.tile.transpose=13,120`。这些对象不是
“假设”或调试标记；进入后续lowering后会形成真实allocation、movement descriptor或target instruction，因此必须按owner和下游语义逐类解释。

根因链必须作为一个pipeline问题一起闭合：

1. current constructive spatial proposal对每个node独立最大化parallel participant，并用iterator factor字典序处理tie；它不传播相邻
   producer/result到consumer/operand的exact `IndexRelation`。实测全部106个node各自使用16 Tile，但weight/activation/matmul/head等沿
   不同语义轴分片；
2. exact demand因此产生672个peer-fragment和1,264个RegionCut per-destination strategy，production没有生成
   `LocalShardResidency`的路径；
3. canonical plan绕过`RegionPlan -> StructuredNodeShardGroup` execution materializer，进入edge-driven legacy actual path；
   `getOrMaterializeSource`按producer/result/exact offsets/sizes缓存，不同destination fragment分别重建producer traversal。一个selected
   node虽只属于一个TileRegion，却实际发射16/31/46/61/91/122次compute；
4. 每份重复producer traversal继续生成consumer-local slice、layout conversion、allocation和global；internal static partial window随后被
   无条件变成explicit `MoveExtractSliceOp`；
5. `elideRedundantFullBufferTransfers`只有实现和unit test，production无调用点，而且它只证明full-buffer transfer，不能补救partial
   fragment或上游重复compute。

第3项是主根因而不是普通cache性能问题。旧canonical materializer先存在；selected `RegionPlan`施工时只为非canonical coordinate新增
execution-driven materializer，并按`plan.regions == canonical`保留旧路径。该实现后来迁入policy-free complete-candidate入口时没有
退役分支；本次search的canonical constructive candidate及attention均进入旧路径。canonical分支在CardModule构造完成后才补
`selectedRegionExecutions`，所以execution relation只是post-hoc记录，不是construction owner。selected-region verifier又把actual emission按
TileRegion收集成node ID集合；重复1次和122次得到同一集合，而且canonical分支没有`selectedRegions`，整段检查不执行。

归因边界如下，避免修复时再次把结果当原因：

- **主根因**：execution identity没有拥有compute construction，edge carrier反向按fragment创建producer traversal；
- **结构性放大器**：逐node独立spatial proposal及缺失local carrier制造大量不同window；
- **次生放大器**：重复traversal继续生成slice/layout/buffer，internal static partial slice再lower为真实gather-scatter；
- **漏检原因**：canonical/noncanonical双实现、post-hoc relation及set-based verifier共同掩盖multiplicity；
- **已排除**：尚未运行的SPM planner、MiniMalloc、frontier state数、source模型规模和temporal tile过细都不是本次膨胀起点；
  production未接入的full-buffer transfer elimination也不可能修复partial fragment与重复compute。

#### Baseline独立IR expansion审计

Baseline与search保持独立controller、plan、materialization contract和result owner。本审计只复用同一只读source artifact和逐stage inventory
方法，不用search plan或期望评价baseline。2026-08-25对同一Llama block运行fresh Release `none`：source导入与Tensor/Linalg输入相同；
baseline在自己的CardModule materialization postcondition处失败，未进入TileRegion→Instr、SPM、DDR或target。Tile 0失败前已经记录2,538个
structured compute emission，相对106个source structured node为23.943x；一个actual TileRegion同时包含15个不同semantic root，而baseline合同
要求每个compute region恰一个root。16个Tile均执行旧edge-driven emitter，`proveStaticReshapeMetadataView`共调用15,504次；实际
`tile_memory_planning_invocations=0`、`tile_to_instr_lowerings=0`和`spm_planning_invocations=0`。

这组证据只签发两个baseline结论：

1. baseline自身没有在canonical/noncanonical plan之间切换，它只有canonical active path；但该path复用了同一个旧edge-driven materializer，
   因而同样发生per-fragment producer rematerialization和compute膨胀；
2. baseline另有独立region-boundary错误：`IndependentDDRStages`要求的root隔离没有在实际TileRegion construction中成立，postcondition正确拒绝了
   15-root region。该错误不能用search的region grouping语义解释或放宽。

失败发生在aggregate CardModule返回前，当前还没有baseline失败前的完整Tile op分类。需要在baseline-owned postcondition之前增加仅由
`--compile-timing`启用的inventory，冻结每Tile总op、compute、slice、layout、allocation、global和movement数量；该instrumentation不得改变
baseline construction或failure。修复后再独立取得bufferized/Instr/target inventory。

| Baseline覆盖类 | 输入/对照 | typed failure | 精确断言 | 直接下游witness |
| --- | --- | --- | --- | --- |
| failure-before-Card inventory | 同一Llama block，`none`，16 Tile | one-root postcondition保持原typed compiler failure | 失败前每Tile op分类、root group、emission multiplicity完整；instrumentation on/off failure一致 | 不进入Instr/SPM，计数明确为零 |
| aligned/ragged root isolation | rank>=3，1024与1025/1031，chain/fanout/matmul/attention | extra/missing/multi-root region拒绝 | baseline自己的每个execution/root all-and-only，edge数量不增加compute owner | baseline CardModule verifier后进入共同无策略Q50.0 leaf |
| baseline downstream expansion | 上游root isolation修复后的同一case | Instr/SPM/target各自保留typed failure | source/Tensor/Tile/bufferized/Instr/target-call逐类计数，metadata view零target call | actual MiniMalloc及package/no-card由baseline自己的accepted owner产生 |

#### 同类actualization路径审计

本表只审计public `search` constructive candidate的TensorProgram→CardModule→TileRegion→Instr→SPM链。Baseline保持独立controller和
materialization contract，不使用search plan、state、preparation或result；本表不评价baseline的IR选择，也不要求两条policy生成相同plan或IR。
下列search链事实不因最初的compute重复修复而自动消失：

| 审计点 | Current证据 | 结论 | 闭合要求 |
| --- | --- | --- | --- |
| canonical/noncanonical Card construction | `plan.regions == canonical`或representation equality直接选择旧`TileMaterializationSession`与新`StructuredNodeShardGroup`两套builder | **已确认活跃错误**；这是本次compute膨胀的主路径分裂 | 删除coordinate equality分流；generic、attention及全部region/representation point进入同一execution-owned builder |
| selected movement donor surgery | selected peer graph先在已生成Tile IR中寻找direct send/receive或external load donor，成功后再创建selected graph并erase旧pair/load/await | **已确认迁移残留**；selected movement不是直接construction，且要求另一种movement IR先存在 | movement builder直接消费execution result和`MovementPlan`创建all-and-only action；不存在donor发现、替换或事后erase |
| RegionPlan与MovementPlan组合 | `lowerStructuredNodeGroupsToCardModule`不接收movement；后续selected peer materializer却要求已有donor | **已确认cross-product缺口**；noncanonical region与peer/relay/fanout没有共同construction owner | 一个prepared candidate同时交付region execution、physical version和movement action；组合positive必须实际进入Instr/SPM |
| execution relation与schedule binding | `StructuredOperationEmissionRelation`只有node ID；Card/region verifier把emission收成set；Instr binding又把同Tile同node的全部NCC issue绑定给一个execution event | **已确认漏检链**；duplicate compute会被当成一个event的多operation实现 | relation携完整execution identity/closure member；verifier比较multiplicity；J只能绑定该execution明确报告的actual operations |
| local carrier production reachability | `LocalShardResidency`只有lowering和unit fixture，production `CardEdgeCarriers`没有创建点；同Tile单piece默认成为RegionCut | **已确认机制未接线**；不是合法的“保守选择” | exact-compatible local edge由current relation/effect/lifetime证明后生成typed local binding；不能靠默认RegionCut |
| dormant edge actions | `RecursiveProducerTiling`、`SpillReload`、`Recompute`同样没有production constructor，只有enum/lowering分支；candidate verifier还明确拒绝recursive action | **已确认历史surface残留**，尚未证明影响本次candidate | 对照current MovementPlan逐项决定接线或删除；没有producer的分支和只覆盖它的fixture不能冒充production capability |
| full-buffer transfer elimination | 实现和大量unit test存在，但production caller为零 | **已确认orphan mechanism**；只覆盖full contiguous transfer，非本次根因 | 在owner明确且不掩盖上游重复后决定接入或删除；partial/view问题另行证明 |
| TileRegion→Instr与SPM planner | named/direct TileRegion conversion共用同一kernel；production `planTileMemory`只调用唯一`PlanSPMMemory`/MiniMalloc，未发现estimate/fallback allocator | **本轮已排除同类双实现** | 保持单路径；修复后用actual candidate重新验证，不把本次静态检查当运行证据 |
| Instr/target expansion | 当前554,688-op Tile IR在15分钟内只lower约13%的slice，未形成完整Instr/target inventory | **尚未判定**；不能据此声明下游无独立膨胀 | 上游唯一construction闭合后重新取得完整bufferized/Instr/target-call计数，再分别归因和设gate |

这些问题属于同一类search actualization未完成迁移：search plan已经描述了selected coordinate，但actual IR仍由canonical donor、post-hoc relation或旧IR
形状驱动。修复不得保留“先生成旧IR、再验证/替换成selected IR”的过渡终态。

Pipeline position:
- Upstream IR / input:
  一个closed `SpatialPlan/RegionPlan/TemporalPlan/RepresentationPlan/MovementPlan`及其exact demand；Tensor/Linalg source仍是单份current IR。
- Current stage responsibility:
  先形成graph-coherent constructive coordinate，再由search materializer直接按selected Region/Temporal/Representation/Movement plan构造IR；
  compute创建helper只对search materializer可见，edge carrier只接收已构造result的SSA value/exact view，不得看到producer op、tiling callback
  或compute builder。bufferization前明确view、shared materialization和copy。
- Output IR / files:
  accepted Tile dataflow中每个operation都能映回唯一source node/execution或typed movement；profile额外输出stage/op inventory，普通编译
  不输出inventory。Instr仍由同一Q50.0路径产生并进入actual SPM planning。
- Downstream consumer:
  `ExecutionStructurePlan`、schedule materialization、TileRegion-to-Instr和唯一`PlanSPMMemory`路径。
- User-level driver / named pipeline:
  public `search`的constructive lane；不增加axis、candidate或debug CLI。
- Explicit non-goals:
  不用shape阈值、estimated footprint或instruction count决定legality；不以单Tile fallback、后置DCE或descriptor cache掩盖上游重复；不改变
  arithmetic/dtype语义。
- Completion criteria:
  同一candidate逐stageinventory可重现；peer fanout共享producer result；exact-compatible local edge不生成RegionCut；metadata view零target
  instruction。search的canonical、noncanonical、attention和generic输入调用同一search-owned materializer，不按plan是否等于canonical/default
  选择另一套transformation。静态call graph证明carrier不能调用compute materializer；定向fanout/layout tests及LLaMA inventory证明重复膨胀消失。
  不新增execution-count verifier、expected inventory或assume。aligned/ragged、fanout/fanin、reshape/transpose及attention均通过actual Instr/SPM下游。

本项覆盖矩阵在修改proposal或materializer前冻结如下：

| 输入等价类 | 代表结构 | 对照 | typed failure | 精确断言 | 直接下游witness |
| --- | --- | --- | --- | --- | --- |
| graph-coherent partition | rank-3/4/5、1024/1025/1031 chain、matmul weight/activation、reshape/head、transpose | independent maximum-axis proposal vs exact-relation propagation | relation不支持传播时保留domain sibling，不伪造同分片 | 每个传播factor满足producer/result与consumer/operand relation；exact demand redistribution严格下降或不变；proposal仍为raw domain member | movement proposal读取同一demand proof |
| construction ownership | 16 Tile、single/main-tail/nested/replica/coupled/pipelined、fanout/fanin | carrier API无producer/tiler vs旧edge-driven call graph | compute helper从carrier可达视为source-organization failure | 固定producer plan时fanout 1/2/15的producer compute IR不变；Temporal/K/J现有verifier继续覆盖各自结构 | actual Tile→Instr/SPM可消费且无新增全局verifier |
| peer fanout sharing | one producer local shard供1/2/15 remote destination及local consumer | fragment-specific recompute vs one execution result+views | fragment union不属于selected result shard则typed failure | send数量保持all-and-only；producer compute数与destination数无关；不同fragment只增加view/send，不增加compute | DTE send/recv/wait与payload coverage不变 |
| local carrier | same-Tile equal/partial exact window、layout-compatible/incompatible、mutation/lifetime交叉 | RegionCut vs local stored/direct binding | alias/mutation或physical relation不能证明时保留explicit movement | compatible edge零DDR cut；shared materialization一次；incompatible edge有唯一copy owner | BufferPlan、effect、completion及SPM lifetime可解释 |
| search single materializer audit | canonical/noncanonical RegionPlan与representation、generic/attention、direct/relay/fanout | 对每类search complete plan追踪唯一production caller和construction owner | 任一search coordinate只能由另一套apply或donor IR实现时按migration contract failure停止 | search内部不存在canonical/default equality驱动的materializer分流、post-hoc execution relation或donor replacement；不读取baseline state/result | search policy-complete Instr verifier后进入共同memory/target leaf |
| IR expansion | Llama block及generic mixed DAG | 修复前fresh inventory vs修复后 | inventory缺stage或owner映射视为profile contract failure | source/Tensor/Tile/Instr/target-call逐类计数；compute倍率由selected executions解释；metadata op不产生target call；无92,976级slice爆炸 | actual TileRegion-to-Instr和SPM planner在finite campaign内执行 |
| regression legality | tiny exhaustive plan domain + 1024/1025 actual chain | proposal/materializer off/on | unsupported/indeterminate/capacity状态不改变类别 | exact raw plan集合不变；修复只影响proposal顺序和同一plan的唯一construction；actual offsets仍只来自MiniMalloc | Q51 oracle、Q50.0 typed result与package/no-card |

### Q52合并施工计划：baseline/search双实现恢复、各自单路径、layout与scalability

本计划是Q52当前唯一执行顺序。Baseline与search保持两套独立production实现：不共享plan、domain、state、candidate、execution schema、
preparation、materialization callback、schedule、ProgramData、package、actual result或cache。`compileCardBaseline`只调用baseline materializer；
`runUnifiedSearch/evaluateCompleteCandidate`只调用search materializer。两条路径只能复用不解释policy语义的窄leaf，例如单operation tiling/
layout emission、TileRegion→Instr conversion kernel、MiniMalloc、DDR和target lowering；不得存在一个根据caller、canonical equality或feature
presence切换行为的shared complete-candidate facade。

Search终态保留`RegionPlan/StructuredNodeShardGroup`所代表的execution-owned方向并补齐generic、attention、movement和physical version。
Baseline终态单独实现其deterministic root、independent-DDR和actual-feedback合同，不转换成search RegionPlan/preparation，也不调用search
materializer。当前`materializeCardCandidate`把两者塞入同一facade，是本次必须退役的边界；旧
`TileMapping -> TileMaterializationSession -> edge-driven producer tiling`不得通过加cache修补。迁移期允许在未提交worktree准备新实现，但切换、
shared facade删除和donor删除属于同一个atomic work item，任何失败不fallback旧实现或另一policy。

#### Baseline与search路径差异合同

| 边界 | Baseline / `none` | Search / `search` |
| --- | --- | --- |
| public owner | `compileCardBaseline`；不创建search session | `runUnifiedSearch`与`PhysicalDataflowPlanningSession`；不调用baseline controller |
| planning input | normal TensorProgram及baseline自己的immutable facts | normal TensorProgram及search自己的typed B--K/I/J domains/continuations |
| candidate policy | 始终只有一个live deterministic candidate；只有actual SPM rejection可以驱动baseline自己的deterministic temporal successor | 可有多个typed partial/complete candidates；每个complete key actualize一次，由actual result controller比较并发布一个winner |
| region语义 | 每个compute region恰一个semantic root；同Tile多root是多个顺序region；跨root shaped dependency由baseline显式independent DDR carrier隔离 | `RegionPlan`可以选择single/multi-root、stored/direct、nested、replica或coupled group；actual region必须逐group等于selected plan |
| spatial/temporal | baseline自己的deterministic assignment与actual-feedback temporal refinement；不枚举search proposal/domain | search的完整Spatial/Temporal domain、proposal和continuation；proposal只改顺序，不改合法集合 |
| representation/layout | baseline deterministic canonical layout合同及其独立actual verifier；不消费search `RepresentationPlan` | search selected `RepresentationPlan`、`PhysicalVersionId`和use binding；derived/shared/alias由search materializer直接实现 |
| movement | baseline deterministic local/DDR/必要peer carrier；不枚举relay/fanout alternative | search selected `MovementPlan`，可表达local/direct/relay/fanout/gather；carrier只消费selected execution/version result |
| storage/structure/schedule | baseline deterministic buffer、order与completion合同；不构造search I/K/J domain | search selected Buffer/K/J plan与plan/actual parity；不得调用baseline canonical completion补缺 |
| materializer owner | 独立baseline materializer及baseline-owned relation/verifier | 独立search materializer及search-owned relation/verifier |
| accepted result | 第一个通过actual gate的baseline candidate直接成为baseline result，不存在winner comparison | search controller从自己的accepted actual results中保留并发布一个winner |
| failure | typed baseline failure；绝不启动search | typed search failure/coverage；绝不运行或返回baseline |

允许复用的是无policy leaf，不是上述任一行的owner或状态：source parsing/normalization、只读`IndexRelation`等基础analysis、单operation
tiling/layout emission、TileRegion→Instr conversion kernel、actual MiniMalloc、DDR/transport verification和target lowering。leaf API只接收该
operation/stage已经完整解释的IR与typed参数，不接收`OptimizationPolicy`、baseline/search plan variant、nullable callback或mode来切换语义。

```text
shared source import / normalization
  |
  +-- none --> baseline planning --> baseline Card/Tile construction
  |                            --> common TileRegion-to-Instr leaf
  |                            --> baseline-owned completion/IR verification
  |
  +-- search -> search domains/controller --> search selected Card/Tile construction
                               --> common TileRegion-to-Instr leaf
                               --> search-owned storage/K/J materialization/verification

two independent, policy-complete Instr results
  --> common actual PlanSPMMemory --> DDR/transport gate --> target/package leaf
```

两条路径只在**各自policy语义已经全部落入verifier-legal Instr IR及current relation之后**汇合。current Q50.0中base/derived
`CardExecutablePreparation`通过dynamic callback在共同函数内部选择baseline/search materialization的结构必须退出：policy-specific construction
移回各自owner，共同downstream只消费policy-complete Instr IR，不能再回调任一controller。

#### 成熟compiler实现对照

| Primary implementation | 实际做法 | 本项采用 | 不采用 |
| --- | --- | --- | --- |
| [LLVM IR well-formedness与Verifier](https://llvm.org/docs/LangRef.html)及pinned `llvm/lib/IR/Verifier.cpp` | 检查类型、CFG、dominance、attribute和module/instruction固有不变量；错误表示输入或transformation产生了非法LLVM IR | verifier只检查对应IR层本身的well-formedness与固有语义 | 不检查某个优化是否采用preferred schedule、op数量或实现形状 |
| [MLIR Dialect Conversion](https://mlir.llvm.org/docs/DialectConversion/)及pinned `DialectConversion.cpp` | transformation直接rewrite当前IR，`ConversionTarget`规定source/target legality，full conversion要求illegal source归零；PassManager可逐pass运行标准verifier | 每个leaf直接正确构造IR，stage target与`verify-each`收尾 | 不复制一套expected output/materialization interpreter |
| [Triton `RemoveLayoutConversions`](https://github.com/triton-lang/triton/blob/main/lib/Dialect/TritonGPU/Transforms/RemoveLayoutConversions.cpp) | 在当前IR上做layout propagation/conflict analysis，随后按dominance order一次rewrite；已有rematerialized value直接复用 | layout优化分析当前SSA并直接删除/复用conversion；失败保持原IR | 不为每个plan另造layout verifier或side model |
| [IREE Codegen lowering config](https://github.com/iree-org/iree/blob/main/compiler/src/iree/compiler/Codegen/Dialect/CPU/IR/IREECPUAttrs.td) | lowering config作为实际pipeline输入；部分跨字段组合甚至不放进attribute verifier，以免过度限制仍可变换的IR | baseline/search各自把已选事实直接交给自己的pipeline；局部op/type verifier只检查本层语义 | 不复制IREE GPU schedule，不增加限制plan域的全局materialization verifier |

结论：本项通过正确的owner/API边界、直接IR rewrite、现有stage verifier和回归测试解决问题；不新增expected-inventory builder、膨胀
legality、shape assume或第二份materialization模型。

#### Verifier职责清理

2026-08-25静态inventory发现compiler当前有52个custom `verify()`定义、3个custom `verifyRegions()`定义（合计55个custom verifier
hooks）、38个显式`mlir::verify`调用点以及多组
materialized-plan verifier。规模本身不是删除理由；判断标准是检查是否属于该IR对象的固有语义、是否有直接下游消费者，以及是否重复解释
construction。已经确认的越界/过重实例包括：

- `CardModuleOp::verify`读取parent module topology、检查sibling card、遍历全部Tile/function/peer及CardDDR symbol/binding闭包；
- collective leaf verifier向上查找module `ExecutionMeshOp`并验证全局partition domain；
- `TileRegionOp::verify`沿def-use、ViewLike、SCF循环/分支递归追踪SPM storage root；
- `verifyMaterializedCardCandidate`重新从actual region/node/loop链解释Region/Temporal plan；
- schedule/structure/storage verifier检查actual op顺序、具体selector operation count以及prologue/kernel/epilogue数量，锁定当前实现形状；
- `elideRedundantFullBufferTransfers`对每个尝试rewrite验证整个Module；同一candidate在若干相邻stage重复整Module verify。

##### 55个custom IR verifier hooks冻结分类

下表按实现family覆盖全部52个`verify()`与3个`verifyRegions()`，数量总和为55；施工时只执行已确定处置，不再重新设计分类。

| Family | 数量 | 分类与处置 |
| --- | ---: | --- |
| `DTEMessageAttr` / `DirectDTEBindingAttr` | 2 | 保留local value/range/tuple关系；target FSM上限属于该target-level attr固有表示范围，不读取外部IR |
| `CardModuleOp` | 1 | **拆分**：body block、直接child/declaration类别、内部Tile ID唯一及CardModule自有symbol table中的CardDDR declaration/symbol/type一致性保留container-local并改用标准SymbolTable工具；parent topology、sibling card ID、available Tile/peer domain及“每Tile绑定全部ABI slot”移到module/card ABI stage一次检查 |
| `TileModuleOp` | 1 | 保留local parent/body/nonnegative ID；parent/card完整domain不在leaf重建 |
| `LinalgExtAttentionOp` | 1 | 保留operand/result、indexing-map role、dtype、tensor/buffer form及algorithm本身局部前置；不加入workload或shape策略 |
| 5个`LinalgExtCollective*Op`及2个combiner `verifyRegions` | 7 | 5个op verifier **拆分**：DPS shape、partition attr非空/非负/唯一、axis/channel保留local，相对module execution mesh的partition上界移到normalization/module stage；2个combiner region verifier保留local |
| `TargetTopologyOp` / `ExecutionMeshOp` | 2 | 保留自身grid/shape/axis/coordinate；direct parent改ODS trait，允许合法namespaced discardable attr，不用手写全attr拒绝表 |
| 7个Tile Move op | 7 | 保留source/result type、static slice/reshape/transpose/broadcast局部relation；优先ODS/interface helper，不读取users或plan |
| 7个Tile Compute op | 7 | 保留operand/result、layout、shape、kind和DPS局部合同；不验证schedule、Tile participation或op数量 |
| 2个Tile Comm op | 2 | 保留buffer/peer/bytes/token局部合同；peer是否属于当前card可用Tile域移到card stage |
| Tile View + Layout op | 2 | 保留ViewLike/layout source-result局部type/relation；是否应消除copy属于transform/test |
| `TileRegionOp::verify` / `verifyRegions` | 2 | 主verifier **拆分**：outer/non-nested及input/result type保留local，递归def-use/SCF/ViewLike SPM-root追踪移到bufferization/Instr-memory stage；body argument/yield region verifier保留local |
| `StorageLoadOp` / `StorageStoreOp` | 2 | 保留DDR/SPM memory space、layout、logical tensor type局部合同 |
| Instr Movement/DTE/Sync/Peripheral/Compute | 19 | 保留Instr层可解释的memref、descriptor、target field width/range、worker/token及compute shape/kind；不读取module plan/cost/preferred lowering |
| **总计** | **55** | 48个hook完整保留local/ODS整理；5个collective op verifier、1个CardModule verifier、1个TileRegion主verifier拆分；零未分类 |

##### 38个whole-IR `mlir::verify`调用点冻结分类

| 文件族 | 数量 | 已确定处置 |
| --- | ---: | --- |
| Driver/Frontend输入与external helper边界 | 3 | 保留：parse/import/helper返回后各验证一次新IR epoch |
| StableHLO normalization/attention | 5 | 每个pass最终function/module verify或named pipeline `verify-each`保留；删除`NormalizeAttention`逐新op verify后又verify function的前一调用 |
| TensorProgram→Tile/Card construction | 14 | `SourceValidation`、旧Card assembly/common、SelectedEdge、CandidateSupport随旧materializer删除；Card→Tile输入/输出保留；Tensor→Tile/SingleRoot各自在最终owned scope一次；SelectedRegion输出一次 |
| selected preparation/schedule/structure | 5 | shared `CompleteCandidatePreparation`调用随facade删除；Schedule mutation后verify与其内部重复verify二合一；ExecutionStructure同样二合一 |
| Tile memory/Card executable/target ABI | 10 | 调用间存在bufferization、Tile→Instr、SPM、DDR、index或target ABI真实mutation，保留每个稳定output边界；切除callback后再按IR epoch删除相邻无mutation重复 |
| Redundant full-buffer transfer | 1 | 不再每个candidate rewrite验证whole Module；只读preflight，fixed-point成功结束后验证一次owned scope |
| **总计** | **38** | 全部call site已有处置，零未分类；最终数量由旧路径删除后的真实stage数决定，不预设配额 |

##### materialized-plan/stage verifier冻结分类

| Current verifier/check | 已确定处置 |
| --- | --- |
| `verifyMaterializedCardCandidate`的Region/node set及loop-chain重放 | 随shared/旧complete materializer删除；search直接按RegionPlan构造，baseline使用独立root materializer；Region/Temporal专项tests保留 |
| `CompleteCandidatePreparation`按node/region寻找actual issue并重放plan | 随shared callback删除；search materializer直接传递selected SSA，baseline不调用 |
| `verifyMaterializedSchedule` | 删除重新证明builder按ClosedSchedulePlan排序/绑worker的全量重放；保留token ownership、completion pending-at-return和current IR validity，收窄到search schedule stage |
| `verifyMaterializedExecutionStructure` | prologue/kernel/epilogue exact op-count公式和selector实现形状移到unit/lit；保留temporary attr清零、SCF verifier和直接下游需要的loop/result有效性 |
| `verifyEmittedStorageObjects` / `verifyRotatingStorageSelections` | inventory相等、`1+2*(slots-1)`及具体cmp/select链移到builder tests；construction直接返回SSA slots/selections |
| `verifySelectedStorageLifetimes` | 保留并收窄为Instr-memory safety check，因为definition/use/completion/release直接决定actual lifetime与MiniMalloc输入 |
| Q63 completion、SPM/DDR range、transport/ABI validation | 保留在各自actual stage；它们验证硬件/ABI可执行性，不是preferred implementation shape |

清理使用下列分类，不增加替代性的总体验证器：

| 检查类别 | 处置 |
| --- | --- |
| operand/result/type/attribute、region结构、effect、field width/range等operation固有不变量 | 优先ODS/trait/interface，确需C++时保留局部custom verifier |
| operation自己拥有的region及block/terminator关系 | 保留在op/region verifier，不离开该operation subtree |
| parent module、sibling、外部target topology、完整Tile/card/ABI resource domain | 移到最近共同parent上的显式stage check；leaf op verifier不向上或横向扫描。container自己拥有的region/symbol table closure可用标准SymbolTable/SymbolUser接口保留 |
| alias/storage root、跨region lifetime、completion pending、transport/ABI资源 | 只有actual bufferization/memory/completion/target直接消费时保留为对应stage analysis/check；不放operation verifier |
| “actual op数量/顺序等于当前builder公式”、canonical/default形状、plan重放 | 由construction直接实现并在定向tests检查；production verifier删除 |
| `mlir::verify`调用 | 每次真正IR mutation后的稳定stage boundary保留一次；同一IR epoch的重复调用及per-attempt whole-Module verify删除 |

本项覆盖矩阵：

| 输入/检查类 | 精确断言 | 直接下游witness |
| --- | --- | --- |
| 55个custom verifier hook inventory | 每项已标成local-keep/ODS-cleanup或split-to-stage，零未分类；local verifier不访问parent module/sibling或op外def-use | parser/roundtrip/negative verifier继续覆盖固有IR语义 |
| Card/Tile/collective topology | leaf verifier只保留局部字段/region；module/card stage一次检查topology、peer、symbol和resource闭包 | compiler正式Card/target pipeline调用stage check，standalone leaf IR仍可局部verify |
| TileRegion storage boundary | op verifier保留输入/result/region局部类型结构；SPM alias/residency由bufferization/Instr-memory stage基于current relation检查 | actual MiniMalloc前owner/lifetime检查仍在，删除leaf递归trace不放宽memory safety |
| plan/materialization checks | builder-specific数量和形状只在unit/lit；有实际下游语义的worker/completion/lifetime检查收窄到对应stage | baseline/search大shape actual pipeline及Q63/SPM/ABI gate通过 |
| 38个whole-IR verify点 | 按IR epoch/stage计数；相邻无mutation重复为零；rewrite candidate不逐次whole-Module verify | named pipeline与正式driver使用`verify-each`，外部tool/package前最终IR再verify |

完成后不以“测试变绿”单独签字：必须列出每个删除检查原来保护的语义、当前new owner或删除理由，并用负例证明必要错误仍在正确stage被拒绝。

| 顺序 | Work item | 单一责任与输出 | 本项精确完成条件 | 本项固定执行流程 |
| ---: | --- | --- | --- | --- |
| 1 | `expansion-evidence-and-contract` | 冻结同一LLaMA source identity及baseline/search逐stage inventory；补全baseline 16 Tile失败前aggregate；冻结两个独立materializer、允许共享的窄leaf、旧路径删除inventory和本节覆盖矩阵 | instrumentation on/off得到相同plan、IR/failure类别；source/post-SPMD/Tensor输入一致；静态call graph证明两条controller在complete-candidate层无共同实现/互调；每个缺失stage明确标成未到达而非零开销 | 读AGENTS/progress→读Q52及相关编号设计/本项矩阵→读涉及硬件/ABI事实→调研成熟compiler的stage/owner instrumentation→查官方与pinned MLIR API→实现诊断/tests→fresh验证→重读设计并按MLIR规范复审→更新状态并提交 |
| 2 | `verifier-contract-cleanup` | 严格按本节已冻结的55/38/materialized-stage分类执行：op verifier只留固有局部语义；module/topology、alias/lifetime、completion/ABI移到真实owner；plan重放/实现形状移到测试；重复whole-IR verify合并 | 分类不在施工时变化；5个collective op verifier、CardModule和TileRegion主verifier完成拆分，48个local hook语义保持；38个call site按表处置；materialized-plan verifier按表删除/收窄；无新增总体验证器，必要负例仍在正确stage失败 | 读AGENTS/progress→读Q54/本节矩阵→读硬件/ABI直接事实→复核LLVM/MLIR/IREE/Triton verifier实践→查pinned源码→逐类改代码/tests→fresh verify-each/build→重读设计/MLIR复审→更新并提交 |
| 3 | `search-execution-materializer` | 直接修复search：补全`StructuredNodeShardGroup`路径，canonical/noncanonical、generic/attention只走这一套；compute helper收归search materializer私有，carrier API只接收SSA result/view | 旧conditional builder与carrier→tiler调用为零；fanout 1/2/15时producer compute IR不变；replica/nested/coupled及main/tail由收窄后的Region/Temporal/K/J检查与tests通过 | 读AGENTS/progress→读C/D/E/G/H/attention设计矩阵→调研MLIR/IREE/Triton实际materialization实现→查pinned MLIR→改代码/tests→fresh Card/Instr验证→重读设计/MLIR复审→更新并提交 |
| 4 | `baseline-root-materializer` | 直接修复baseline：独立deterministic materializer按root创建region并显式建立independent-DDR carrier，不构造或调用search plan/materializer | 当前15-root region消失；每个baseline compute region恰一个semantic root；baseline include/CMake/call graph不触及search materializer/domain/preparation | 读AGENTS/progress→读Q49.P矩阵→读硬件completion/DDR事实→调研成熟baseline pipeline实现→查pinned MLIR→改代码/tests→fresh baseline Card/Instr验证→重读设计/MLIR复审→更新并提交 |
| 5 | `movement-and-spatial-closure` | 直接修复movement与search proposal：两条policy carrier都只消费已有SSA；接通local carrier；search沿exact `IndexRelation`协调相邻分片轴，减少peer/RegionCut fragments | carrier源文件不能include/call compute tiler；fanout只增加必要view/transport；compatible local edge零DDR；search proposal不改变raw合法域或SPM合法集合 | 读AGENTS/progress→分别读Q49.P与A/B/H矩阵→读硬件transport事实→调研图一致分区/fanout实现→查MLIR API→改代码/tests→actual Instr/SPM验证→重读设计/硬件/MLIR复审→更新并提交 |
| 6 | `layout-version-and-view-closure` | 直接接通现有layout实现：search `PhysicalVersionBuilder`进入production，baseline保留独立deterministic layout；shared conversion复用SSA，alias/view不造copy；relation-aware full-buffer elimination接到offset规划前 | `PhysicalVersionBuilder`和full-buffer eliminator均有真实production caller；shared use只生成一份conversion；metadata view零target call；partial/non-equivalent copy仍保留 | 读AGENTS/progress→分别读baseline layout与G/I/memory矩阵→调研MLIR/Triton layout propagation/removal实现→查pinned API→改代码/tests→fresh Instr/SPM验证→重读设计/MLIR复审→更新并提交 |
| 7 | `legacy-path-retirement` | 切换两个独立owner并直接删除shared `materializeCardCandidate`、旧sessions、edge-owned compute、donor surgery、无producer action及active文档/tests/CMake | baseline只调用baseline materializer，search只调用search materializer；旧symbol/source/doc为零；失败不fallback旧path或另一policy | 读AGENTS/progress→读迁移表→逐项对照old capability/new owner/test→调研API retirement实践→查MLIR ownership→切换并删除→fresh build/static/downstream验证→重读设计/diff复审→更新并提交 |
| 8 | `scale-regression-and-q52-rebase` | 用1024/1025/1031、rank 3--6验证上述修复，并在新IR上重新profile既有memo/DP/bound/priority/LNS，只保留exact on/off等价且有收益的改动 | fanout/layout/view定向计数通过；完整Tile/Instr/target inventory形成；MiniMalloc实际运行；相关unit/lit/named/driver parity fresh通过 | 读AGENTS/progress→读Q52/各axis矩阵→读硬件资源事实→调研search scalability实现→查pinned MLIR→实现/清理/tests→fresh真实规模验证→重读设计/MLIR复审→更新并提交 |
| 9 | `llama-baseline-search-acceptance` | 同一只读current FP16 LLaMA source分别启动fresh `none`和`search`进程、ProgramData与输出目录，顺序执行；两者各自生成current package并strict readback/no-card | 每次Release不超过15分钟且无timeout/OOM/skip；baseline不进入search，search不调用baseline/fallback并发布自己的winner；两者实际进入Instr、MiniMalloc、DDR和target；问题专项及膨胀回归测试全部通过 | 读AGENTS/progress→重读Q49.P/Q52/Q53验收矩阵→确认current source/tool→fresh顺序运行→逐项核对设计与MLIR规范→更新progress/working plan并提交 |

任一checkpoint失败留在本项修复，不跳到后项，不把旧路径、DCE、descriptor cache、单Tile或历史package作为fallback。第9项通过前Q52保持
`doing`，Q53保持`queued`。

#### IR expansion验收

不为IR膨胀新增production assume、legality、analysis、expected builder或专用verifier。实现正确性由代码职责、现有MLIR stage verifier和测试共同
证明：

| 已确认问题 | 必须发生的代码修改 | 验收证据 |
| --- | --- | --- |
| search canonical/noncanonical两套complete builder | search只保留`StructuredNodeShardGroup`方向；删除plan equality分流和旧builder | static caller/source gate为零；canonical/noncanonical、generic/attention都通过同一search路径 |
| carrier按fragment重建producer compute | compute tiler/helper变为search或baseline materializer私有；carrier函数只接收现成SSA value/view | carrier source/include/call graph中compute materializer为零；fanout 1/2/15定向case的producer compute op数完全相同 |
| baseline把15个root放进一个region | baseline materializer按deterministic root直接创建独立region，不从whole-function SSA component重新group | baseline大shapecase逐region检查一个structured root；LLaMA不再触发15-root failure |
| spatial proposal制造大量不协调fragment | search proposal用exact `IndexRelation`协调相邻partition，仍返回普通raw domain member | proposal on/off合法plan集合相同；peer/RegionCut piece实测下降；不进入SPM legality控制流 |
| selected layout builder和full-buffer elimination没有production caller | 将现有builder/eliminator接入各自明确stage；shared conversion复用SSA，alias/view不造copy | production caller static gate；shared-use 1/2/15只有一份conversion；alias/view零target movement；partial copy负例保持 |
| Tile/Instr IR整体膨胀 | 删除上述根因后重新统计所有stage，不用DCE或descriptor cache遮蔽 | source→Tile→Instr→SPM→target完整inventory；所有stage实际到达；standard MLIR verifier/ConversionTarget和现有Region/Temporal/K/J verifier通过 |

1024/1025/1031与small oracle继续验证原有tiling、tail、pipeline、layout和movement语义；它们不是第二份全局materialization模型。SPM allocation
owner totality继续使用现有current relation合同，因为MiniMalloc反馈直接需要该信息，不为膨胀审计新增owner种类或控制流。

对冻结LLaMA artifact再增加具体回归门禁：source/post-SPMD/Tensor inventory保持142/144/434；search修复前基线为554,688 Tile ops、
40,960 compute emission和92,976 extract-slice，修复后total/slice必须严格下降且execution duplicate为零；baseline修复前Tile 0基线为
30,668 ops、2,538 compute emission和6,048 extract-slice，修复后该Tile三项必须严格下降、16 Tile完整inventory可用且15-root region消失。
严格下降与问题专项case、静态call graph、现有IR verifier和最终package/no-card必须同时通过；不设进入compiler逻辑的op阈值。

#### 旧路径强制删除清单

production切换完成时至少检查以下current残留；仍有功能的leaf先迁到新owner，不能保留wrapper或fallback：

- `TileMaterializationSession`、`TileMaterializationSourceSession`、public `lowerTensorProgramToCardModule`及
  `lowerPreparedTensorProgramToCardModule`完整Card builder；
- shared `materializeCardCandidate(CompleteCandidatePlan, mode, ...)` facade，以及用base/derived `CardExecutablePreparation` callback在共同
  executable函数内选择baseline/search construction的dynamic dispatch；终态由两个policy owner分别编排窄leaf；
- `usesSelectedRegions`/`usesSelectedRepresentations`决定complete materializer的分支；
- carrier侧`getOrMaterializeSource -> materializeCandidateRootTileValue`及按fragment重建producer compute的调用链；
- selected movement中的`oldPairs`、`oldLoads`、direct donor扫描/替换/erase；
- Card/Region/J按node集合、region cardinality或post-hoc `appendRequiredExecutionNodeRelations`恢复execution identity；
- `RootRegionEmitter`从whole-function SSA component重新选择root grouping的责任；终态只emit caller-prepared groups；
- `BufferVersions/lookupAny`按first use或layout enum选择physical version的责任；search use直接引用`PhysicalVersionId`，baseline use直接引用
  baseline自己的deterministic layout identity，不共享version schema；
- 没有current plan producer的`RecursiveProducerTiling`、`SpillReload`、`Recompute`枚举/lowering/tests；`LocalShardResidency`必须接通而非删除；
- active设计、`memory/`、CMake和tests中把上述路径描述为current或supported的内容。archive只作为明确历史，不得被current导航或completion引用。
