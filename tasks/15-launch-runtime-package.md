...
Conv、Pool、UnPool、structured TDMA DataMove 和 Peripheral metadata 只接受对应 instr-level target
kind attr、shape descriptor 和 kind-specific arity 字段；package metadata 不能把这些硬件 wrapper
family 降级成 opaque ABI call 或忽略掉。
