...
- `test/Tools/wafer-opt-hf-megatron-transformer-block.test` 覆盖更接近 LLM 的真实 frontend gate：
  ...
  memory-planned instruction IR；target LLVM lowering 已有 hand-written instr/group gate，HF program-chain
  target LLVM integration、package/no-card runtime required-symbol gate、真实 board execution、数值 correctness
