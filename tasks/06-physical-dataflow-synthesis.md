...
每个 movement candidate 先在 clone-local owner 上完成 canonical Instr、completion 和实际 SPM/DDR resource analysis，
只将 actual aggregate cost 交给同一 cohort 的 comparator；target LLVM/ABI/package lowering 默认只对排序前两个 finalists 执行。
Finalist target failure 不会回写或修复 pre-target owner；候选失败后销毁，下一 finalist 仍从自己的 actual owner 进入唯一 target leaf。
