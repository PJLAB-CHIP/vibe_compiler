...
| `Transforms/PhysicalDataflow/{CompleteRankMaterialization,LayoutMovementOptimization,ReadyOrder,StructuredOpInterfaceModels}.cpp`及对应unit | layout/movement actual rewrite、ready-order与interface mechanics | Q50.C–E已迁入single-root、partial merge、connected partition、multi-sink和完整temporal traversal，并删除旧`CompleteTraversal.cpp`；`CompleteRankMaterialization`只剩residency/movement donor归Q50.H，layout/movement与ready-order/interface分别由Q50.G/H、Q50.J迁入。剩余donor在承接前保持dormant，不进入production target |
