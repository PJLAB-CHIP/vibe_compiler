...
| `Transforms/PhysicalDataflow/{CompleteRankMaterialization,ReadyOrder,StructuredOpInterfaceModels}.cpp`及对应unit | residency/movement、ready-order与interface mechanics | Q50.C–G已迁入single-root、partial merge、connected partition、multi-sink、完整temporal及typed physical-version domain/apply；旧`CompleteTraversal.cpp`、`LayoutMovementOptimization.cpp`与PBQP/top-proposal专属测试已删除。`CompleteRankMaterialization`只剩residency/movement donor归Q50.H，ready-order/interface由Q50.J迁入；剩余donor在承接前保持dormant，不进入production target |
| `Transforms/Scheduling/{FixedSlotPipeline,RankTileMaterialization,WorkerPlacement}.cpp`及对应unit | loop pipeline、selective spill和worker placement mechanics | Q50.C–E已迁入direct final-TileModule node-shard与temporal materialization；`RankTileMaterialization`只剩selective spill mechanics由Q50.H承接，loop pipeline与worker placement分别归Q50.K、Q50.J；不得恢复fixed-slot selector或rank candidate API |
