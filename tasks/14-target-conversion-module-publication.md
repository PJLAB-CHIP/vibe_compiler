...
- Completion gate:
  ...
  metadata/ABI/exports/digest fresh readback；任一 Tile 失败时无部分 artifact 可见；Q53 source→package/no-card
...
- 经过 admission 的 Tile-local Instr module。
...
type 或 slot mismatch 均在 publication 前失败。
...
- unsupported dtype、layout、geometry或字段范围在 conversion/preflight失败，不生成 fallback call。
...
publication在私有 staging root中完成 LLVM IR、object、CRT、device link、ELF 和 readback。只有以下条件全部成立
才一次性发布 target root：
...
## 5. Profile-only target publication

profiling以同一次 accepted final artifact为事实源。instrumented capture module可以是额外内部publication，但：
...
- profile companion仍使用显式物理三元组，并校验它与 production manifest逐 Tile一致；
...
- non-identity `tile_id`/`launch_slot` mapping，包含 aggregate与非aggregate publication；
