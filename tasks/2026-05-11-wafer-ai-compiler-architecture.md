...
| Candidate decision | `tasks/2026-06-11-wafer-r3-2h-candidate-decision-design.md` | R3.2h V0 已实现 | bounded traversal tile shape、same-domain output coverage、matmul/generic reduction split、representative tile classes、R3.2e-g gates、`tile_search` 选择策略、passing candidate artifact | 新 allocator、失败计划 IR、runtime allocation/import、packet bitfield、不同 output domain 或 dynamic reduction 伪支持 |
