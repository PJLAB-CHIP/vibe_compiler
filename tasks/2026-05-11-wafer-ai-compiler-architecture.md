...
- `#wafer.memory<ddr, *>`：device/global DDR 的目标侧地址空间，由 DDR memory planning
  验证显式 view/range/resource demand；launch/runtime/package 负责把已接受的 requirement
  映射到运行时分配对象。
...
source/destination address space；差别在于 resource requirement、owner 和 lowering 层级不同。
DDR 的 default arena、view/range、compiler-managed/resident planned range、external allocation、
constant residency 和 bandwidth 设计见 `tasks/2026-05-25-wafer-ddr-memory-planning-design.md`。
...
| DDR memory planning | explicit DDR view/range/descriptor、external allocation requirement、compiler-managed/resident/inter-group requirement、planned DDR ranges、lifetime/reuse、default arena capacity、largest-contiguous/bandwidth diagnostics |
...
| SPM bufferization | `tasks/2026-05-21-wafer-spm-bufferization-design.md` | 草案 | `#wafer.memory<spm, *>` demand、liveness、range/alignment、allocation、placement realization input | DDR planned ranges、runtime allocation mapping、collective algorithm、host launch |
...
| Instruction IR | `tasks/2026-06-05-wafer-instruction-ir-design.md` | 草案 | `wafer.instr.*`、Wafer-tagged memref graph、issue family、memref read/write/issue effect | SPM offset、DDR planned range/runtime mapping、raw packet、C ABI call、重复 storage IR |
...
| DDR memory planning | `tasks/2026-05-25-wafer-ddr-memory-planning-design.md` | 草案 | `#wafer.memory<ddr, *>` demand、explicit view/range/descriptor、external/compiler-managed/resident/inter-group requirement、planned DDR ranges、lifetime/reuse、default arena capacity/largest-contiguous/bandwidth | tensor fusion、SPM offset、runtime allocation/import、packet bitfield |
