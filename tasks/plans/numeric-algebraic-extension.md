...
各自数值合同允许的f16/bf16/f32。scalar reassociation、tree balance和distribution/factorization
...
- Current stage responsibility:
  ...
  使用与integer相同的结构、shape、layout和资源检查。scalar algebra rewrite直接产生candidate；
  integer overflow/no-wrap规则和真实target能力检查保留。
- Output artifact / IR:
...
- Completion gate:
  f16和bf16的scalar algebra覆盖actual mutation、frontier、lowering和paired output qualification；
  generic reduction切分、GEMM K切分和Ring collective按各自现有typed
...
- 代数candidate的SSA matcher复用同一结构，只按op family选择`arith.addi/addf`、
  `subi/subf`和`muli/mulf`。支持的float直接改写；integer仍拒绝带no-wrap promise而改变
  poison边界的改写。
...
- physical-dataflow rewrite：无标注f16/bf16输入产生reassociate、balance、distribution和factor actual mutation；
