...
| 一个根因的候选修改稳定 | 正式编译目标case一次，配合少量共享该机制的回归；完成全输出PyTorch及必要A/B | 目标整网必须实际验证；其它相关重编合并到本轮稳定检查点，不能只用小case声称模型收益 |
| 第一、二轮结束 | 审核完整关键集合的影响表，只执行本轮受影响、疑似退化或覆盖不足的项 | 每个相关大case对稳定版本集中准备一次；已测且身份未变不再重编/重跑，不相关项记录延后依据 |
| 第三轮最终收口 | 对完整冻结矩阵补齐最终版本缺少或失效的编译/no-card/数值资格，对变化的关键产物做匹配性能验收 | 集中补齐一次；已具备最终版本有效证据的项复用，不再重复一套全量资格 |
...
开始优化前冻结关键集合，之后不得为通过验收删行。它是必须保护的集合，不是每次必跑清单；执行频率由上述影响表决定。
...
- **执行范围**：每项修改先测目标和直接受影响关键case；每轮末审核完整集合的影响与证据，仅补本轮必要验证。
