...
最终冻结版本后完整矩阵实际板测一次，逐项PyTorch及普通计时；仅补失效/缺失的编译产物，不每轮全量重编或全量profile。
中间迭代的免测规则不用于最终实卡验收，最终版本有效prepared产物仍可复用以省去编译。
LLaMA单独设强制性能检查：可能影响它的修改在稳定后接受前验证，每轮最终版本也必须具备资格；
同时比较最初正确快版本和上一接受版本，防止累计退化。其它大模型继续按影响范围安排，具体配置/触发和复用规则由扩展矩阵拥有。
