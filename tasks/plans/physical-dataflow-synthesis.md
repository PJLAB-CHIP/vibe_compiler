...
| 10 | `structured-logical-egraph-normalization` | `pending` | 05号normalization唯一拥有post-attention pure Tensor/Linalg graph的bounded access-relation e-graph；通过hermetic pinned `egg` Rust staticlib和不暴露MLIR对象的薄C ABI实现core，C++只负责current IR importer、proof和一次性rewrite；先复用pinned MLIR folds，再处理剩余static reshape/transpose/broadcast/canonical concat、pure elementwise及受限contraction/reduction access graph；candidate attention展开调用同一scoped kernel | 不自研或fallback第二个C++ e-graph；C ABI schema/allocator/panic/error和Cargo/CMake offline build合同闭合；不修改scalar/combiner/dtype、不复制compute occurrence、不越attention/collective/SCF/effect barrier；deterministic work budget耗尽保持原verified IR；global/scoped输出由直接consumer读取；on/off以exact proof保持语义、downstream representability和determinism，不要求raw choice identity | 读AGENTS/progress→读05/06/19及本项矩阵→调研`egg`/`egglog`、TENSAT、Glenside和成熟tensor optimizer，确认`egg`版本/许可/build/FFI/extractor边界→先完成pinned `egg` staticlib+C ABI integration spike及malformed/panic/ownership测试→查官方及pinned MLIR patterns/API→实现typed language/e-class analysis/rules/extractor/instrumentation→fresh真实规模graph→planning witness→重读设计/完整diff/MLIR复审→更新并提交 |
...
| pinned `egg` build与C ABI | hermetic offline build；empty/single/dense records；连续/并行请求；malformed tag/length/relation ID、allocator与forced panic seam | configure/build缺依赖直接失败；runtime adapter返回typed internal failure且不发布partial rewrite | Cargo lock/source revision/许可固定；POD roundtrip exact；无MLIR pointer跨ABI；alloc/free all-and-only；无global e-graph state | named/driver同一adapter和real component normalization |
