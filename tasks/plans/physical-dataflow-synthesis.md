...
- Completion criteria:
  下表的15个checkpoint按线性顺序通过；current production不再有future SSA/buffer/event/schedule的跨stage owner；
...
实际到达本项输出和已闭合下游，不允许只由手写fixture或未注册helper代签。第10项同时修复function output没有真正DPS
闭合而生成冗余DDR→DDR publication copy的实现bug，并保留有SSA/alias/effect证据的必要copy；第11项把shared temporal
tiling/fusion mechanics替换为compact current-IR transformation；
第12项对这条baseline current-IR链运行完整门禁；第13项最后建立search structural producer，并在同一修改中接通既有consumer、切换production和删除旧
shadow路径。最终search运行顺序固定为：

```text
structural choice
  -> 第13项：actual structural Card/TileRegion
     （第11项唯一compact temporal tile-and-fuse mechanics）
  -> 第10项：current-IR layout/view/bufferization与actual output destination
...
第13项以前不为同一policy注册第二条compiler pipeline，也不把未被真实上游和直接consumer共同读取的新stage标成完成。
Baseline和search仍是两条policy-owned路径；第6--10项只逐步替换baseline内部的shared downstream stages，search旧链只保留到
第13项原子cutover，不能作为这些新stage的完成证据。第11项只替换shared transformation mechanics，不增加第三个materializer
或policy路径。Structural producer、search production cutover和旧complete/shadow
...
| --- | --- | --- | --- |
| memory入口内的function-boundary bufferization | 第10项`current-ir-layout-bufferization` | 第6项只从leaf删除调用并让缺失boundary typed失败；第10项在movement前接通唯一production transformation | 第10→9→8→7→6链中bufferization恰一次；leaf调用次数为0 |
...
| bufferization/layout产生的普通allocation及其IR位置 | 第10项 | 第10项在创建transaction中确定dominance、alias和effect；不把placement另存成plan | actual allocation/view/copy all-and-only，1024/1025/1031 lifetime与MiniMalloc witness一致 |
...
1. 第10项先冻结function-boundary bufferization前后的output/result SSA和copy inventory，按alias conflict、旧值保留、out-of-place、
   layout/memory-space materialization及纯publication分类。修正DPS/out-parameter binding，使冗余DDR→DDR publication copy为0；
   必要copy保留witness并交给typed movement；删除Tile-to-Instr临时建Region/staging的fallback。
2. 第11项删除`LocalUseDelivery`、`DirectNestedValue`、`StoredRegionValue`、`ReconstructedRegionValue`和`rewireDirectSSA`等
   future materialization协议。Region choice只保留group membership；先生成actual operations/current SSA，再决定fusion。
3. 使用pinned MLIR SCF tiling/fusion API替换parallel/reduction手写三段递归builder。Tile内使用`scf.for`；fusion control直接
   检查current producer/use、indexing relation、use count、effect和region boundary。Explicit recompute/replica先创建actual op。
4. 先生成一个bounded-tail canonical loop nest；只有static Tile/Instr直接要求时才peel最后一个partial iteration并运行
   canonicalization。删除front peel、按wave复制body和无delivery通用fusion worklist/cache。
5. 第12项先重签完整baseline；第13项再原子切换search并删除旧shadow/complete owner；第14项比较refinement前后的actual
   fusion/copy/IR/Instr/work inventory；第15项最后重跑同源LLaMA none/search验收。

第10--11项不改变SPM admission：每个candidate仍先形成上述actual IR，再由唯一MiniMalloc返回accepted offset或typed actual
...
| 10 | `current-ir-layout-bufferization` | `done` | 唯一拥有function-boundary/region-local bufferization、observable output DPS binding以及layout/view/alias产生的普通allocation与其IR位置；冻结structural TileRegion→layout-resolved endpoint合同，从current SSA构造完整layout域和query-local exact PBQP；baseline与search共享solver/transform，baseline每attempt求解应用一次 | solver全局tie-break与flat oracle一致；hard factor只表达exact legality；能够从current IR精确投影时分别使用NE、Vector/CT、SPM movement和instruction-control soft term，descriptor multiplicity等任一必要事实未知时整个soft set禁用；bufferization恰一次且在movement前；每个observable result直接写唯一actual destination；仅因DPS缺失产生的冗余DDR→DDR publication copy为0，必要copy有exact SSA/alias/effect witness；movement closure后未分类`memref.copy`为0；删除Instr copy-only TileRegion/staging fallback；same-layout/unused op为0、shared conversion一个SSA、exact alias零copy；fresh baseline source经第10→9→8→7→6项成功 | 读AGENTS/progress→读06–11/19及本项矩阵→读NE/CT与DDR/SPM事实→调研PBQP、resource-aware projection、One-Shot Bufferization destination-passing和function output→查pinned `BufferizableOpInterface`/DPS API→修复solver/layout/output binding并删除copy fallback→fresh solver oracle、copy inventory及baseline全链→重读设计/硬件/MLIR复审→更新并提交 |
| 11 | `compact-temporal-tile-and-fuse` | `pending` | 将baseline/search共享的temporal loop与producer-fusion mechanics收敛到pinned MLIR SCF tile-and-fuse；Region choice只保留membership，materializer先创建actual operations/current SSA，fusion再直接读取该IR；static Instr要求只由late remainder peeling满足 | 一个traversal一个canonical `scf.for` nest；1024无remainder clone，1025/1031只产生必要main/remainder且不peel first；未融合producer在consumer loop外只物化一次，融合producer只存在于对应loop；multi-use/reduction/contraction/collective/cross-region无exact证明时保持barrier；除actual explicit recompute外iteration domain无重叠；`LocalUseDelivery`、`DirectNestedValue`、`StoredRegionValue`、`ReconstructedRegionValue`、`rewireDirectSSA`、旧三段builder和通用fusion worklist/cache零production残留；fresh输出顺序通过第10→9→8→7→6项 | 读AGENTS/progress→读06/07/10–11/19及本项矩阵→读static Instr/tail事实→调研MLIR SCF/Linalg tiling、producer fusion、reduction tiling、loop peeling和成熟compiler fusion control→查pinned API→先固化current膨胀/重算回归→迁移current-IR loop/fusion/remainder及窄exact adapters并删除旧owner→fresh 1024/1025/1031 baseline/search focused链→重读设计/完整diff/MLIR复审→更新并提交 |
| 12 | `baseline-current-ir-integration-gate` | `pending` | 对第6--11项已逐步接通的baseline唯一路径运行完整none门禁；不首次接线、不重做root构造 | baseline不调用search state/domain/materializer；每region一semantic root；第11→10→9→8→7→6项和actual MiniMalloc均到达；冗余DDR→DDR publication copy为0，必要copy已typed；fresh current FP16 LLaMA none在15分钟内生成package并strict readback/no-card | 读AGENTS/progress→读06–16及本项矩阵→确认current baseline/source/package边界→读相关硬件/ABI事实→调研deterministic baseline集成验证→查pinned API→补完整gate/tests→fresh focused矩阵和none package/no-card→重读设计/MLIR复审→更新并提交 |
| 13 | `search-current-ir-cutover-and-shadow-retirement` | `pending` | spatial/region/temporal choice用第11项唯一current-IR mechanics物化candidate-owned structural Card/TileRegion；search先使用第10项同一PBQP assignment，再遍历完整raw layout alternatives；movement、execution structure和Instr schedule choice分别在current IR上立即变换并进入第6项；同批重接controller、切换production并删除旧shadow/complete路径 | PBQP on/off不改变raw合法域；PBQP result只决定首个proposal且不进入candidate key；controller从Accepted current Instr的fresh cost/schedule analysis比较resource-aware objective；只累加current IR明确排序的work，排序依赖未证overlap或缺失rate时typed incomparable；pre-structural state无future value/buffer/event；Accepted owner不重建；旧type/builder/domain/state/caller/CMake/test/current-doc零production残留；baseline路径不变 | 读AGENTS/progress→读05–17/Q52删除清单与本项矩阵→读NE/CT、overlap和target-profile事实→调研resource-aware cost、current-IR search transaction和原子API migration→查pinned Rewriter/IRMapping/ownership API→实现producer/current-IR枚举/PBQP proposal/final objective/controller接线并同批删除→fresh search全链、engine-cost反例、PBQP on/off与baseline隔离验证→重读设计/硬件/完整diff/MLIR复审→更新并提交 |
| 14 | `scale-regression-and-inventory` | `pending` | 在新actual-IR pipeline上profile并仅保留有证据的PBQP、memo、priority、DP和LNS，补齐actual fusion、producer occurrence、copy、layout conversion、transfer elimination与Instr只读汇总 | 完整Tile/TileRegion/current SSA edge/producer-occurrence/copy/layout-conversion/buffer/movement/eliminated-transfer/execution-structure/Instr/target inventory；refinement不按wave复制compute closure；exhaustive PBQP on/off保持raw/accepted set与winner，budgeted差异保持typed coverage；instrumentation/safe optimization on/off等价；actual MiniMalloc到达 | 读AGENTS/progress→读06/Q52及本项矩阵→读resource事实→调研search scalability和PBQP proposal算法→查pinned MLIR/LLVM→改代码/tests→fresh真实规模验证→重读设计/MLIR复审→更新并提交 |
| 15 | `llama-baseline-search-acceptance` | `pending` | 同一current FP16 LLaMA source顺序运行独立none和search事务 | 每次Release≤15分钟；各自package strict readback/no-card；两条路径互不调用；均实际进入Instr/MiniMalloc/DDR/target；冗余DDR→DDR publication copy和Instr copy-only Region为0，必要copy已typed；search没有temporal-refinement静态IR突增 | 读AGENTS/progress→重读06/Q52/Q53及本项矩阵→确认current source/tool和runtime/ABI边界→fresh顺序运行→逐项核对设计与MLIR/runtime规范→更新状态并提交 |
...
| 输入等价类 | shape/结构 | typed failure | 精确断言 | 直接下游witness |
| --- | --- | --- | --- | --- |
| generic/attention execution | single/nested/replica/coupled、fanout；rank 3–6，1024/1025/1031 | selected structural recipe不可表达时typed failure | 每execution all-and-only一次，fanout不重复compute | 第11项compact mechanics与第13项search structural cutover |
...
| observable output DPS与copy regression | single/multi-result、loop-carried result、chain/fanout；rank 3–6，1024/1025/1031；16 Tile | output无法绑定唯一destination或copy必要性缺少SSA/alias/effect witness时本stage失败，不在下游猜测 | 每个result直接写唯一actual destination；冗余DDR→DDR publication copy为0；必要copy all-and-only并在movement closure后typed；Tile-to-Instr创建copy-only Region/SPM staging次数为0 | 第9→8→7→6项与第12项baseline gate |
...
- function-boundary冗余DDR→DDR publication copy在Instr conversion中临时创建TileRegion、SPM staging和RDMA/WDMA的fallback；
  第10项必须修复产生该copy的output DPS/bufferization实现并删除fallback；有witness的必要copy仍由typed movement表达；
- `LocalUseDelivery`、`DirectNestedValue`、`StoredRegionValue`、`ReconstructedRegionValue`、`rewireDirectSSA`及依赖它们重放
  ...
  SSA legality而递归融合任意`TilingInterface` producer的通用worklist/cache；第11项只保留pinned SCF
...
只有第13项完成后才重新解释profile和优化search工作：
