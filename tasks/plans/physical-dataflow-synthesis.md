...
当前直接项：`egraph-relation-consumer-closure`；随后线性执行`temporal-relation-consumer-closure`、
`layout-movement-relation-consumer-closure`和第16--20项。
...
| ---: | --- | --- | --- | --- |
| C1 | `egraph-relation-consumer-closure` | 保持05号一次性egg owner，把单root请求改为ordered multi-root pure component和共享DAG extraction；mixed general reshape作为可暂时不可物化的exact `Access`参与composition，并以一类relation-driven reshape-through-compute rule产生标准Tensor/Linalg等价图；删除egg外`propagateMultiUseProjectedAccesses` | 不新增greedy/DRR/C++等价旁路或第二次e-graph；request/result ABI只保留multi-root形式；2/15 roots共享producer只导入/物化一次；中间不可物化relation可以继续探索但不能被extract；最终identity、standard reshape或downstream-accepted Linalg signature才提交；全部roots一次preflight和原子替换；computeId、scalar/combiner、iterator/reduction语义和DPS role不变；attention/collective/call/SCF/effect barrier不变 | 读AGENTS/progress→读05/06/19及本项矩阵→调研egg multi-root/shared extraction、Glenside/TENSAT及current IREE fusion→查pinned MLIR/egg API→先补current设计→实现单一ABI、multi-root importer/extractor/materializer和dynamic rule→删除旧旁路→fresh graph/StructuredDAG/ExactDemand矩阵→重读设计/完整diff/MLIR复审→更新并提交 |
| C2 | `temporal-relation-consumer-closure` | 重新打开第13项未闭合边界：Temporal query保留完整composed `IndexRelation`，完整choice显式覆盖independent与all-use joint traversal；对general static reshape的parametric rectangle/有限static-shape exact pieces、broadcast invariant和无隐式重算window立即物化actual SCF/Tensor/Linalg | raw per-op temporal size/order域不因relation proof删点；joint是query-local typed transformation choice，不是future op清单；general reshape只在actual main/tail可形成一个rectangle或work-bounded互斥static-shape pieces时融合，禁止bounding box、逐元素loop或dynamic-piece fallback；multi-use只有全部current uses、相同domain/tile/order和同一exact demand被joint choice捕获时共享一次producer tile；其它choice保持一个独立producer且不clone；第15项后成功case完整intermediate allocation/copy为0 | 读AGENTS/progress→读06/07/19及本项矩阵→调研pinned SCF multi-root tiling、tensor reshape slice与IREE tile fusion→查pinned API→先补current设计→实现choice-local relation query、joint SCF和actual slice materialization→fresh main/tail/fanout/layout witness→重读设计/完整diff/MLIR复审→更新并提交 |
| C3 | `layout-movement-relation-consumer-closure` | 从第15项current tensor/view SSA和第16项将消费的actual endpoint审计每个`IndexRelation` consumer；删除view触发的`compactOnly`降级。DPS与standard view的actual alias进入同一个PBQP value group，每个layout state组合canonical logical relation和两端physical relation作硬过滤；movement已有完整symbolic relation consumer保持不变 | 每个修改有定义、current caller和下游witness；reshape/cast只有physical element mapping、footprint、alignment、padding、write injectivity及标准memref view全部成立时zero-copy；不兼容state通过既有activation形成一个actual materialization或typed无解；slice/insert缺base-offset/range/alias/effect proof时fail closed；没有实际降级证据的movement API不扩展；SPM和completion合法性不读取relation估算 | 读AGENTS/progress→读06/08/19及本项矩阵→调研One-Shot Bufferization、PBQP alias group和symbolic transfer实践→查pinned API→列完整caller分类→先补current设计→逐consumer实现/删除死API→fresh layout/copy/Instr witness→重读设计/完整diff/MLIR复审→更新并提交 |
| 16 | `current-ir-downstream-orchestration` | 在不建立complete materializer的前提下，把第15项layout-resolved owner依次交给deterministic structured-to-Tile、current movement/boundary、execution-structure immediate apply、TileRegion→Instr、worker/order/fresh completion、standalone Tile fanout和唯一`compileCanonicalInstructionTilesToExecutable` actual leaf；把Tile-to-Instr私有的只读descriptor query抽到shared analysis并接回第15项PBQP resource projection；每个atomic stage仍由原owner实现，orchestration只固定current-IR调用与typed handoff | structured compute和movement all-and-only消费current memref/endpoint relation并形成physical TileRegion；descriptor query与actual lowering逐项一致且不保存plan；local/DDR/peer、Serialized/pipelined、tail/rotating slot、Instr、join/wait硬件witness与actual MiniMalloc全部到达；standalone fanout只move body；legacy combined facade和`CompleteCandidatePreparation`为0；输入choice不枚举、失败不repair/fallback、Accepted owner不重建 | 读AGENTS/progress→读06--14/19及本项矩阵→读movement/completion/memory硬件事实→调研MLIR staged lowering与LLVM pass-pipeline ownership→查pinned conversion/SCF API→抽取shared descriptor query并逐stage接唯一current transform、删除隐藏facade→fresh 1024/1025/1031 layout projection→movement→execution→Instr→completion→fanout→leaf与typed failure验证→重读设计/完整diff/硬件/MLIR复审→更新并提交 |
...
### C1. E-graph relation consumer closure

| 输入等价类 | Shape / 结构 | Typed/optimization failure | 精确断言 | 下游witness |
| --- | --- | --- | --- | --- |
| ordered multi-root pure component | 1/2/15 roots；chain/diamond/fanout；rank 3--6；1024/1025/1031 | root type、shared node、ABI range、relation callback或materializer任一不完整时整个component保持；budget exhaustion byte-identical | request/result只有multi-root schema；共享Input/Access/Compute node在union DAG中一次；全部root order/type保持；computeId集合及每个scalar/combiner/iterator/init逐项不变；第二次运行byte-equivalent | StructuredDAG和ExactDemand从一次物化后的current SSA直接建立 |
| general reshape equality saturation | collapse/expand与transpose/broadcast/concat/elementwise/reduction/contraction连续链；parallel-only、reduction-only及mixed-kind reassociation；中间relation不可单独物化 | 一个group混合parallel/reduction、最终relation既非identity/standard reshape也不能形成accepted Linalg maps时不extract；generic proof work limit保持component | 至少一个case由reshape-through-compute、composition和identity多条egg rule连续闭合；无C++/DRR/greedy旁路；reduction-only reshape保持row-major线性次序与总domain，mixed-kind不改；producer SSA不复制 | C2从最终current Tensor/Linalg graph重建完整relation |
| barrier与ABI | attention邻接ordinary component；collective/call/SCF/effect；malformed roots/nodes/children/relation、forced panic | barrier两侧不能进入同一component；invalid/work/internal状态区分 | attention op及region byte-identical；C/Rust allocator all-and-only；无旧single-root ABI symbol或`propagateMultiUseProjectedAccesses` caller | named pipeline与production driver调用同一个egg pass |

### C2. Temporal relation consumer closure

| 输入等价类 | Shape / 结构 | Typed/optimization failure | 精确断言 | 下游witness |
| --- | --- | --- | --- | --- |
| general collapse/expand tile | rank 3--6；1024/1025/1031；full、aligned main、ragged tail、一个rectangle与有限pieces | relation unknown/work limit、non-unit stride、dynamic piece shape、piece work超限或会生成逐元素loop时joint不可选，independent仍在raw domain | actual producer slices union与consumer tile image完全相等且互斥；main/tail static；无element-count trip loop；成功case完整producer被删除 | 第15项后对应完整intermediate allocation/copy为0 |
| compatible multi-use | 2/15 uses；single/multi-result producer；相同及不同tile/order；all-use和observable extra use | 任一use未捕获、DPS init、effect、domain/tile/order或exact demand不同则joint不可选 | independent和joint均为typed choice；joint只有一个common SCF loop、一次actual producer tile和all-and-only root results；动态producer执行次数不随use数增长；无clone | Layout、actual buffer roots和MiniMalloc直接读取joint current IR |
| broadcast/window | broadcast axis位于loop order内外；disjoint、halo overlap；1024/1025/1031 | 无支配hoist位置或不同tile demand重叠会隐式重算时joint不可选 | broadcast producer位于所有重复轴之外；仅互斥或actual shared halo被证明时融合；独立case保留原producer一次 | Instr inventory无relation fallback制造的完整copy或重复compute |

### C3. Layout/movement relation consumer closure

| 输入等价类 | Shape / 结构 | Typed/optimization failure | 精确断言 | 下游witness |
| --- | --- | --- | --- | --- |
| reshape/cast view edge | Tensor/NTensor/Cx/NCx；rank 3--6；1024/1025/1031；singleton、block/tail、1/2/15 read uses | composed physical mapping、footprint、alignment、padding、write injectivity或memref representation失败时state非zero-copy | DPS init/result与view alias同group；每个PBQP state exact过滤；compatible outer reshape零allocation/copy并保持Cx；channel-changing reshape恰一个shared actual materialization；intervening write拆cohort | One-Shot输出actual memref view/materialization并由第16项descriptor query消费 |
| slice/insert与movement caller审计 | static/ragged slice、loop-carried insert、local/DDR/peer、identity/projected/general relation | range/alias/effect/lifetime不完整保持现有路径；无caller API不因“能力强”而启用 | caller逐一归类为完整consumer、实际降级、死API；只修实际降级；冗余DDR publication copy为0且necessary copy有typed witness | physical TileRegion、Instr transfer与actual memory owner完整 |

### 16. Current-IR downstream orchestration

| 输入等价类 | Shape / 结构 | Typed failure | 精确断言 | 下游witness |
| --- | --- | --- | --- | --- |
| layout-resolved Linalg/memref与第15项hard-only PBQP state | elementwise/convert、GEMM/reduce、attention state；Tensor/NTensor/Cx/NCx；rank 2--6，1024/1025/1031 | source scalar/combiner、layout tuple或descriptor relation不能精确lower时typed unsupported；query work/overflow保持indeterminate | deterministic structured-to-Tile all-and-only消费Linalg；只读descriptor query与actual RDMA/WDMA/GS command count/bytes/multiplicity逐项一致；接回PBQP后known cohort启用resource soft projection，unknown cohort仍整体hard-only；无future Instr inventory | movement transformation直接读取current Tile/memref，PBQP projected-vs-actual由第19项汇总 |
