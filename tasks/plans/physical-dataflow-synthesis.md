...
| 11 | `legacy-shadow-materializer-retirement` | `pending` | 按本节五个线性批次先切断`none`/`search`旧产品caller，再把仍正确的算法、rewrite kernel和oracle迁到直接current-IR owner，随后从Schedule/Storage等末端向前删除complete/shadow schema、旧group/delivery materializer、隐藏facade和无consumer孤儿API；此项不实现替代pipeline | repo保持build/link；两种policy在CardExecutable边界明确`operation_not_supported`且analysis/candidate/fallback/MiniMalloc/package次数为0；retained API不再include Representation/Movement/Buffer/Event/Schedule shadow type；旧type/source/public header/CMake/test/current-doc及产品链接符号全仓为0；direct normalization、Spatial/ExactDemand/PBQP/FA-FD oracle、current movement/execution/completion mechanics和`compileCanonicalInstructionTilesToExecutable`仍通过 | 读AGENTS/progress→读01/05–19/Q52删除与迁移账本及本项矩阵→逐definition/caller/include/CMake/test/link symbol分类→调研成熟compiler破坏性cutover与typed unavailable边界→查pinned LLVM/MLIR ownership/API→严格执行route cut→asset isolation→shadow graph deletion→orphan/test cleanup→zero-residual五批→fresh build、routing、direct retained-assets、actual leaf、source/IR organization和链接符号验证→重读设计/完整diff/MLIR复审→更新并提交 |
...
| TensorProgram→Card/Tile低层construction | 只把single-op typed builder、Card/Tile assembly、partial-reduction tiling及确有direct consumer的exact reshape/insert adapter留在Conversion private kernel | `StructuredNodeShardGroup` complete recipe、`SelectedRegionMaterialization` replay、`rewireDirectSSA`、手写三段wave递归和通用future fusion worklist/cache |
...
| Attention | attention op interface、`AttentionSpatialConstraints`、FA/FD state算术、Linalg op construction和独立reference/oracle分别留在05号IR/第12空间约束/第14 selected transform；Linalg builder移出Planning replay owner | `AttentionWorkDescription`、canonical projection、`SelectedAttentionDecomposition`、baseline attention materializer、future action/value/scratch/physical-version/occurrence inventory |
...
| orphan、compatibility与链接负担清理 | host library、wafer-opt/wafer-compile、registered unit/lit；fresh configured build | 只有明确current consumer才能阻止删除；test-only caller改走正式API，不能把adapter继续链接进产品 | `StaticBufferRange`、`StaticSchedulePlan`、baseline temporal setter、planning stringifier、`getRootRegionWork`、`convertTileRegionToInstrModule`和retired complete materializer顶层符号在source/header/CMake/test及产品link symbol中为0；无空stub或旧名alias | source/IR organization、public-header self-containment、named/driver direct-stage parity和第16项正式adapter |
