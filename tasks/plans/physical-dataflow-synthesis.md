...
- 同一current FP16 HF Llama block以batch 2分别fresh capture sequence 1024和1025。两次正式pipeline均`verify-each`通过且无budget
  exhaustion；normalization均为431→383个op、删除23个Access，12个component中9个改变，其中6个changed component实际应用两类以上
  dynamic rule；ABI记录667→583、serialized bytes 10840→9424。当前Release host profile分别约92 ms/32.1 MiB和94 ms/32.1 MiB；
  这些数值只校准work budget，不进入compiler选择。
...
| e-graph budget/extraction/instrumentation | tiny independent oracle与fresh HF/LLaMA ordinary component | deterministic relation/e-node/match/rebuild/extraction limit不是legality failure | 相同budget产生相同IR/diagnostic；至少一个fresh真实component由两条以上rules有效改变；记录relation/e-node/e-class/match/iteration/wall/RSS | 第15项scale inventory与第16项完整pipeline reachability |
