...
闭合而生成DDR→DDR copy的实现bug；第11项把shared temporal tiling/fusion mechanics替换为compact current-IR transformation；
...
| function-boundary bufferization产生的DDR→DDR `memref.copy`与Instr copy-only TileRegion fallback | 第10项`current-ir-layout-bufferization`的output DPS正确性 | output在bufferization前绑定唯一actual destination；修复产生copy的实现并删除Instr conversion临时建Region/staging的fallback，不新增DDR-copy stage | bufferization后compiler-created DDR→DDR copy为0；每个observable result唯一publication；Instr conversion创建copy-only TileRegion次数为0 |
...
1. 第10项先冻结function-boundary bufferization前后的output/result SSA和DDR→DDR `memref.copy` inventory，修正DPS/
   out-parameter binding，使每个observable result直接写唯一actual destination；删除Tile-to-Instr临时建Region/staging的fallback。
...
| observable output DPS与copy regression | single/multi-result、loop-carried result、chain/fanout；rank 3–6，1024/1025/1031；16 Tile | output无法绑定唯一destination或One-Shot不能保持result/destination关系时本stage失败，不在下游补copy | 每个result直接写唯一actual destination；bufferization后DDR→DDR `memref.copy`为0；Tile-to-Instr创建copy-only Region/SPM staging次数为0 | 第9→8→7→6项与第12项baseline gate |
...
- function-boundary DDR→DDR `memref.copy`在Instr conversion中临时创建TileRegion、SPM staging和RDMA/WDMA的fallback；
  第10项必须修复产生该copy的output DPS/bufferization实现并删除fallback，不新增DDR-copy stage；
