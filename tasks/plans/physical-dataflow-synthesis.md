...
Q49–Q51和Q52第1--9项的详细证据在`tasks/archive/physical-dataflow-synthesis-working-history.md`；Q52当前重建从第10项起的
完成证据暂随本计划保留，Q52整体闭合后再统一归档。
...
| search execution materializer | typed construction与FA/FD实现的历史对照、semantic fixtures和覆盖oracle；第11项只保留已有直接owner与witness的资产后删除旧wrapper | 旧group/materializer API、future value/movement/storage/schedule、零consumer builder或继续可用的search route |
...
2. `legacy-shadow-materializer-retirement`先删除错误的旧none/search product routes、complete-candidate/baseline wrappers以及future
   ...
   纯spatial/connected-partition/free-temporal/PBQP/relation算法与FA/FD semantic/spatial fixtures只有迁到直接owner后才保留；零consumer
...
2. **隔离保留资产。** 逐个把有直接后续owner的算法或rewrite kernel移出旧schema依赖。迁移后的public/internal API只能接收current
   operation/value、显式choice和query-local typed result；不能继续include `RepresentationPlan`、`MovementPlan`、`BufferPlan`、
   `EventGraph`、`ExecutionStructurePlan`或`SchedulePlan`来“暂时复用类型”。每项先有direct test和下一consumer编译witness，再删旧owner。
3. **从末端向前删除shadow graph。** 先删Schedule/Event/Storage及其materialization，再删future Movement/Representation和
   `PhysicalVersion`，随后删post-Temporal PlanningState/continuation/memo、CompleteCandidate/FullFeasibility、baseline reclose和旧
   Region group/delivery materializer。这样每一步都只移除已经没有consumer的层，不用compatibility typedef或空stub维持编译。
4. **删除孤儿与test-only产品接口。** 删除无production/current-design consumer的分析、adapter、便利函数及only-purpose fixture；仍有
   current边界价值的测试改用named pipeline或直接owner API。测试需要调用某个生产library符号本身不是保留该符号的理由。
5. **零残留收口。** 同批清理public header、source、CMake source/test registration、include/link edge、active设计和diagnostic；fresh
   configured build不得复用已删object，产品二进制不得再导出或静态带入retired top-level symbol。Archive只作历史，不参与零残留搜索。

保留资产与删除边界固定如下；“迁移”表示抽出算法，不表示把旧文件原样换目录：
...
ID/record分离，其算法说明和可复用oracle迁到直接owner后删除实现，不能以“后续可能会用”为理由继续链接。相反，已有明确direct
consumer的`PhysicalLayoutRelation`、`TransferRealizability`、`NCCCompletionAnalysis`、MiniMalloc和target leaf不能因旧route断开而删除。
...
- fresh host build和706项unit全部通过；默认lit 233项全部通过；Tools配置中实际可执行的24项全部通过。其余3项没有执行，也不计为
  通过：feature-off install属于另一构建配置，target-model source/bulk还要求未编译的numeric/bulk/SystemC依赖并依赖尚未恢复的
  source→package产品链。8个同样必经旧`none`路径的FP16/BF16 no-card CTest已取消注册，case、CPU oracle和runner保持；第17项恢复
  `none`时必须重新注册并在对应配置实际执行这些case及target-model纵向。
- source/IR organization、4个public-link smoke和3个feature-off link-closure检查全部通过；active source/header/CMake/test及
  `wafer-compile`链接符号对删除清单的扫描为零。
...
| orphan、compatibility与链接负担清理 | host library、wafer-opt/wafer-compile、registered unit/lit/CTest；fresh configured build | 只有明确current consumer才能阻止删除；test-only caller改走正式API，不能把adapter继续链接进产品 | `StaticBufferRange`、`StaticSchedulePlan`、baseline temporal setter、planning stringifier、`getRootRegionWork`、`convertTileRegionToInstrModule`和retired complete materializer顶层符号在source/header/CMake/test及产品link symbol中为0；无空stub或旧名alias；第11项不把未编译feature或旧source→package路径造成的`Unsupported`计为通过，当前必经旧`none`路径的no-card CTest取消注册而不删除case/oracle | source/IR organization、public-header self-containment、named/driver direct-stage parity和第16项正式adapter；第17项恢复`none`时重新注册并实际执行保留的no-card与configured target-model source/bulk纵向 |
...
| 第11项保留但不能执行的产品测试资产 | conv mixed DAG、attention prefill/decode、LLaMA block的FP16/BF16 no-card；configured formal/bulk target-model source纵向 | 任一case仍在CardExecutable边界返回unavailable、跳过或只运行feature-off反向测试均不能签发完成 | 8个none no-card CTest重新注册并逐项执行；target-model source/bulk lit在numeric/bulk/SystemC-enabled配置实际执行；feature-off install测试仍只属于其alternate configuration，不计入本项通过数 | Q53从同一fresh none package继续host/model/no-card资格验证 |
...
- 无current设计或production consumer的`StaticBufferRange`、`StaticSchedulePlan`/`estimateStaticSchedulePlanDurations`、
  `CardBaselineTemporalTiling`、planning stringifier、`getRootRegionWork`便利函数和`convertTileRegionToInstrModule`兼容adapter；
  确有current consumer的小类型先拆到其直接owner，测试改走正式API后删除only-purpose source/CMake registration；
