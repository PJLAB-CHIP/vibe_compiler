...
| 10 | `current-ir-layout-bufferization` | `pending` | 唯一拥有function-boundary/region-local bufferization、observable output DPS binding以及layout/view/alias产生的普通allocation与其IR位置；冻结structural TileRegion→layout-resolved endpoint合同，从current SSA构造完整layout域和query-local exact PBQP；baseline与search共享solver/transform，baseline每attempt求解应用一次 | solver全局tie-break与flat oracle一致；hard factor只表达exact legality；soft cost分别投影NE、Vector/CT、SPM movement和instruction-control；bufferization恰一次且在movement前；每个observable result直接写唯一actual destination；仅因DPS缺失产生的冗余DDR→DDR publication copy为0，必要copy有exact SSA/alias/effect witness；movement closure后未分类`memref.copy`为0；删除Instr copy-only TileRegion/staging fallback；same-layout/unused op为0、shared conversion一个SSA、exact alias零copy；fresh baseline source经第10→9→8→7→6项成功 | 读AGENTS/progress→读06–11/19及本项矩阵→读NE/CT与DDR/SPM事实→调研PBQP、resource-aware projection、One-Shot Bufferization destination-passing和function output→查pinned `BufferizableOpInterface`/DPS API→修复solver/layout/output binding并删除copy fallback→fresh solver oracle、copy inventory及baseline全链→重读设计/硬件/MLIR复审→更新并提交 |
