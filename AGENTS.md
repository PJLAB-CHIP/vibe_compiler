...
1. **用户当前明确、无歧义的要求**。
2. **本文件规定的工作流程和仓库规范**：用户目标决定“做什么”，本文件流程决定“怎么做”。
   除非用户明确要求只讨论、不落文件，执行任务时不能跳过上下文读取、设计同步、pipeline contract、
   验证和提交这些流程要求；不能因为赶进度或局部自信把它们降级成建议。
3. **任务的完成标准**：文档一致、代码能编译、测试能通过、功能真的工作。
...
每次进入执行阶段，先按本流程推进。它是工作方式的约束，不是排在实现之后的收尾建议。

1. **确认工作区**：先看 `git status`，识别已有改动；不得回滚用户或其它工具的无关改动。
2. **读取任务上下文**：按任务范围读取 `AGENTS.md`、`tasks/README.md`、`tasks/progress.md` 任务队列、
   相关编号设计文档，以及必要的 `docs/`、`memory/` 和代码。
3. **确认任务队列位置**：以 `tasks/progress.md` 的任务队列作为执行管控入口；当前要做的事必须能落到
   一个队列项，或者先更新队列和对应设计文档。
4. **先设计后实现**：非小修、非纯文本错别字任务必须先确认或补齐对应编号设计文档和
   pipeline contract，再写代码或工具逻辑。
5. **按 IR / artifact 边界施工**：实现说明和代码改动必须说明消费什么上游 artifact、产出什么下游
   可验证 IR / artifact，不能只围绕 pass、脚本、单个 case 或临时任务号组织。
6. **做匹配验证**：按任务类型运行测试、构建、脚本自检或文本一致性搜索；声称完成前必须有本轮新鲜
   验证结果。
7. **同步记录和提交**：如果任务状态、设计边界或稳定经验变化，同步对应文档和 `memory/`；
   任务收尾必须提交本次相关改动。

如果用户明确要求“先讨论”“别急着改”，只进入讨论状态，不执行文件修改。除此之外，不能用“先做一点
看看”“后面再补文档/验证/提交”替代上述流程。若流程中的某一步因为环境或外部依赖无法完成，必须在
结果中明确说明缺口、已做验证和剩余风险。
...
- `tasks/progress.md`：当前任务队列，记录每步任务的状态、对应设计文档、要做什么、完成要求和
  不算完成的情况。
...
- `tasks/progress.md`：当前任务队列、doing/next/blocked 项、对应设计文档和完成要求。
...
上述冲突优先级用于处理设计、事实和任务状态冲突；不降低本文件中工作流程硬约束的执行优先级。
遇到冲突时，不要悄悄合并；在修改中收敛成一个清晰边界。`AGENTS.md` 只保存稳定协作规则和
仓库导航，不记录当前动态任务状态、短期路线图状态或还在讨论中的 IR 设计选择。
...
3. 读 `tasks/progress.md`，确认当前任务队列、doing/next/blocked 项、对应设计文档和完成要求。
...
`memory/` 是稳定经验库，不是任务状态板，也不是设计文档替代品。每次收尾时都要判断本轮是否产生
可复用经验；有就写，没有就说明无需沉淀，不要让长期经验停留在对话里。

- `memory/bugs.md`：记录遇到的问题、失败现象、误导来源、根因、修复方式和防复发模式。包括代码 bug、
  构建/测试失败、设计文档误读、任务队列误判、旧经验误导等已经确认的问题。
- `memory/general_dev.md`：记录可复用开发模式、构建/测试命令、调试入口、仓库约定、稳定 workflow 和
  长期注意事项。它回答“以后遇到同类开发任务怎么做”。
- 其它 `memory/` 文件只放明确归类的稳定经验；没有稳定结论时不要新增或硬写。
- 如果本轮发现 `memory/` 和当前编号设计文档、`tasks/progress.md` 任务队列或代码事实冲突，要在同一
  批改动里更新或删除过时 memory，不能继续让旧经验误导后续工作。
- 不要把临时任务状态、个人推测、未验证 workaround、单个 case 的偶然现象或尚未收敛的 IR 设计写进
  `memory/`。这些内容应留在任务队列、设计文档的待讨论问题或本轮汇报里。
...
2. **做验证**：按任务类型运行测试、构建、脚本自检或文本一致性搜索。声称端到端或主线 gate
   通过时，必须确认相关测试实际执行而不是被 `unsupported` / skipped；`ctest passed` 不能替代
   lit unsupported 清单、外部依赖 feature 配置和真实输入链路检查。
3. **沉淀经验**：遇到的问题、根因和修复模式写入 `memory/bugs.md`；开发中可复用的命令、入口、
   检查方式和 workflow 写入 `memory/general_dev.md`；如果经验尚不稳定，不要硬写。
