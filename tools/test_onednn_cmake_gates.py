...
def common_command(args: argparse.Namespace) -> list[str]:
    command = [
        ...
        definition("WAFER_ENABLE_TARGET_NUMERIC_BACKEND", "ON"),
    ]
        ...
        result.extend(
            [
                ConfigurationCase(
                    ...
                    "missing",
                    "missing",
                    "requires a managed oneDNN root",
                ),
                ConfigurationCase(
                    "missing-record",
                    True,
                    "directory",
                    "missing",
                    "requires the completed managed oneDNN dependency record",
                ),
                ConfigurationCase(
                    "invalid-record",
                    True,
                    "directory",
                    "invalid",
                    "oneDNN dependency identity/conformance check failed",
                ),
            ]
        )
    return result
...
def run_case(
    ...
    record = managed_root / "onednn-deps.json"
    ...
    command.extend(
        [
            ...
            definition("WAFER_ONEDNN_DEPS_ROOT", managed_root),
            definition("WAFER_ONEDNN_DEPS_RECORD", record),
        ]
    )
    ...
    print(f"{case.name}: rejected with expected onednn dependency diagnostic")
...
def main() -> int:
    ...
    temporary = pathlib.Path(
        tempfile.mkdtemp(prefix="onednn-cmake-gates-", dir=args.work_root.resolve())
    )
    selected = cases(args)
    try:
    ...
    print(f"checked {len(selected)} fail-closed onednn CMake configurations")
    return 0
