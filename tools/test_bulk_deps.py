...
def make_fixture(root: pathlib.Path) -> tuple[pathlib.Path, dict[str, object]]:
    ...
    record_path = root / "bulk-model-deps.json"
    record_path.write_text(canonical_json(record), encoding="utf-8")
    return record_path, record


class BulkDependencyRecordTest(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory(prefix="wafer-bulk-deps-")
