#!/usr/bin/env python3
"""Exercise fail-closed managed oneDNN configuration with real CMake runs."""
...
def common_command(args: argparse.Namespace) -> list[str]:
    command = [
        ...
        definition("WAFER_ENABLE_BULK_MODEL_DEPS", "ON"),
    ]
        ...
        result.extend(
            [
                ConfigurationCase(
                    ...
                    "missing",
                    "missing",
                    "requires a managed bulk-model root",
                ),
                ConfigurationCase(
                    "missing-record",
                    True,
                    "directory",
                    "missing",
                    "requires the completed managed bulk-model dependency record",
                ),
                ConfigurationCase(
                    "invalid-record",
                    True,
                    "directory",
                    "invalid",
                    "bulk-model dependency identity/conformance check failed",
                ),
            ]
        )
    return result
...
def run_case(
    args: argparse.Namespace, temporary: pathlib.Path, case: ConfigurationCase
) -> None:
    case_root = temporary / case.name
    case_root.mkdir()
    managed_root = case_root / "managed-root"
    record = managed_root / "bulk-model-deps.json"
    ...
    command.extend(
        [
            ...
            definition(
                ...
                "ON" if case.numeric_enabled else "OFF",
            ),
            definition("WAFER_BULK_MODEL_DEPS_ROOT", managed_root),
            definition("WAFER_BULK_MODEL_DEPS_RECORD", record),
        ]
    )
    ...
    expected = " ".join(case.expected_diagnostic.split())
    if expected not in normalized:
        raise RuntimeError(
            f"{case.name}: expected diagnostic {case.expected_diagnostic!r} was absent\n"
            f"command: {' '.join(command)}\n{completed.stdout}"
        )
    print(f"{case.name}: rejected with expected bulk dependency diagnostic")
...
def main() -> int:
    ...
    temporary = pathlib.Path(
        tempfile.mkdtemp(prefix="bulk-cmake-gates-", dir=args.work_root.resolve())
    )
    selected = cases(args)
    try:
    ...
    print(f"checked {len(selected)} fail-closed bulk CMake configurations")
    return 0
