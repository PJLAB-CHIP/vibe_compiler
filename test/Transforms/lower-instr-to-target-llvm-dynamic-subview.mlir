...
// RUN: not wafer-opt --wafer-lower-instr-to-target-llvm='target-profile=wafer-tx81-single-card-kernel-v1' %t/cx.mlir 2>&1 | FileCheck %s --check-prefix=CX
// RUN: not wafer-opt --wafer-lower-instr-to-target-llvm='target-profile=wafer-tx81-single-card-kernel-v1' %t/ncx.mlir 2>&1 | FileCheck %s --check-prefix=NCX
// RUN: not wafer-opt --wafer-lower-instr-to-target-llvm='target-profile=wafer-tx81-single-card-kernel-v1' %t/dynamic-size.mlir 2>&1 | FileCheck %s --check-prefix=SIZE
// RUN: not wafer-opt --wafer-lower-instr-to-target-llvm='target-profile=wafer-tx81-single-card-kernel-v1' %t/dynamic-stride.mlir 2>&1 | FileCheck %s --check-prefix=STRIDE
...
// BOUND: unsupported_target_address: dynamic DDR tensor subview offset #0 requires constant non-negative scf.for bounds and a positive constant step
...
// RANGE: target_geometry_mismatch: dynamic DDR tensor subview dimension #0 may access source coordinate 4 outside static extent 4

//--- spm.mlir

func.func @reject_dynamic_spm_subview() {
  %spm = memref.alloc() {wafer.spm.offset = #wafer.spm_offset<65536>}
...
// SPM: unsupported_target_address: dynamic subview requires matching #wafer.memory<ddr, tensor> source and result types
...
// CX: unsupported_target_address: dynamic subview requires matching #wafer.memory<ddr, tensor> source and result types
...
// NCX: unsupported_target_address: dynamic subview requires matching #wafer.memory<ddr, tensor> source and result types
...
// SIZE: unsupported_target_address: dynamic DDR tensor subview requires static source and result shapes
...
// STRIDE: unsupported_target_address: dynamic DDR tensor subview requires static non-negative sizes and positive strides
