    ...
    ^bb0(%arg0: memref<8xf32, #wafer.memory<ddr, tensor>>):
      ...
      %recv = wafer.tile.peer_recv %recv_buffer
          {peer = 1 : i64, bytes = 32 : i64,
           message = #wafer.dte_message<communication = 41, phase = peer_dataflow, round = 0, slice = 3>}
          : memref<8xf32, #wafer.memory<spm, tensor>> -> !async.token
      %send = wafer.tile.peer_send %send_buffer
          {peer = 1 : i64, bytes = 32 : i64,
           message = #wafer.dte_message<communication = 42, phase = peer_dataflow, round = 0, slice = 3>}
          : memref<8xf32, #wafer.memory<spm, tensor>> -> !async.token
...
// CHECK-SAME: message = #wafer.dte_message<communication = 41, phase = peer_dataflow, round = 0, slice = 3>
// CHECK-SAME: peer = 1 : i64
// CHECK: %[[SEND:.+]] = wafer.instr.dte_send %[[SEND_BUFFER]]
// CHECK-SAME: message = #wafer.dte_message<communication = 42, phase = peer_dataflow, round = 0, slice = 3>
// CHECK-SAME: peer = 1 : i64
