...
func.func @reject_all_reduce_bitpacked_layout(
  ...
  ^bb0(%arg0: memref<4xf32, #wafer.memory<ddr, tensor>>):
    %input = "builtin.unrealized_conversion_cast"()
        : () -> memref<4xi1, #wafer.memory<spm, cx>>
    ...
    %result = wafer.tile.all_reduce #wafer.reduce_kind<sum> %input using %recv
        {local_rank = 0 : i64, group_size = 2 : i64,
         rank_group = array<i64: 0, 1>, bytes = 256 : i64,
...
// CHECK: tile.all_reduce lowering requires a static byte-addressable physical footprint
