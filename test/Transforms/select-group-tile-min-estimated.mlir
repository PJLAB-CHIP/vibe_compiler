...
// RUN: wafer-opt --wafer-select-group-tile='tile-search=min-estimated-time tile-search-effort=quick print-candidate-summary candidate-parallelism=2' %s 2>&1 | FileCheck --implicit-check-not=selected_group --check-prefixes=PARALLEL,IR %s
...
// PARALLEL: wafer.select_group_tile selected group @elementwise_last_dim_1000#0
