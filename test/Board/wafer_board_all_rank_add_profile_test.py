...
def engine_rows() -> list[dict[str, object]]:
    ...
    for engine in ("CT", "NE", "RDMA", "WDMA", "TDMA"):
        rows.append(
            {
                "engine": engine,
                "busy_cycles": 8 if engine == "CT" else 0,
                "busy_cycles_valid": True,
                "activity_window_count": 1 if engine == "CT" else 0,
                "wait_window_cycles": None,
                ...
                "timeline_events": [
                    {
                        "tile": tile,
                        "engine": "CT",
                        "activity_window_cycles": 8,
                        "trace_entry_offset_begin": 1,
                        "trace_entry_offset_end": 9,
                    }
                    for tile in range(HARNESS.RANK_COUNT)
                ],
            },
...
class AllRankAddProfileGateTest(unittest.TestCase):
    ...
    def test_rejects_bad_permissions_and_negative_cycles(self) -> None:
        fixture = ProfileReportFixture(self.root)
