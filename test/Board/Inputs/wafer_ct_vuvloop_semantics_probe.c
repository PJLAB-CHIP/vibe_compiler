...
static void wafer_ctvl_write_record(volatile uint64_t *record) {
...
static uint32_t wafer_ctvl_decode(const volatile uint64_t *request,
  ...
  if (request[WAFER_CTVL_REQ_MAGIC] != WAFER_CTVL_REQUEST_MAGIC ||
      request[WAFER_CTVL_REQ_SCHEMA_AND_WORDS] !=
          (((uint64_t)WAFER_CTVL_SCHEMA << 32) | WAFER_CTVL_REQUEST_WORDS) ||
      request[WAFER_CTVL_REQ_OPCODE] != WAFER_CTVL_OPCODE ||
      request[WAFER_CTVL_REQ_DTYPE] != WAFER_CTVL_DTYPE ||
      request[WAFER_CTVL_REQ_LHS_OWNED_BYTES] != WAFER_CTVL_LHS_OWNED_BYTES ||
      request[WAFER_CTVL_REQ_RHS_OWNED_BYTES] != WAFER_CTVL_RHS_OWNED_BYTES ||
  ...
  if (selected->unit_elem_count != 64U ||
      (uint64_t)selected->full_elem_count * selected->unit_elem_count !=
          (uint64_t)selected->elem_count * selected->full_unit_elem_count ||
      request[WAFER_CTVL_REQ_DISPOSITION] != selected->disposition ||
      request[WAFER_CTVL_REQ_ELEM_COUNT] != selected->elem_count ||
      request[WAFER_CTVL_REQ_UNIT_ELEM_COUNT] != selected->unit_elem_count ||
      request[WAFER_CTVL_REQ_FULL_ELEM_COUNT] != selected->full_elem_count ||
...
static uint32_t
wafer_ctvl_output_physical_span(const WaferCTVLSelection *selected) {
  uint32_t bytes = selected->full_elem_count * WAFER_CTVL_ELEMENT_BYTES;
...
static uint64_t wafer_ctvl_issue(const WaferCTVLSelection *selected) {
  ...
  uint32_t lhs = (uint32_t)(WAFER_CTVL_SPM_LHS + WAFER_CTVL_BODY_OFFSET);
  uint32_t rhs = (uint32_t)(WAFER_CTVL_SPM_RHS + WAFER_CTVL_BODY_OFFSET);
  uint32_t output = (uint32_t)(WAFER_CTVL_SPM_OUTPUT + WAFER_CTVL_BODY_OFFSET);
  ...
  instruction.param.full_unit_elem_count = selected->full_unit_elem_count;
  return TsmExecute(&instruction);
}

static void wafer_ctvl_init_record(volatile uint64_t *record, uint32_t status) {
  ...
  record[WAFER_CTVL_REC_SCHEMA_AND_WORDS] =
      ((uint64_t)WAFER_CTVL_SCHEMA << 32) | WAFER_CTVL_RECORD_WORDS;
  record[WAFER_CTVL_REC_STATUS] = status;
  record[WAFER_CTVL_REC_OUTPUT_DDR_OFFSET] = WAFER_CTVL_OUTPUT_DDR_OFFSET;
  record[WAFER_CTVL_REC_OUTPUT_SLOT_BYTES] = WAFER_CTVL_OUTPUT_SLOT_BYTES;
  record[WAFER_CTVL_REC_BODY_OFFSET] = WAFER_CTVL_BODY_OFFSET;
  record[WAFER_CTVL_REC_OUTPUT_OWNED_BYTES] = WAFER_CTVL_OUTPUT_CAPTURE_BYTES;
...
wafer_tx81_instruction_family_probe(uint64_t request_ddr, uint64_t payload_ddr,
                                    uint64_t output_ddr) {
  wafer_ctvl_invalidate(request_ddr,
                        WAFER_CTVL_REQUEST_WORDS * sizeof(uint64_t));
  ...
  if (status == WAFER_CTVL_STATUS_OK) {
    ...
    record[WAFER_CTVL_REC_UNIT_ELEM_COUNT] = selected.unit_elem_count;
    record[WAFER_CTVL_REC_FULL_ELEM_COUNT] = selected.full_elem_count;
    record[WAFER_CTVL_REC_FULL_UNIT_ELEM_COUNT] = selected.full_unit_elem_count;
    record[WAFER_CTVL_REC_SAMPLE] = request[WAFER_CTVL_REQ_SAMPLE];
    record[WAFER_CTVL_REC_REQUEST_GUARD] = request[WAFER_CTVL_REQ_GUARD];
    ...
    wafer_tx81_rdma_v3(payload_ddr + WAFER_CTVL_PAYLOAD_LHS_OFFSET,
                       ...
                       WAFER_CTVL_LHS_SLOT_BYTES, 0, 0, 0, 1, 1, 1, Fmt_UINT8,
                       0U);
    wafer_tx81_rdma_v3(payload_ddr + WAFER_CTVL_PAYLOAD_RHS_OFFSET,
                       WAFER_CTVL_SPM_RHS, WAFER_CTVL_RHS_SLOT_BYTES,
                       WAFER_CTVL_RHS_SLOT_BYTES, 0, 0, 0, 1, 1, 1, Fmt_UINT8,
                       0U);
    wafer_tx81_rdma_v3(payload_ddr + WAFER_CTVL_PAYLOAD_OUTPUT_SEED_OFFSET,
    ...
    wafer_tx81_wdma_v3(
        WAFER_CTVL_SPM_OUTPUT, output_ddr + WAFER_CTVL_OUTPUT_DDR_OFFSET,
        WAFER_CTVL_OUTPUT_SLOT_BYTES, WAFER_CTVL_OUTPUT_SLOT_BYTES, 0, 0, 0, 1,
        1, 1, Fmt_UINT8, 0U);
  ...
  wafer_ctvl_write_record(record);
}
