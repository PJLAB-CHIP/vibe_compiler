...
#ifndef WAFER_IFP_SLOTS_PER_TILE
#error "WAFER_IFP_SLOTS_PER_TILE must match the package entry layout"
#endif

__attribute__((visibility("hidden")))
const uint64_t wafer_ifp_slots_per_tile = WAFER_IFP_SLOTS_PER_TILE;
...
static void wafer_ifp_copy_spm_bytes_ncc(uint64_t destination, uint64_t source,
   ...
   */
  wafer_tx81_gather_scatter(source, destination, bytes, bytes, 0U, 0U, 0U,
...
wafer_ifp_dispatch_reduce_capability(const WaferIFPDescriptor *descriptor,
  ...
  switch (operation) {
  case 0:
    wafer_tx81_reduce_sum(input, output, dimension, n, h, w, c, format, 0U);
    break;
  case 1:
    wafer_tx81_reduce_avg(input, output, dimension, n, h, w, c, format, 0U);
    break;
  case 2:
    wafer_tx81_reduce_max(input, output, dimension, n, h, w, c, format, 0U);
    break;
  case 3:
    wafer_tx81_reduce_min(input, output, dimension, n, h, w, c, format, 0U);
    break;
  default:
    return 0;
  }
...
wafer_ifp_dispatch_pool_capability(const WaferIFPDescriptor *descriptor,
  ...
  switch (operation) {
  case 0:
    wafer_tx81_pool_avg(input, output, OP_FUNC_CGRATensor_PoolOp_T_T_avg, 1,
                           src_h, src_w, 64, 1, dst_h, dst_w, 64, pad_top,
                           pad_bottom, pad_left, pad_right, kernel_x, kernel_y,
                           stride_x, stride_y, format, 0U);
    break;
  case 1:
    wafer_tx81_pool_sum(input, output, OP_FUNC_CGRATensor_PoolOp_T_T_sum, 1,
                           src_h, src_w, 64, 1, dst_h, dst_w, 64, pad_top,
                           pad_bottom, pad_left, pad_right, kernel_x, kernel_y,
                           stride_x, stride_y, format, 0U);
    break;
  case 2:
    wafer_tx81_pool_max(input, output, OP_FUNC_CGRATensor_PoolOp_T_T_max, 1,
                           src_h, src_w, 64, 1, dst_h, dst_w, 64, pad_top,
                           pad_bottom, pad_left, pad_right, kernel_x, kernel_y,
                           stride_x, stride_y, format, 0U);
    break;
  case 3:
    ...
    break;
  case 4:
    wafer_tx81_pool_min(input, output, OP_FUNC_CGRATensor_PoolOp_T_T_min, 1,
                           src_h, src_w, 64, 1, dst_h, dst_w, 64, pad_top,
                           pad_bottom, pad_left, pad_right, kernel_x, kernel_y,
                           stride_x, stride_y, format, 0U);
    break;
...
static int wafer_ifp_dispatch(const WaferIFPDescriptor *descriptor,
  ...
  case WAFER_IFP_CASE_CT_NEG_BF16:
    wafer_tx81_elementwise_neg(input_a, output, elements, format, 0U);
    break;
  ...
  case WAFER_IFP_CASE_CT_ADD_F16_TAIL130:
    wafer_tx81_elementwise_add(input_a, input_b, output, 130, Fmt_FP16, 0U);
    break;
  case WAFER_IFP_CASE_CT_ADD_BF16_TAIL130:
    wafer_tx81_elementwise_add(input_a, input_b, output, 130, Fmt_BF16, 0U);
    break;
  ...
  case WAFER_IFP_CASE_CT_POW2_BF16:
    wafer_tx81_elementwise_pow2(input_a, output, elements, format, 0U);
    break;
  case WAFER_IFP_CASE_CT_RELU_F16:
  case WAFER_IFP_CASE_CT_RELU_BF16:
    wafer_tx81_elementwise_relu(input_a, output, elements, format, 0U);
    break;
  case WAFER_IFP_CASE_CONVERT_I8_F16_ZP0:
    wafer_tx81_convert_int8_fp16(input_a, output, elements, 0, 0, 0U);
    break;
  case WAFER_IFP_CASE_CONVERT_I8_BF16_ZP0:
    wafer_tx81_convert_int8_bf16(input_a, output, elements, 0, 0, 0U);
    break;
  case WAFER_IFP_CASE_CONVERT_BF16_F16_PLAIN:
    wafer_tx81_convert_bf16_fp16(input_a, output, elements, 0, 0, 0U);
    break;
  case WAFER_IFP_CASE_CONVERT_F16_BF16_ROUND:
    wafer_tx81_convert_fp16_bf16(input_a, output, elements, 0, 0, 0U);
    break;
  case WAFER_IFP_CASE_CONVERT_F16_I16_ROUND:
    wafer_tx81_convert_fp16_int16(input_a, output, elements, 0, 0, 0U);
    break;
  case WAFER_IFP_CASE_REDUCE_SUM_F16:
    wafer_tx81_reduce_sum(input_a, output, 1, 1, 1, 4, 64, Fmt_FP16, 0U);
    break;
  case WAFER_IFP_CASE_REDUCE_MAX_F16:
    wafer_tx81_reduce_max(input_a, output, 1, 1, 1, 4, 64, Fmt_FP16, 0U);
    break;
  case WAFER_IFP_CASE_REDUCE_MIN_BF16:
    wafer_tx81_reduce_min(input_a, output, 1, 1, 1, 4, 64, Fmt_BF16, 0U);
    break;
  case WAFER_IFP_CASE_REDUCE_AVG_BF16:
    wafer_tx81_reduce_avg(input_a, output, 1, 1, 1, 4, 64, Fmt_BF16, 0U);
    break;
  case WAFER_IFP_CASE_SELECT_F16:
  case WAFER_IFP_CASE_SELECT_BF16:
    wafer_tx81_bit2fp(input_b, auxiliary, elements, format, 0U);
  ...
  case WAFER_IFP_CASE_GEMM_F16:
    wafer_tx81_gemm(input_a, input_b, output, 1, 16, 16, 1, Fmt_FP16, 0U);
    break;
  case WAFER_IFP_CASE_GEMM_F16_ACCUM_ROUND:
    wafer_tx81_gemm(input_a, input_b, output, 1, 16, 16, 1, Fmt_FP16, 0U);
    break;
  case WAFER_IFP_CASE_GEMM_F16_M4:
    wafer_tx81_gemm(input_a, input_b, output, 4, 16, 16, 1, Fmt_FP16, 0U);
    break;
  case WAFER_IFP_CASE_GEMM_F16_BATCH2_M8:
    wafer_tx81_gemm(input_a, input_b, output, 8, 16, 16, 2, Fmt_FP16, 0U);
    break;
  case WAFER_IFP_CASE_GEMM_F16_N17:
    wafer_tx81_gemm(input_a, input_b, output, 1, 16, 17, 1, Fmt_FP16, 0U);
    break;
  case WAFER_IFP_CASE_GEMM_F16_K17:
    wafer_tx81_gemm(input_a, input_b, output, 1, 17, 16, 1, Fmt_FP16, 0U);
    break;
  case WAFER_IFP_CASE_GEMM_F16_N65:
    wafer_tx81_gemm(input_a, input_b, output, 1, 16, 65, 1, Fmt_FP16, 0U);
    break;
  ...
  case WAFER_IFP_CASE_GEMM_BF16_ACCUM_ROUND:
    wafer_tx81_gemm(input_a, input_b, output, 1, 16, 16, 1, Fmt_BF16, 0U);
    break;
  case WAFER_IFP_CASE_GEMM_BF16_BATCH2_M8:
    wafer_tx81_gemm(input_a, input_b, output, 8, 16, 16, 2, Fmt_BF16, 0U);
    break;
  case WAFER_IFP_CASE_GEMM_BF16_K17:
    wafer_tx81_gemm(input_a, input_b, output, 1, 17, 16, 1, Fmt_BF16, 0U);
    break;
  case WAFER_IFP_CASE_GEMM_BF16_N65:
    wafer_tx81_gemm(input_a, input_b, output, 1, 16, 65, 1, Fmt_BF16, 0U);
    break;
  ...
  case WAFER_IFP_CASE_TDMA_IMG2COL_BF16:
    wafer_tx81_tdma_img2col(input_a, output, 1, 3, 3, 64, 1, 4, 4, 64, 0, 0,
  ...
  case WAFER_IFP_CASE_POOL_BF16:
    wafer_tx81_pool_max(input_a, output, OP_FUNC_CGRATensor_PoolOp_T_T_max,
                           1, 2, 4, 64, 1, 1, 2, 64, 0, 0, 0, 0, 2, 2, 2, 2,
                           format, 0U);
    break;
  case WAFER_IFP_CASE_POOL_AVG_F16:
    wafer_tx81_pool_avg(input_a, output, OP_FUNC_CGRATensor_PoolOp_T_T_avg,
                           1, 2, 4, 64, 1, 1, 2, 64, 0, 0, 0, 0, 2, 2, 2, 2,
                           Fmt_FP16, 0U);
    break;
  case WAFER_IFP_CASE_POOL_SUM_F16:
    wafer_tx81_pool_sum(input_a, output, OP_FUNC_CGRATensor_PoolOp_T_T_sum,
                           1, 2, 4, 64, 1, 1, 2, 64, 0, 0, 0, 0, 2, 2, 2, 2,
                           Fmt_FP16, 0U);
    break;
  case WAFER_IFP_CASE_POOL_MIN_F16:
    wafer_tx81_pool_min(input_a, output, OP_FUNC_CGRATensor_PoolOp_T_T_min,
                           1, 2, 4, 64, 1, 1, 2, 64, 0, 0, 0, 0, 2, 2, 2, 2,
                           Fmt_FP16, 0U);
    break;
  case WAFER_IFP_CASE_POOL_INDEXED_MIN_F16:
    wafer_tx81_pool_indexedmin(input_a, output, output + 256U,
  ...
  case WAFER_IFP_CASE_POOL_INDEXED_MAX_F16_ASYMMETRIC:
    wafer_tx81_pool_indexedmax(input_a, output, output + 512U,
  ...
  case WAFER_IFP_CASE_UNPOOL_AVG_F32:
    wafer_tx81_unpool_avg(input_a, output,
  ...
  case WAFER_IFP_CASE_UNPOOL_INDEX_F16_ASYMMETRIC_OBSERVED:
    ...
    wafer_tx81_unpool_unpool(input_b, output,
  ...
  case WAFER_IFP_CASE_UNPOOL_MASK_F16_ASYMMETRIC:
    wafer_tx81_pool_indexedmax(
        input_a, input_b, auxiliary, OP_FUNC_CGRATensor_PoolOp_T_T_indexedmax,
        1, 3, 5, 64, 1, 2, 2, 64, 0, 0, 0, 0, 3, 2, 2, 1, Fmt_FP16, 0U);
    wafer_tx81_unpool_mask(input_b, output,
  ...
  case WAFER_IFP_CASE_PERIPHERAL_ARGMAX_F16:
    wafer_tx81_peripheral_argmax(input_a, output, output + 4,
  ...
  case WAFER_IFP_CASE_PERIPHERAL_ARGMIN_NEGATIVE_F16_OBSERVED:
    wafer_tx81_peripheral_argmin(input_a, output, output + 4,
  ...
  case WAFER_IFP_CASE_PERIPHERAL_LUT16_F16:
    wafer_tx81_peripheral_lut16(input_a, input_b, output,
  ...
  case WAFER_IFP_CASE_PERIPHERAL_BILINEAR_F16:
    wafer_tx81_peripheral_bilinear(
        ...
        Fmt_FP16, 1, 1, 2, 64, 1, 1, 2, 64, 0, 0, 0, 0, 0U);
    break;
  ...
  case WAFER_IFP_CASE_PERIPHERAL_LUT32_OBSERVED:
    wafer_tx81_peripheral_lut32(input_a, input_b, output,
  ...
  case WAFER_IFP_CASE_PERIPHERAL_RANDGEN_F16_OBSERVED:
    wafer_tx81_peripheral_rand_gen(input_a, input_b, output, output + 512U,
                                      output + 1024U,
  ...
  case WAFER_IFP_CASE_PERIPHERAL_ELEMMASK_F16_OBSERVED:
    wafer_tx81_peripheral_elem_mask(
        ...
        Fmt_FP16, 0, UINT32_C(0x00003c00), 50U, RND_STOCHASTIC, 0U);
    break;
  default:
    return 1;
  }
...
static uint32_t wafer_ifp_decode(const volatile uint64_t *request,
  ...
  if (request[WAFER_IFP_REQ_MAGIC] != WAFER_IFP_REQUEST_MAGIC ||
      request[WAFER_IFP_REQ_WORD_COUNT] !=
          (WAFER_IFP_REQUEST_WORDS) ||
...
static void wafer_ifp_init_record(volatile uint64_t *record, uint32_t status) {
  ...
  record[WAFER_IFP_REC_WORD_COUNT] =
      WAFER_IFP_RECORD_WORDS;
