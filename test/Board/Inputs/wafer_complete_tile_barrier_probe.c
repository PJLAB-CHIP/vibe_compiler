...
__attribute__((visibility("hidden"))) const uint64_t wafer_barrier_status_slot =
    WAFER_BARRIER_STATUS_SLOT;
...
wafer_tx81_complete_tile_barrier_probe(uint32_t tile_id, uint64_t input_ddr,
  ...
  *epoch2 = wafer_barrier_epoch2_marker(tile_id);
  ...
  uint64_t status = request_valid == 0 ? WAFER_BARRIER_STATUS_INVALID_REQUEST
