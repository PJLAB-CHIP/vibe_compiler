...
static void wafer_ctv_write_record(volatile uint64_t *record) {
...
static uint32_t wafer_ctv_special_ordinal(uint32_t opcode, uint32_t domain) {
...
static uint32_t wafer_ctv_domain_disposition(uint32_t opcode, uint32_t domain) {
...
static uint32_t wafer_ctv_bool_output(uint32_t opcode) {
  return ((opcode >= 30U && opcode <= 77U &&
           ((opcode - 30U) % 8U == 1U || (opcode - 30U) % 8U == 3U ||
            (opcode - 30U) % 8U == 6U || (opcode - 30U) % 8U == 7U)) ||
          (opcode >= 88U && opcode <= 97U));
}

static uint32_t wafer_ctv_vs(uint32_t opcode) {
  return ((opcode >= 6U && opcode <= 29U && (opcode - 6U) % 4U == 1U) ||
          (opcode >= 30U && opcode <= 77U &&
           ((opcode - 30U) % 8U == 2U || (opcode - 30U) % 8U == 3U)));
}

static uint32_t wafer_ctv_vuv(uint32_t opcode) {
  return ((opcode >= 6U && opcode <= 29U &&
           ((opcode - 6U) % 4U == 2U || (opcode - 6U) % 4U == 3U)) ||
          (opcode >= 30U && opcode <= 77U && (opcode - 30U) % 8U >= 4U) ||
          (opcode >= 82U && opcode <= 87U) || (opcode >= 92U && opcode <= 97U));
}

static uint32_t wafer_ctv_loop(uint32_t opcode) {
  return ((opcode >= 6U && opcode <= 29U && (opcode - 6U) % 4U == 3U) ||
          (opcode >= 30U && opcode <= 77U &&
           ((opcode - 30U) % 8U == 5U || (opcode - 30U) % 8U == 7U)) ||
          (opcode >= 85U && opcode <= 87U) || (opcode >= 95U && opcode <= 97U));
}
...
static uint32_t wafer_ctv_scalar_bits(uint32_t dtype) {
  return dtype == WAFER_CTV_F32 ? UINT32_C(0x40000000) : UINT32_C(0x4000);
}
...
static uint32_t wafer_ctv_decode(const volatile uint64_t *request,
  ...
  uint32_t tail_elements = is_loop != 0 ? WAFER_CTV_TAIL_LOOP_FULL_ELEMENTS
                                        : WAFER_CTV_TAIL_ELEMENTS;
  if (opcode > 110U || dtype > WAFER_CTV_BOOL ||
      (elements != WAFER_CTV_MAIN_ELEMENTS && elements != tail_elements) ||
  ...
  if (domain == WAFER_CTV_DOMAIN_NORMAL) {
    uint32_t dtype_ordinal = dtype == WAFER_CTV_BOOL ? WAFER_CTV_F16 : dtype;
    expected_case = WAFER_CTV_CASE_BASE + opcode * WAFER_CTV_CASE_STRIDE +
                    dtype_ordinal * 2U + shape;
  } else {
    uint32_t special_ordinal = wafer_ctv_special_ordinal(opcode, domain);
    if (special_ordinal == UINT32_MAX || elements != WAFER_CTV_MAIN_ELEMENTS)
      return WAFER_CTV_STATUS_UNSUPPORTED_CASE;
    expected_case = WAFER_CTV_SPECIAL_CASE_BASE + special_ordinal * 3U + dtype;
  }
  ...
  uint32_t disposition = wafer_ctv_domain_disposition(opcode, domain);
  ...
  uint32_t unit_elements = is_loop != 0 ? WAFER_CTV_VUVLOOP_UNIT_ELEMENTS
                                        : (elements == WAFER_CTV_MAIN_ELEMENTS
                                               ? WAFER_CTV_MAIN_UNIT_ELEMENTS
                                               : WAFER_CTV_TAIL_UNIT_ELEMENTS);
  uint32_t elem_count = is_loop == 0 ? elements
                                     : (elements == WAFER_CTV_MAIN_ELEMENTS
  ...
  if (wafer_ctv_has_second_operand(opcode) != 0) {
    uint32_t second_elements =
        is_vuv == 0 ? elements
                    : (is_loop != 0 ? full_unit_elements : unit_elements);
...
static uint64_t wafer_ctv_issue(const WaferCTVCase *selected) {
  ...
  uint32_t output = (uint32_t)(WAFER_CTV_SPM_OUTPUT + WAFER_CTV_BODY_OFFSET);
  instruction.inter_type = I_CGRA | I_WORKER0;
  instruction.ctrl.rnd_mode = RND_NEAREST_EVEN;
  instruction.ctrl.src0_format = (uint8_t)wafer_ctv_format(selected->dtype);
  ...
  instruction.param.dst0_end = output + selected->output_span - 1U;
  ...
  if (selected->full_elements != 0) {
    ...
    instruction.param.full_unit_elem_count = selected->full_unit_elements;
  }
  return TsmExecute(&instruction);
}

static void wafer_ctv_init_record(volatile uint64_t *record, uint32_t status) {
...
wafer_tx81_instruction_family_probe(uint64_t request_ddr, uint64_t payload_ddr,
  ...
  if (status == WAFER_CTV_STATUS_OK) {
    ...
    record[WAFER_CTV_REC_REQUEST_GUARD] = request[WAFER_CTV_REQ_GUARD];
    record[WAFER_CTV_REC_INPUT_A_BYTES] = selected.input_a_bytes;
    record[WAFER_CTV_REC_INPUT_B_BYTES] = selected.input_b_bytes;
    record[WAFER_CTV_REC_SCALAR_BITS] = selected.scalar_bits;
    record[WAFER_CTV_REC_UNIT_ELEMENTS] = selected.unit_elements;
