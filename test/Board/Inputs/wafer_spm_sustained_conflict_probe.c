...
static void wafer_ssc_init_record(volatile uint64_t *record, uint32_t status) {
...
static volatile uint64_t *wafer_ssc_row_record(volatile uint64_t *record,
                                               uint32_t row) {
  return record + WAFER_SSC_ROW_RECORD_BASE_WORD +
         row * WAFER_SSC_ROW_RECORD_WORDS;
}

static void wafer_ssc_init_row(volatile uint64_t *row_record, uint32_t status) {
  ...
  row_record[WAFER_SSC_ROW_PORT_RESPONSE_GUARD] = WAFER_SSC_PORT_RESPONSE_GUARD;
...
static uint32_t wafer_ssc_decode(const volatile uint64_t *wire,
  ...
  if (wire[WAFER_SSC_REQ_MAGIC] != WAFER_SSC_REQUEST_MAGIC ||
      ...
      wire[WAFER_SSC_REQ_SPM_CELL_STRIDE] != WAFER_SSC_SPM_CELL_STRIDE ||
      wire[WAFER_SSC_REQ_SPM_READ0_OFFSET] != WAFER_SSC_SPM_READ0_OFFSET ||
      wire[WAFER_SSC_REQ_SPM_READ1_OFFSET] != WAFER_SSC_SPM_READ1_OFFSET ||
      wire[WAFER_SSC_REQ_SPM_WRITE_OFFSET] != WAFER_SSC_SPM_WRITE_OFFSET ||
      wire[WAFER_SSC_REQ_CANDIDATE_OFFSET] != WAFER_SSC_CANDIDATE_OFFSET ||
      wire[WAFER_SSC_REQ_CONTROL_OFFSET] != WAFER_SSC_CONTROL_OFFSET ||
      wire[WAFER_SSC_REQ_RESOURCE_BYTES] != WAFER_SSC_RESOURCE_BYTES ||
      ...
      wire[WAFER_SSC_REQ_PORT_RESPONSE_BASE] != WAFER_SSC_PORT_RESPONSE_BASE ||
      wire[WAFER_SSC_REQ_PORT_RESPONSE_STRIDE] !=
          WAFER_SSC_PORT_RESPONSE_STRIDE ||
      wire[WAFER_SSC_REQ_SPM_PORT_PMU_BASE] != WAFER_SSC_SPM_PORT_PMU_BASE ||
      ...
      wire[WAFER_SSC_REQ_ROW_RECORD_WORDS] != WAFER_SSC_ROW_RECORD_WORDS ||
      wire[WAFER_SSC_REQ_RESOURCE_CANARY] != WAFER_SSC_RESOURCE_CANARY ||
  ...
  uint32_t work_index = request->work_bytes == 4096U
                            ? 0U
                            : (request->work_bytes == 16384U ? 1U : UINT32_MAX);
  if (work_index == UINT32_MAX ||
      request->rounds != request->work_bytes / WAFER_SSC_TRANSFER_BYTES ||
      request->rounds == 0U || request->rounds > WAFER_SSC_MAX_ROUNDS ||
      request->issue_order > 1U || request->sample >= WAFER_SSC_REPEATS ||
    ...
    return WAFER_SSC_STATUS_BAD_REQUEST;
  uint64_t owned_bytes = (uint64_t)request->rounds * WAFER_SSC_SPM_CELL_STRIDE;
  if (WAFER_SSC_SPM_BASE < WAFER_MDC_SPM_ALLOCATABLE_BEGIN ||
      owned_bytes == 0U ||
      WAFER_SSC_SPM_BASE + owned_bytes > WAFER_MDC_SPM_ALLOCATABLE_END ||
      WAFER_SSC_OUTPUT_ARCHIVE_BASE +
              WAFER_SSC_ROW_COUNT * WAFER_SSC_OUTPUT_ARCHIVE_STRIDE >
          WAFER_SSC_PORT_RESPONSE_BASE ||
      WAFER_SSC_OUTPUT_ARCHIVE_STRIDE < owned_bytes ||
      WAFER_SSC_PORT_RESPONSE_BASE +
              WAFER_SSC_ROW_COUNT * WAFER_SSC_PORT_RESPONSE_STRIDE >
...
static uint32_t wafer_ssc_spm_port_pmu_read32(uint32_t offset) {
  return *(const volatile uint32_t *)(uintptr_t)(WAFER_SSC_SPM_PORT_PMU_BASE +
                                                 offset);
}

static void wafer_ssc_spm_port_pmu_write32(uint32_t offset, uint32_t value) {
  *(volatile uint32_t *)(uintptr_t)(WAFER_SSC_SPM_PORT_PMU_BASE + offset) =
      value;
}

static uint64_t wafer_ssc_spm_port_pmu_read64(uint32_t low_offset,
                                              uint32_t stability_bit,
                                              uint32_t *stable_mask) {
  uint32_t low = 0U;
  uint32_t high_after = 0U;
  for (uint32_t retry = 0U; retry < WAFER_MDC_STABLE_RETRIES; ++retry) {
    uint32_t high_before = wafer_ssc_spm_port_pmu_read32(low_offset + 4U);
...
static WaferSSCPortPMU wafer_ssc_read_spm_port_pmu(void) {
  ...
  uint32_t enable_before = wafer_ssc_spm_port_pmu_read32(SPM1_PMU_EN);
...
static void wafer_ssc_record_spm_port_pmu(volatile uint64_t *row_record,
                                          const WaferSSCPortPMU *before,
                                          const WaferSSCPortPMU *boundary,
                                          const WaferSSCPortPMU *after) {
  row_record[WAFER_SSC_ROW_SPM_PORT_PMU_ENABLE_BEFORE] = before->enable;
  row_record[WAFER_SSC_ROW_SPM_PORT_PMU_ENABLE_BOUNDARY] = boundary->enable;
  row_record[WAFER_SSC_ROW_SPM_PORT_PMU_ENABLE_AFTER] = after->enable;
  row_record[WAFER_SSC_ROW_SPM_PORT_PMU_STABLE_BEFORE] = before->stable_mask;
  row_record[WAFER_SSC_ROW_SPM_PORT_PMU_STABLE_BOUNDARY] =
      boundary->stable_mask;
  row_record[WAFER_SSC_ROW_SPM_PORT_PMU_STABLE_AFTER] = after->stable_mask;
  row_record[WAFER_SSC_ROW_SPM_PORT0_T2_BEFORE] = before->port0_t2;
  row_record[WAFER_SSC_ROW_SPM_PORT0_T2_BOUNDARY] = boundary->port0_t2;
  row_record[WAFER_SSC_ROW_SPM_PORT0_T2_AFTER] = after->port0_t2;
  row_record[WAFER_SSC_ROW_SPM_PORT0_T3_BEFORE] = before->port0_t3;
  row_record[WAFER_SSC_ROW_SPM_PORT0_T3_BOUNDARY] = boundary->port0_t3;
  row_record[WAFER_SSC_ROW_SPM_PORT0_T3_AFTER] = after->port0_t3;
  row_record[WAFER_SSC_ROW_SPM_PORT6_T2_BEFORE] = before->port6_t2;
  row_record[WAFER_SSC_ROW_SPM_PORT6_T2_BOUNDARY] = boundary->port6_t2;
  row_record[WAFER_SSC_ROW_SPM_PORT6_T2_AFTER] = after->port6_t2;
  row_record[WAFER_SSC_ROW_SPM_PORT6_T3_BEFORE] = before->port6_t3;
  row_record[WAFER_SSC_ROW_SPM_PORT6_T3_BOUNDARY] = boundary->port6_t3;
  row_record[WAFER_SSC_ROW_SPM_PORT6_T3_AFTER] = after->port6_t3;
}

static uint32_t wafer_ssc_restore_spm_port_pmu(uint32_t original_enable,
                                               volatile uint64_t *row_record,
                                               uint32_t *scope_flags) {
  wafer_ssc_set_spm_port_pmu_enable(original_enable);
  uint32_t restored = wafer_ssc_spm_port_pmu_read32(SPM1_PMU_EN);
...
static uint32_t wafer_ssc_fail_work(WaferSSCWorkState *work) {
  ...
  if (wafer_mdc_drain_worker(WAFER_SSC_WORKER, &work->final_control)) {
    work->pending = 0U;
...
static uint32_t wafer_ssc_complete_pending(WaferSSCWorkState *work) {
  if (work->pending == 0U)
    return 1U;
  if (!wafer_mdc_drain_worker(WAFER_SSC_WORKER, &work->final_control)) {
    work->cleanup_attempted = 1U;
...
static void wafer_ssc_record_work_state(volatile uint64_t *row_record,
                                        uint32_t phase, uint32_t accepted_word,
                                        uint32_t pending_word,
                                        uint32_t final_control_word,
                                        const WaferSSCWorkState *work) {
...
static uint32_t wafer_ssc_issue_setup_rdma(uint64_t source,
  ...
  rdma->ConfigStrideIteration(&packet, bytes, 0U, 1U, 0U, 1U, 0U, 1U);
  wafer_mdc_set_packet_worker(&packet.inter_type, WAFER_SSC_WORKER);
  TsmDeleteRdma(rdma);
...
static uint32_t wafer_ssc_issue_archive_wdma(uint64_t source,
  ...
  wdma->ConfigStrideIteration(&packet, bytes, 0U, 1U, 0U, 1U, 0U, 1U);
  wafer_mdc_set_packet_worker(&packet.inter_type, WAFER_SSC_WORKER);
  TsmDeleteWdma(wdma);
  if (TsmExecute(&packet) != 1)
    return wafer_ssc_fail_work(work);
...
static uint32_t wafer_ssc_issue_seed_chunk(uint64_t payload_ddr, uint64_t spm,
                                           uint32_t bytes,
                                           WaferSSCWorkState *work) {
  if (!wafer_ssc_issue_setup_rdma(payload_ddr + WAFER_SSC_PAYLOAD_CANARY_OFFSET,
                                  spm, bytes, work))
    return 0U;
...
static uint32_t wafer_ssc_seed_owned_spm(const WaferSSCRequest *request,
  ...
  while (cursor < owned_bytes) {
    uint32_t chunk = owned_bytes - cursor > WAFER_SSC_MAX_DMA_CHUNK
                         ? WAFER_SSC_MAX_DMA_CHUNK
                         : (uint32_t)(owned_bytes - cursor);
    if (!wafer_ssc_issue_seed_chunk(payload_ddr, WAFER_SSC_SPM_BASE + cursor,
                                    chunk, work))
      return 0U;
    cursor += chunk;
  }
  for (uint32_t round = 0; round < request->rounds; ++round) {
    uint64_t cell =
        WAFER_SSC_SPM_BASE + (uint64_t)round * WAFER_SSC_SPM_CELL_STRIDE;
    if (!wafer_ssc_issue_setup_rdma(
            payload_ddr + WAFER_SSC_PAYLOAD_READ0_OFFSET +
                (uint64_t)round * WAFER_SSC_TRANSFER_BYTES,
            cell + WAFER_SSC_SPM_READ0_OFFSET, WAFER_SSC_TRANSFER_BYTES, work))
      return 0U;
    ...
    if (!wafer_ssc_issue_setup_rdma(
            payload_ddr + WAFER_SSC_PAYLOAD_READ1_OFFSET +
                (uint64_t)round * WAFER_SSC_TRANSFER_BYTES,
            cell + WAFER_SSC_SPM_READ1_OFFSET, WAFER_SSC_TRANSFER_BYTES, work))
      return 0U;
    if (work->pending == WAFER_SSC_MAX_PENDING_SETUP &&
        !wafer_ssc_complete_pending(work))
      return 0U;
...
static uint32_t wafer_ssc_prepare_packets(
    ...
    WaferMDCPreparedInstruction packets[2][WAFER_SSC_MAX_ROUNDS]) {
  for (uint32_t round = 0; round < request->rounds; ++round) {
    uint64_t cell =
        WAFER_SSC_SPM_BASE + (uint64_t)round * WAFER_SSC_SPM_CELL_STRIDE;
    ...
    if (arith == 0)
      return 0U;
    arith->AddVV(&packets[0][round].packet.ct, read0, read1, write,
                 WAFER_SSC_TRANSFER_BYTES / sizeof(uint16_t), RND_NEAREST_EVEN,
                 Fmt_FP16);
    wafer_mdc_set_packet_worker(&packets[0][round].packet.ct.inter_type,
                                WAFER_SSC_WORKER);
    ...
    if (rdma == 0)
      return 0U;
    rdma->AddSrcDst(&packets[1][round].packet.rdma,
                    payload_ddr + WAFER_SSC_PAYLOAD_RDMA_OFFSET +
                        (uint64_t)round * WAFER_SSC_TRANSFER_BYTES,
                    write + relative_offset, Fmt_UINT8);
    rdma->ConfigStrideIteration(&packets[1][round].packet.rdma,
                                WAFER_SSC_TRANSFER_BYTES, 0U, 1U, 0U, 1U, 0U,
                                1U);
    wafer_mdc_set_packet_worker(&packets[1][round].packet.rdma.inter_type,
                                WAFER_SSC_WORKER);
    TsmDeleteRdma(rdma);
  }
  return 1U;
...
static uint32_t wafer_ssc_prepare_matched_wdma(
  ...
  for (uint32_t round = 0; round < request->rounds; ++round) {
    uint64_t cell =
        WAFER_SSC_SPM_BASE + (uint64_t)round * WAFER_SSC_SPM_CELL_STRIDE;
    uint64_t source = cell + WAFER_SSC_SPM_WRITE_OFFSET + relative_offset;
    packets[round].engine = WAFER_MDC_ENGINE_WDMA;
    TsmWdma *wdma = TsmNewWdma();
    if (wdma == 0)
      return 0U;
    wdma->AddSrcDst(&packets[round].packet.wdma, source,
                    output_ddr + readback_offset +
                        (uint64_t)round * WAFER_SSC_TRANSFER_BYTES,
                    Fmt_UINT8);
    wdma->ConfigStrideIteration(&packets[round].packet.wdma,
                                WAFER_SSC_TRANSFER_BYTES, 0U, 1U, 0U, 1U, 0U,
                                1U);
    wafer_mdc_set_packet_worker(&packets[round].packet.wdma.inter_type,
                                WAFER_SSC_WORKER);
    TsmDeleteWdma(wdma);
  }
  return 1U;
...
static uint32_t wafer_ssc_issue_lane(
    WaferMDCPreparedInstruction packets[2][WAFER_SSC_MAX_ROUNDS], uint32_t lane,
    uint32_t rounds, WaferSSCWorkState *work) {
  for (uint32_t round = 0; round < rounds; ++round) {
    if (TsmExecute(wafer_mdc_prepared_packet(&packets[lane][round])) != 1)
      return wafer_ssc_fail_work(work);
    wafer_ssc_track_accepted(work);
  }
  return 1U;
...
static uint32_t wafer_ssc_issue_measured(
  ...
  if (schedule == WAFER_MDC_SCHEDULE_SERIAL) {
    if (!wafer_ssc_issue_lane(packets, first, request->rounds, work) ||
        !wafer_ssc_complete_pending(work) ||
        !wafer_ssc_issue_lane(packets, second, request->rounds, work) ||
        !wafer_ssc_complete_pending(work))
      return 0U;
    return 1U;
  }
  for (uint32_t round = 0; round < request->rounds; ++round) {
    if (TsmExecute(wafer_mdc_prepared_packet(&packets[first][round])) != 1)
      return wafer_ssc_fail_work(work);
    wafer_ssc_track_accepted(work);
    if (TsmExecute(wafer_mdc_prepared_packet(&packets[second][round])) != 1)
      return wafer_ssc_fail_work(work);
    wafer_ssc_track_accepted(work);
  }
  return wafer_ssc_complete_pending(work);
...
static uint32_t wafer_ssc_archive_owned_spm(const WaferSSCRequest *request,
                                            uint64_t output_ddr,
  ...
  while (cursor < owned_bytes) {
    uint32_t chunk = owned_bytes - cursor > WAFER_SSC_MAX_DMA_CHUNK
                         ? WAFER_SSC_MAX_DMA_CHUNK
                         : (uint32_t)(owned_bytes - cursor);
    if (!wafer_ssc_issue_archive_wdma(WAFER_SSC_SPM_BASE + cursor,
                                      output_ddr + archive_offset + cursor,
                                      chunk, work))
      return 0U;
    cursor += chunk;
    if (work->pending == WAFER_SSC_MAX_PENDING_SETUP &&
...
static void wafer_ssc_write_row_contract(
    volatile uint64_t *row_record, const WaferSSCRequest *request, uint32_t row,
    uint32_t ordinal, uint64_t relative_offset, uint32_t schedule,
  ...
  row_record[WAFER_SSC_ROW_TRANSFER_BYTES] = WAFER_SSC_TRANSFER_BYTES;
  row_record[WAFER_SSC_ROW_SPM_BASE] = WAFER_SSC_SPM_BASE;
  row_record[WAFER_SSC_ROW_OWNED_SPM_BYTES] = owned_bytes;
  row_record[WAFER_SSC_ROW_SPM_CELL_STRIDE] = WAFER_SSC_SPM_CELL_STRIDE;
  row_record[WAFER_SSC_ROW_SPM_READ0_OFFSET] = WAFER_SSC_SPM_READ0_OFFSET;
  row_record[WAFER_SSC_ROW_SPM_READ1_OFFSET] = WAFER_SSC_SPM_READ1_OFFSET;
  row_record[WAFER_SSC_ROW_SPM_WRITE_OFFSET] = WAFER_SSC_SPM_WRITE_OFFSET;
  ...
  row_record[WAFER_SSC_ROW_SPM_PORT_PMU_BASE] = WAFER_SSC_SPM_PORT_PMU_BASE;
  ...
  row_record[WAFER_SSC_ROW_PORT_READBACK_BYTES] = request->work_bytes;
}

static uint32_t wafer_ssc_execute_row(const WaferSSCRequest *request,
                                      uint32_t row, uint32_t ordinal,
                                      uint64_t request_ddr,
                                      uint64_t payload_ddr, uint64_t output_ddr,
                                      volatile uint64_t *row_record) {
  uint32_t address_class = row / 2U;
  uint32_t schedule = row % 2U;
  uint64_t relative_offset = address_class == 0U ? WAFER_SSC_CANDIDATE_OFFSET
                                                 : WAFER_SSC_CONTROL_OFFSET;
  wafer_ssc_init_row(row_record, WAFER_SSC_STATUS_PREPARE_FAILED);
  wafer_ssc_write_row_contract(row_record, request, row, ordinal,
                               relative_offset, schedule, request_ddr,
                               payload_ddr, output_ddr);

  WaferSSCWorkState setup_work = {0};
  if (!wafer_ssc_seed_owned_spm(request, payload_ddr, &setup_work)) {
    wafer_ssc_record_work_state(row_record, WAFER_SSC_CLEANUP_SETUP,
                                WAFER_SSC_ROW_SETUP_ACCEPTED,
                                WAFER_SSC_ROW_SETUP_PENDING_AFTER,
                                WAFER_SSC_ROW_SETUP_FINAL_CONTROL, &setup_work);
    row_record[WAFER_SSC_ROW_STATUS] = setup_work.poisoned
  ...
  wafer_ssc_record_work_state(row_record, WAFER_SSC_CLEANUP_SETUP,
                              ...
                              WAFER_SSC_ROW_SETUP_PENDING_AFTER,
                              WAFER_SSC_ROW_SETUP_FINAL_CONTROL, &setup_work);
  WaferMDCPreparedInstruction packets[2][WAFER_SSC_MAX_ROUNDS] = {0};
  WaferMDCPreparedInstruction wdma_packets[WAFER_SSC_MAX_ROUNDS] = {0};
  uint64_t readback_offset = WAFER_SSC_PORT_RESPONSE_BASE +
                             (uint64_t)row * WAFER_SSC_PORT_RESPONSE_STRIDE;
  if (!wafer_ssc_prepare_packets(request, payload_ddr, relative_offset,
                                 packets) ||
      !wafer_ssc_prepare_matched_wdma(request, relative_offset, output_ddr,
                                      readback_offset, wdma_packets)) {
    row_record[WAFER_SSC_ROW_STATUS] = WAFER_SSC_STATUS_PREPARE_FAILED;
    return WAFER_SSC_STATUS_PREPARE_FAILED;
  }

  uint32_t scope_flags = 0U;
  uint32_t original_port_enable = wafer_ssc_spm_port_pmu_read32(SPM1_PMU_EN);
  row_record[WAFER_SSC_ROW_SPM_PORT_PMU_ENABLE_ORIGINAL] = original_port_enable;
  wafer_ssc_set_spm_port_pmu_enable(WAFER_SSC_SPM_PORT_PMU_REQUIRED_ENABLE);
  ...
  if (!(scope_flags & WAFER_SSC_PORT_SCOPE_BEFORE_STABLE)) {
    wafer_ssc_record_spm_port_pmu(row_record, &port_before, &port_boundary,
                                  &port_after);
    (void)wafer_ssc_restore_spm_port_pmu(original_port_enable, row_record,
                                         &scope_flags);
    row_record[WAFER_SSC_ROW_SPM_PORT_PMU_SCOPE_FLAGS] = scope_flags;
    row_record[WAFER_SSC_ROW_STATUS] = WAFER_SSC_STATUS_PMU_SCOPE_FAILED;
  ...
  if (!wafer_ssc_issue_measured(request, schedule, packets, &measured_work)) {
    wafer_ssc_record_work_state(row_record, WAFER_SSC_CLEANUP_MEASURED_PAIR,
    ...
    row_record[WAFER_SSC_ROW_STATUS] =
        measured_work.poisoned ? WAFER_SSC_STATUS_CLEANUP_FAILED
  ...
  if (!(scope_flags & WAFER_SSC_PORT_SCOPE_BOUNDARY_STABLE) ||
      !wafer_ssc_issue_matched_wdma(request, wdma_packets,
                                    &matched_wdma_work)) {
    wafer_ssc_record_work_state(row_record, WAFER_SSC_CLEANUP_MATCHED_WDMA,
                                WAFER_SSC_ROW_MATCHED_WDMA_ACCEPTED,
                                WAFER_SSC_ROW_MATCHED_WDMA_PENDING_AFTER,
                                WAFER_SSC_ROW_WDMA_FINAL_CONTROL,
                                &matched_wdma_work);
    wafer_ssc_record_spm_port_pmu(row_record, &port_before, &port_boundary,
                                  &port_after);
    uint32_t restored = wafer_ssc_restore_spm_port_pmu(
  ...
  for (uint32_t engine = 0; engine < WAFER_SSC_ENGINES; ++engine)
    if (engine != WAFER_MDC_ENGINE_CT && engine != WAFER_MDC_ENGINE_RDMA)
      other_instructions +=
          (uint32_t)(after.instructions[engine] - before.instructions[engine]);
  ...
  row_record[WAFER_SSC_ROW_WORKER_CT_INST_DELTA] =
      (uint32_t)(after.worker_instructions[WAFER_SSC_WORKER]
                                          [WAFER_MDC_ENGINE_CT] -
  ...
  for (uint32_t engine = 0; engine < WAFER_SSC_ENGINES; ++engine)
    if (engine != WAFER_MDC_ENGINE_WDMA)
      wdma_other_instructions += (uint32_t)(wdma_after.instructions[engine] -
                                            wdma_before.instructions[engine]);
  row_record[WAFER_SSC_ROW_WDMA_INST_DELTA] =
      (uint32_t)(wdma_after.instructions[WAFER_MDC_ENGINE_WDMA] -
                 wdma_before.instructions[WAFER_MDC_ENGINE_WDMA]);
  row_record[WAFER_SSC_ROW_WDMA_OTHER_INST_DELTA] = wdma_other_instructions;
  row_record[WAFER_SSC_ROW_WORKER_WDMA_INST_DELTA] =
      (uint32_t)(wdma_after.worker_instructions[WAFER_SSC_WORKER]
  ...
  row_record[WAFER_SSC_ROW_FLAGS] =
      ...
      WAFER_SSC_FLAG_PORT_PMU_RECORDED | WAFER_SSC_FLAG_PORT_MATCHED_READBACK |
      WAFER_SSC_FLAG_PORT_ENABLE_RESTORED | WAFER_SSC_FLAG_CLEANUP_EVIDENCE;

  uint64_t archive_offset = WAFER_SSC_OUTPUT_ARCHIVE_BASE +
                            (uint64_t)row * WAFER_SSC_OUTPUT_ARCHIVE_STRIDE;
  WaferSSCWorkState archive_work = {0};
  if (!wafer_ssc_archive_owned_spm(request, output_ddr, archive_offset,
                                   &archive_work)) {
    wafer_ssc_record_work_state(
        row_record, WAFER_SSC_CLEANUP_ARCHIVE, WAFER_SSC_ROW_ARCHIVE_ACCEPTED,
        WAFER_SSC_ROW_ARCHIVE_PENDING_AFTER,
        WAFER_SSC_ROW_ARCHIVE_FINAL_CONTROL, &archive_work);
    row_record[WAFER_SSC_ROW_STATUS] = archive_work.poisoned
                                           ? WAFER_SSC_STATUS_CLEANUP_FAILED
  ...
  wafer_ssc_record_work_state(
      row_record, WAFER_SSC_CLEANUP_ARCHIVE, WAFER_SSC_ROW_ARCHIVE_ACCEPTED,
      WAFER_SSC_ROW_ARCHIVE_PENDING_AFTER, WAFER_SSC_ROW_ARCHIVE_FINAL_CONTROL,
      &archive_work);
...
wafer_tx81_instruction_family_probe(uint64_t request_ddr, uint64_t payload_ddr,
                                    uint64_t output_ddr) {
  wafer_mdc_cache_range(request_ddr, WAFER_SSC_RESOURCE_BYTES, 1U);
  wafer_mdc_cache_range(payload_ddr, WAFER_SSC_RESOURCE_BYTES, 1U);
  ...
  for (uint32_t row = 0; row < WAFER_SSC_ROW_COUNT; ++row)
    wafer_ssc_init_row(wafer_ssc_row_record(record, row), status);
  ...
  if (status == WAFER_SSC_STATUS_OK) {
    ...
    for (uint32_t ordinal = 0; ordinal < WAFER_SSC_ROW_COUNT; ++ordinal) {
      uint32_t row = (ordinal + request.sample) % WAFER_SSC_ROW_COUNT;
