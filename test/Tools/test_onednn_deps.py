...
def make_fixture(root: pathlib.Path) -> tuple[pathlib.Path, dict[str, object]]:
    ...
    source_top = "oneDNN-" + versions["WAFER_ONEDNN_COMMIT"]
    source = root / "sources" / source_top
    source.mkdir(parents=True)
    (source / "LICENSE").write_text("fixture source\n", encoding="utf-8")
    archive = root / "downloads" / f"oneDNN-{versions['WAFER_ONEDNN_COMMIT']}.tar.gz"
    ...
    for name in sorted(REQUIRED_FILES):
        ...
        if name != "onednn-archive":
            ...
            path.write_bytes(f"managed onednn fixture: {name}\n".encode())
        files[name] = {
    ...
    record_path = root / "onednn-deps.json"
    record_path.write_text(canonical_json(record), encoding="utf-8")
    return record_path, record


class OneDNNDependencyRecordTest(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory(prefix="wafer-onednn-deps-")
        self.root = pathlib.Path(self.temporary.name)
        self.record_path, self.record = make_fixture(self.root)
        self.versions = load_versions()
        self.versions["WAFER_ONEDNN_SHA256"] = self.record["dependency"][
            "archive_sha256"
        ]
    ...
    def test_valid_record_produces_closed_absolute_snapshot(self) -> None:
        ...
        self.assertEqual(pathlib.Path(snapshot["root"]), self.root)
        self.assertTrue(
            all(
                pathlib.Path(item["path"]).is_absolute()
    ...
    def test_changed_source_tree_and_gate_log_are_rejected(self) -> None:
        source = (
            self.root
            / "sources"
            / ("oneDNN-" + self.versions["WAFER_ONEDNN_COMMIT"])
            / "LICENSE"
        )
        ...
        (self.root / first_gate["log"]).write_text(
            "tampered gate log\n", encoding="utf-8"
        )
    ...
    def test_build_option_and_gate_closure_are_rejected(self) -> None:
        changed = copy.deepcopy(self.record)
        changed["build"]["options"]["DNNL_CPU_RUNTIME"] = "OMP"
        self.rewrite(changed)
