...
module {
  ...
  func.func @partial_f16_nt() {
    ...
    } : memref<2x2x1025xf16, #wafer.memory<spm, ncx>>, memref<2x4x1025xf16, #wafer.memory<spm, ncx>> into memref<2x2x4xf16, #wafer.memory<spm, ncx>>
    return
  }

  func.func @partial_f16_tn() {
    ...
    } : memref<2x1025x2xf16, #wafer.memory<spm, ncx>>, memref<2x1025x4xf16, #wafer.memory<spm, ncx>> into memref<2x2x4xf16, #wafer.memory<spm, ncx>>
    return
  }

  func.func @partial_f16_tt() {
    %a = memref.alloc() {wafer.spm.offset = #wafer.spm_offset<65536>} : memref<2x1025x2xf16, #wafer.memory<spm, ncx>>
    %b = memref.alloc() {wafer.spm.offset = #wafer.spm_offset<196608>} : memref<2x4x1025xf16, #wafer.memory<spm, ncx>>
    %c = memref.alloc() {wafer.spm.offset = #wafer.spm_offset<458752>} : memref<2x2x4xf16, #wafer.memory<spm, ncx>>
    %p = memref.alloc() {wafer.spm.offset = #wafer.spm_offset<524288>} : memref<2x2x4xf32, #wafer.memory<spm, ncx>>
    wafer.instr.gemm %a, %b psum(%p : memref<2x2x4xf32, #wafer.memory<spm, ncx>>) into %c {
      ...
      lhs_orientation = #wafer.gemm_orientation<transpose>,
      rhs_orientation = #wafer.gemm_orientation<transpose>
    } : memref<2x1025x2xf16, #wafer.memory<spm, ncx>>, memref<2x4x1025xf16, #wafer.memory<spm, ncx>> into memref<2x2x4xf16, #wafer.memory<spm, ncx>>
    return
  }

  func.func @partial_bf16_nn() {
  ...
  func.func @partial_bf16_nt() {
    ...
    wafer.instr.gemm %a, %b psum(%p : memref<2x2x4xf32, #wafer.memory<spm, ncx>>) into %c {
      ...
      lhs_orientation = #wafer.gemm_orientation<normal>,
      rhs_orientation = #wafer.gemm_orientation<transpose>
    } : memref<2x2x1025xbf16, #wafer.memory<spm, ncx>>, memref<2x4x1025xbf16, #wafer.memory<spm, ncx>> into memref<2x2x4xbf16, #wafer.memory<spm, ncx>>
    return
  }

  func.func @partial_bf16_tn() {
    ...
    wafer.instr.gemm %a, %b psum(%p : memref<2x2x4xf32, #wafer.memory<spm, ncx>>) into %c {
      ...
      lhs_orientation = #wafer.gemm_orientation<transpose>,
      rhs_orientation = #wafer.gemm_orientation<normal>
    } : memref<2x1025x2xbf16, #wafer.memory<spm, ncx>>, memref<2x1025x4xbf16, #wafer.memory<spm, ncx>> into memref<2x2x4xbf16, #wafer.memory<spm, ncx>>
    return
  }

  func.func @partial_bf16_tt() {
    %a = memref.alloc() {wafer.spm.offset = #wafer.spm_offset<65536>} : memref<2x1025x2xbf16, #wafer.memory<spm, ncx>>
    %b = memref.alloc() {wafer.spm.offset = #wafer.spm_offset<196608>} : memref<2x4x1025xbf16, #wafer.memory<spm, ncx>>
    %c = memref.alloc() {wafer.spm.offset = #wafer.spm_offset<458752>} : memref<2x2x4xbf16, #wafer.memory<spm, ncx>>
    %p = memref.alloc() {wafer.spm.offset = #wafer.spm_offset<524288>} : memref<2x2x4xf32, #wafer.memory<spm, ncx>>
    wafer.instr.gemm %a, %b psum(%p : memref<2x2x4xf32, #wafer.memory<spm, ncx>>) into %c {
      ...
      lhs_orientation = #wafer.gemm_orientation<transpose>,
      rhs_orientation = #wafer.gemm_orientation<transpose>
    } : memref<2x1025x2xbf16, #wafer.memory<spm, ncx>>, memref<2x4x1025xbf16, #wafer.memory<spm, ncx>> into memref<2x2x4xbf16, #wafer.memory<spm, ncx>>
    return
  }

}

// CHECK-LABEL: llvm.func @partial_f16_nn
// CHECK: llvm.mlir.constant(4 : i32) : i32
// CHECK-NEXT: %{{.*}} = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[INPUT:.*]] = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[OUTPUT:.*]] = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[PSUM_FORMAT:.*]] = llvm.mlir.constant(5 : i32) : i32
// CHECK: llvm.call @wafer_tx81_gemm({{.*}}, %[[INPUT]], %[[OUTPUT]], %[[PSUM_FORMAT]], {{.*}})

// CHECK-LABEL: llvm.func @partial_f16_nt
// CHECK: llvm.mlir.constant(4 : i32) : i32
// CHECK-NEXT: %{{.*}} = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[INPUT:.*]] = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[OUTPUT:.*]] = llvm.mlir.constant(2 : i32) : i32
...
// CHECK: llvm.call @wafer_tx81_gemm_oriented({{.*}}, %[[INPUT]], %[[OUTPUT]], %[[PSUM_FORMAT]], %[[LHS]], %[[RHS]], {{.*}})

// CHECK-LABEL: llvm.func @partial_f16_tn
// CHECK: llvm.mlir.constant(4 : i32) : i32
// CHECK-NEXT: %{{.*}} = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[INPUT:.*]] = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[OUTPUT:.*]] = llvm.mlir.constant(2 : i32) : i32
...
// CHECK-LABEL: llvm.func @partial_f16_tt
// CHECK: llvm.mlir.constant(4 : i32) : i32
// CHECK-NEXT: %{{.*}} = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[INPUT:.*]] = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[OUTPUT:.*]] = llvm.mlir.constant(2 : i32) : i32
...
// CHECK-LABEL: llvm.func @partial_bf16_nn
// CHECK: llvm.mlir.constant(4 : i32) : i32
// CHECK-NEXT: %{{.*}} = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[INPUT:.*]] = llvm.mlir.constant(3 : i32) : i32
// CHECK-NEXT: %[[OUTPUT:.*]] = llvm.mlir.constant(3 : i32) : i32
// CHECK-NEXT: %[[PSUM_FORMAT:.*]] = llvm.mlir.constant(5 : i32) : i32
// CHECK: llvm.call @wafer_tx81_gemm({{.*}}, %[[INPUT]], %[[OUTPUT]], %[[PSUM_FORMAT]], {{.*}})

// CHECK-LABEL: llvm.func @partial_bf16_nt
// CHECK: llvm.mlir.constant(4 : i32) : i32
// CHECK-NEXT: %{{.*}} = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[INPUT:.*]] = llvm.mlir.constant(3 : i32) : i32
// CHECK-NEXT: %[[OUTPUT:.*]] = llvm.mlir.constant(3 : i32) : i32
...
// CHECK-LABEL: llvm.func @partial_bf16_tn
// CHECK: llvm.mlir.constant(4 : i32) : i32
// CHECK-NEXT: %{{.*}} = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[INPUT:.*]] = llvm.mlir.constant(3 : i32) : i32
// CHECK-NEXT: %[[OUTPUT:.*]] = llvm.mlir.constant(3 : i32) : i32
...
// CHECK-LABEL: llvm.func @partial_bf16_tt
// CHECK: llvm.mlir.constant(4 : i32) : i32
// CHECK-NEXT: %{{.*}} = llvm.mlir.constant(2 : i32) : i32
// CHECK-NEXT: %[[INPUT:.*]] = llvm.mlir.constant(3 : i32) : i32
// CHECK-NEXT: %[[OUTPUT:.*]] = llvm.mlir.constant(3 : i32) : i32
