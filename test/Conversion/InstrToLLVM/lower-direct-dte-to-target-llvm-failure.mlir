// RUN: split-file %s %t
// RUN: not wafer-opt --pass-pipeline='builtin.module(wafer-lower-instr-to-target-llvm{card-id=0 tile-id=0 transport-status-argument-index=0})' %t/missing-binding.mlir 2>&1 | FileCheck %s --check-prefix=MISSING
// RUN: not wafer-opt --pass-pipeline='builtin.module(wafer-lower-instr-to-target-llvm{card-id=0 tile-id=0 transport-status-argument-index=0})' %t/missing-selector.mlir 2>&1 | FileCheck %s --check-prefix=SELECTOR
