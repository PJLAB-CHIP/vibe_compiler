...
mapPartition(const NodeExecutionPartition &source,
             const SpatialRootDomainFacts &sourceFacts,
  ...
  llvm::SmallVector<uint32_t, 4> independentAxes;
  auto inverse = relation.inverse(limits);
  if (inverse.isExact()) {
    auto image = inverse.get()->getRectangularTileImage(
        context, target.iteratorExtents, sourceFacts.iteratorExtents,
        target.iteratorExtents, limits);
    if (image.isExact())
      independentAxes = image.image->invariantDimensions;
  }
  ...
  for (auto [axis, pieces] : llvm::enumerate(intervals)) {
    ...
    if (llvm::is_contained(independentAxes, axis) && pieces.size() == 1 &&
        pieces.front().offset == 0 &&
    ...
    if (cells > availableTiles.size() / pieces.size())
      return std::nullopt;
    cells *= pieces.size();
...
mlir::FailureOr<SpatialPlan> propagateSpatialPartitions(
  ...
  auto propose = [&](const StructuredDAGEdge &edge,
    ...
    return mapPartition(*source, forward ? *p : *c, target,
                        result.nodes[positions.at(target.root)],
                        problem.getStructuralProblem().getAvailableTiles(),
                        *relation, producer->getContext(), limits);
  };
