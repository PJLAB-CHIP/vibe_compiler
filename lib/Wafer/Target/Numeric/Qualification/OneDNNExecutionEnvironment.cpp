//===- OneDNNExecutionEnvironment.cpp - Managed oneDNN environment ----===//
...
#ifndef WAFER_ONEDNN_DEPENDENCY_RECORD_SHA256
#error "managed onednn dependency record identity is required"
#endif
#ifndef WAFER_ONEDNN_LIBRARY_SHA256
#error "managed oneDNN library identity is required"
#endif
#ifndef WAFER_ONEDNN_VERSION
#error "managed oneDNN version is required"
#endif
#ifndef WAFER_ONEDNN_COMMIT
...
createManagedOneDNNExecutionEnvironment() {
  ...
  static std::optional<OneDNNExecutionEnvironment> cached;
  ...
  if (failure)
    return onednnError(
        OneDNNTensorNumericErrorCode::BackendConfigurationFailure, *failure);
  ...
  appendField(backendStream, "schema", "wafer-onednn-backend");
  appendField(backendStream, "name", "oneDNN");
  appendField(backendStream, "version", WAFER_ONEDNN_VERSION);
  appendField(backendStream, "commit", WAFER_ONEDNN_COMMIT);
  const std::string dependencyRecordDigest =
      (llvm::Twine("sha256:") + WAFER_ONEDNN_DEPENDENCY_RECORD_SHA256).str();
  const std::string libraryDigest =
      (llvm::Twine("sha256:") + WAFER_ONEDNN_LIBRARY_SHA256).str();
  appendField(backendStream, "dependency_record", dependencyRecordDigest);
  appendField(backendStream, "library", libraryDigest);
  OneDNNBackendDescriptor backend("oneDNN", WAFER_ONEDNN_VERSION,
                                  WAFER_ONEDNN_COMMIT, dependencyRecordDigest,
  ...
  appendField(environmentStream, "schema", "wafer-onednn-environment");
  ...
  cached = OneDNNExecutionEnvironment(
