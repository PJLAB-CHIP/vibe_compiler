//===- OneDNNQualification.cpp - Canonical qualification files --------===//
...
static constexpr llvm::StringLiteral kCurrentSpecSchema =
    "wafer-onednn-qualification-spec";
...
environmentJSON(const OneDNNExecutionEnvironment &environment) {
  const OneDNNBackendDescriptor &backend = environment.getBackend();
  return llvm::json::Object{
      {"schema", "wafer-onednn-execution-environment"},
...
llvm::Error validateEnvironmentJSON(const llvm::json::Object &object,
  ...
  if (llvm::Error error = requireFields(
          ...
          "onednn execution environment"))
    return error;
  auto string = [&](llvm::StringRef name) {
    return requireString(object, name, "onednn execution environment");
  };
  ...
  llvm::Expected<std::string> dependencyDigest = requireDigest(
      object, "dependency_record_digest", "onednn execution environment");
  llvm::Expected<std::string> libraryDigest =
      requireDigest(object, "library_digest", "onednn execution environment");
  llvm::Expected<std::string> backendDigest =
      requireDigest(object, "backend_digest", "onednn execution environment");
  llvm::Expected<llvm::StringRef> cpuName = string("host_cpu_name");
  llvm::Expected<std::string> featuresDigest = requireDigest(
      object, "host_features_digest", "onednn execution environment");
  llvm::Expected<std::string> platformDigest = requireDigest(
      object, "host_platform_digest", "onednn execution environment");
  ...
  llvm::Expected<uint64_t> mxcsr =
      requireUnsigned(object, "mxcsr_control", "onednn execution environment");
  llvm::Expected<llvm::StringRef> threadRuntime = string("thread_runtime");
  llvm::Expected<uint64_t> workerCount =
      requireUnsigned(object, "worker_count", "onednn execution environment");
  ...
  llvm::Expected<uint64_t> cache = requireUnsigned(
      object, "primitive_cache_capacity", "onednn execution environment");
  llvm::Expected<std::string> environmentDigest = requireDigest(
      object, "environment_digest", "onednn execution environment");
  if (llvm::Error error = takeExpectedErrors(
    ...
    return error;
  if (!fenvRound)
    return invalid("onednn execution environment identity is incomplete");
  if (*schema != "wafer-onednn-execution-environment" ||
    ...
    return invalid("onednn execution environment policy/readback mismatch");
  return llvm::Error::success();
}
...
parseSpecObject(const llvm::json::Object &object) {
  llvm::Expected<llvm::StringRef> schema =
      requireString(object, "schema", "onednn qualification spec");
  if (!schema)
    return schema.takeError();
  if (*schema != kCurrentSpecSchema)
    return invalid("onednn qualification spec schema mismatch");
  ...
  llvm::Expected<llvm::StringRef> formatText =
      requireString(object, "format", "onednn qualification spec");
  llvm::Expected<uint64_t> m =
      requireUnsigned(object, "m", "onednn qualification spec");
  llvm::Expected<uint64_t> k =
      requireUnsigned(object, "k", "onednn qualification spec");
  llvm::Expected<uint64_t> n =
      requireUnsigned(object, "n", "onednn qualification spec");
  llvm::Expected<uint64_t> batch =
      requireUnsigned(object, "batch_count", "onednn qualification spec");
  llvm::Expected<llvm::StringRef> lhsLayoutText =
      requireString(object, "lhs_layout", "onednn qualification spec");
  llvm::Expected<llvm::StringRef> rhsLayoutText =
      requireString(object, "rhs_layout", "onednn qualification spec");
  llvm::Expected<llvm::StringRef> destinationLayoutText =
      requireString(object, "destination_layout", "onednn qualification spec");
  llvm::Expected<uint64_t> seed =
      requireUnsigned(object, "seed", "onednn qualification spec");
  if (llvm::Error error =
  ...
  if (llvm::Error error =
    ...
    return error;
  if (!explicitPayload)
    return OneDNNQualificationSpec::create(*format, *m, *k, *n, *batch,
                                           *lhsLayout, *rhsLayout,
                                           *destinationLayout, *seed);
  llvm::Expected<llvm::StringRef> lhsHex =
      requireString(object, "lhs_physical", "onednn qualification spec");
  llvm::Expected<llvm::StringRef> rhsHex =
      requireString(object, "rhs_physical", "onednn qualification spec");
  llvm::Expected<llvm::StringRef> destinationHex = requireString(
      object, "destination_template_physical", "onednn qualification spec");
  ...
  if (llvm::Error error =
    ...
    return error;
  return OneDNNQualificationSpec::createWithPhysicalPayload(
...
llvm::Expected<OneDNNQualificationSpec> OneDNNQualificationSpec::create(
  ...
  if (format != LogicalFormat::F16 && format != LogicalFormat::BF16 &&
    ...
    return invalid("onednn qualification only accepts f16, bf16 or f32");
  if (m == 0 || k == 0 || n == 0 || batchCount == 0 ||
    ...
    return invalid(
        "onednn qualification dimensions must be positive uint16 and "
        "seed must fit int64");
  ...
  OneDNNQualificationSpec provisional(format, m, k, n, batchCount, lhsLayout,
                                      rhsLayout, destinationLayout, seed,
                                      /*explicitPhysicalPayload=*/false, {}, {},
                                      {}, "");
  std::string canonical = canonicalJSON(specJSON(provisional));
  return OneDNNQualificationSpec(format, m, k, n, batchCount, lhsLayout,
                                 rhsLayout, destinationLayout, seed,
                                 /*explicitPhysicalPayload=*/false, {}, {}, {},
                                 sha256(canonical));
}
...
llvm::Expected<OneDNNQualificationSpec>
OneDNNQualificationSpec::createWithPhysicalPayload(
    LogicalFormat format, uint64_t m, uint64_t k, uint64_t n,
    uint64_t batchCount, PhysicalTensorLayout lhsLayout,
    PhysicalTensorLayout rhsLayout, PhysicalTensorLayout destinationLayout,
  ...
  llvm::Expected<OneDNNQualificationSpec> generated =
  ...
  OneDNNQualificationSpec provisional(
...
loadOneDNNQualificationSpec(llvm::StringRef path) {
  ...
  if (!object)
    return invalid("onednn qualification spec root must be an object");
  llvm::Expected<OneDNNQualificationSpec> spec = parseSpecObject(*object);
  if (!spec)
    return spec.takeError();
  if (spec->getDigest() != parsed->digest)
    return invalid("onednn qualification spec digest readback mismatch");
  return spec;
}
