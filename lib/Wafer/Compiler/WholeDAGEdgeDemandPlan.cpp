//===- WholeDAGEdgeDemandPlan.cpp - Exact logical edge demand ---------===//
...
public:
  mlir::FailureOr<const ExactEdgeRelation *> get(const CardDAGAnalysis &dag,
                                                 const CardDAGEdge &edge,
                                                 std::string *failureReason) {
...
mlir::LogicalResult
appendEdgeDemandPlan(const CardDAGAnalysis &dag, const CardDAGEdge &edge,
                     const WholeDAGNodePlacement &producerPlacement,
                     const WholeDAGNodePlacement &consumerPlacement,
                     WholeDAGEdgeDemandPlan *result,
  ...
  llvm::SmallVector<WholeDAGEdgeProducerShardOwnership, 4> ownership;
  ...
  for (PhysicalTileId source : producerPlacement.tiles) {
    ...
    ownedDomain = ownedDomain ? ownedDomain->unionSet(*sourceDomain.set)
                              : *sourceDomain.set;
    ownership.push_back(WholeDAGEdgeProducerShardOwnership{
  ...
  for (PhysicalTileId destination : consumerPlacement.tiles) {
    ...
    if (result)
      result->demands.push_back(WholeDAGEdgeDemand{
...
class WholeDAGEdgeDemandPlanner::Impl {
public:
...
WholeDAGEdgeDemandPlanner::WholeDAGEdgeDemandPlanner(const CardDAGAnalysis &dag)
    : impl(std::make_unique<Impl>(dag)) {}

WholeDAGEdgeDemandPlanner::~WholeDAGEdgeDemandPlanner() = default;
WholeDAGEdgeDemandPlanner::WholeDAGEdgeDemandPlanner(
    WholeDAGEdgeDemandPlanner &&) noexcept = default;
WholeDAGEdgeDemandPlanner &WholeDAGEdgeDemandPlanner::operator=(
    WholeDAGEdgeDemandPlanner &&) noexcept = default;

mlir::FailureOr<WholeDAGEdgeDemandPlan> WholeDAGEdgeDemandPlanner::derive(
    CardDAGEdgeID edgeID, const WholeDAGNodePlacement &producerPlacement,
    const WholeDAGNodePlacement &consumerPlacement,
    std::string *failureReason) {
  if (failureReason)
    failureReason->clear();
  ...
  if (!edge)
    return fail<WholeDAGEdgeDemandPlan>(
...
mlir::FailureOr<WholeDAGEdgeDemandPlan> WholeDAGEdgeDemandPlanner::derive(
  ...
  WholeDAGEdgeDemandPlan result;
  for (const CardDAGEdge &edge : impl->dag.getEdges()) {
...
deriveWholeDAGEdgeDemandPlan(const CardDAGAnalysis &dag, CardDAGEdgeID edgeID,
                             const WholeDAGNodePlacement &producerPlacement,
                             const WholeDAGNodePlacement &consumerPlacement,
                             std::string *failureReason) {
  WholeDAGEdgeDemandPlanner planner(dag);
