//===- WholeDAGPlacementFrontier.cpp - Joint node placement ------------===//
...
llvm::SmallVector<PartialPlacementState, 24> retainPlacementFrontier(
...
deriveWholeDAGPlacementFrontier(const CardDAGAnalysis &dag,
                                const PhysicalTopology &topology,
                                PhysicalCardId cardId,
                                WholeDAGPlacementFrontierStatistics *statistics,
                                std::string *failureReason) {
  if (failureReason)
    failureReason->clear();
  WholeDAGPlacementFrontierStatistics localStatistics;
  WholeDAGPlacementFrontierStatistics &resultStatistics =
      statistics ? *statistics : localStatistics;
  resultStatistics = {};
  if (dag.getNodes().empty()) {
    setFailure(failureReason, "whole-DAG placement frontier requires a DAG");
    return mlir::failure();
  }
  std::optional<llvm::ArrayRef<PhysicalTileId>> availableTiles =
      topology.getAvailableTileIds(cardId);
  if (!availableTiles || availableTiles->empty()) {
    setFailure(failureReason,
               "whole-DAG placement frontier requires available Tiles");
    return mlir::failure();
  }
  ...
  if (groups.empty()) {
    setFailure(failureReason,
               "whole-DAG placement frontier has no connected Tile group");
    return mlir::failure();
  }

  llvm::SmallVector<PartialPlacementState, 24> frontier(1);
  EdgeTransitionLegalityCache transitionCache(dag);
  for (const CardDAGNode &node : dag.getNodes()) {
    resultStatistics.expandedStates =
        saturatingAdd(resultStatistics.expandedStates, frontier.size());
    llvm::SmallVector<PartialPlacementState, 128> expanded;
    for (const PartialPlacementState &state : frontier) {
    ...
    if (expanded.empty()) {
      setFailure(failureReason,
                 "whole-DAG placement frontier cannot place a DAG node");
      return mlir::failure();
    }
    frontier = retainPlacementFrontier(dag, std::move(expanded));
  }
  ...
  if (dag.getNodes().size() <= availableTiles->size()) {
    ...
    if (complete &&
        llvm::none_of(frontier, [&](const PartialPlacementState &state) {
          return samePartialPlacement(state, singletonPipeline);
        }))
      frontier.push_back(std::move(singletonPipeline));
  }
  ...
  for (PartialPlacementState &state : frontier) {
    ...
    if (resourceEquivalent) {
      ...
      // independent of unordered frontier bucket iteration order.
      const WholeDAGPlacementCandidate &representative =
