//===- CoordinatedStructuredFrontier.cpp - Current-IR frontier facts ----===//
...
using Dimension = CoordinatedStructuredDimension;
using Knowledge = CoordinatedStructuredFactKnowledge;
using Metric = CoordinatedStructuredMetric;
...
static void appendSymbolicIndexIdentity(llvm::raw_ostream &stream,
                                        mlir::Value value) {
  value = resolveInvariantValue(value);
    ...
    if (auto loop = mlir::dyn_cast<mlir::scf::ForOp>(parent)) {
      ...
      for (mlir::Value bound :
        ...
        if (std::optional<int64_t> value = getProvenConstantIndex(bound))
          ...
          appendSymbolicIndexIdentity(stream, bound);
        stream << ',';
      }
      stream << ")/";
    }
  ...
  if (includeAttributes) {
    ...
    for (mlir::NamedAttribute attribute : operation->getAttrs()) {
      ...
      if (name == mlir::SymbolTable::getSymbolAttrName() ||
        ...
        continue;
      // Symbol spelling and paths are not structured frontier facts. Calls
...
struct RankFacts {
  std::array<Metric, kCoordinatedStructuredDimensionCount> selection;
...
static RankFacts deriveRankFacts(const CoordinatedRankTileProgram &rank) {
  std::array<MetricAccumulator, kCoordinatedStructuredDimensionCount>
      accumulators;
  ...
  rank.module.get().walk([&](mlir::Operation *operation) {
    ...
    auto addEstimated = [&](Dimension dimension, std::optional<uint64_t> amount,
      ...
      std::string modelIdentity = model.str();
      if (!multiplicity.exact) {
        modelIdentity.append(":static-site-dynamic-control:");
        modelIdentity.append(multiplicity.disposition);
      }
      accumulator.addEstimate(*amount, modeledMultiplicity, identity,
                              modelIdentity);
    };
  ...
  for (size_t metricIndex = 0;
       metricIndex < kCoordinatedStructuredDimensionCount; ++metricIndex)
...
static bool usesMaximumReduction(Dimension dimension) {
  ...
  llvm_unreachable("invalid structured frontier dimension");
}
...
deriveCoordinatedStructuredFrontierFacts(
  ...
  CoordinatedStructuredFrontierFacts result;
  for (size_t metricIndex = 0;
       metricIndex < kCoordinatedStructuredDimensionCount; ++metricIndex) {
...
CoordinatedStructuredFrontierOrder compareCoordinatedStructuredFrontierFacts(
  ...
  if (left.futureLiveInterface != right.futureLiveInterface ||
    ...
    return CoordinatedStructuredFrontierOrder::Incomparable;
  ...
  if (leftBetter)
    return CoordinatedStructuredFrontierOrder::LeftDominates;
  if (rightBetter)
    return CoordinatedStructuredFrontierOrder::RightDominates;
  return CoordinatedStructuredFrontierOrder::Equivalent;
}
...
planCoordinatedStructuredFrontierInsertion(
  ...
  CoordinatedStructuredInsertionPlan plan;
  ...
  for (auto [existingIndex, state] : llvm::enumerate(existing)) {
    if (!state.facts || state.stableSemanticOrdinal < 0)
      return mlir::failure();
    switch (compareCoordinatedStructuredFrontierFacts(*candidate.facts,
                                                      *state.facts)) {
    case CoordinatedStructuredFrontierOrder::LeftDominates:
      if (!state.reservedBaseline)
        plan.eraseIndices.push_back(existingIndex);
      break;
    case CoordinatedStructuredFrontierOrder::RightDominates:
      if (!candidate.reservedBaseline)
        plan.retainCandidate = false;
      break;
    case CoordinatedStructuredFrontierOrder::Equivalent:
      ...
      break;
    case CoordinatedStructuredFrontierOrder::Incomparable:
      break;
    }
