//===- RankCandidateSearch.cpp - Rank candidate search -----------------===//
...
static std::string computeCoordinatedTileVariantContentDigestImpl(
  ...
  llvm::SHA256 hasher;
  for (const CoordinatedRankTileModule &rank : variant.ranks) {
...
mlir::FailureOr<CoordinatedTileVariant> materializeCoordinatedTileRepair(
  ...
  for (const CoordinatedRankTileModule &rank : parent.ranks) {
...
deriveStructuredCandidates(mlir::ModuleOp sourceModule,
                           const RankCandidateSearchConfig &config,
...
struct RankCandidateSearchSession::Impl {
  ...
  RankCandidateSearchConfig config;
  ...
  materializeRank(const CoordinatedStructuralProposal &proposal,
    ...
    case CoordinatedStructuralRecipeKind::Traversal: {
      ...
      return materializeCompleteRankCandidateTileModule(
    ...
    case CoordinatedStructuralRecipeKind::Connections:
      return materializeCompleteRankConnectionChoicesTileModule(
...
RankCandidateSearchSession::RankCandidateSearchSession(
    std::unique_ptr<Impl> impl)
    : impl(std::move(impl)) {}

RankCandidateSearchSession::~RankCandidateSearchSession() = default;
RankCandidateSearchSession::RankCandidateSearchSession(
    RankCandidateSearchSession &&) noexcept = default;
RankCandidateSearchSession &RankCandidateSearchSession::operator=(
    RankCandidateSearchSession &&) noexcept = default;

mlir::FailureOr<std::unique_ptr<RankCandidateSearchSession>>
RankCandidateSearchSession::create(
  ...
  return std::unique_ptr<RankCandidateSearchSession>(
      new RankCandidateSearchSession(std::move(state)));
}

mlir::FailureOr<std::unique_ptr<CoordinatedTileVariant>>
RankCandidateSearchSession::admitNextActualCandidate() {
...
mlir::LogicalResult RankCandidateSearchSession::completeActiveCandidate(
...
bool RankCandidateSearchSession::exhausted() const {
...
llvm::StringRef
RankCandidateSearchSession::getStructuredCandidateSetDigest() const {
  return impl ? llvm::StringRef(impl->statistics.structuredCandidateSetDigest)
              : llvm::StringRef();
}

llvm::StringRef
RankCandidateSearchSession::getActualCandidateSetDigest() const {
...
mlir::FailureOr<RankCandidateSet> buildRankCandidateSet(
  ...
  auto session = RankCandidateSearchSession::create(
      sourceModule, config, ledger, structuredStatistics);
  if (mlir::failed(session))
    return mlir::failure();
  RankCandidateSet candidates;
  ...
  if (candidates.empty() || !candidates.front().reservedBaseline)
    return mlir::failure();
  std::string digest = computeRankCandidateSetDigest(candidates);
