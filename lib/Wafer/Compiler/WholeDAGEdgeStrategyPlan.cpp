...
lowerDemandPlanToCanonicalStrategies(const CardDAGAnalysis &dag,
                                     const WholeDAGEdgeDemandPlan &demandPlan,
                                     std::string *failureReason) {
  WholeDAGEdgeStrategyPlan result;
  std::unordered_map<CardDAGEdgeID, uint32_t> nextPayloadSlice;
  for (const WholeDAGEdgeDemand &demand : demandPlan.demands) {
    const CardDAGEdge *edge = dag.getEdge(demand.edge);
    const CardDAGNode *producerNode =
    ...
    for (const WholeDAGEdgeProducerShardOwnership &ownership :
      ...
      if (*bytes > std::numeric_limits<uint64_t>::max() - result.totalPeerBytes)
        return fail<WholeDAGEdgeStrategyPlan>(failureReason,
...
public:
  explicit Impl(const CardDAGAnalysis &dag) : dag(dag), demandPlanner(dag) {}

  const CardDAGAnalysis &dag;
  WholeDAGEdgeDemandPlanner demandPlanner;
};
...
deriveWholeDAGEdgeStrategyPlan(const CardDAGAnalysis &dag, CardDAGEdgeID edgeID,
  ...
  if (failureReason)
    failureReason->clear();
  mlir::FailureOr<WholeDAGEdgeDemandPlan> demandPlan =
      deriveWholeDAGEdgeDemandPlan(dag, edgeID, producerPlacement,
                                   consumerPlacement, failureReason);
  if (mlir::failed(demandPlan))
    return mlir::failure();
  return lowerWholeDAGEdgeDemandPlanToCanonicalStrategies(dag, *demandPlan,
                                                          failureReason);
}
...
mlir::FailureOr<WholeDAGEdgeStrategyPlan> deriveWholeDAGEdgeStrategyPlan(
    ...
    llvm::ArrayRef<WholeDAGNodePlacement> requestedPlacements,
    std::string *failureReason) {
  WholeDAGEdgeStrategyPlanner planner(dag);
