//===- WholeCardResourceVerification.cpp - Whole-card resource verification
//===//
...
verifyWholeCardResources(llvm::ArrayRef<mlir::ModuleOp> physicalTileModules,
  ...
  if (physicalTileModules.size() != physicalTileIds.size() ||
    ...
    return mlir::ModuleOp(physicalTileModules.front()).emitOpError()
           << "whole_card_resource_verification: physical Tile domain has "
           << physicalTileModules.size()
  ...
  for (auto [expected, actual] : llvm::enumerate(sortedTileIds)) {
    if (actual == static_cast<int64_t>(expected))
      continue;
    return mlir::ModuleOp(physicalTileModules.front()).emitOpError()
           << "whole_card_resource_verification: physical Tile ids must cover "
  ...
  if (policy.spmAddressLimit < policy.spmAddressBase)
    return diagnosticAnchor.emitOpError(
        "whole_card_resource_verification: target SPM range is invalid");
  ...
  for (auto [module, tileId] :
    ...
    if (!closure)
      return diagnosticModule.emitOpError()
             << "whole_card_resource_verification: call closure "
                "is invalid: "
  ...
  for (auto &&[tile, tileCost] : llvm::enumerate(cost.tileCosts)) {
    ...
    if (!tileCost.spmHighWaterBytes.isKnown())
      return tileModule.emitOpError()
             << "whole_card_resource_verification: physical Tile " << tileId
  ...
  if (cost.aggregateNoC.aggregateTransmitBytes.isKnown() &&
    ...
    return diagnosticAnchor.emitOpError()
           << "whole_card_resource_verification: matched whole-card NoC "
...
verifyWholeCardResources(llvm::ArrayRef<mlir::ModuleOp> physicalTileModules,
                         llvm::ArrayRef<PhysicalTileId> physicalTileIds,
                         const ExecutionConfig &executionConfig) {
  return detail::verifyWholeCardResources(physicalTileModules, physicalTileIds,
                                          executionConfig);
}
