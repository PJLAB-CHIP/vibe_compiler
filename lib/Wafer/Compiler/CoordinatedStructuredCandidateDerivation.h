//===- CoordinatedStructuredCandidateDerivation.h --------------*- C++ -*-===//

#ifndef WAFER_COMPILER_COORDINATEDSTRUCTUREDCANDIDATEDERIVATION_H
#define WAFER_COMPILER_COORDINATEDSTRUCTUREDCANDIDATEDERIVATION_H
...
/// Every candidate family enters the same coordinated frontier through this
...
/// parameters. It never owns an IR clone, an executable-finalization
...
/// One pre-materialization frontier member. Facts are conservative projections
/// from the immutable source and typed recipe. A dimension that cannot be
/// proved before replay remains Unknown with a stable disposition; it is never
/// treated as zero. canonicalKey and coverageClass are invocation-local search
/// keys and are not serialized into selected IR or an artifact.
...
#endif // WAFER_COMPILER_COORDINATEDSTRUCTUREDCANDIDATEDERIVATION_H
