//===- RankCandidateEvaluation.cpp - Rank candidate evaluation
//-----------===//
...
std::string computeEvaluatedRankCandidateSetDigest(
  ...
  llvm::SHA256 hasher;
  for (const EvaluatedRankCandidate &executable : candidates) {
    std::string header =
        (llvm::Twine("semantic:") +
...
static CoordinatedScheduleActionEstimate deriveScheduleRecipeEstimate(
    ...
    const analysis::CardInstructionProgramCost &canonicalCost) {
...
mlir::LogicalResult
RankCandidateEvaluationScheduler::recordMandatoryBaselineAccepted() {
...
RankCandidateEvaluationScheduler::chooseNextLane(
  ...
  if (seedAvailable && expansionAvailable)
    return preferSeed ? RankCandidateEvaluationLane::Seed
                      : RankCandidateEvaluationLane::Expansion;
  return seedAvailable ? RankCandidateEvaluationLane::Seed
                       : RankCandidateEvaluationLane::Expansion;
}

mlir::LogicalResult RankCandidateEvaluationScheduler::recordAttempt(
    RankCandidateEvaluationLane lane, bool exactAccepted) {
  if (!baselineAccepted || !canAttempt())
    return mlir::failure();
  ++attemptCount;
  if (exactAccepted)
    ++exactAcceptedCount;
  preferSeed = lane == RankCandidateEvaluationLane::Expansion;
  return exactAcceptedCount <= kMaximumCoordinatedExactScheduleActions
             ? mlir::success()
             : mlir::failure();
}

bool RankCandidateEvaluationScheduler::canAttempt() const {
...
bool isRecoverableRankInstrLoweringFailure(
  ...
  return failure.kind == RankInstrLoweringFailureKind::RankInstrLowering ||
         failure.kind == RankInstrLoweringFailureKind::SPMAllocation;
}
...
buildCoordinatedScheduleRecipePlan(
  ...
  analysis::CardInstructionProgramCost canonicalCost =
      analysis::analyzeCardInstructionProgramCost(
...
struct RankCandidateEvaluationCursor::Impl {
...
RankCandidateEvaluationCursor::RankCandidateEvaluationCursor(
...
RankCandidateEvaluationCursor::RankCandidateEvaluationCursor(
    RankCandidateEvaluationCursor &&) noexcept = default;
RankCandidateEvaluationCursor &
RankCandidateEvaluationCursor::operator=(
    RankCandidateEvaluationCursor &&) noexcept = default;

int64_t RankCandidateEvaluationCursor::getStableSemanticOrdinal() const {
  return impl ? impl->stableSemanticOrdinal : -1;
}
bool RankCandidateEvaluationCursor::isReservedBaseline() const {
  return impl && impl->reservedBaseline;
}
size_t RankCandidateEvaluationCursor::getCanonicalParentCount() const {
  return impl ? impl->canonicalParents.size() : 0;
}
bool RankCandidateEvaluationCursor::seedAttempted() const {
  return impl && impl->didSeedAttempt;
}
bool RankCandidateEvaluationCursor::exactSeedAccepted() const {
  return impl && impl->seedExactAccepted;
}
bool RankCandidateEvaluationCursor::exhausted() const {
...
beginRankCandidateEvaluation(
    ...
    RankCandidateSelectionMode selectionMode, unsigned rankPipelineParallelism,
    llvm::ArrayRef<const CoordinatedCommunicationActionProvider *>
        communicationProviders) {
  ...
  if (ledger.getRankCount() != rankCount ||
    ...
    failure.kind = RankInstrLoweringFailureKind::RankDomain;
    ...
    if (!tileVariant.reservedBaseline &&
      ...
      failure.kind = RankInstrLoweringFailureKind::WorkLedger;
      failure.gate = "work-ledger";
    }
    return mlir::failure();
  }

  auto state = std::make_unique<RankCandidateEvaluationCursor::Impl>();
  ...
  for (auto [rankIndex, result] : llvm::enumerate(canonicalResults)) {
    ...
    if (result.conversionFailed || result.verificationFailed) {
      failure.kind = RankInstrLoweringFailureKind::RankInstrLowering;
      failure.logicalRank = tileVariant.ranks[rankIndex].logicalRank;
      failure.gate =
  ...
  state->selectedTileIR.reserve(tileVariant.ranks.size());
  {
    ...
    for (const CoordinatedRankTileModule &rank : tileVariant.ranks)
  ...
  const CoordinatedScheduleActionFamily actionFamily =
      selectionMode == RankCandidateSelectionMode::
  ...
  return std::unique_ptr<RankCandidateEvaluationCursor>(
      new RankCandidateEvaluationCursor(std::move(state)));
}

mlir::FailureOr<RankCandidateEvaluationStep>
advanceRankCandidateEvaluation(
    RankCandidateEvaluationCursor &cursor,
    CandidateEvaluationReservation reservation) {
  if (!cursor.impl)
    return mlir::failure();
  RankCandidateEvaluationCursor::Impl &state = *cursor.impl;
  RankCandidateEvaluationStep step;
  if (state.recipePlan.nextRecipe >=
      state.recipePlan.materializationOrder.size()) {
    step.kind = RankCandidateEvaluationStepKind::Exhausted;
    return step;
  }
  ...
  auto reject = [&](RankInstrLoweringFailure failure)
      -> mlir::FailureOr<RankCandidateEvaluationStep> {
    if (mlir::failed(closeAttempt()))
      return mlir::failure();
    if (seed)
      state.seedExactAccepted = false;
    step.kind = RankCandidateEvaluationStepKind::RecoverableRejected;
  ...
  const RankScheduleActionKey actionKey{action->readyOrderKind,
    ...
    workers = runBoundedRankPipelines(
        ...
        [&](size_t rankIndex) {
          mlir::FailureOr<mlir::OwningOpRef<mlir::ModuleOp>> planned =
              planTileMemory(std::move(action->rankModules[rankIndex]),
    ...
    if (!result.module) {
      ...
      failure.kind =
          result.failure.kind ==
              ...
              ? RankInstrLoweringFailureKind::SPMAllocation
              : RankInstrLoweringFailureKind::RankInstrLowering;
  ...
  mlir::FailureOr<RankExecutableSet> accepted =
      lowerRankInstrModules(
  ...
  if (mlir::failed(accepted)) {
    RankInstrLoweringFailure failure;
    failure.kind = RankInstrLoweringFailureKind::RankCandidateExactGate;
  ...
  step.kind = RankCandidateEvaluationStepKind::Accepted;
...
evaluateRankCandidate(
    ...
    llvm::ArrayRef<const CoordinatedCommunicationActionProvider *>
        communicationProviders) {
  auto cursor = beginRankCandidateEvaluation(
  ...
  std::vector<EvaluatedRankCandidate> evaluatedCandidates;
  ...
  while (!(*cursor)->exhausted() &&
    ...
    auto step = advanceRankCandidateEvaluation(**cursor, reservation);
    if (mlir::failed(step))
      return mlir::failure();
    if (step->kind ==
        RankCandidateEvaluationStepKind::RecoverableRejected) {
    ...
    } else if (step->kind == RankCandidateEvaluationStepKind::Accepted) {
      ++exactAccepted;
      evaluatedCandidates.push_back(std::move(*step->candidate));
      if (mlir::failed(reduceEvaluatedRankCandidates(
