//===- LowerRankInstrModules.cpp - Rank Instr module lowering ----------===//
...
#include "RankResourceCostValidation.h"
...
struct RankExecutableDomain {
  RankExecutableDomain(
...
static mlir::FailureOr<RankExecutableDomain> buildCandidateRankExecutables(
  ...
  wafer::support::ScopedCompileTimingSpan totalTiming(
      "rank-instr-lowering", "candidate", "rank-executables");
  ...
  {
    wafer::support::ScopedCompileTimingSpan timing("rank-instr-lowering",
...

  {
    wafer::support::ScopedCompileTimingSpan timing(
        "rank-instr-lowering", "candidate", "index-lowering");
    std::vector<uint8_t> failedRanks(rankCount);
    runBoundedRankModulePipelines(
        moduleViews,
        [&](size_t rank) {
  ...
  {
    wafer::support::ScopedCompileTimingSpan timing(
        "rank-instr-lowering", "candidate", "program-resources");
    resourceCost = validateRankResourceCost(moduleViews, executionConfig);
  }
  if (mlir::failed(resourceCost)) {
    failedCheck = "program-resources";
    return mlir::failure();
  }
  ...
  for (size_t rank = 0; rank < rankCount; ++rank) {
    ...
    llvm::StringRef tileEvidence = selectedTileIR[rank]
                                       ...
                                       : llvm::StringRef{};
    ranks.push_back(CardExecutableBuilder::makeRank(
  ...
  return RankExecutableDomain(std::move(ranks),
                                  std::move(*runtimeLaunchContract),
                                  std::move(*resourceCost));
}

static mlir::FailureOr<RankExecutableSet> verifyCandidateTargetLowering(
    RankExecutableDomain candidate, const ExecutionConfig &executionConfig,
    RankCandidateSearchStatistics *statistics, std::string &failedCheck,
    unsigned rankPipelineParallelism) {
  if (statistics)
  ...
  return RankExecutableSet(std::move(candidate.ranks),
...
mlir::FailureOr<RankExecutableSet> lowerRankInstrModules(
    ...
    std::string *failedCheck, RankCandidateSearchStatistics *statistics,
    unsigned rankPipelineParallelism) {
  auto fail = [&](llvm::StringRef check, llvm::StringRef message) {
    ...
    return mlir::FailureOr<RankExecutableSet>(mlir::failure());
  };

  std::string check = "unknown";
  if (statistics)
    ++statistics->tileModuleLoweringAttempts;
  mlir::FailureOr<RankExecutableDomain> rankExecutables =
  ...
  if (mlir::failed(rankExecutables))
    return fail(check,
                "card candidate failed validation '" + check + "'");
  if (statistics)
    ++statistics->tileModuleLoweringSuccesses;

  mlir::FailureOr<RankExecutableSet> result =
