...
/// every Tile variant; an individual executable-finalization cursor never
...
inline constexpr uint32_t kMaximumCoordinatedLiveFinalizationCursors = 8;
...
inline constexpr uint32_t kMaximumCoordinatedStructuralFrontierStates = 64;
...
struct ExecutableFinalizationReservation {
  uint64_t id = 0;
};
...
struct CoordinatedWorkLedgerSnapshot {
  ...
  uint64_t finalizationReserved = 0;
...
/// generation, executable finalization, and repair. Mandatory baseline
/// generation and executable finalization plus a finite repair allowance are
...
public:
  ...
  ExecutableFinalizationReservation getMandatoryBaselineReservation() const {
  ...
  /// finalization or repair reservation.
  ...
  tryReserveExecutableFinalization(const CoordinatedWorkEstimate &upperBound);
  ...
  /// Closes one attempted schedule action atomically, whether exact admission
  /// accepted or rejected it. Unused upper-bound credits are released; an
  /// actual count beyond any reserved class fails.
  mlir::LogicalResult
  completeExecutableFinalization(ExecutableFinalizationReservation reservation,
                                 const CoordinatedWorkEstimate &actual);

  /// Releases a rejected non-mandatory candidate before executable
  /// finalization. The mandatory baseline reservation cannot be released.
  mlir::LogicalResult
  releaseExecutableFinalization(ExecutableFinalizationReservation reservation);

  /// Consumes the pre-reserved repair allowance. It cannot borrow mandatory
  /// baseline or admitted executable-finalization credits.
  ...
  getExecutableFinalizationUpperBound(int64_t rankCount);
  static mlir::FailureOr<CoordinatedWorkEstimate>
  getExecutableScheduleAttemptUpperBound(int64_t rankCount);

private:
  struct FinalizationReservationRecord {
    uint64_t id = 0;
  ...
  FinalizationReservationRecord *findReservation(uint64_t id);
  const FinalizationReservationRecord *findReservation(uint64_t id) const;
  ...
  std::array<uint64_t, kCoordinatedWorkKindCount> consumedByKind = {};
  std::vector<FinalizationReservationRecord> finalizationReservations;
  ExecutableFinalizationReservation mandatoryBaselineReservation;
};

/// One real rank entry in an all-rank Tile candidate. The module is the only
/// semantic artifact. selectedTileIR is optional immutable debug evidence for
/// externally constructed fixtures; production frontier construction leaves
...
struct CoordinatedTileVariant {
  ...
  ExecutableFinalizationReservation finalizationReservation;
...
enum class CoordinatedStructuredFactKnowledge : uint8_t {
  Known,
  Estimated,
  Unknown,
  Unsupported,
...
struct CoordinatedStructuredMetric {
  uint64_t value = 0;
  CoordinatedStructuredFactKnowledge knowledge =
      CoordinatedStructuredFactKnowledge::Known;
  ...
  /// state, not an IR attribute or serialized artifact.
...
enum class CoordinatedStructuredDimension : uint8_t {
...
inline constexpr size_t kCoordinatedStructuredDimensionCount =
    static_cast<size_t>(CoordinatedStructuredDimension::Count);

struct CoordinatedStructuredFrontierFacts {
  std::array<CoordinatedStructuredMetric, kCoordinatedStructuredDimensionCount>
      selection;
...
enum class CoordinatedStructuredFrontierOrder : uint8_t {
  LeftDominates,
  RightDominates,
  Equivalent,
  Incomparable,
...
struct CoordinatedStructuredFrontierView {
  int64_t stableSemanticOrdinal = 0;
  bool reservedBaseline = false;
  const CoordinatedStructuredFrontierFacts *facts = nullptr;
};

struct CoordinatedStructuredInsertionPlan {
  bool retainCandidate = false;
  llvm::SmallVector<size_t, 8> eraseIndices;
};

/// Re-derives all transient structured frontier facts from the actual
/// complete-rank Tile clone. No proposal object, name convention, finalization
/// offset or historical schedule participates.
mlir::FailureOr<CoordinatedStructuredFrontierFacts>
deriveCoordinatedStructuredFrontierFacts(const CoordinatedTileVariant &variant);

/// Verifies shared collective parameters and all-and-only peer message
/// matching directly across the actual rank Tile programs. This is a C2
...
CoordinatedStructuredFrontierOrder compareCoordinatedStructuredFrontierFacts(
    const CoordinatedStructuredFrontierFacts &left,
    const CoordinatedStructuredFrontierFacts &right);
...
bool canStillSatisfyCoordinatedProductionPromotion(
    const CoordinatedStructuredFrontierFacts &candidate,
    const CoordinatedStructuredFrontierFacts &baseline);
...
/// solver.
mlir::FailureOr<CoordinatedStructuredInsertionPlan>
planCoordinatedStructuredFrontierInsertion(
    CoordinatedStructuredFrontierView candidate,
    llvm::ArrayRef<CoordinatedStructuredFrontierView> existing);

struct CoordinatedStructuredFrontierStatistics {
  ...
  uint64_t finalizationAdmissionDenied = 0;
  uint64_t generationAdmissionDenied = 0;
  ...
  uint64_t actualCandidateAdmissions = 0;
  ...
  std::string structuralFrontierDigest;
  std::string actualAdmissionDigest;
};
...
/// dispositions are reported after executable finalization closes its work
/// reservation. The compatibility disposition transfers the still-live
/// reservation to the legacy returned frontier.
...
/// Invocation-local owner of the structural frontier, actual-clone admission,
/// and the deterministic work ledger protocol. The source module is borrowed
...
class CoordinatedDataflowSearchSession {
public:
  static mlir::FailureOr<std::unique_ptr<CoordinatedDataflowSearchSession>>
  create(mlir::ModuleOp sourceModule,
         const CoordinatedDataflowSearchConfig &config,
         CoordinatedWorkLedger &ledger,
         CoordinatedStructuredFrontierStatistics *statistics = nullptr);

  ~CoordinatedDataflowSearchSession();
  CoordinatedDataflowSearchSession(
      CoordinatedDataflowSearchSession &&) noexcept;
  CoordinatedDataflowSearchSession &
  operator=(CoordinatedDataflowSearchSession &&) noexcept;
  CoordinatedDataflowSearchSession(const CoordinatedDataflowSearchSession &) =
      delete;
  CoordinatedDataflowSearchSession &
  operator=(const CoordinatedDataflowSearchSession &) = delete;

  /// Returns nullptr on deterministic exhaustion or policy stop. Materialized
  /// and actual-frontier-rejected proposals are skipped internally and trigger
  /// stable-order backfill from the still-live structural frontier.
  ...
  llvm::StringRef getStructuralFrontierDigest() const;
  llvm::StringRef getActualAdmissionDigest() const;

private:
  struct Impl;
  explicit CoordinatedDataflowSearchSession(std::unique_ptr<Impl> impl);
...
/// under one executable-finalization reservation. No Tile-to-Instr, completion,
/// SPM, DDR, transport, ABI, cost selection, or winner commit occurs here.
mlir::FailureOr<CoordinatedTileFrontier> buildCoordinatedTileFrontier(
    ...
    CoordinatedWorkLedger &ledger,
    CoordinatedStructuredFrontierStatistics *structuredStatistics = nullptr);
...
/// unplaced actual Tile parent. Executable-finalization reservation is admitted
