//===- PhysicalTileMemoryPlanning.cpp - Physical Tile memory planning ----===//
...
mlir::FailureOr<mlir::OwningOpRef<mlir::ModuleOp>> planPhysicalTileMemory(
  ...
  if (!module) {
    if (failure)
      failure->kind = PhysicalTileMemoryPlanningFailureKind::Contract;
    return mlir::failure();
  }

  auto recordFailure = [&](PhysicalTileMemoryPlanningFailureKind kind) {
    if (failure)
      failure->kind = kind;
  };
  ...
  if (containsTileDataflowOperations(module->getOperation())) {
    recordFailure(PhysicalTileMemoryPlanningFailureKind::Contract);
    module->emitError()
        << "physical_tile_memory_planning_requires_canonical_instr_ir: Tile "
  ...
  if (hasPreexistingPhysicalAssignments(*module)) {
    recordFailure(
        PhysicalTileMemoryPlanningFailureKind::PreexistingPhysicalAssignments);
    module->emitError()
        << "physical_tile_memory_planning_contains_preexisting_physical_"
  ...
  wafer::support::recordCompileWork(
      wafer::support::CompileWorkKind::PhysicalTileMemoryPlanning);
  if (mlir::failed(mlir::verify(*module))) {
    recordFailure(PhysicalTileMemoryPlanningFailureKind::Verification);
    module->emitError(
        "physical-Tile memory-planning input failed verification");
    return mlir::failure();
  }
  ...
  if (mlir::failed(
    ...
    recordFailure(
        PhysicalTileMemoryPlanningFailureKind::InstrMemoryPlanningPreparation);
    return mlir::failure();
  }
  if (materializationRelations)
  ...
  if (!selectedBufferRequests.empty() &&
    ...
    recordFailure(
        PhysicalTileMemoryPlanningFailureKind::SelectedBufferMaterialization);
    return mlir::failure();
  }
  if (mlir::failed(
  ...
  mlir::LogicalResult spmResult = runPassPipeline(
      *module, "physical-tile-spm-planning", [&](mlir::OpPassManager &manager) {
        wafer::addAssignSPMOffsetsPass(manager, spmOptions, &spmFailure);
      });
  if (mlir::failed(spmResult)) {
    recordFailure(PhysicalTileMemoryPlanningFailureKind::SPMAllocation);
    if (failure)
  ...
  if (hasWholeCardAssignments(*module)) {
    recordFailure(
        PhysicalTileMemoryPlanningFailureKind::PreexistingPhysicalAssignments);
    module->emitError(
        "physical_tile_memory_planning_created_premature_whole_card_facts");
    return mlir::failure();
  }
