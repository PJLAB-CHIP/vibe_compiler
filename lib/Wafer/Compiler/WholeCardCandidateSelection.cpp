//===- WholeCardCandidateSelection.cpp - Final all-rank choice ---------===//
...
static bool
hasValidAdmittedIdentities(llvm::ArrayRef<EvaluatedWholeCardCandidate> variants,
...
static bool
hasNoReservedRankIdentity(const EvaluatedWholeCardCandidate &candidate) {
  return !candidate.variant.ranks.empty() && !candidate.reservedBaseline;
}

static bool matchesStaticFixedSlotQualification(
    const EvaluatedWholeCardCandidate &candidate) {
...
static bool
matchesCollectiveCharacterization(const EvaluatedWholeCardCandidate &candidate,
...
static bool matchesQualification(const EvaluatedWholeCardCandidate &candidate,
                                 WholeVariantSelectionMode mode) {
  switch (mode) {
  case WholeVariantSelectionMode::QualifyStaticFixedSlot:
...
struct EligibleCandidate {
  size_t index = 0;
  WholeCardSelectionBasis basis = WholeCardSelectionBasis::EstimatedBenefit;
  analysis::WholeCardResourceDurationEstimate duration;
};

static std::optional<EligibleCandidate> evaluatePromotion(
    size_t index, llvm::ArrayRef<WholeCardCandidateCostView> variants,
  ...
  const WholeCardCandidateCostView &candidate = variants[index];
  ...
  if (candidateUpper.isKnown() && baselineLower.isKnown() &&
      marginClears(candidateUpper.value, baselineLower.value,
                   policy.productionBenefitMarginPermille)) {
    result.basis = WholeCardSelectionBasis::ProvenBenefit;
    return result;
  }
  ...
  if (!candidateNominal.isKnown() || !baselineNominal.isKnown() ||
    ...
    if (candidateNominal.isKnown() && baselineNominal.isKnown() &&
        ...
        hasKnownPrimitiveWorkPromotion(*candidate.resourceCost, baselineCost,
                                       policy)) {
      result.basis = WholeCardSelectionBasis::EstimatedBenefit;
      return result;
    }
...
mlir::FailureOr<WholeCardCandidateSelection> selectWholeCardCandidateByCost(
  ...
  WholeCardCandidateSelection plan;
...
selectWholeCardQualificationCandidate(
  ...
  WholeCardQualificationSelection plan;
  for (auto [index, variant] : llvm::enumerate(variants))
...
mlir::LogicalResult reduceEvaluatedWholeCardCandidates(
  ...
  if (baselineIndex || promotionBaselineCost) {
    if (selectionMode == WholeVariantSelectionMode::ReservedBaseline) {
      ...
      llvm::SmallVector<WholeCardCandidateCostView, 16> views;
      views.reserve(variants.size());
      for (const EvaluatedWholeCardCandidate &variant : variants)
        views.push_back(
  ...
  std::vector<EvaluatedWholeCardCandidate> reduced;
...
mlir::FailureOr<EvaluatedWholeCardCandidate> selectEvaluatedWholeCardCandidate(
  ...
  if (isQualificationSelectionMode(selectionMode)) {
    mlir::FailureOr<WholeCardQualificationSelection> qualification =
        selectWholeCardQualificationCandidate(variants, selectionMode);
    ...
    const EvaluatedWholeCardCandidate &winner =
  ...
  for (const EvaluatedWholeCardCandidate &variant : variants)
    views.push_back({variant.stableSemanticOrdinal, variant.reservedBaseline,
                     ...
                     variant.scheduleActionOrdinal});
  mlir::FailureOr<WholeCardCandidateSelection> plan =
      selectWholeCardCandidateByCost(views, selectionMode);
  if (mlir::failed(plan))
    return mlir::failure();
  if (statistics)
    statistics->paretoRetainedVariants += plan->paretoIndices.size();

  const EvaluatedWholeCardCandidate &winner = variants[plan->selectedIndex];
  diagnostics << "wafer-compile: coordinated whole-rank selection"
              ...
              << " implementation_alternative="
              << (winner.implementationAlternativeOrigin ? "true" : "false")
              << " basis=" << stringifyWholeCardSelectionBasis(plan->basis)
              << " nominal_ps=";
...
llvm::StringRef
stringifyWholeCardSelectionBasis(WholeCardSelectionBasis basis) {
  switch (basis) {
  case WholeCardSelectionBasis::ReservedBaseline:
    return "reserved-baseline";
  case WholeCardSelectionBasis::EstimatedBenefit:
    return "estimated-benefit";
  case WholeCardSelectionBasis::ProvenBenefit:
