//===- StructuredDAGSchedulePlan.cpp - Accepted-IR candidate plan ------------===//
...
using analysis::CardInstructionProgramCost;
...
bool conservesRawCost(const CardInstructionProgramCost &whole,
                      llvm::ArrayRef<CardInstructionProgramCost> slices) {
  ...
  for (size_t tile = 0; tile < whole.tileCosts.size(); ++tile) {
    ...
    for (const CardInstructionProgramCost &slice : slices)
...
bool tileSetsIntersect(llvm::ArrayRef<TileId> lhs,
  ...
  return llvm::any_of(
      lhs, [&](TileId tile) { return llvm::is_contained(rhs, tile); });
}

std::vector<llvm::BitVector>
computeDAGReachability(const StructuredDAGAnalysis &dag) {
...
llvm::BitVector getObservableDAGNodes(const StructuredDAGAnalysis &dag) {
  llvm::BitVector observable(dag.getNodes().size());
  llvm::SmallVector<StructuredDAGNodeID, 16> worklist;
  for (llvm::ArrayRef<StructuredDAGNodeID> roots : dag.getObservableOutputRootNodes())
    worklist.append(roots.begin(), roots.end());

  while (!worklist.empty()) {
    StructuredDAGNodeID node = worklist.pop_back_val();
    if (observable.test(node))
      continue;
    observable.set(node);
    for (StructuredDAGEdgeID edgeId : dag.getNodes()[node].incomingEdges)
...
bool hasCompleteObservableNodeCoverage(const StructuredDAGAnalysis &dag,
  ...
  while (changed) {
    ...
    for (const StructuredDAGNode &node : dag.getNodes()) {
      ...
      for (StructuredDAGEdgeID edgeId : node.outgoingEdges) {
        StructuredDAGNodeID successor = dag.getEdges()[edgeId].consumer;
...
StaticScheduleStage
makeProgramResourceStage(const CardInstructionProgramCost &cost,
...
mlir::FailureOr<StaticSchedulePlan> buildAcceptedStructuredDAGSchedulePlan(
    ...
    llvm::SmallVectorImpl<CardInstructionProgramCost> &phaseCosts,
      ...
      mlir::WalkResult walk = function.walk([&](mlir::Operation *operation) {
        ...
        if (!operationNodeSet.none() && !residual) {
          ...
          for (int relationNode = operationNodeSet.find_first();
            ...
            StructuredDAGNodeID candidate = static_cast<StructuredDAGNodeID>(relationNode);
        ...
        if (node && !llvm::is_contained(placements[*node]->tiles,
          ...
          message << "accepted operation is outside its DAG node placement"
                  ...
                  << " placement_tiles=[";
          for (auto [index, tileId] :
               llvm::enumerate(placements[*node]->tiles)) {
            if (index != 0)
              message << ',';
            message << tileId.getValue();
          }
  ...
  for (size_t phase = 0; phase < phaseCount; ++phase) {
    llvm::SmallVector<analysis::TileInstructionProgramSlice, 16>
        tileSlices;
    tileSlices.reserve(executable.tiles.size());
    for (auto [tileIndex, tile] : llvm::enumerate(executable.tiles))
      tileSlices.push_back({tile.getTileId(),
                            entries[tileIndex].getOperation(),
                            operations[phase][tileIndex]});
    phaseCosts.push_back(analysis::analyzeCardInstructionProgramCostSlice(
        tileSlices, policy));
  }
  ...
  steps.push_back(StaticScheduleStep::forStage(makeProgramResourceStage(
  ...
  llvm::SmallVector<llvm::SmallVector<StructuredDAGNodeID, 4>, 16> successors(
      dag.getNodes().size());
  for (const StructuredDAGEdge &edge : dag.getEdges()) {
  ...
  while (!scheduled.all()) {
    llvm::SmallVector<StructuredDAGNodeID, 8> ready;
    for (const StructuredDAGNode &node : dag.getNodes())
    ...
    llvm::SmallVector<StructuredDAGNodeID, 8> dispatched;
    for (StructuredDAGNodeID node : ready) {
      const StructuredDAGNodePlacement &placement = *placements[node];
      if (llvm::any_of(dispatched, [&](StructuredDAGNodeID other) {
    ...
    StaticScheduleStage stage;
    for (StructuredDAGNodeID node : dispatched) {
      StaticScheduleBranch branch;
      branch.dependentWork.push_back(
    ...
    for (StructuredDAGNodeID node : dispatched) {
      scheduled.set(node);
      for (StructuredDAGNodeID successor : successors[node]) {
