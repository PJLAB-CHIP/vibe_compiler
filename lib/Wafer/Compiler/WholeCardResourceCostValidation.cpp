//===- WholeCardResourceCostValidation.cpp - Validate resource costs ----===//
...
requireKnown(mlir::ModuleOp anchor, llvm::StringRef name,
  ...
  return anchor.emitOpError()
         << "whole_card_resource_cost_validation: " << name << " is "
...
validateWholeCardResourceCost(llvm::ArrayRef<mlir::ModuleOp> rankModules,
  ...
  if (rankModules.size() != static_cast<size_t>(executionConfig.getRankCount()))
    return mlir::ModuleOp(rankModules.front()).emitOpError()
           << "whole_card_resource_cost_validation: rank domain has "
  ...
  if (policy.spmAddressLimit < policy.spmAddressBase)
    return diagnosticAnchor.emitOpError(
        "whole_card_resource_cost_validation: target SPM range is invalid");
  ...
  for (mlir::ModuleOp module : rankModules) {
    ...
    if (!closure)
      return module.emitOpError()
             << "whole_card_resource_cost_validation: accepted call closure "
                "is invalid: "
  ...
  for (auto &&[rank, rankCost] : llvm::enumerate(cost.rankCosts)) {
    ...
    if (rankCost.spmHighWaterBytes.value > perRankSPMCapacity)
      return rankModule.emitOpError()
             << "whole_card_resource_cost_validation: rank " << rank
  ...
  if (cost.aggregateNoC.aggregateTransmitBytes.value !=
    ...
    return diagnosticAnchor.emitOpError()
           << "whole_card_resource_cost_validation: matched all-rank NoC "
...
validateWholeCardResourceCost(llvm::ArrayRef<mlir::ModuleOp> rankModules,
                              const ExecutionConfig &executionConfig) {
  return detail::validateWholeCardResourceCost(rankModules, executionConfig);
}
