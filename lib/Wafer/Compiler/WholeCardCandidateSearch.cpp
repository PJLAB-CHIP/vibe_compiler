//===- WholeCardCandidateSearch.cpp - Whole-card Tile candidates --------===//

#include "WholeCardCandidateSearch.h"

#include "BoundedRankExecutor.h"
#include "StructuredCandidateDerivation.h"
...
buildStructuredTraversalProposals(mlir::ModuleOp sourceModule) {
  ...
  // simply absent from the actual candidates.
  if (shape)
    proposals.push_back(
  ...
  // failed materialization never enters the candidates.
  if (shape) {
...
static StructuredCandidateCost
makeUnknownPrecloneFacts(llvm::StringRef physicalVersions,
                         llvm::StringRef loopReuseAndEffects) {
  StructuredCandidateCost facts;
  for (StructuredCandidateMetric &metric : facts.selection) {
    metric.knowledge = StructuredCandidateMetricKnowledge::Unknown;
...
static CoordinatedStructuralProposal makeConnectionStructuralProposal(
  ...
  auto setEstimate = [&](StructuredCandidateCostDimension dimension,
                         uint64_t value) {
    StructuredCandidateMetric &metric =
        structural.facts.selection[static_cast<size_t>(dimension)];
    metric.value = value;
    metric.knowledge = estimateOverflow
                           ? StructuredCandidateMetricKnowledge::Overflow
                           : StructuredCandidateMetricKnowledge::Estimated;
  ...
  setEstimate(StructuredCandidateCostDimension::DDRAggregateReadBytes,
              estimatedDDRReadBytes);
  setEstimate(StructuredCandidateCostDimension::DDRAggregateWriteBytes,
              estimatedDDRWriteBytes);
  setEstimate(StructuredCandidateCostDimension::DDRMaximumRankIssueWork,
              estimatedDDRIssueWork);
  setEstimate(StructuredCandidateCostDimension::InstrAggregateWork,
              estimatedTraversalTrips);
  setEstimate(StructuredCandidateCostDimension::TileUnderutilization,
  ...
  setEstimate(StructuredCandidateCostDimension::ComputeMaximumRankWork,
              estimatedComputeElements);
  setEstimate(StructuredCandidateCostDimension::AllRankCouplingWork,
...
static CoordinatedStructuralProposal makeImplementationStructuralProposal(
  ...
  auto setEstimate = [&](StructuredCandidateCostDimension dimension,
    ...
    StructuredCandidateMetric &metric =
        structural.facts.selection[static_cast<size_t>(dimension)];
    metric.value = *value;
    metric.knowledge = StructuredCandidateMetricKnowledge::Estimated;
...
static std::string
computeActualCandidateSetDigest(llvm::ArrayRef<std::string> candidateKeys) {
  llvm::SHA256 hasher;
  for (const std::string &candidateKey : candidateKeys) {
    hasher.update(candidateKey);
    hasher.update("\n");
  }
  return llvm::toHex(hasher.final(), /*LowerCase=*/true);
}
...
mlir::FailureOr<CoordinatedWorkEstimate>
CoordinatedWorkLedger::getCandidateEvaluationUpperBound(int64_t rankCount) {
  if (rankCount <= 0)
    return mlir::failure();
  uint64_t ranks = static_cast<uint64_t>(rankCount);
  CoordinatedWorkEstimate estimate;
...
CoordinatedWorkLedger::CoordinatedWorkLedger(
  ...
  candidateEvaluationReservations.push_back(
      {mandatoryBaselineReservation.id, /*mandatoryBaseline=*/true,
       /*scheduleAttemptSlots=*/1, std::move(evaluationUpperBound)});
...
CoordinatedWorkLedger::create(int64_t rankCount, uint64_t capacity,
  ...
  mlir::FailureOr<CoordinatedWorkEstimate> evaluation =
      getCandidateEvaluationUpperBound(rankCount);
  if (mlir::failed(generation) || mlir::failed(evaluation))
    return mlir::failure();
  std::optional<uint64_t> generationTotal = generation->getTotal();
  std::optional<uint64_t> evaluationTotal = evaluation->getTotal();
  if (!generationTotal || !evaluationTotal)
    return mlir::failure();
  std::optional<uint64_t> mandatory =
      checkedAdd(*generationTotal, *evaluationTotal);
  if (!mandatory)
    return mlir::failure();
  ...
  return CoordinatedWorkLedger(rankCount, capacity, repairReserve,
                               std::move(*generation), std::move(*evaluation));
}

CoordinatedWorkLedger::CandidateEvaluationReservationRecord *
CoordinatedWorkLedger::findReservation(uint64_t id) {
  auto found =
      llvm::find_if(candidateEvaluationReservations,
                    [&](const CandidateEvaluationReservationRecord &record) {
                      return record.id == id;
                    });
  return found == candidateEvaluationReservations.end() ? nullptr : &*found;
}

const CoordinatedWorkLedger::CandidateEvaluationReservationRecord *
...
CoordinatedWorkLedger::tryReserveCandidateEvaluation(
    const CoordinatedWorkEstimate &upperBound) {
  ...
  if (!credits || *credits == 0 || !canReserve(*credits) ||
    ...
    return std::nullopt;
  CandidateEvaluationReservation reservation{nextReservationId++};
  candidateEvaluationReservations.push_back({reservation.id,
...
CoordinatedWorkLedger::tryReserveExecutableScheduleAttempt() {
  ...
  if (mlir::failed(upperBound))
    return std::nullopt;
  return tryReserveCandidateEvaluation(*upperBound);
}

mlir::LogicalResult CoordinatedWorkLedger::completeCandidateEvaluation(
    CandidateEvaluationReservation reservation,
    const CoordinatedWorkEstimate &actual) {
  CandidateEvaluationReservationRecord *record =
      findReservation(reservation.id);
  ...
  addConsumed(actual);
  llvm::erase_if(candidateEvaluationReservations,
                 [&](const CandidateEvaluationReservationRecord &candidate) {
                   return candidate.id == reservation.id;
                 });
  return mlir::success();
}

mlir::LogicalResult CoordinatedWorkLedger::releaseCandidateEvaluation(
    CandidateEvaluationReservation reservation) {
  CandidateEvaluationReservationRecord *record =
      findReservation(reservation.id);
...
CoordinatedWorkLedgerSnapshot CoordinatedWorkLedger::getSnapshot() const {
  ...
  for (const CandidateEvaluationReservationRecord &record :
       candidateEvaluationReservations)
    snapshot.evaluationWorkReserved += *record.upperBound.getTotal();
  uint64_t reservedWork = consumed + mandatoryGenerationReserved +
                          snapshot.evaluationWorkReserved + repairReserved;
  snapshot.unreserved = reservedWork <= capacity ? capacity - reservedWork : 0;
  return snapshot;
}
...
verifyCoordinatedTileVariant(const CoordinatedTileVariant &variant,
  ...
  if (expectedRankCount <= 0 ||
      ...
      variant.evaluationReservation.id == 0 ||
  ...
  for (auto [expectedRank, rank] : llvm::enumerate(variant.ranks)) {
    ...
    if (hasPhysicalFacts) {
      rank.module.get().emitError() << "coordinated Tile variant contains "
                                       "evaluation-owned physical facts at "
                                       "logical rank "
                                    << expectedRank;
      return mlir::failure();
    }
...
mlir::FailureOr<CoordinatedTileVariant> materializeCoordinatedTileRepair(
  ...
  mlir::FailureOr<CoordinatedWorkEstimate> evaluationUpperBound =
      CoordinatedWorkLedger::getCandidateEvaluationUpperBound(
          ledger.getRankCount());
  if (mlir::failed(evaluationUpperBound))
    return fail("cannot derive repair executable-evaluation upper bound");
  std::optional<CandidateEvaluationReservation> reservation =
      ledger.tryReserveCandidateEvaluation(*evaluationUpperBound);
  if (!reservation)
    return fail("coordinated repair has no executable-evaluation reservation");
  ...
  if (!ledger.tryConsumeRepair(repairWork)) {
    (void)ledger.releaseCandidateEvaluation(*reservation);
  ...
  sibling.evaluationReservation = *reservation;
...
std::string computeWholeCardTileCandidateSetDigest(
    const WholeCardTileCandidateSet &candidates) {
  llvm::SHA256 hasher;
...
deriveStructuredCandidates(mlir::ModuleOp sourceModule,
                           const WholeCardCandidateSearchConfig &config,
                           CoordinatedWorkLedger &ledger,
                           StructuredCandidateSearchStatistics &statistics) {
  ...
  if (config.reservedBaselineOnly ||
    ...
    statistics.structuredCandidateSetDigest =
    ...
    for (const StructuredImplementationAlternativeProvider *provider :
      ...
      llvm::sort(points, [targetSPMWindowBytes](const auto &left,
        ...
        return std::make_tuple(
                   ...
                   leftEstimate.reductionTileCount, left->getKey().stableKey) <
               std::make_tuple(
                   ...
                   rightEstimate.reductionTileCount, right->getKey().stableKey);
      });
      if (points.size() > kMaximumStructuredCandidateStates) {
        ...
        auto retain = [&](size_t index) {
          if (selected[index] ||
              retained.size() >= kMaximumStructuredCandidateStates)
            return;
      ...
      // before adding physical siblings that change the baseline. Point-major
      // Cartesian order would let the three residency actions for a few early
      // points consume the shared structural beam and starve later parameter
      // points.
  ...
  for (CoordinatedStructuralProposal &proposal : proposals) {
    ...
    llvm::SmallVector<StructuredCandidateCostView, 16> existing;
    for (const CoordinatedStructuralProposal &state : pareto)
      existing.push_back(
          {state.stableSemanticOrdinal, state.reservedBaseline, &state.facts});
    mlir::FailureOr<StructuredParetoInsertion> insertion =
        planStructuredParetoInsertion({proposal.stableSemanticOrdinal,
                                       proposal.reservedBaseline,
                                       &proposal.facts},
                                      existing);
  ...
  if (pareto.size() > kMaximumStructuredCandidateStates) {
    ...
    retain(0);
    for (size_t index = 1; index < pareto.size() &&
                           retained.size() < kMaximumStructuredCandidateStates;
         ++index)
  ...
  // Candidate ordering is itself a bounded coverage beam: the mandatory
  ...
  std::vector<bool> selected(pareto.size(), false);
  llvm::StringSet<> coverageClasses;
  auto retain = [&](size_t index) {
    if (selected[index])
      return;
    selected[index] = true;
  ...
  for (size_t index = 1; index < pareto.size(); ++index)
    if (coverageClasses.insert(pareto[index].coverageClass).second)
      retain(index);
  for (size_t index = 1; index < pareto.size(); ++index)
    retain(index);
  ...
  statistics.structuredCandidateSetDigest =
      computeStructuralProposalDigest(pareto);
  return pareto;
}

} // namespace

struct WholeCardCandidateSearchSession::Impl {
  ...
  WholeCardCandidateSearchConfig config;
  CoordinatedWorkLedger *ledger = nullptr;
  StructuredCandidateSearchStatistics statistics;
  StructuredCandidateSearchStatistics *externalStatistics = nullptr;
  std::vector<CoordinatedStructuralProposal> structuredCandidates;
  ...
  std::vector<std::string> actualCandidateKeys;
  CoordinatedWorkEstimate evaluationUpperBound;
  ...
  void updateStatistics() {
    statistics.structuralPendingAtStop =
        ...
        std::min(nextProposal, structuredCandidates.size());
  ...
  materialize(const CoordinatedStructuralProposal &proposal,
    ...
    candidate.implementationAlternativeOrigin =
        ...
        CoordinatedStructuralRecipeKind::ImplementationAlternative;
    candidate.evaluationReservation = reservation;
...
WholeCardCandidateSearchSession::WholeCardCandidateSearchSession(
    std::unique_ptr<Impl> impl)
    : impl(std::move(impl)) {}

WholeCardCandidateSearchSession::~WholeCardCandidateSearchSession() = default;
WholeCardCandidateSearchSession::WholeCardCandidateSearchSession(
    WholeCardCandidateSearchSession &&) noexcept = default;
WholeCardCandidateSearchSession &WholeCardCandidateSearchSession::operator=(
    WholeCardCandidateSearchSession &&) noexcept = default;

mlir::FailureOr<std::unique_ptr<WholeCardCandidateSearchSession>>
WholeCardCandidateSearchSession::create(
    mlir::ModuleOp sourceModule, const WholeCardCandidateSearchConfig &config,
    CoordinatedWorkLedger &ledger,
    StructuredCandidateSearchStatistics *statistics) {
  ...
  mlir::FailureOr<CoordinatedWorkEstimate> evaluation =
      CoordinatedWorkLedger::getCandidateEvaluationUpperBound(config.rankCount);
  if (mlir::failed(evaluation))
    return mlir::failure();
  state->evaluationUpperBound = *evaluation;
  mlir::FailureOr<std::vector<CoordinatedStructuralProposal>> proposals =
      deriveStructuredCandidates(sourceModule, config, ledger,
                                 state->statistics);
  if (mlir::failed(proposals) || proposals->empty() ||
      !proposals->front().reservedBaseline)
    return mlir::failure();
  state->structuredCandidates = std::move(*proposals);
  state->updateStatistics();
  return std::unique_ptr<WholeCardCandidateSearchSession>(
      new WholeCardCandidateSearchSession(std::move(state)));
}

mlir::FailureOr<std::unique_ptr<CoordinatedTileVariant>>
WholeCardCandidateSearchSession::admitNextActualCandidate() {
  ...
  if (impl->statistics.successfulActualCandidates >=
      ...
      impl->nextProposal >= impl->structuredCandidates.size()) {
    impl->updateStatistics();
    return std::unique_ptr<CoordinatedTileVariant>();
  }

  while (impl->nextProposal < impl->structuredCandidates.size()) {
    const CoordinatedStructuralProposal &proposal =
        impl->structuredCandidates[impl->nextProposal++];
    ...
    if (proposal.reservedBaseline) {
      ...
      std::optional<CandidateEvaluationReservation> optionalReservation =
          impl->ledger->tryReserveCandidateEvaluation(
              impl->evaluationUpperBound);
      if (!optionalReservation) {
        ++impl->statistics.evaluationReservationDenied;
      ...
      if (proposal.recipe.kind ==
          CoordinatedStructuralRecipeKind::ImplementationAlternative) {
        // The prepared structured module is the actual isolated clone and
        // is consumed in place by ordinary lowering. Only an implementation
        ...
        if (!providerCloneCount) {
          (void)impl->ledger->releaseCandidateEvaluation(reservation);
          return mlir::failure();
        }
      ...
      if (!impl->ledger->tryConsumeGeneration(
        ...
        ++impl->statistics.generationReservationDenied;
        (void)impl->ledger->releaseCandidateEvaluation(reservation);
        impl->stoppedForBudget = true;
        impl->statistics.structuralBudgetExhausted = true;
        impl->updateStatistics();
    ...
    if (mlir::failed(materialized)) {
      ...
      if (proposal.reservedBaseline ||
          mlir::failed(impl->ledger->releaseCandidateEvaluation(reservation))) {
    ...
    mlir::FailureOr<StructuredCandidateCost> facts =
        deriveStructuredCandidateCost(*materialized);
    ...
    if (!proposal.reservedBaseline &&
      ...
      impl->materializationBackfillPending = true;
      if (mlir::failed(impl->ledger->releaseCandidateEvaluation(reservation)))
        return mlir::failure();
      continue;
    }
    llvm::SmallVector<StructuredCandidateCostView, 16> existing;
    ...
    mlir::FailureOr<StructuredParetoInsertion> insertion =
        planStructuredParetoInsertion({proposal.stableSemanticOrdinal,
                                       proposal.reservedBaseline, &*facts},
                                      existing);
    if (mlir::failed(insertion))
      return mlir::failure();
    if (!insertion->retainCandidate) {
      bool equivalent = llvm::any_of(
          impl->actualFacts, [&](const Impl::ActualFactState &state) {
            return compareStructuredCandidateCost(*facts, state.facts) ==
                   StructuredCandidateDominance::Equivalent;
          });
      if (equivalent)
        ++impl->statistics.equivalentCandidatesRejected;
      else
    ...
    impl->actualCandidateKeys.push_back(
        (llvm::Twine(proposal.stableSemanticOrdinal) + ":" +
         llvm::Twine(proposal.reservedBaseline ? 1 : 0) + ":" + contentDigest)
            .str());
    impl->statistics.actualCandidateSetDigest =
        computeActualCandidateSetDigest(impl->actualCandidateKeys);
    ++impl->statistics.retainedActualCandidates;
    if (proposal.recipe.kind == CoordinatedStructuralRecipeKind::Connections)
...
mlir::LogicalResult WholeCardCandidateSearchSession::completeActiveCandidate(
...
bool WholeCardCandidateSearchSession::exhausted() const {
...
llvm::StringRef
WholeCardCandidateSearchSession::getStructuredCandidateSetDigest() const {
  return impl ? llvm::StringRef(impl->statistics.structuredCandidateSetDigest)
              : llvm::StringRef();
}

llvm::StringRef
WholeCardCandidateSearchSession::getActualCandidateSetDigest() const {
  return impl ? llvm::StringRef(impl->statistics.actualCandidateSetDigest)
              : llvm::StringRef();
}

mlir::FailureOr<WholeCardTileCandidateSet> buildWholeCardTileCandidateSet(
    mlir::ModuleOp sourceModule, const WholeCardCandidateSearchConfig &config,
    CoordinatedWorkLedger &ledger,
    StructuredCandidateSearchStatistics *structuredStatistics) {
  auto session = WholeCardCandidateSearchSession::create(
      sourceModule, config, ledger, structuredStatistics);
  if (mlir::failed(session))
    return mlir::failure();
  WholeCardTileCandidateSet candidates;
  while (candidates.size() < config.maximumSuccessfulActualCandidates) {
  ...
  if (candidates.empty() || !candidates.front().reservedBaseline)
    return mlir::failure();
  std::string digest = computeWholeCardTileCandidateSetDigest(candidates);
  for (CoordinatedTileVariant &variant : candidates)
    variant.candidateSetDigest = digest;
  return candidates;
}
