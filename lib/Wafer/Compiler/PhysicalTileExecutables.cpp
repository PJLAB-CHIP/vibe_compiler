//===- PhysicalTileExecutables.cpp - Physical Tile executables ----------===//
...
printAcceptedInstructionWork(llvm::raw_ostream &diagnostics,
                             const detail::WholeCardExecutable &accepted) {
...
static llvm::Expected<PhysicalTileExecutables>
compileTensorProgramModuleToPhysicalTileExecutables(
  ...
  detail::WholeCardExecutable &accepted = compiled->executable;
  ...
  if (tiles.size() !=
    ...
    return fail("physical Tile executable domain is incomplete");

  diagnostics << "wafer-compile: compile-stats stage=physical-tile-executables"
              << " wall_ms=" << detail::elapsedCompileMilliseconds(totalStart)
              << " peak_rss_kib=" << detail::getCompilePeakRSSKiB() << "\n";
  return PhysicalTileExecutablesBuilder::makeExecutables(
      executionConfig, std::move(accepted.runtimeLaunchContract), context,
      std::move(tiles));
}

static llvm::Expected<PhysicalTileExecutables> buildPhysicalTileExecutablesImpl(
    std::shared_ptr<mlir::MLIRContext> &context, mlir::ModuleOp tensorModule,
  ...
  const detail::CompileClock::time_point totalStart =
      detail::CompileClock::now();
  wafer::support::ScopedCompileTimingSpan physicalTileExecutablesTiming(
      "stage", "tensor-program-to-executable", "physical-tile-executables");
  return compileTensorProgramModuleToPhysicalTileExecutables(
      context, tensorModule, program, executionConfig, optimizations,
      diagnostics, failAfterLaunchSlot, totalStart, irTrace);
}

llvm::Expected<PhysicalTileExecutables> detail::buildPhysicalTileExecutables(
    std::shared_ptr<mlir::MLIRContext> &context, mlir::ModuleOp tensorModule,
    frontend::FrontendProgramVerificationResult program,
    ExecutionConfig executionConfig, OptimizationConfig optimizations,
  ...
  return buildPhysicalTileExecutablesImpl(
      context, tensorModule, std::move(program), executionConfig, optimizations,
      diagnostics, failAfterLaunchSlot, discardedTrace);
}

llvm::Expected<PhysicalTileExecutables>
detail::buildPhysicalTileExecutablesWithIRTrace(
    std::shared_ptr<mlir::MLIRContext> &context, mlir::ModuleOp tensorModule,
    frontend::FrontendProgramVerificationResult program,
    ExecutionConfig executionConfig, OptimizationConfig optimizations,
    llvm::raw_ostream &diagnostics, std::optional<int64_t> failAfterLaunchSlot,
  ...
  return buildPhysicalTileExecutablesImpl(
      context, tensorModule, std::move(program), executionConfig, optimizations,
      diagnostics, failAfterLaunchSlot, irTrace);
}
