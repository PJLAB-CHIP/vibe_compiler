//===- LowerWholeCardInstrModules.cpp - Exact all-rank check --------===//

#include "LowerWholeCardInstrModules.h"
...
#include "WholeCardResourceCostValidation.h"
...
lowerAcceptedRankIndexExpressions(mlir::ModuleOp module) {
  ...
  wafer::support::attachCompileTiming(manager, "result-rank-index-lowering");
...
/// output stores. It must not use a private DDR allocation as an intermediate
/// spill. Candidate labels are deliberately not used as this semantic proof.
} // namespace
...
verifyAcceptedRankModule(mlir::ModuleOp module, const ExecutionConfig &config,
  ...
  if (mlir::isa<InstrDTESendOp, InstrDTERecvOp, InstrDTEWaitOp>(illegal))
    return illegal->emitOpError(
        "does not satisfy the result executable transport contract");
  return illegal->emitOpError(
      "is not legal in an result static-rank executable");
}
...
struct WholeCardRankExecutables {
  WholeCardRankExecutables(
...
static mlir::FailureOr<WholeCardRankExecutables> buildCandidateRankExecutables(
    ...
    const frontend::FrontendProgramVerificationResult &program,
    const ExecutionConfig &executionConfig, std::string &failedCheck,
    unsigned rankPipelineParallelism) {
  wafer::support::ScopedCompileTimingSpan totalTiming(
      "whole-card-instr-lowering", "candidate", "rank-executables");
  ...
  if (rankCount == 0 ||
    ...
    failedCheck = "rank-domain";
    return mlir::failure();
  }
  const bool fixedSlot =
      actionKey.bufferingKind == CoordinatedBufferingKind::StaticFixedSlot;
  if ((fixedSlot && actionKey.bufferingPlanOrdinal == 0) ||
      (!fixedSlot && actionKey.bufferingPlanOrdinal != 0) ||
      (actionKey.workerPlacementKind ==
           CoordinatedWorkerPlacementKind::DisjointComponents &&
       actionKey.workerPlacementPlanOrdinal == 0) ||
      (actionKey.workerPlacementKind ==
           CoordinatedWorkerPlacementKind::Unplaced &&
       actionKey.workerPlacementPlanOrdinal != 0)) {
    failedCheck = "scheduling-action-key";
    return mlir::failure();
  }
  ...
  for (mlir::OwningOpRef<mlir::ModuleOp> &owned : modules) {
    if (!owned) {
      failedCheck = "rank-materialization";
      return mlir::failure();
    }
  ...
  {
    wafer::support::ScopedCompileTimingSpan timing("whole-card-instr-lowering",
                                                   "candidate", "ddr-planning");
...

  {
    wafer::support::ScopedCompileTimingSpan timing(
        "whole-card-instr-lowering", "candidate", "index-lowering");
    std::vector<uint8_t> failedRanks(rankCount);
    runBoundedRankModulePipelines(
        moduleViews,
        [&](size_t rank) {
        ...
        rankPipelineParallelism == 0 ? kMaximumBoundedRankPipelineWorkers
                                     : rankPipelineParallelism);
    if (llvm::is_contained(failedRanks, uint8_t{1})) {
      failedCheck = "result-index-lowering";
      return mlir::failure();
    }
  }
  ...
  if (fixedSlot) {
    ...
    if (mlir::failed(wafer::specializePeriodicDirectDTESites(
      ...
      failedCheck = "direct-dte-site-specialization";
      return mlir::failure();
    }
  ...
  {
    wafer::support::ScopedCompileTimingSpan timing(
        "whole-card-instr-lowering", "candidate", "whole-card-resources");
    resourceCost = validateWholeCardResourceCost(moduleViews, executionConfig);
  }
  if (mlir::failed(resourceCost)) {
    failedCheck = "whole-card-resources";
    return mlir::failure();
  }
  ...
  for (size_t rank = 0; rank < rankCount; ++rank) {
    ...
    if (mlir::failed(verifyAcceptedRankModule(
      ...
      failedCheck = "result-rank-verifier";
      return mlir::failure();
    }
    ...
    if (!closure) {
      ...
      failedCheck = "result-call-closure";
      return mlir::failure();
    }
    ...
    if (mlir::failed(bindings)) {
      failedCheck = "rank-resource-projection";
      return mlir::failure();
    }
    llvm::StringRef tileEvidence = selectedTileIR[rank]
                                       ? llvm::StringRef(*selectedTileIR[rank])
                                       : llvm::StringRef{};
    ranks.push_back(PhysicalTileExecutablesBuilder::makeRank(
  ...
  if (llvm::any_of(ranks, [&](const RankExecutable &rank) {
    ...
    failedCheck = "runtime-launch-contract";
    return mlir::failure();
  }
  ...
  return WholeCardRankExecutables(std::move(ranks),
                                  std::move(*runtimeLaunchContract),
                                  std::move(*resourceCost));
}

static mlir::FailureOr<WholeCardExecutableResult> verifyCandidateTargetLowering(
    WholeCardRankExecutables candidate, const ExecutionConfig &executionConfig,
    WholeVariantSelectionStatistics *statistics, std::string &failedCheck,
    unsigned rankPipelineParallelism) {
  if (statistics)
  ...
  const unsigned workers = runBoundedRankModulePipelines(
      ...
      rankPipelineParallelism == 0 ? kMaximumBoundedRankPipelineWorkers
                                   : rankPipelineParallelism);
  if (statistics) {
    statistics->targetRankVerificationInvocations += candidate.ranks.size();
  ...
  for (TargetRankFailure failure : rankFailures) {
    if (failure == TargetRankFailure::Preparation) {
      failedCheck = "target-abi-preparation";
      return mlir::failure();
    }
    if (failure == TargetRankFailure::Lowering) {
      failedCheck = "target-abi-lowering";
      return mlir::failure();
    }
  }
  return WholeCardExecutableResult(std::move(candidate.ranks),
...
mlir::FailureOr<WholeCardExecutableResult> lowerWholeCardInstrModules(
    ...
    std::string *failedCheck, WholeVariantSelectionStatistics *statistics,
    unsigned rankPipelineParallelism) {
  auto fail = [&](llvm::StringRef check, llvm::StringRef message) {
    if (failedCheck)
      *failedCheck = check.str();
    if (!message.empty())
      diagnostics << "wafer-compile: " << message << '\n';
    return mlir::FailureOr<WholeCardExecutableResult>(mlir::failure());
  };

  std::string check = "unknown";
  if (statistics)
    ++statistics->physicalTileModuleLoweringAttempts;
  mlir::FailureOr<WholeCardRankExecutables> rankExecutables =
      buildCandidateRankExecutables(std::move(rankModules), selectedTileIR,
                                    actionKey, program, executionConfig, check,
                                    rankPipelineParallelism);
  if (mlir::failed(rankExecutables))
    return fail(check,
                "whole-card candidate failed validation '" + check + "'");
  if (statistics)
    ++statistics->physicalTileModuleLoweringSuccesses;

  mlir::FailureOr<WholeCardExecutableResult> result =
      verifyCandidateTargetLowering(std::move(*rankExecutables),
                                    executionConfig, statistics, check,
                                    rankPipelineParallelism);
