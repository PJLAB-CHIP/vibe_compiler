//===- NoCIntermediateRouting.cpp - Route intermediate values ----------===//

#include "Wafer/Transforms/Transport/NoCIntermediateRouting.h"

#include "Wafer/Analysis/PhysicalDataflow/GlobalTileRelation.h"
...
static std::optional<BoundaryTileKey>
resolveBoundaryTileKey(mlir::Value value, int64_t rank,
                       llvm::ArrayRef<BoundaryDescriptor> boundaries) {
  auto equivalent = [](const BoundaryTileKey &lhs, const BoundaryTileKey &rhs) {
  ...
  std::optional<BoundaryTileKey> identity;
  for (const BoundaryDescriptor &boundary : boundaries) {
    ...
    if (!hashRankBoundaryTileView(*resolved, *slice, boundary.globalShape,
                                  boundary.localShape))
      return std::nullopt;
    BoundaryTileKey current{&boundary, slice, std::move(*resolved)};
...
static bool equivalentBoundaryTileKey(const BoundaryTileKey &lhs,
                                      const BoundaryTileKey &rhs) {
  if (!lhs.boundary || !rhs.boundary || !lhs.rankSlice || !rhs.rankSlice ||
      lhs.boundary->kind != rhs.boundary->kind ||
      lhs.boundary->programIndex != rhs.boundary->programIndex ||
      lhs.view.leafType != rhs.view.leafType ||
...
public:
  ...
  bool prove(mlir::Value lhs, mlir::Operation *lhsBefore, mlir::Value rhs,
    ...
    std::optional<BoundaryTileKey> lhsBoundary =
        resolveBoundaryTileKey(lhs, lhsRank, boundaries);
    std::optional<BoundaryTileKey> rhsBoundary =
        resolveBoundaryTileKey(rhs, rhsRank, boundaries);
    if (lhsBoundary || rhsBoundary)
      return finish(lhsBoundary && rhsBoundary &&
                    equivalentBoundaryTileKey(*lhsBoundary, *rhsBoundary));
...
struct RequiredOutputGroup {
  llvm::SmallVector<RequiredOutputWriter, 16> writers;
};
...
static bool isFinalRequiredOutputWriter(mlir::ModuleOp module,
  ...
  module.walk([&](mlir::Operation *operation) {
    if (!valid || operation == writer.getOperation())
      return;
    auto effects = mlir::dyn_cast<mlir::MemoryEffectOpInterface>(operation);
    if (!effects)
      return;
    ...
    // required final writer.
    valid = operation->getBlock() == writer->getBlock() &&
            operation->isBeforeInBlock(writer);
  });
  return valid;
}

static std::optional<RequiredOutputWriter>
findRequiredOutputWriter(mlir::ModuleOp module, int64_t logicalRank,
  ...
  if (stores.size() != 1)
    return std::nullopt;
  InstrWDMAOp writer = stores.front();

  std::optional<int64_t> sourceBytes =
      getPhysicalBytes(writer.getSource().getType());
  std::optional<int64_t> destinationBytes =
      getPhysicalBytes(writer.getDest().getType());
  auto sourceType =
      mlir::dyn_cast<mlir::MemRefType>(writer.getSource().getType());
  auto destinationType =
      mlir::dyn_cast<mlir::MemRefType>(writer.getDest().getType());
  if (!sourceBytes || !destinationBytes || *sourceBytes != *destinationBytes ||
      ...
      !isCompleteStore(writer, *sourceBytes))
    return std::nullopt;
  ...
  mlir::Value destinationRoot =
      resolveTransparentDataflowAlias(writer.getDest());
  if (!destinationRoot ||
      !isFinalRequiredOutputWriter(module, writer, destinationRoot))
    return std::nullopt;

  return RequiredOutputWriter{logicalRank,
                              &binding,
                              rankSlice,
                              writer,
                              writer.getSource(),
...
static llvm::SmallVector<RequiredOutputGroup, 4> collectRequiredOutputGroups(
  ...
  for (const frontend::ProgramBoundaryBinding &binding :
    ...
    for (auto [rank, module] : llvm::enumerate(modules)) {
      std::optional<RequiredOutputWriter> writer =
          findRequiredOutputWriter(module, static_cast<int64_t>(rank), binding,
      ...
      if (!group.writers.empty()) {
        const RequiredOutputWriter &anchor = group.writers.front();
        if (writer->bytes != anchor.bytes ||
            writer->source.getType() != anchor.source.getType() ||
            compareStaticTiles(writer->globalTile, anchor.globalTile) !=
                StaticTileRelation::Equivalent ||
            !haveEquivalentStructuredOperationPaths(writer->writer,
                                                    anchor.writer)) {
          group.writers.clear();
          break;
        }
        ProducedValueEquivalence equivalence(boundaries, anchor.logicalRank,
                                             writer->logicalRank);
        if (!equivalence.prove(anchor.source, anchor.writer, writer->source,
                               writer->writer)) {
          group.writers.clear();
          break;
        }
      }
      group.writers.push_back(std::move(*writer));
    }
    if (group.writers.size() != modules.size() || group.writers.size() < 2)
      continue;
    ...
    const size_t selected =
        static_cast<size_t>(binding.programIndex) % group.writers.size();
    std::rotate(group.writers.begin(), group.writers.begin() + selected,
                group.writers.end());
...
static void materializeRequiredOutputGroup(const RequiredOutputGroup &group,
                                           ...
                                           NoCFanoutKind kind) {
  const RequiredOutputWriter &owner = group.writers.front();
  InstrWDMAOp ownerWriter = owner.writer;
  auto message =
      DTEMessageAttr::get(ownerWriter.getContext(), communicationId,
                          DTEProtocolPhase::PeerDataflow, /*round=*/2,
                          /*payloadSlice=*/owner.binding->programIndex);

  mlir::OpBuilder ownerBuilder(ownerWriter);
  if (kind == NoCFanoutKind::Direct) {
    for (const RequiredOutputWriter &peer : llvm::drop_begin(group.writers)) {
      auto send = ownerBuilder.create<CommPeerSendOp>(
          ownerWriter.getLoc(), ownerBuilder.getType<mlir::async::TokenType>(),
          owner.source, ownerBuilder.getI64IntegerAttr(peer.logicalRank),
          ownerBuilder.getI64IntegerAttr(owner.bytes), message);
      ownerBuilder.create<mlir::async::AwaitOp>(ownerWriter.getLoc(),
                                                send.getToken());
    }
  } else {
    const RequiredOutputWriter &peer = group.writers[1];
    auto send = ownerBuilder.create<CommPeerSendOp>(
        ownerWriter.getLoc(), ownerBuilder.getType<mlir::async::TokenType>(),
        owner.source, ownerBuilder.getI64IntegerAttr(peer.logicalRank),
        ownerBuilder.getI64IntegerAttr(owner.bytes), message);
  ...
  for (size_t index = 1; index < group.writers.size(); ++index) {
    const RequiredOutputWriter &peer = group.writers[index];
    InstrWDMAOp peerWriter = peer.writer;
    mlir::OpBuilder builder(peerWriter);
    ...
    const int64_t source = kind == NoCFanoutKind::Direct
                               ? owner.logicalRank
                               : group.writers[index - 1].logicalRank;
    auto recv = builder.create<CommPeerRecvOp>(
        peerWriter.getLoc(), builder.getType<mlir::async::TokenType>(),
        received.getResult(), builder.getI64IntegerAttr(source),
        builder.getI64IntegerAttr(peer.bytes), message);
    auto await = builder.create<mlir::async::AwaitOp>(peerWriter.getLoc(),
                                                      recv.getToken());
    if (kind == NoCFanoutKind::ReceiveForward &&
        index + 1 < group.writers.size()) {
      builder.setInsertionPointAfter(await);
      auto send = builder.create<CommPeerSendOp>(
          peerWriter.getLoc(), builder.getType<mlir::async::TokenType>(),
          received.getResult(),
          builder.getI64IntegerAttr(group.writers[index + 1].logicalRank),
          builder.getI64IntegerAttr(peer.bytes), message);
      builder.create<mlir::async::AwaitOp>(peerWriter.getLoc(),
                                           send.getToken());
    }
    peerWriter->setOperand(0, received.getResult());
...
bool hasNoCIntermediateRoutingOpportunity(
    llvm::ArrayRef<mlir::ModuleOp> modules,
    const frontend::FrontendProgramVerificationResult &program) {
  if (modules.size() < 2 ||
...
bool hasNoCReplicatedOutputDataflowOpportunity(
    llvm::ArrayRef<mlir::ModuleOp> modules,
    const frontend::FrontendProgramVerificationResult &program) {
  if (modules.size() < 2 ||
      modules.size() != static_cast<size_t>(program.logicalRankCount) ||
...
unsigned materializeNoCIntermediateRoutes(
    llvm::MutableArrayRef<mlir::ModuleOp> modules,
    const frontend::FrontendProgramVerificationResult &program,
    int64_t &communicationId, NoCFanoutKind kind) {
  wafer::support::ScopedCompileTimingSpan timing(
      "transformation", "materializeNoCIntermediateRoutes", "total");
  ...
  auto phaseTiming = std::make_unique<wafer::support::ScopedCompileTimingSpan>(
      "analysis-phase", "materializeNoCIntermediateRoutes",
  ...
  phaseTiming = std::make_unique<wafer::support::ScopedCompileTimingSpan>(
      "transformation-phase", "materializeNoCIntermediateRoutes",
...
unsigned materializeNoCReplicatedOutputDataflow(
    llvm::MutableArrayRef<mlir::ModuleOp> modules,
    const frontend::FrontendProgramVerificationResult &program,
    int64_t &communicationId, NoCFanoutKind kind) {
  wafer::support::ScopedCompileTimingSpan timing(
      "transformation", "materializeNoCReplicatedOutputDataflow", "total");
  if (modules.size() < 2 || communicationId < 0 ||
      modules.size() != static_cast<size_t>(program.logicalRankCount) ||
      program.distributedOutputs.empty())
    return 0;
  llvm::SmallVector<BoundaryDescriptor, 8> boundaries =
      getBoundaryDescriptors(program);
  auto phaseTiming = std::make_unique<wafer::support::ScopedCompileTimingSpan>(
      "analysis-phase", "materializeNoCReplicatedOutputDataflow",
  ...
  phaseTiming = std::make_unique<wafer::support::ScopedCompileTimingSpan>(
      "transformation-phase", "materializeNoCReplicatedOutputDataflow",
