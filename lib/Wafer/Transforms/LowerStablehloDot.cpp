  ...
  void runOnOperation() final {
    ...
    for (mlir::stablehlo::DotGeneralOp dot : dots) {
      ...
      mlir::TypedAttr zeroAttr =
          getZeroAttr(builder, resultType.getElementType());
      if (!zeroAttr) {
      ...
      auto zero =
          builder.create<mlir::arith::ConstantOp>(dot.getLoc(), zeroAttr);
      auto empty = builder.create<mlir::tensor::EmptyOp>(
          dot.getLoc(), resultType.getShape(), resultType.getElementType());
      auto filled = builder.create<mlir::linalg::FillOp>(
          dot.getLoc(), mlir::TypeRange{resultType}, mlir::ValueRange{zero},
