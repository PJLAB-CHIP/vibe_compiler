...
static mlir::FailureOr<wafer::TileRegionOp>
materializeTileRegion(wafer::GroupOp group, mlir::linalg::MatmulOp matmul) {
  auto lhsType =
...
static mlir::LogicalResult materializeGroup(wafer::GroupOp group,
  ...
  if (!matmul)
    return group.emitOpError(
        "cannot materialize multi-tile no-comm: expected one linalg.matmul "
        "body op");
  if (group.getNumResults() != 1 || group.getOuts().size() != 1)
    return group.emitOpError(
        "cannot materialize multi-tile no-comm: expected one group result "
        "and out");
  if (matmul.getInputs().size() != 2 || matmul.getOutputs().size() != 1 ||
      matmul->getNumResults() != 1)
    return matmul.emitOpError(
        "cannot materialize multi-tile no-comm: expected tensor matmul with "
        "two inputs, one out, and one result");

  wafer::TileRegionOp lastTileRegion;
  for (int64_t rank = 0; rank < logicalRankCount; ++rank) {
    mlir::FailureOr<wafer::TileRegionOp> tileRegion =
        materializeTileRegion(group, matmul);
    if (mlir::failed(tileRegion))
      return matmul.emitOpError(
          "cannot materialize multi-tile no-comm: matmul tensors must be "
          "ranked");
    lastTileRegion = *tileRegion;
  }

  group->replaceAllUsesWith(lastTileRegion->getResults());
  group.erase();
  return mlir::success();
}

struct MaterializeMultiTileNoCommPass
    : public mlir::PassWrapper<MaterializeMultiTileNoCommPass,
                               mlir::OperationPass<mlir::ModuleOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(MaterializeMultiTileNoCommPass)

  llvm::StringRef getArgument() const final {
    return "wafer-materialize-multi-tile-no-comm";
  }
  ...
  void runOnOperation() final {
    ...
    module.walk([&](wafer::GroupOp group) { groups.push_back(group); });
    if (groups.empty())
      return;

    llvm::SmallVector<wafer::PlacementMapOp> placementMaps;
    module.walk([&](wafer::PlacementMapOp map) { placementMaps.push_back(map); });
    if (placementMaps.size() != 1) {
      module.emitError(
          "materialize multi-tile no-comm requires exactly one "
          "wafer.placement.map");
      signalPassFailure();
      return;
    }
    ...
    int64_t logicalRankCount =
        placementMaps.front().getLogicalRankCountAttr().getInt();
    for (wafer::GroupOp group : groups) {
      if (mlir::failed(materializeGroup(group, logicalRankCount))) {
        signalPassFailure();
        return;
      }
    }
...
std::unique_ptr<mlir::Pass> createMaterializeMultiTileNoCommPass() {
  return std::make_unique<MaterializeMultiTileNoCommPass>();
}
