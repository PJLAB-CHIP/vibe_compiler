//===- OneDNNQualificationComparison.cpp - Formal/backend comparison ---===//
...
compareOutputs(const FormalTensorNumericResult &formal,
               const OneDNNTensorStorage &backend) {
  llvm::Expected<std::vector<RawLogicalValue>> backendValues =
      unpackOneDNNTensorLogicalValues(backend);
  if (!backendValues)
...
runQualification(const OneDNNExecutionEnvironment &environment,
  ...
  llvm::Expected<std::vector<RawLogicalValue>> lhs =
      unpackOneDNNTensorLogicalValues(testCase.getInputs()[0]);
  llvm::Expected<std::vector<RawLogicalValue>> rhs =
      unpackOneDNNTensorLogicalValues(testCase.getInputs()[1]);
  ...
  llvm::Expected<detail::UnqualifiedOneDNNExecutionResult> backend =
      detail::executeOneDNNTensorForQualification(
          environment, testCase.getOperation(), testCase.getInputs(),
          testCase.getDestinationTemplate(), onednnBudget);
  if (!backend)
  ...
  llvm::Expected<OneDNNTensorStorage> formalStorage =
      OneDNNTensorStorage::create(destinationTemplate.getKey(),
                                  std::move(*formalPhysical));
  if (!formalStorage)
    return formalStorage.takeError();
  return QualificationRun{std::move(testCase), std::move(*formal),
                          std::move(*backend), *comparison,
                          computeOneDNNTensorStorageDigest(*formalStorage)};
}
