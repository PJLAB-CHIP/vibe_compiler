//===- ExecutableCallClosure.cpp - Executable call closure ---------------===//
...
llvm::Expected<ExecutableCallClosure>
analyzeExecutableCallClosure(mlir::ModuleOp module,
                             std::optional<llvm::StringRef> expectedEntry) {
  ExecutableCallClosure closure;
  analysis::DirectCallGraphAnalysis callGraph(module.getOperation());
  if (!callGraph.hasModuleScope())
    return invalid("executable call closure requires a module");
  ...
  for (mlir::func::FuncOp function : closure.functions) {
    if (function.isDeclaration())
      return invalid(("executable call closure contains external function @" +
                      function.getSymName())
                         .str());
  ...
  if (expectedEntry && closure.entry.getSymName() != *expectedEntry)
    return invalid("entry symbol disagrees with the executable");
  ...
  for (mlir::func::FuncOp function : closure.functions) {
    ...
    for (auto [index, type] :
      ...
      if (!wafer::isWaferDDRMemRefType(type))
        return invalid(("executable function @" + function.getSymName() +
  ...
  if (!callGraph.getUnsupportedCallOperations().empty()) {
    ...
    return invalid(("executable call closure contains unsupported indirect or "
  ...
  if (callGraph.hasRecursiveCycle()) {
    auto function =
        mlir::dyn_cast<mlir::func::FuncOp>(callGraph.getRecursiveCycleOrigin());
    return invalid(
        ("executable call closure is recursive at @" + function.getSymName())
            .str());
  }
