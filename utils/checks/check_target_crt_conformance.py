...
def check_ncc_worker_command_abi(
    ...
    if len(declarations) != 115:
        fail(
            "runtime CRT worker ABI: expected 115 public declarations, found "
            f"{len(declarations)}"
        )
    signatures = {
    ...
    if len(ordinary_symbols) != 103:
        fail(
            "runtime CRT worker ABI: expected 103 current ordinary "
...
def main() -> int:
    ...
    header_text = read_text(
        repo_root / "runtime" / "crt" / "include" / "wafer_tx81_crt.h"
    )
    source_text = read_text(repo_root / "runtime" / "crt" / "src" / "wafer_tx81_crt.c")
