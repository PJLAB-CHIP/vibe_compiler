...
BOARD_TEST_DIRECTORY = (
    pathlib.Path(__file__).resolve().parents[2] / "test" / "Board"
)
for board_python_directory in (
    BOARD_TEST_DIRECTORY / "Support",
    BOARD_TEST_DIRECTORY / "Cases",
):
    if str(board_python_directory) not in sys.path:
        sys.path.insert(0, str(board_python_directory))
